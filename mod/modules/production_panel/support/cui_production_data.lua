...
function GetDistrictData(district)
    ...
    local goldData = GetDistrictPurchaseData(district, m_city, "YIELD_GOLD",
                                             goldSource,
    ...
    local faithData = GetDistrictPurchaseData(district, m_city, "YIELD_FAITH",
                                              faithSource,
...
function GetDistrictPurchaseData(district, city, yield, source, key)
    ...
    if CityManager.CanStartCommand(city, CMD.PURCHASE, true, param, false) then
        local canStart, results = CityManager.CanStartCommand(city,
                                                              CMD.PURCHASE,
                                                              false, param, true)
        ...
        if not canAfford then
            ...
            tooltip = tooltip .. "[COLOR:Red]" .. Locale.Lookup(key) ..
                          "[ENDCOLOR]"
        end
...
function GetBuildingData(building, district)
    ...
    local bToolTip = ToolTipHelper.GetBuildingToolTip(building.Hash, m_playerID,
                                                      m_city)
    local fToolTip = GetFailureToolTip(canStart, results)
    local cToolTip = GetProduceCostToolTip(cost, progress)
    ...
    if building.PurchaseYield == "YIELD_GOLD" then
        local goldSource = m_player:GetTreasury()
        local goldData = GetBuildingPurchaseData(building, m_city, "YIELD_GOLD",
                                                 goldSource,
                                                 "LOC_BUILDING_INSUFFICIENT_FUNDS")
        data.GoldUnlock = goldData.Unlock
        data.GoldCost = goldData.Cost
    ...
    -- faith data
    if building.PurchaseYield == "YIELD_FAITH" or
        m_city:GetGold():IsBuildingFaithPurchaseEnabled(building.Hash) then
        local faithSource = m_player:GetReligion()
        local faithData = GetBuildingPurchaseData(building, m_city,
                                                  "YIELD_FAITH", faithSource,
                                                  "LOC_BUILDING_INSUFFICIENT_FAITH")
        data.FaithUnlock = faithData.Unlock
        data.FaithCost = faithData.Cost
        data.FaithEnable = faithData.Enable
...
function GetWonderData(wonder)
    ...
    if (not canStart) and (not isNil(results)) then
        if results[CityOperationResults.NO_SUITABLE_LOCATION] then
            local purchaseablePlots =
                GetCityRelatedPlotIndexesWondersAlternative(m_city, wonder.Hash)
    ...
    local bToolTip = ToolTipHelper.GetBuildingToolTip(wonder.Hash, m_playerID,
                                                      m_city)
    local fToolTip = GetFailureToolTip(canStart, results)
    local cToolTip = GetProduceCostToolTip(cost, progress)
...
function GetUnitData(unit, formation)
    ...
    if unit.PurchaseYield == "YIELD_GOLD" then
        local goldSource = m_player:GetTreasury()
        local goldData = GetUnitPurchaseData(unit, m_city, "YIELD_GOLD",
                                             goldSource,
                                             "LOC_BUILDING_INSUFFICIENT_FUNDS",
                                             formation)
        data.GoldUnlock = goldData.Unlock
    ...
    -- faith data
    if unit.PurchaseYield == "YIELD_FAITH" or
        m_city:GetGold():IsUnitFaithPurchaseEnabled(unit.Hash) then
        local faithSource = m_player:GetReligion()
        local faithData = GetUnitPurchaseData(unit, m_city, "YIELD_FAITH",
                                              faithSource,
                                              "LOC_BUILDING_INSUFFICIENT_FAITH",
                                              formation)
        data.FaithUnlock = faithData.Unlock
...
function GetUnitPurchaseData(unit, city, yield, source, key, formation)
    ...
    if CityManager.CanStartCommand(city, CMD.PURCHASE, true, standard, false) then
        ...
        data.Unlock = true
        data.Cost = city:GetGold():GetPurchaseCost(yieldType, unit.Hash,
                                                   formation)
        data.Enable = canAfford and noFailure
        data.ReasonTT = tooltip
        data.CostTT = GetPurchaseCostToolTip(data.Cost, yield)
...
function GetProjectData(project)
    ...
    local bToolTip = ToolTipHelper.GetProjectToolTip(project.Hash, m_playerID,
                                                     m_city)
    local fToolTip = GetFailureToolTip(canStart, results)
    local cToolTip = GetProduceCostToolTip(cost, progress)
