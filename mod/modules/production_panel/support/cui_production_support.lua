...
function CuiCanProduceDistrict(city, queue, district)
    ...
    if not canProduce then
        ...
        --
        if notSuitable then
            local plots = GetCityRelatedPlotIndexesDistrictsAlternative(city,
                                                                        district.Hash)
    ...
    if (not canProduce) and results then
        ...
        if not isNil(failureReasons) then
            ...
            for i, v in ipairs(failureReasons) do
                ...
                allReasons = allReasons .. "[COLOR:Red]" .. Locale.Lookup(v) ..
                                 "[ENDCOLOR]"
            end
            return allReasons
        end
...
function GetPurchaseCostToolTip(cost, yield)
    ...
    if cost ~= 0 then
        ...
        if yield == "YIELD_GOLD" then
            concatString = " [ICON_GOLD] " ..
                               Locale.Lookup("LOC_YIELD_GOLD_NAME")
        elseif yield == "YIELD_FAITH" then
            concatString = " [ICON_FAITH] " ..
                               Locale.Lookup("LOC_YIELD_FAITH_NAME")
        end
        tooltip = Locale.Lookup("LOC_HUD_PURCHASE") .. ": " .. cost ..
                      concatString
    end
    return tooltip
end
...
function GetProduceCostToolTip(cost, progress)
    local tooltip = ""
    if cost ~= 0 then
        local concatString = " [ICON_Production] " ..
                                 Locale.Lookup("LOC_HUD_PRODUCTION")
        local costString = tostring(cost)
        if progress > 0 then
            costString = tostring(progress) .. "/" .. tostring(cost)
        end
        tooltip =
            Locale.Lookup("LOC_HUD_PRODUCTION_COST") .. ": " .. costString ..
                concatString
    end
    return tooltip
end
...
RepeatableProject = {
    ...
    "LOC_PROJECT_ENHANCE_DISTRICT_THEATER_NAME", "LOC_PROJECT_CARNIVAL_NAME",
    ...
    -- expansion 2
    "LOC_PROJECT_CARBON_RECAPTURE_NAME", "LOC_PROJECT_ORBITAL_LASER_NAME",
    "LOC_PROJECT_TERRESTRIAL_LASER_NAME", "LOC_PROJECT_SEND_AID_NAME",
...
function RepeatProjects()
    ...
    for i, city in player:GetCities():Members() do
        ...
        if RepeatedProjectsList[cityName] then
            ...
            CityManager.RequestOperation(city, CityOperationTypes.BUILD,
                                         tParameters)
        end
    end
end
