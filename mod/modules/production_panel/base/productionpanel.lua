...
local TXT_PRODUCTION_ITEM_DECONTAMINATE =
    Locale.Lookup("LOC_PRODUCTION_ITEM_DECONTAMINATE")
local TXT_HUD_CITY_WILL_NOT_COMPLETE = Locale.Lookup(
                                           "LOC_HUD_CITY_WILL_NOT_COMPLETE")
local TXT_HUD_CITY_DISTRICT_BUILT_TT = Locale.Lookup(
                                           "LOC_HUD_CITY_DISTRICT_BUILT_TT")
local TXT_COST = Locale.Lookup("LOC_HUD_PRODUCTION_COST")
local TXT_PRODUCTION = Locale.Lookup("LOC_HUD_PRODUCTION")
local LOC_HUD_UNIT_PANEL_FLEET_SUFFIX = Locale.Lookup(
                                            "LOC_HUD_UNIT_PANEL_FLEET_SUFFIX")
local TXT_HUD_UNIT_PANEL_CORPS_SUFFIX = Locale.Lookup(
                                            "LOC_HUD_UNIT_PANEL_CORPS_SUFFIX")
local TXT_HUD_UNIT_PANEL_ARMADA_SUFFIX =
    Locale.Lookup("LOC_HUD_UNIT_PANEL_ARMADA_SUFFIX")
local TXT_HUD_UNIT_PANEL_ARMY_SUFFIX = Locale.Lookup(
                                           "LOC_HUD_UNIT_PANEL_ARMY_SUFFIX")
local TXT_DISTRICT_REPAIR_LOCATION_FLOODED =
    Locale.Lookup("LOC_DISTRICT_REPAIR_LOCATION_FLOODED")
...
local m_purchaseListIM = InstanceManager:new("NestedList", "Top",
                                             Controls.PurchaseList)
local m_purchaseFaithListIM = InstanceManager:new("NestedList", "Top",
                                                  Controls.PurchaseFaithList)
local m_queueListIM = InstanceManager:new("NestedList", "Top",
                                          Controls.QueueList)
...
-- ===========================================================================
local cui_prodIM = InstanceManager:new("CuiGroupInstance", "Top",
                                       Controls.ProductionList)
local cui_goldIM = InstanceManager:new("CuiGroupInstance", "Top",
                                       Controls.PurchaseList)
local cui_faithIM = InstanceManager:new("CuiGroupInstance", "Top",
                                        Controls.PurchaseFaithList)
local cui_queueIM = InstanceManager:new("CuiGroupInstance", "Top",
                                        Controls.QueueList)
local cui_itemIM = InstanceManager:new("CuiItemInstance", "Top",
                                       Controls.CuiItemContainer)
...
function ShowProperList(eListMode)
    ...
    Controls.PurchaseMenu:SetHide(not (m_CurrentListMode ==
                                      LISTMODE.PURCHASE_GOLD))
    Controls.PurchaseFaithMenu:SetHide(not (m_CurrentListMode ==
                                           LISTMODE.PURCHASE_FAITH))
    Controls.ProductionListScroll:SetHide(
        not (m_CurrentListMode == LISTMODE.PRODUCTION))
    Controls.QueueListContainer:SetHide(not (m_CurrentListMode ==
                                            LISTMODE.PROD_QUEUE))
end
...
function PurchaseUnit(city, unitEntry)
    ...
    if (unitEntry.Yield == "YIELD_GOLD") then
        tParameters[CityCommandTypes.PARAM_YIELD_TYPE] =
            GameInfo.Yields["YIELD_GOLD"].Index
        UI.PlaySound("Purchase_With_Gold")
    else
        tParameters[CityCommandTypes.PARAM_YIELD_TYPE] =
            GameInfo.Yields["YIELD_FAITH"].Index
...
function PurchaseDistrict(city, districtEntry)
    ...
    if (bNeedsPlacement) then
        ...
        if (districtEntry.Yield == "YIELD_GOLD") then
            local tParameters = {}
            tParameters[CityOperationTypes.PARAM_DISTRICT_TYPE] =
                districtEntry.Hash
            tParameters[CityCommandTypes.PARAM_YIELD_TYPE] =
                GameInfo.Yields["YIELD_GOLD"].Index
            UI.SetInterfaceMode(InterfaceModeTypes.DISTRICT_PLACEMENT,
                                tParameters)
        else
            local tParameters = {}
            tParameters[CityOperationTypes.PARAM_DISTRICT_TYPE] =
...
function OnCitySelectionChanged(owner, cityID, i, j, k, isSelected, isEditable)
    ...
    if owner == localPlayerId and isSelected then
        ...
        if (ContextPtr:IsHidden() == false) and
            Controls.PauseDismissWindow:IsStopped() and
            Controls.AlphaIn:IsStopped() then Refresh() end
    end
end
...
function OnInterfaceModeChanged(eOldMode, eNewMode)
    ...
    if eNewMode == InterfaceModeTypes.CITY_MANAGEMENT or eNewMode ==
        InterfaceModeTypes.VIEW_MODAL_LENS then
...
-- ===========================================================================
function OnUnitSelectionChanged(playerID, unitID, hexI, hexJ, hexK, bSelected,
                                bEditable)
...
function View(data)
    ...
    Controls.PurchaseListScroll:SetScrollValue(
        GetScrollPosition(LISTMODE.PURCHASE_GOLD))
    PopulateList(data, LISTMODE.PURCHASE_FAITH, m_purchaseFaithListIM)
    Controls.PurchaseFaithListScroll:SetScrollValue(
        GetScrollPosition(LISTMODE.PURCHASE_FAITH))
    PopulateList(data, LISTMODE.PROD_QUEUE, m_queueListIM)
    Controls.QueueListScroll:SetScrollValue(
        GetScrollPosition(LISTMODE.PROD_QUEUE))
...
function ResizeProductionScrollList()
    local contentOffset = Controls.TabContainer:GetSizeY() +
                              Controls.TabContainer:GetOffsetY()
...
-- ===========================================================================
function OnTopStackContainerSizeChanged() ResizeProductionScrollList() end

-- ===========================================================================
function OnTabContainerSizeChanged() ResizeProductionScrollList() end
...
function PopulateCurrentProduction(data)
    m_hasProductionToShow = RefreshCurrentProduction(Controls, data.Owner,
                                                     data.City:GetID())
end
...
function PopulateWonders(data, listMode, listIM)
    ...
    if (wonderList.wonderListIM ~= nil) then
        ...
        wonderList.wonderListIM = InstanceManager:new("BuildingListInstance",
                                                      "Root", wonderList.List)
    end
    ...
    for i, item in ipairs(data.BuildingItems) do
        ...
        if shouldShow then
            ...
            if numberOfTurns == -1 then
                ...
                turnsStrTT = numberOfTurns ..
                                 Locale.Lookup("LOC_HUD_CITY_TURNS_TO_COMPLETE",
                                               item.TurnsLeft)
            end
            ...
            wonderListing.Button:RegisterCallback(Mouse.eMouseEnter, function()
    ...
    if (wonderList.wonderListIM.m_iAllocatedInstances <= 0) then
        ...
        wonderList.Header:RegisterCallback(Mouse.eMouseEnter, function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end)
        wonderList.Header:RegisterCallback(Mouse.eLClick,
                                           function()
            OnExpand(wonderList)
        end)
        wonderList.HeaderOn:RegisterCallback(Mouse.eMouseEnter, function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end)
    ...
    wonderList.Top:RegisterSizeChanged(function()
...
function PopulateProjects(data, listMode, listIM)
    ...
    if (projectList.projectListIM ~= nil) then
        ...
        projectList.projectListIM = InstanceManager:new("ProjectListInstance",
                                                        "Root", projectList.List)
    end
    ...
    for i, item in ipairs(data.ProjectItems) do
        ...
        if listMode ~= LISTMODE.PROD_QUEUE then
            ...
            shouldShow = CuiIsProjectRepeatable(item) or
                             not CuiIsProjectInQueue(item)
        end
        --
        if shouldShow then
            ...
            local numberOfTurns = item.TurnsLeft
            if numberOfTurns == -1 then numberOfTurns = "999+" end
            ...
            if not m_isTutorialRunning then
                ...
                projectListing.Button:SetTag(
                    UITutorialManager:GetHash(item.Type))
            end
        end
    end
...
function PopulateDistrictsWithNestedBuildings(data, listMode, listIM)
    ...
    if (districtList.districtListIM ~= nil) then
        ...
        districtList.districtListIM = InstanceManager:new(
                                          "DistrictListInstance", "Root",
                                          districtList.List)
    end
    ...
    for i, item in ipairs(data.DistrictItems) do
        ...
        if (GameInfo.DistrictReplaces[item.Type] ~= nil) then
            districtType = GameInfo.DistrictReplaces[item.Type]
        ...
        if shouldShow then
            ...
            if (item.HasBeenBuilt and GameInfo.Districts[item.Type].OnePerCity ==
                true and not item.Repair and not item.Contaminated and
                item.Progress == 0) then
                turnsStrTT = TXT_HUD_CITY_DISTRICT_BUILT_TT
                turnsStr = "[ICON_Checkmark]"
            elseif item.ContaminatedTurns > 0 then
                turnsStrTT = Locale.Lookup("LOC_TOOLTIP_PLOT_CONTAMINATED_TEXT",
                                           item.ContaminatedTurns)
                ...
                if numberOfTurns == -1 then
                    ...
                    turnsStrTT = numberOfTurns ..
                                     Locale.Lookup(
                                         "LOC_HUD_CITY_TURNS_TO_COMPLETE",
                                         item.TurnsLeft)
                end
                turnsStr = numberOfTurns .. "[ICON_Turn]"
            end
            ...
            local uniqueBuildingAreaName =
                BUILDING_DRAWER_PREFIX .. districtType

            table.insert(m_TypeNames, districtType)

            if districtList[uniqueBuildingIMName] == nil then
                districtList[uniqueBuildingIMName] =
                    InstanceManager:new("BuildingListInstance", "Root",
                                        districtListing.BuildingStack)
            else
                districtList[uniqueBuildingIMName]:ResetInstances()
                districtList[uniqueBuildingIMName].m_ParentControl =
                    districtListing.BuildingStack
            end

            districtList[uniqueBuildingAreaName] =
                districtListing.BuildingDrawer
            districtListing.CompletedArea:SetHide(true)

            if (item.Disabled) then
                if (item.HasBeenBuilt and
                    GameInfo.Districts[item.Type].OnePerCity == true and
                    not item.Repair) then
                    districtListing.CompletedArea:SetHide(false)
                    districtListing.Disabled:SetHide(true)
                end
            end

            districtListing.Button:RegisterCallback(Mouse.eMouseEnter,
                                                    function()
                UI.PlaySound("Main_Menu_Mouse_Over")
            end)
            ...
            if not m_isTutorialRunning then
                districtListing.Button:RegisterCallback(Mouse.eRClick,
                                                        function()
                    LuaEvents.OpenCivilopedia(item.Type)
                end)
            else
    ...
    if (districtList.districtListIM.m_iAllocatedInstances <= 0) then
        ...
        districtList.Header:RegisterCallback(Mouse.eMouseEnter, function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end)
        districtList.HeaderOn:RegisterCallback(Mouse.eLClick, function()
            OnCollapse(districtList)
        end)
        districtList.HeaderOn:RegisterCallback(Mouse.eMouseEnter, function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end)
    end
    ...
    for i, buildingItem in ipairs(data.BuildingItems) do
        ...
        --
        if shouldShow then
            local uniqueDrawerName = BUILDING_DRAWER_PREFIX ..
                                         buildingItem.PrereqDistrict
            local uniqueIMName = BUILDING_IM_PREFIX ..
                                     buildingItem.PrereqDistrict
            ...
            if (pDistrictBuildingIM ~= nil) then
                ...
                local districtBuildingAreaControl =
                    districtList[uniqueDrawerName]
                ...
                if numberOfTurns == -1 then
                    ...
                    turnsStrTT = numberOfTurns ..
                                     Locale.Lookup(
                                         "LOC_HUD_CITY_TURNS_TO_COMPLETE",
                                         buildingItem.TurnsLeft)
                end
                turnsStr = numberOfTurns .. "[ICON_Turn]"
                buildingListing.CostText:SetToolTipString(turnsStrTT)
                buildingListing.CostText:SetText(turnsStr)

                buildingListing.Button:RegisterCallback(Mouse.eMouseEnter,
                                                        function()
                    UI.PlaySound("Main_Menu_Mouse_Over")
                end)
                buildingListing.Button:RegisterCallback(Mouse.eLClick,
                                                        function()
                    BuildBuilding(data.City, buildingItem)
                end)

                if not m_isTutorialRunning then
                    buildingListing.Button:RegisterCallback(Mouse.eRClick,
                                                            function()
                        LuaEvents.OpenCivilopedia(buildingItem.Type)
                    end)
                else
                    buildingListing.Button:SetTag(
                        UITutorialManager:GetHash(buildingItem.Type))
                end
            end
        end
    end

    districtList.Top:RegisterSizeChanged(
        function()
...
function PopulateDistrictsWithoutNestedBuildings(data, listMode, listIM)
    ...
    for i, item in ipairs(data.DistrictPurchases) do
        if ((item.Yield == "YIELD_GOLD" and listMode == LISTMODE.PURCHASE_GOLD) or
            (item.Yield == "YIELD_FAITH" and listMode == LISTMODE.PURCHASE_FAITH)) then
            local districtListing = districtList.districtListIM:GetInstance()
            ...
            if (item.Yield == "YIELD_GOLD") then
                costStr = Locale.Lookup("LOC_PRODUCTION_PURCHASE_GOLD_TEXT",
                                        item.Cost)
            else
                costStr = Locale.Lookup("LOC_PRODUCTION_PURCHASE_FAITH_TEXT",
                                        item.Cost)
            end
    ...
    if (buildingList.buildingListIM ~= nil) then
        ...
        buildingList.buildingListIM = InstanceManager:new(
                                          "BuildingListInstance", "Root",
                                          buildingList.List)
    end
    ...
    if (buildingList.buildingListIM.m_iAllocatedInstances <= 0) then
        ...
        m_maxPurchaseSize = m_maxPurchaseSize + HEADER_Y + SEPARATOR_Y
        buildingList.Header:RegisterCallback(Mouse.eMouseEnter, function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end)
        buildingList.Header:RegisterCallback(Mouse.eLClick, function()
            OnExpand(buildingList)
        end)
        buildingList.HeaderOn:RegisterCallback(Mouse.eMouseEnter, function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end)
...
function PopulateUnits(data, listMode, listIM)
    ...
    local primaryColor, secondaryColor =
        UI.GetPlayerColors(Players[Game.GetLocalPlayer()]:GetID())
    ...
    if (unitList.unitListIM ~= nil) then
        ...
        unitList.unitListIM = InstanceManager:new("UnitListInstance", "Root",
                                                  unitList.List)
    end
    if (unitList.civilianListIM ~= nil) then
        unitList.civilianListIM:ResetInstances()
    else
        unitList.civilianListIM = InstanceManager:new("CivilianListInstance",
                                                      "Root", unitList.List)
    end
    ...
    for i, item in ipairs(unitData) do
        local unitListing
        if ((item.Yield == "YIELD_GOLD" and listMode == LISTMODE.PURCHASE_GOLD) or
            (item.Yield == "YIELD_FAITH" and listMode == LISTMODE.PURCHASE_FAITH) or
            (listMode == LISTMODE.PRODUCTION and item.IsCurrentProduction ==
                false) or listMode == LISTMODE.PROD_QUEUE) then
            ...
            local costStr = ""
            local costStrTT = ""
            if (listMode == LISTMODE.PRODUCTION or listMode ==
                LISTMODE.PROD_QUEUE) then
                ...
                if (item.Yield == "YIELD_GOLD") then
                    ...
                    costStr = Locale.Lookup(
                                  "LOC_PRODUCTION_PURCHASE_FAITH_TEXT",
                                  item.Cost)
                end
                if item.CantAfford then
                    costStr = "[COLOR:Red]" .. costStr .. "[ENDCOLOR]"
            ...
            if unitListing.ReligionIcon then
                ...
                if item.ReligiousStrength and item.ReligiousStrength > 0 then
                    if unitListing.ReligionIcon then
                        local religionType =
                            data.City:GetReligion():GetMajorityReligion()
                        if religionType > 0 then
                            local religion = GameInfo.Religions[religionType]
                            local religionIcon =
                                "ICON_" .. religion.ReligionType
                            local religionColor =
                                UI.GetColorValue(religion.Color)
                            local religionName =
                                Game.GetReligion():GetName(religion.Index)

                            unitListing.ReligionIcon:SetIcon(religionIcon)
                            unitListing.ReligionIcon:SetColor(religionColor)
                            unitListing.ReligionIcon:LocalizeAndSetToolTip(
                                religionName)
            ...
            unitListing.Button:RegisterCallback(Mouse.eMouseEnter, function()
                UI.PlaySound("Main_Menu_Mouse_Over")
            end)
            if (listMode == LISTMODE.PRODUCTION or listMode ==
                LISTMODE.PROD_QUEUE) then
            ...
            if item.Corps or item.Army then
                ...
                unitListing.CorpsArmyArrow:RegisterCallback(Mouse.eLClick,
                                                            function()
                    OnCorpsToggle(unitList, unitListing)
                end)
                unitListing.CorpsArmyDropdownButton:RegisterCallback(
                    Mouse.eLClick,
                    function()
                        OnCorpsToggle(unitList, unitListing)
                    end)
            ...
            if item.Corps then
                ...
                if (listMode == LISTMODE.PRODUCTION or listMode ==
                    ...
                    if (item.Progress > 0) then
                        ...
                        if (unitProgress < 1) then
                            unitListing.ProductionCorpsProgress:SetPercent(
                                unitProgress)
                        else
                    ...
                    local turnsStr, turnsStrTT =
                        GetTurnsToCompleteStrings(item.CorpsTurnsLeft)
                    ...
                    if (item.Yield == "YIELD_GOLD") then
                        costStr = Locale.Lookup(
                                      "LOC_PRODUCTION_PURCHASE_GOLD_TEXT",
                                      item.CorpsCost)
                    else
                        costStr = Locale.Lookup(
                                      "LOC_PRODUCTION_PURCHASE_FAITH_TEXT",
                ...
                if (listMode == LISTMODE.PRODUCTION or listMode ==
                    LISTMODE.PROD_QUEUE) then
                    unitListing.TrainCorpsButton:RegisterCallback(Mouse.eLClick,
                                                                  function()
            ...
            if item.Army then
                ...
                if (listMode == LISTMODE.PRODUCTION or listMode ==
                    ...
                    if (item.Progress > 0) then
                        ...
                        unitListing.ProductionArmyProgress:SetPercent(
                            unitProgress)
                        if (unitProgress < 1) then
                            unitListing.ProductionArmyProgress:SetPercent(
                                unitProgress)
                    ...
                    local turnsStr, turnsStrTT =
                        GetTurnsToCompleteStrings(item.ArmyTurnsLeft)
                ...
                if (listMode == LISTMODE.PRODUCTION or listMode ==
                    LISTMODE.PROD_QUEUE) then
                    unitListing.TrainArmyButton:RegisterCallback(Mouse.eLClick,
                                                                 function()
    ...
    if (unitList.unitListIM.m_iAllocatedInstances <= 0 and
        unitList.civilianListIM.m_iAllocatedInstances <= 0) then
        unitList.Top:SetHide(true)
    else
        unitList.Header:RegisterCallback(Mouse.eLClick,
                                         function() OnExpand(unitList) end)
        unitList.HeaderOn:RegisterCallback(Mouse.eLClick,
                                           function()
            OnCollapse(unitList)
        end)
    end

    unitList.Top:RegisterSizeChanged(function()
...
function PopulateList(data, listMode, listIM)
    ...
    if (listMode == LISTMODE.PURCHASE_FAITH or listMode ==
        LISTMODE.PURCHASE_GOLD) then
...
-- ===========================================================================
function OnLocalPlayerChanged() Refresh() end
    ...
    if isDisabled and results ~= nil then
        ...
        if pFailureReasons ~= nil and table.count(pFailureReasons) > 0 then
            ...
            for i, v in ipairs(pFailureReasons) do
                allReasons = allReasons .. "[NEWLINE][NEWLINE][COLOR:Red]" ..
                                 Locale.Lookup(v) .. "[ENDCOLOR]"
            end
            return allReasons
        end
    end
...
function ComposeProductionCostString(iProductionProgress, iProductionCost)
    ...
    if iProductionCost ~= 0 then
        ...
        return "[NEWLINE][NEWLINE]" .. TXT_COST .. ": " .. costString ..
                   " [ICON_Production] " .. TXT_PRODUCTION
    end
    return ""
end
...
function ComposeUnitCorpsStrings(unit, iProdProgress, pBuildQueue)
    ...
    local tooltip = ToolTipHelper.GetUnitToolTip(unit.Hash,
                                                 MilitaryFormationTypes.CORPS_MILITARY_FORMATION,
                                                 pBuildQueue)
    ...
    tooltip = tooltip .. ComposeProductionCostString(iProdProgress,
                                                     pBuildQueue:GetUnitCorpsCost(
                                                         unit.Index))
...
function ComposeUnitForPurchase(row, pCity, sYield, pYieldSource, sCantAffordKey)
    ...
    if CityManager.CanStartCommand(pCity, CityCommandTypes.PURCHASE, true,
                                   tParameters, false) then
        local isCanStart, results = CityManager.CanStartCommand(pCity,
        ...
        local sToolTip = ToolTipHelper.GetUnitToolTip(row.Hash,
                                                      MilitaryFormationTypes.STANDARD_MILITARY_FORMATION,
                                                      pCity:GetBuildQueue()) ..
                             allReasons
        ...
        local TXT_INSUFFIENT_YIELD = "[NEWLINE][NEWLINE][COLOR:Red]" ..
                                         Locale.Lookup(sCantAffordKey) ..
                                         "[ENDCOLOR]"
        ...
        sToolTip = sToolTip .. "[NEWLINE]---" ..
                       ComposeProductionCostString(nProductionProgress,
                                                   nProductionCost)
        ...
        local kUnit = {
            ...
            Yield = sYield,
            Cost = pCityGold:GetPurchaseCost(YIELD_TYPE, row.Hash,
                                             MilitaryFormationTypes.STANDARD_MILITARY_FORMATION),
        ...
        if results ~= nil then
            ...
            if kUnit.Corps then
                kUnit.CorpsCost = pCityGold:GetPurchaseCost(YIELD_TYPE,
                                                            row.Hash,
                                                            MilitaryFormationTypes.CORPS_MILITARY_FORMATION)
                kUnit.CorpsTooltip, kUnit.CorpsName =
                    ComposeUnitCorpsStrings(row, nProdProgress, pBuildQueue)
                kUnit.CorpsDisabled = not pYieldSource:CanAfford(nCityID,
                                                                 row.Hash,
                                                                 MilitaryFormationTypes.CORPS_MILITARY_FORMATION)
                if kUnit.CorpsDisabled then
                    kUnit.CorpsTooltip =
                        kUnit.CorpsTooltip .. TXT_INSUFFIENT_YIELD
                end
                tParameters[CityCommandTypes.PARAM_MILITARY_FORMATION_TYPE] =
                    MilitaryFormationTypes.CORPS_MILITARY_FORMATION
                local bCanPurchase, kResults =
                    CityManager.CanStartCommand(pCity,
                                                CityCommandTypes.PURCHASE,
                                                false, tParameters, true)
                kUnit.CorpsDisabled = not bCanPurchase
                if (not bCanPurchase) then
                    local sFailureReasons =
                        ComposeFailureReasonStrings(kUnit.CorpsDisabled,
                                                    kResults)
            ...
            if kUnit.Army then
                kUnit.ArmyCost = pCityGold:GetPurchaseCost(YIELD_TYPE, row.Hash,
                                                           MilitaryFormationTypes.ARMY_MILITARY_FORMATION)
                kUnit.ArmyTooltip, kUnit.ArmyName =
                    ComposeUnitArmyStrings(row, nProdProgress, pBuildQueue)
                kUnit.ArmyDisabled = not pYieldSource:CanAfford(nCityID,
                                                                row.Hash,
                                                                MilitaryFormationTypes.ARMY_MILITARY_FORMATION)
                if kUnit.ArmyDisabled then
                    kUnit.ArmyTooltip = kUnit.ArmyTooltip ..
                                            TXT_INSUFFIENT_YIELD
                end
                tParameters[CityCommandTypes.PARAM_MILITARY_FORMATION_TYPE] =
                    MilitaryFormationTypes.ARMY_MILITARY_FORMATION
                local bCanPurchase, kResults =
                ...
                if (not bCanPurchase) then
                    local sFailureReasons =
                        ComposeFailureReasonStrings(kUnit.ArmyDisabled, kResults)
...
function ComposeBldgForPurchase(pRow, pCity, sYield, pYieldSource,
                                sCantAffordKey)
    local YIELD_TYPE = GameInfo.Yields[sYield].Index
    ...
    if CityManager.CanStartCommand(pCity, CityCommandTypes.PURCHASE, true,
                                   tParameters, false) then
        local isCanStart, pResults = CityManager.CanStartCommand(pCity,
                                                                 CityCommandTypes.PURCHASE,
                                                                 false,
                                                                 tParameters,
                                                                 true)
        ...
        local sToolTip = ToolTipHelper.GetBuildingToolTip(pRow.Hash, playerID,
                                                          pCity) .. sAllReasons
        local isCantAfford = false

        -- Affordability check
        if not pYieldSource:CanAfford(pCity:GetID(), pRow.Hash) then
            sToolTip = sToolTip .. "[NEWLINE][NEWLINE][COLOR:Red]" ..
                           Locale.Lookup(sCantAffordKey) .. "[ENDCOLOR]"
            isDisabled = true
            isCantAfford = true
        end
        ...
        sToolTip = sToolTip ..
                       ComposeProductionCostString(iProductionProgress,
                                                   iProductionCost)
...
function ComposeDistrictForPurchase(pRow, pCity, sYield, pYieldSource,
                                    sCantAffordKey)
    local YIELD_TYPE = GameInfo.Yields[sYield].Index
    ...
    if CityManager.CanStartCommand(pCity, CityCommandTypes.PURCHASE, true,
        ...
        local isDisabled = not isCanStart
        local sAllReasons = ComposeFailureReasonStrings(isDisabled, pResults)
        local sToolTip = ToolTipHelper.GetDistrictToolTip(pRow.Hash) ..
                             sAllReasons
        local isCantAfford = false
...
function GetData()
    ...
    local new_data = {
        ...
        Damage = pPlayer:GetDistricts():FindID(pSelectedCity:GetDistrictID())
            :GetDamage(),
    ...
    for row in GameInfo.Districts() do
        if row.Hash == m_CurrentProductionHash then
            ...
            if (GameInfo.DistrictReplaces[row.DistrictType] ~= nil) then
                new_data.CurrentProductionType =
                    GameInfo.DistrictReplaces[row.DistrictType]
                        .ReplacesDistrictType
            else
        ...
        local isInPanelList = (row.Hash ~= m_CurrentProductionHash or
                                  not row.OnePerCity) and not row.InternalOnly
        local bHasProducedDistrict = cityDistricts:HasDistrict(row.Index)
        if isInPanelList and
            (buildQueue:CanProduce(row.Hash, true) or bHasProducedDistrict) then
            local isCanProduceExclusion, results =
                buildQueue:CanProduce(row.Hash, false, true)
            ...
            local plots = GetCityRelatedPlotIndexesDistrictsAlternative(
                              pSelectedCity, row.Hash)
            if plots == nil or table.count(plots) == 0 then
                ...
                elseif results ~= nil then
                    local pFailureReasons =
                        results[CityCommandResults.FAILURE_REASONS]
                    if pFailureReasons ~= nil and table.count(pFailureReasons) >
                        0 then
                        for i, v in ipairs(pFailureReasons) do
            ...
            local allReasons = ComposeFailureReasonStrings(isDisabled, results)
            local sToolTip = ToolTipHelper.GetToolTip(row.DistrictType,
                                                      Game.GetLocalPlayer()) ..
                                 allReasons

            local iProductionCost = buildQueue:GetDistrictCost(row.Index)
            local iProductionProgress =
                buildQueue:GetDistrictProgress(row.Index)
            sToolTip = sToolTip ..
                           ComposeProductionCostString(iProductionProgress,
                                                       iProductionCost)
            ...
            if bIsContaminated then
                for _, pDistrict in cityDistricts:Members() do
                    ...
                    if kDistrictDef.PrimaryKey == row.DistrictType then
                        ...
                        local pDistrictPlot =
                            Map.GetPlot(pDistrict:GetX(), pDistrict:GetY())
                        iContaminatedTurns =
                            kFalloutManager:GetFalloutTurnsRemaining(
                                pDistrictPlot:GetIndex())
                    end
                end
            end
        ...
        local isAllowed, kDistrict = ComposeDistrictForPurchase(row,
                                                                pSelectedCity,
                                                                "YIELD_GOLD",
                                                                playerTreasury,
    ...
    for row in GameInfo.Buildings() do
        ...
        if row.Hash ~= m_CurrentProductionHash and
            (not row.MustPurchase or cityBuildings:IsPillaged(row.Hash)) and
            bCanProduce then
            local isCanStart, results = buildQueue:CanProduce(row.Hash, false,
                                                              true)
            local isDisabled = not isCanStart

            -- Did it fail and it is a Wonder?  If so, if it failed because of *just* NO_SUITABLE_LOCATION, we can look for an alternate.
            if (isDisabled and row.IsWonder and results ~= nil and
                results[CityOperationResults.NO_SUITABLE_LOCATION] ~= nil and
                results[CityOperationResults.NO_SUITABLE_LOCATION] == true) then
                local pPurchaseablePlots =
                    GetCityRelatedPlotIndexesWondersAlternative(pSelectedCity,
                                                                row.Hash)
            ...
            local allReasons = ComposeFailureReasonStrings(isDisabled, results)
            local sToolTip = ToolTipHelper.GetBuildingToolTip(row.Hash,
                                                              playerID,
                                                              pSelectedCity) ..
                                 allReasons
            ...
            local iProductionProgress =
                buildQueue:GetBuildingProgress(row.Index)
            sToolTip = sToolTip ..
                           ComposeProductionCostString(iProductionProgress,
                                                       iProductionCost)
        ...
        if row.PurchaseYield == "YIELD_GOLD" then
            local isAllowed, kBldg = ComposeBldgForPurchase(row, pSelectedCity,
                                                            "YIELD_GOLD",
                                                            playerTreasury,
                                                            "LOC_BUILDING_INSUFFICIENT_FUNDS")
            if isAllowed then
        ...
        -- Can it be purchased with faith?
        if row.PurchaseYield == "YIELD_FAITH" or
            cityGold:IsBuildingFaithPurchaseEnabled(row.Hash) then
            local isAllowed, kBldg = ComposeBldgForPurchase(row, pSelectedCity,
                                                            "YIELD_FAITH",
                                                            playerReligion,
                                                            "LOC_BUILDING_INSUFFICIENT_FAITH")
    ...
    table.sort(new_data.BuildingItems, function(a, b)
        if a.IsWonder then return false end
        if a.Disabled == false and b.Disabled == true then return true end
        return false
    end)
    ...
    for row in GameInfo.Units() do
        ...
        kBuildParameters.MilitaryFormationType =
            MilitaryFormationTypes.STANDARD_MILITARY_FORMATION

        -- Can it be built normally?
        if not row.MustPurchase and
            buildQueue:CanProduce(kBuildParameters, true) then
            local isCanProduceExclusion, results =
                buildQueue:CanProduce(kBuildParameters, false, true)
            local isDisabled = not isCanProduceExclusion
            ...
            if results ~= nil then
                if results[CityOperationResults.CAN_TRAIN_CORPS] then
                    kBuildParameters.MilitaryFormationType =
                        MilitaryFormationTypes.CORPS_MILITARY_FORMATION
                    local bCanProduceCorps, kResults =
                        buildQueue:CanProduce(kBuildParameters, false, true)
                    kUnit.Corps = true
                    kUnit.CorpsDisabled = not bCanProduceCorps
                    kUnit.CorpsCost = buildQueue:GetUnitCorpsCost(row.Index)
                    kUnit.CorpsTurnsLeft =
                        buildQueue:GetTurnsLeft(row.Hash,
                                                MilitaryFormationTypes.CORPS_MILITARY_FORMATION)
                    kUnit.CorpsTooltip, kUnit.CorpsName =
                        ComposeUnitCorpsStrings(row, nProductionProgress,
                                                buildQueue)
                    local sFailureReasons =
                        ComposeFailureReasonStrings(kUnit.CorpsDisabled,
                                                    kResults)
                ...
                if results[CityOperationResults.CAN_TRAIN_ARMY] then
                    kBuildParameters.MilitaryFormationType =
                        MilitaryFormationTypes.ARMY_MILITARY_FORMATION
                    local bCanProduceArmy, kResults =
                        buildQueue:CanProduce(kBuildParameters, false, true)
                    kUnit.Army = true
                    ...
                    kUnit.ArmyTurnsLeft =
                        buildQueue:GetTurnsLeft(row.Hash,
                                                MilitaryFormationTypes.ARMY_MILITARY_FORMATION)
                    kUnit.ArmyTooltip, kUnit.ArmyName =
                        ComposeUnitArmyStrings(row, nProductionProgress,
                                               buildQueue)
                    local sFailureReasons =
                        ComposeFailureReasonStrings(kUnit.ArmyDisabled, kResults)
                    kUnit.ArmyTooltip = kUnit.ArmyTooltip .. sFailureReasons
        ...
        -- Can it be purchased with gold?
        if row.PurchaseYield == "YIELD_GOLD" then
            local isAllowed, kUnit = ComposeUnitForPurchase(row, pSelectedCity,
                                                            "YIELD_GOLD",
                                                            playerTreasury,
                                                            "LOC_BUILDING_INSUFFICIENT_FUNDS")
            if isAllowed then
        ...
        -- Can it be purchased with faith?
        if row.PurchaseYield == "YIELD_FAITH" or
            cityGold:IsUnitFaithPurchaseEnabled(row.Hash) then
            local isAllowed, kUnit = ComposeUnitForPurchase(row, pSelectedCity,
                                                            "YIELD_FAITH",
                                                            playerReligion,
                                                            "LOC_BUILDING_INSUFFICIENT_FAITH")
    ...
    if (pBuildQueue == nil) then pBuildQueue = pSelectedCity:GetBuildQueue() end
    ...
    for row in GameInfo.Projects() do
        ...
        if buildQueue:CanProduce(row.Hash, true) then
            ...
            local allReasons = ComposeFailureReasonStrings(isDisabled, results)
            local sToolTip = ToolTipHelper.GetProjectToolTip(row.Hash) ..
                                 allReasons
...
-- ===========================================================================
function OnProductionClose() if not ContextPtr:IsHidden() then Close() end end
...
--	LUA Event
-- ===========================================================================
function OnTutorialProductionOpen() Open() end

-- ===========================================================================
--	LUA Event
-- ===========================================================================
function OnProductionOpenForQueue() Open() end

-- ===========================================================================
--	LUA Event
-- ===========================================================================
function OnCityPanelPurchasePlot() Close() end
...
function CreateCorrectTabs()
    ...
    local purchaseFaithLabelX = Controls.PurchaseFaithTab:GetTextControl()
                                    :GetSizeX()
    ...
    Controls.ProductionTab:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
    Controls.PurchaseTab:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
    Controls.PurchaseFaithTab:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
    Controls.MiniProductionTab:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
    Controls.MiniPurchaseTab:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
    Controls.MiniPurchaseFaithTab:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
    ...
    m_tabs = CreateTabs(Controls.TabRow, tabSizeX,
...
function GetScrollPosition(modeIndex)
    local scrollPosTable = m_PlayerScrollPositions[Game.GetLocalPlayer()]
    if scrollPosTable ~= nil then return scrollPosTable[modeIndex] end
...
function CloseAfterNewProduction()
    if not m_isQueueOpen and not m_isManagerOpen then Close() end
end
...
function RefreshQueue(playerID, cityID)
    if m_isTutorialRunning or m_tutorialTestMode then return end
    ...
    for i = 1, MAX_QUEUE_SIZE do
        ...
        local queueInstance = CreateQueueInstance(m_QueueInstanceIM,
                                                  Controls.QueueStack, playerID,
                                                  cityID, i, entry)
        queueInstance.Top:RegisterCallback(Mouse.eLClick, function()
            OnItemClicked(queueInstance, queueInstance.Top)
        end)
        queueInstance.Top:RegisterCallback(Mouse.eRClick,
                                           function() RemoveQueueItem(i) end)
...
function OnItemClicked(kParentControl, kButtonControl)
    ...
    elseif m_kSelectedQueueItem.Index ~= kParentControl[FIELD_QUEUE_INDEX] then
        SwapQueueItem(m_kSelectedQueueItem.Index,
                      kParentControl[FIELD_QUEUE_INDEX])
    else
        DeselectItem()
    end
end
...
function OnBuildingListSizeChanged(listIM, sizeY)
    ...
    if kListParent then kListParent[FIELD_LIST_BUILDING_SIZE_Y] = sizeY end
end

-- ===========================================================================
function OnWonderListSizeChanged(listIM, sizeY)
    local kListParent = listIM.m_ParentControl
    if kListParent then kListParent[FIELD_LIST_WONDER_SIZE_Y] = sizeY end
end

-- ===========================================================================
function OnUnitListSizeChanged(listIM, sizeY)
    local kListParent = listIM.m_ParentControl
    if kListParent then kListParent[FIELD_LIST_UNIT_SIZE_Y] = sizeY end
end
...
function OnWondersButtonClicked()
    if m_CurrentListMode == LISTMODE.PRODUCTION then
        ScrollToWonderList(Controls.ProductionList,
                           Controls.ProductionListScroll)
    elseif m_CurrentListMode == LISTMODE.PROD_QUEUE then
...
function ScrollToWonderList(kListStack, kListScroll)
    local desiredScrollValue = kListStack[FIELD_LIST_BUILDING_SIZE_Y] /
                                   (kListStack:GetSizeY() -
                                       kListScroll:GetSizeY())
...
function OnUnitsButtonClicked()
    ...
    elseif m_CurrentListMode == LISTMODE.PURCHASE_FAITH then
        ScrollToUnitList(Controls.PurchaseFaithList,
                         Controls.PurchaseFaithListScroll)
    end
end
...
function ScrollToUnitList(kListStack, kListScroll)
    local desiredScrollValue = (kListStack[FIELD_LIST_BUILDING_SIZE_Y] +
                                   kListStack[FIELD_LIST_WONDER_SIZE_Y]) /
                                   (kListStack:GetSizeY() -
                                       kListScroll:GetSizeY())
    kListScroll:SetScrollValue(desiredScrollValue)
end
...
function OnProjectsButtonClicked()
    if m_CurrentListMode == LISTMODE.PRODUCTION then
        ScrollToProjectList(Controls.ProductionList,
                            Controls.ProductionListScroll)
    elseif m_CurrentListMode == LISTMODE.PROD_QUEUE then
...
function ScrollToProjectList(kListStack, kListScroll)
    local desiredScrollValue = (kListStack[FIELD_LIST_BUILDING_SIZE_Y] +
                                   kListStack[FIELD_LIST_WONDER_SIZE_Y] +
                                   kListStack[FIELD_LIST_UNIT_SIZE_Y]) /
                                   (kListStack:GetSizeY() -
                                       kListScroll:GetSizeY())
    kListScroll:SetScrollValue(desiredScrollValue)
end
...
function GetBuildInsertMode(tParameters)
    if m_isQueueOpen or m_isManagerOpen then
        if IsBuildQueueFull(city) then
            ...
            tParameters[CityOperationTypes.PARAM_INSERT_MODE] =
                CityOperationTypes.VALUE_REPLACE_AT
            tParameters[CityOperationTypes.PARAM_QUEUE_DESTINATION_LOCATION] =
                MAX_QUEUE_SIZE
        else
            -- Append to end of queue
            tParameters[CityOperationTypes.PARAM_INSERT_MODE] =
                CityOperationTypes.VALUE_APPEND
        end
    else
        tParameters[CityOperationTypes.PARAM_INSERT_MODE] =
            CityOperationTypes.VALUE_REPLACE_AT
...
function IsBuildQueueFull()
    ...
    if pSelectedCity then
        ...
        if pBuildQueue ~= nil then
            ...
            if iQueueSize - 1 >= MAX_QUEUE_SIZE then return true end
        end
    end
        ...
        if pBuildQueue ~= nil then
            for i = 1, MAX_QUEUE_SIZE do
                ...
                if queueEntry and queueEntry.DistrictType then
                    local pDistrictDef =
                        GameInfo.Districts[queueEntry.DistrictType]
        ...
        if pBuildQueue ~= nil then
            for i = 1, MAX_QUEUE_SIZE do
                ...
                if queueEntry and queueEntry.BuildingType then
                    local pBuildingDef =
                        GameInfo.Buildings[queueEntry.BuildingType]
        ...
        if pBuildQueue ~= nil then
            for i = 1, MAX_QUEUE_SIZE do
                ...
                if queueEntry and queueEntry.ProjectType then
                    local pProjectDef =
                        GameInfo.Projects[queueEntry.ProjectType]
...
function CuiInit()
    ...
    -- new version
    Controls.SwitchVersionButton:RegisterCallback(Mouse.eLClick,
                                                  CuiOnNewModeClick)
    CuiNewVersionToggle(cui_newVersion)
end
...
function Initialize()
    ...
    Controls.ProductionListScroll:RegisterScrollCallback(
        OnProductionListScrolled)
    Controls.PurchaseListScroll:RegisterScrollCallback(OnPurchaseListScrolled)
    Controls.PurchaseFaithListScroll:RegisterScrollCallback(
        OnPurchaseFaithListScrolled)
    ...
    Controls.TopStackContainer:RegisterSizeChanged(
        OnTopStackContainerSizeChanged)
    Controls.TabContainer:RegisterSizeChanged(OnTabContainerSizeChanged)
    Controls.BuildingsButton:RegisterCallback(Mouse.eLClick,
                                              OnBuildingsButtonClicked)
    Controls.WondersButton:RegisterCallback(Mouse.eLClick,
                                            OnWondersButtonClicked)
    Controls.UnitsButton:RegisterCallback(Mouse.eLClick, OnUnitsButtonClicked)
    Controls.ProjectsButton:RegisterCallback(Mouse.eLClick,
                                             OnProjectsButtonClicked)
    Controls.CurrentProductionButton:RegisterCallback(Mouse.eLClick, function()
        OnItemClicked(Controls, Controls.CurrentProductionButton)
    end)
    Controls.CurrentProductionButton:RegisterCallback(Mouse.eRClick, function()
        RemoveQueueItem(0)
    end)
    ...
    LuaEvents.CityBannerManager_ProductionToggle.Add(
        OnCityBannerManagerProductionToggle)
    ...
    LuaEvents.NotificationPanel_ChooseProduction.Add(
        OnNotificationPanelChooseProduction)
    LuaEvents.StrageticView_MapPlacement_ProductionOpen.Add(
        OnStrategicViewMapPlacementProductionOpen)
    LuaEvents.Tutorial_ProductionOpen.Add(OnTutorialProductionOpen)
    LuaEvents.ProductionManager_SelectedIndexChanged.Add(
        OnManagerSelectedIndexChanged)
end
Initialize()
