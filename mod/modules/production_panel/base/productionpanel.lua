...
function PopulateWonders(data, listMode, listIM)
    ...
    for i, item in ipairs(data.BuildingItems) do
        ...
        if shouldShow then
            ...
            wonderListing.Button:RegisterCallback(
                Mouse.eMouseEnter,
                function()
    ...
    if (wonderList.wonderListIM.m_iAllocatedInstances <= 0) then
        ...
        wonderList.Header:RegisterCallback(
            Mouse.eMouseEnter,
            function()
                UI.PlaySound("Main_Menu_Mouse_Over")
            end
        ...
        wonderList.HeaderOn:RegisterCallback(
            Mouse.eMouseEnter,
            function()
                UI.PlaySound("Main_Menu_Mouse_Over")
            end
    ...
    wonderList.Top:RegisterSizeChanged(
        function()
...
function PopulateProjects(data, listMode, listIM)
    ...
    for i, item in ipairs(data.ProjectItems) do
        ...
        if shouldShow then
            ...
            projectListing.Button:RegisterCallback(
                Mouse.eLClick,
                function()
    ...
    if (projectList.projectListIM.m_iAllocatedInstances <= 0) then
        ...
        projectList.Header:RegisterCallback(
            Mouse.eLClick,
            function()
                OnExpand(projectList)
            end
        )
        projectList.HeaderOn:RegisterCallback(
            Mouse.eLClick,
            function()
...
function PopulateDistrictsWithNestedBuildings(data, listMode, listIM)
    ...
    for i, item in ipairs(data.DistrictItems) do
        ...
        if shouldShow then
            ...
            if
                (item.HasBeenBuilt and GameInfo.Districts[item.Type].OnePerCity == true and not item.Repair and
                    not item.Contaminated and
                    item.Progress == 0)
             then
            ...
            if districtList[uniqueBuildingIMName] == nil then
                districtList[uniqueBuildingIMName] =
                    InstanceManager:new("BuildingListInstance", "Root", districtListing.BuildingStack)
            else
            ...
            districtListing.Button:RegisterCallback(
                Mouse.eMouseEnter,
                function()
                    UI.PlaySound("Main_Menu_Mouse_Over")
                end
    ...
    if (districtList.districtListIM.m_iAllocatedInstances <= 0) then
        ...
        districtList.Header:RegisterCallback(
            Mouse.eLClick,
            function()
        ...
        districtList.HeaderOn:RegisterCallback(
            Mouse.eLClick,
            function()
        ...
        if shouldShow then
            ...
            if (pDistrictBuildingIM ~= nil) then
                ...
                if numberOfTurns == -1 then
                    ...
                    turnsStrTT =
                        numberOfTurns .. Locale.Lookup("LOC_HUD_CITY_TURNS_TO_COMPLETE", buildingItem.TurnsLeft)
                end
                turnsStr = numberOfTurns .. "[ICON_Turn]"
                buildingListing.CostText:SetToolTipString(turnsStrTT)
                buildingListing.CostText:SetText(turnsStr)

                buildingListing.Button:RegisterCallback(
                    Mouse.eMouseEnter,
                    function()
                        UI.PlaySound("Main_Menu_Mouse_Over")
                    end
...
function PopulateDistrictsWithoutNestedBuildings(data, listMode, listIM)
    ...
    for i, item in ipairs(data.DistrictPurchases) do
        if
            ((item.Yield == "YIELD_GOLD" and listMode == LISTMODE.PURCHASE_GOLD) or
                (item.Yield == "YIELD_FAITH" and listMode == LISTMODE.PURCHASE_FAITH))
         then
            local districtListing = districtList.districtListIM:GetInstance()
    ...
    if (buildingList.buildingListIM.m_iAllocatedInstances <= 0) then
        ...
        m_maxPurchaseSize = m_maxPurchaseSize + HEADER_Y + SEPARATOR_Y
        buildingList.Header:RegisterCallback(
            Mouse.eMouseEnter,
            function()
                UI.PlaySound("Main_Menu_Mouse_Over")
            end
        ...
        buildingList.HeaderOn:RegisterCallback(
            Mouse.eMouseEnter,
            function()
                UI.PlaySound("Main_Menu_Mouse_Over")
            end
...
function PopulateUnits(data, listMode, listIM)
    ...
    for i, item in ipairs(unitData) do
        ...
        if
            ((item.Yield == "YIELD_GOLD" and listMode == LISTMODE.PURCHASE_GOLD) or
                ...
                (listMode == LISTMODE.PRODUCTION and item.IsCurrentProduction == false) or
                listMode == LISTMODE.PROD_QUEUE)
         then
            ...
            unitListing.Button:RegisterCallback(
                Mouse.eMouseEnter,
                function()
                    UI.PlaySound("Main_Menu_Mouse_Over")
                end
            ...
            if item.Corps or item.Army then
                ...
                unitListing.CorpsArmyArrow:RegisterCallback(
                    Mouse.eLClick,
                    function()
            ...
            if item.Corps then
                ...
                if (listMode == LISTMODE.PRODUCTION or listMode == LISTMODE.PROD_QUEUE) then
                    unitListing.TrainCorpsButton:RegisterCallback(
                        Mouse.eLClick,
                        function()
            ...
            if item.Army then
                ...
                if (listMode == LISTMODE.PRODUCTION or listMode == LISTMODE.PROD_QUEUE) then
                    unitListing.TrainArmyButton:RegisterCallback(
                        Mouse.eLClick,
                        function()
    ...
    if (unitList.unitListIM.m_iAllocatedInstances <= 0 and unitList.civilianListIM.m_iAllocatedInstances <= 0) then
        ...
        unitList.Header:RegisterCallback(
            Mouse.eLClick,
            function()
                OnExpand(unitList)
            end
        )
        unitList.HeaderOn:RegisterCallback(
            Mouse.eLClick,
            function()
                OnCollapse(unitList)
            end
    ...
    unitList.Top:RegisterSizeChanged(
        function()
...
function ComposeUnitCorpsStrings(unit, iProdProgress, pBuildQueue)
    local tooltip =
        ToolTipHelper.GetUnitToolTip(unit.Hash, MilitaryFormationTypes.CORPS_MILITARY_FORMATION, pBuildQueue)
...
function ComposeUnitForPurchase(row, pCity, sYield, pYieldSource, sCantAffordKey)
    ...
    if CityManager.CanStartCommand(pCity, CityCommandTypes.PURCHASE, true, tParameters, false) then
        local isCanStart, results =
            CityManager.CanStartCommand(pCity, CityCommandTypes.PURCHASE, false, tParameters, true)
        local isDisabled = not isCanStart
        local allReasons = ComposeFailureReasonStrings(isDisabled, results)
        local sToolTip =
            ToolTipHelper.GetUnitToolTip(
            row.Hash,
        ...
        if results ~= nil then
            ...
            if kUnit.Corps then
                kUnit.CorpsCost =
                    pCityGold:GetPurchaseCost(YIELD_TYPE, row.Hash, MilitaryFormationTypes.CORPS_MILITARY_FORMATION)
                kUnit.CorpsTooltip, kUnit.CorpsName = ComposeUnitCorpsStrings(row, nProdProgress, pBuildQueue)
                kUnit.CorpsDisabled =
                    not pYieldSource:CanAfford(nCityID, row.Hash, MilitaryFormationTypes.CORPS_MILITARY_FORMATION)
            ...
            if kUnit.Army then
                kUnit.ArmyCost =
                    pCityGold:GetPurchaseCost(YIELD_TYPE, row.Hash, MilitaryFormationTypes.ARMY_MILITARY_FORMATION)
                kUnit.ArmyTooltip, kUnit.ArmyName = ComposeUnitArmyStrings(row, nProdProgress, pBuildQueue)
                kUnit.ArmyDisabled =
                    not pYieldSource:CanAfford(nCityID, row.Hash, MilitaryFormationTypes.ARMY_MILITARY_FORMATION)
...
function GetData()
    ...
    for row in GameInfo.Districts() do
        ...
        local isAllowed, kDistrict =
            ComposeDistrictForPurchase(
            row,
            pSelectedCity,
            "YIELD_GOLD",
            playerTreasury,
    ...
    for row in GameInfo.Buildings() do
        ...
        if
            row.Hash ~= m_CurrentProductionHash and (not row.MustPurchase or cityBuildings:IsPillaged(row.Hash)) and
                bCanProduce
         then
            ...
            if
                (isDisabled and row.IsWonder and results ~= nil and
                    results[CityOperationResults.NO_SUITABLE_LOCATION] ~= nil and
                    results[CityOperationResults.NO_SUITABLE_LOCATION] == true)
             then
        ...
        if row.PurchaseYield == "YIELD_GOLD" then
            local isAllowed, kBldg =
                ComposeBldgForPurchase(
                row,
                pSelectedCity,
                "YIELD_GOLD",
                playerTreasury,
    ...
    for row in GameInfo.Units() do
        ...
        if not row.MustPurchase and buildQueue:CanProduce(kBuildParameters, true) then
            ...
            local sToolTip =
                ToolTipHelper.GetUnitToolTip(row.Hash, MilitaryFormationTypes.STANDARD_MILITARY_FORMATION, buildQueue) ..
                sAllReasons
            ...
            if results ~= nil then
                if results[CityOperationResults.CAN_TRAIN_CORPS] then
                    ...
                    kUnit.CorpsTurnsLeft =
                        buildQueue:GetTurnsLeft(row.Hash, MilitaryFormationTypes.CORPS_MILITARY_FORMATION)
                ...
                if results[CityOperationResults.CAN_TRAIN_ARMY] then
                    ...
                    kUnit.ArmyTurnsLeft =
                        buildQueue:GetTurnsLeft(row.Hash, MilitaryFormationTypes.ARMY_MILITARY_FORMATION)
        ...
        -- Can it be purchased with gold?
        if row.PurchaseYield == "YIELD_GOLD" then
            local isAllowed, kUnit =
                ComposeUnitForPurchase(
                row,
                pSelectedCity,
                "YIELD_GOLD",
                playerTreasury,
...
function CreateCorrectTabs()
    ...
    Controls.ProductionTab:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    )
    Controls.PurchaseTab:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    )
    Controls.PurchaseFaithTab:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    )
    Controls.MiniProductionTab:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    )
    Controls.MiniPurchaseTab:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    )
    Controls.MiniPurchaseFaithTab:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
...
function RefreshQueue(playerID, cityID)
    ...
    for i = 1, MAX_QUEUE_SIZE do
        ...
        queueInstance.Top:RegisterCallback(
            Mouse.eLClick,
            function()
...
function ScrollToUnitList(kListStack, kListScroll)
    local desiredScrollValue =
        (kListStack[FIELD_LIST_BUILDING_SIZE_Y] + kListStack[FIELD_LIST_WONDER_SIZE_Y]) /
...
function Initialize()
    ...
    Controls.CurrentProductionButton:RegisterCallback(
        Mouse.eLClick,
        function()
