...
local m_MovementZoneOfControl = UILens.CreateLensLayerHash(
                                    "Movement_Zone_Of_Control")
local m_HexColoringWaterAvail = UILens.CreateLensLayerHash(
                                    "Hex_Coloring_Water_Availablity")
local m_HexColoringReligion =
    UILens.CreateLensLayerHash("Hex_Coloring_Religion")
...
m_HexColoringGreatPeople = UILens.CreateLensLayerHash(
                               "Hex_Coloring_Great_People")
...
function RealizeMoveRadius(kUnit)
    ...
    if kUnit ~= nil and (not GameInfo.Units[kUnit:GetUnitType()].IgnoreMoves) and
        (not UI.IsGameCoreBusy()) then
        ...
        if kUnitInfo ~= nil and (kUnitInfo.Spy or kUnitInfo.MakeTradeRoute) then
            ...
            if not kUnit:HasMovedIntoZOC() then
                ...
                kMovePlots = {}
                kZOCPlots = {}
                table.insert(kMovePlots,
                             Map.GetPlot(kUnit:GetX(), kUnit:GetY()):GetIndex())
            end

            local isShowingTarget = kUnit:GetAttacksRemaining() > 0

            -- Extract attack indicator locations and extensions to movement range
            if isShowingTarget and kAttackPlots ~= nil and
                table.count(kAttackPlots) > 0 then
            ...
            local pResults = UnitManager.GetOperationTargets(kUnit,
                                                             UnitOperationTypes.RANGE_ATTACK)
            local pAllPlots = pResults[UnitOperationResults.PLOTS]
            if pAllPlots ~= nil then
                for i, modifier in ipairs(
                                       pResults[UnitOperationResults.MODIFIERS]) do
                    if modifier == UnitOperationResults.MODIFIER_IS_TARGET then
                        ...
                        if isUnique then
                            table.insert(kAttackIndicators,
                                         {"AttackRange_Target", kPlotId})
                        end
                    end
                end
            end
            ...
            if table.count(kZOCPlots) > 0 or table.count(kAttackIndicators) > 0 then
                UILens.SetLayerHexesArea(m_MovementZoneOfControl, eLocalPlayer,
                                         kZOCPlots, kAttackIndicators)
            end

            -- Lay down movement border around movable and attack-movable hexes
            if kMovePlots ~= nil and table.count(kMovePlots) > 0 then
                UILens.SetLayerHexesArea(m_MovementRange, eLocalPlayer,
                                         kMovePlots)
            end
        end
    end
    ...
    if kUnit ~= nil and (not UI.IsGameCoreBusy()) then
        ...
        if playerID == Game.GetLocalPlayer() then
            ...
            if kUnitGreatPerson ~= nil and kUnitGreatPerson:IsGreatPerson() then
                local greatPersonInfo =
                    GameInfo.GreatPersonIndividuals[kUnitGreatPerson:GetIndividual()]
                -- Highlight an area around the Great Person (if they have an area of effect trait)
                local areaHighlightPlots = {}
                if (greatPersonInfo ~= nil and
                    greatPersonInfo.AreaHighlightRadius ~= nil) then
                    areaHighlightPlots =
                        kUnitGreatPerson:GetAreaHighlightPlots()
                end
                -- Highlight the plots the Great Person could use its action on
                local activationPlots = {}
                if (greatPersonInfo ~= nil and
                    greatPersonInfo.ActionEffectTileHighlighting ~= nil and
                    greatPersonInfo.ActionEffectTileHighlighting) then
                    local rawActivationPlots =
                        kUnitGreatPerson:GetActivationHighlightPlots()
                    for _, plotIndex in ipairs(rawActivationPlots) do
                        table.insert(activationPlots,
                                     {"Great_People", plotIndex})
                    end
                end
                UILens.SetLayerHexesArea(m_HexColoringGreatPeople, playerID,
                                         areaHighlightPlots, activationPlots)
                UILens.ToggleLayerOn(m_HexColoringGreatPeople)
            elseif (kUnitArchaeology ~= nil and
                GameInfo.Units[kUnit:GetUnitType()].ExtractsArtifacts == true) then
                -- Highlight plots that can activated by Archaeologists
                local activationPlots = {}
                local rawActivationPlots =
                    kUnitArchaeology:GetActivationHighlightPlots()
                for _, plotIndex in ipairs(rawActivationPlots) do
...
function OnUnitSelectionChanged(playerID, unitID, hexI, hexJ, hexK, isSelected,
    ...
    if pPlayer ~= nil then
        ...
        if isSelected then
            ...
            if GameInfo.Units[kUnit:GetUnitType()].FoundCity and
                (not m_isDisableWaterAvailLens) and
            ...
            if kUnit ~= nil then
                ...
                if GameInfo.Units[kUnit:GetUnitType()].FoundCity and
                    (not m_isDisableWaterAvailLens) then
...
function OnInterfaceModeChanged(eOldMode, eNewMode)
    if eNewMode == InterfaceModeTypes.VIEW_MODAL_LENS then UI.DeselectAll() end
end
...
function OnCombatVisEnd(aCombatVisData)
    ...
    for _, i in ipairs(aCombatVisData) do
        if i.playerID == localPlayerID then isInvolvesLocalPlayer = true end
    end
    ...
    if isInvolvesLocalPlayer then
        ...
        if pUnit ~= nil and aCombatVisData[CombatVisType.ATTACKER].componentID ==
            pUnit:GetID() and pUnit:GetMovesRemaining() == 0 then
...
function Initialize()
    ...
    LuaEvents.TutorialUIRoot_DisableSettleHintLens.Add(
        OnTutorialUIRoot_DisableSettleHintLens)
end
Initialize()
