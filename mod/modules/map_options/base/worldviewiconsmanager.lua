...
local KEY_CURRENT_ICON_INFO = "currentIcon"
local KEY_PREVIOUS_ICON_INFO = "prevIcon"
local KEY_PLOT_INDEX = "plotIndex"

local g_InstanceManager = InstanceManager:new("IconSetInstance", "Anchor",
                                              Controls.IconContainer)
local g_MapIcons = {}
local m_kUntouchedPlots = {} -- Used to prevent multiple calls to mess with animation start for a plot change
local m_isShowResources = UserConfiguration.ShowMapResources()
local m_isShowRecommendations = true
local m_kSettlerTooltip = {} -- Table of controls for custom tooltip access.

local m_techsThatUnlockResources = {}
local m_civicsThatUnlockResources = {}
local m_techsThatUnlockImprovements = {}
...
local m_RecommendedImprovementPlots = {}
...
local m_RecommendedSettlementPlots = {}
...
function GetStartingPlotPlayer(pPlot)
    ...
    for i = 0, GameDefines.MAX_PLAYERS - 1 do
        ...
        if (playerConfig:IsInUse()) then
            ...
            if (location.x == x and location.y == y) then return i end
        end
    end
...
-- ===========================================================================
function OnEndFade(pInstance)
    local pCurrentIconInfo = pInstance[KEY_CURRENT_ICON_INFO]
    if (pCurrentIconInfo ~= nil) then
        local textureOffsetX = pCurrentIconInfo.textureOffsetX
        local textureOffsetY = pCurrentIconInfo.textureOffsetY
        local textureSheet = pCurrentIconInfo.textureSheet
        pInstance.ResourceIcon:SetTexture(textureOffsetX, textureOffsetY,
                                          textureSheet)
...
-------------------------------------------------------------------------------
function SetResourceIcon(pInstance, pPlot, type, state)
    ...
    if (pPlot and resourceInfo ~= nil) then
        ...
        if (feature) then featureType = feature.FeatureType end

        local terrain = GameInfo.Terrains[pPlot:GetTerrainType()]
        if (terrain) then terrainType = terrain.TerrainType end
        ...
        local textureOffsetX, textureOffsetY, textureSheet =
            IconManager:FindIconAtlas(iconName, 256)
        if (textureSheet ~= nil) then
            pInstance[KEY_PREVIOUS_ICON_INFO] =
                DeepCopy(pInstance[KEY_CURRENT_ICON_INFO])
            if pInstance[KEY_PREVIOUS_ICON_INFO] ~= nil then
                pInstance.ResourceIcon:SetTexture(
                    pInstance[KEY_PREVIOUS_ICON_INFO].textureOffsetX,
                    pInstance[KEY_PREVIOUS_ICON_INFO].textureOffsetY,
                    pInstance[KEY_PREVIOUS_ICON_INFO].textureSheet)
                pInstance.ResourceIcon:SetHide(not m_isShowResources)
            else
                pInstance.ResourceIcon:SetHide(true)
            end
            pInstance.NextResourceIcon:SetTexture(textureOffsetX,
            ...
            if (resourceInfo.ResourceClassType == "RESOURCECLASS_BONUS") then
                table.insert(toolTipItems,
                             Locale.Lookup("LOC_TOOLTIP_BONUS_RESOURCE"))
            elseif (resourceInfo.ResourceClassType == "RESOURCECLASS_LUXURY") then
                table.insert(toolTipItems,
                             Locale.Lookup("LOC_TOOLTIP_LUXURY_RESOURCE"))
            elseif (resourceInfo.ResourceClassType == "RESOURCECLASS_STRATEGIC") then
                table.insert(toolTipItems,
                             Locale.Lookup("LOC_TOOLTIP_STRATEGIC_RESOURCE"))
            elseif (resourceInfo.ResourceClassType == "RESOURCECLASS_ARTIFACT") then
                table.insert(toolTipItems,
                             Locale.Lookup("LOC_TOOLTIP_ARTIFACT_RESOURCE"))
                table.insert(toolTipItems, Locale.Lookup(
                                 "LOC_TOOLTIP_ARTIFACT_RESOURCE_DETAILS"))
            end
            ...
            for row in GameInfo.Improvement_ValidResources() do
                if (row.ResourceType == resourceType) then
                    if (terrainType == "TERRAIN_COAST") then
                        if ("DOMAIN_SEA" ==
                            GameInfo.Improvements[row.ImprovementType].Domain) then
                ...
                if (table.count(tValidImprovements) > 1) then
                    for i, improvement in ipairs(tValidImprovements) do
                        ...
                        if (valid_feature == true and valid_terrain == true) then
                            resourceTechType =
                                GameInfo.Improvements[improvementType]
                                    .PrereqTech
                            resourceCivicType =
                                GameInfo.Improvements[improvementType]
                                    .PrereqCivic
                    ...
                    resourceTechType = GameInfo.Improvements[improvementType]
                                           .PrereqTech
                    resourceCivicType = GameInfo.Improvements[improvementType]
                                            .PrereqCivic
                end
            end
            ...
            if (resourceTechType ~= nil) then
                ...
                if (localPlayer ~= nil) then
                    ...
                    if (techType ~= nil and
                        not playerTechs:HasTech(techType.Index)) then
                        table.insert(toolTipItems,
                                     "[COLOR:Civ6Red](" ..
            ...
            if (resourceCivicType ~= nil) then
                ...
                if (localPlayer ~= nil) then
                    ...
                    if (civicType ~= nil and
                        not playerCulture:HasCivic(civicType.Index)) then
                        table.insert(toolTipItems,
                                     "[COLOR:Civ6Red](" ..
                                         Locale.Lookup("LOC_TOOLTIP_REQUIRES") ..
                                         " " .. Locale.Lookup(civicType.Name) ..
                                         ")[ENDCOLOR]")
                    end
                end
            end
            ...
            pInstance.ResourceIcon:SetToolTipString(
                table.concat(toolTipItems, "[NEWLINE]"))
        end
        -- CUI: toggle improved resource icons
        local cui_ShowImproved = CuiSettings:GetBoolean(
...
function GetInstanceAt(plotIndex)
    ...
    if (pInstance == nil) then
        ...
        pInstance.RecommendationIconTexture:UnloadTexture()
        pInstance.RecommendationIcon:SetHide(true)
        pInstance.AlphaAnim:RegisterEndCallback(
            function() OnEndFade(pInstance) end)
...
-------------------------------------------------------------------------------
function GetInstanceAllocatedAt(plotIndex) return g_MapIcons[plotIndex] end
...
function GetNonEmptyAt(plotIndex, state)
    ...
    local pLocalPlayerVis = PlayerVisibilityManager.GetPlayerVisibility(
                                eObserverID)
    if (pLocalPlayerVis ~= nil) then
        ...
        -- Have a Resource?
        local eResource = pLocalPlayerVis:GetLayerValue(
                              VisibilityLayerTypes.RESOURCES, plotIndex)
        local bHideResource = (pPlot ~= nil and
                                  (pPlot:GetDistrictType() > 0 or pPlot:IsCity()))
        ...
        if (pPlot) then
            -- Starting plot?
            if pPlot:IsStartingPlot() and WorldBuilder.IsActive() and
                WorldBuilder.GetWBAdvancedMode() then
                pInstance = GetInstanceAt(plotIndex)
                pInstance.RecommendationIconTexture:TrySetIcon(
                    "ICON_UNITOPERATION_FOUND_CITY", 256)
...
-------------------------------------------------------------------------------
function RemoveAll(plotIndex) ReleaseInstanceAt(plotIndex) end
...
function ChangeToMidFog(plotIndex)
    ...
    if (pIconSet ~= nil) then
        pIconSet.NextResourceIcon:SetHide(
            (not m_isShowResources) or (not pIconSet.ResourceIcon:HasTexture()))
        pIconSet.RecommendationIcon:SetHide(
            (not m_isShowRecommendations) or
                (not pIconSet.RecommendationIconTexture:HasTexture()))
...
function OnResearchCompleted(player, tech, isCanceled)
...
function OnCivicCompleted(player, civic, isCanceled)
    if player == Game.GetLocalPlayer() then
...
function OnResourceVisibilityChanged(x, y, resourceType, visibilityType)
    ...
    if plotIndex == -1 then return end
...
function OnPlotMarkersChanged(x, y)
    ...
    if (pLocalPlayerVis ~= nil) then
        ...
        local visibilityType = pLocalPlayerVis:GetState(x, y)
        local plotIndex = GetUntouchPlotIndex(x, y)
        if plotIndex == -1 then return end
...
function GetUntouchPlotIndex(x, y)
    ...
    if WorldBuilder.IsActive() then
        ...
        if m_kUntouchedPlots[plotIndex] == nil or m_kUntouchedPlots[plotIndex] ~=
            gameTurn then
...
function RestoreResourceIconState()
    ...
    if (bChangedValue ~= m_isShowResources) then
        ...
        for _, pIconSet in pairs(g_MapIcons) do
            pIconSet.ResourceIcon:SetHide(
                (not m_isShowResources) or
                    (not pIconSet.ResourceIcon:HasTexture()))
            pIconSet.IconStack:CalculateSize()
            pIconSet.IconStack:ReprocessAnchoring()
        end
    end
...
----------------------------------------------------------------
function OnUserOptionsActivated() RestoreResourceIconState() end

-- ===========================================================================
function OnBeginWonderReveal() ContextPtr:SetHide(true) end

-- ===========================================================================
function OnEndWonderReveal() ContextPtr:SetHide(false) end
...
function ClearImprovementRecommendations()
    ...
    for i, plotIndex in ipairs(m_RecommendedImprovementPlots) do
        ...
        pRecommendedPlotInstance.ImprovementRecommendationBackground:SetHide(
            true)
    end
...
-- ===========================================================================
function AddImprovementRecommendationsForCity(pCity, pSelectedUnit)
    local pCityAI = pCity:GetCityAI()
    if pCityAI then
        local recommendList = pCityAI:GetImprovementRecommendationsForBuilder(
                                  pSelectedUnit:GetComponentID())
        for key, value in pairs(recommendList) do
            local pRecommendedPlotInstance =
                GetInstanceAt(value.ImprovementLocation)

            -- Get improvement info
            local pImprovementInfo =
                GameInfo.Improvements[value.ImprovementHash]

            -- Update icon
            pRecommendedPlotInstance.ImprovementRecommendationIcon:TrySetIcon(
                pImprovementInfo.Icon, 256)

            -- Update tooltip
            pRecommendedPlotInstance.ImprovementRecommendationIcon:SetToolTipString(
                Locale.Lookup("LOC_TOOLTIP_IMPROVEMENT_RECOMMENDATION",
                              pImprovementInfo.Name))

            -- Show recommendation and add to list for clean up later
            pRecommendedPlotInstance.ImprovementRecommendationBackground:SetHide(
                false)
            table.insert(m_RecommendedImprovementPlots,
                         value.ImprovementLocation)
        end
    end
end
...
function AddSettlementRecommendations()
    ...
    if pLocalPlayer == nil then
        UI.DataAssert(
            "Could not obtain a player object to make settler recommendations for player id: ",
            localPlayerID)
        return
    end
    ...
    local NUM_RECOMMENDATIONS = 5
    local pSettlementRecommendations = pGrandAI:GetSettlementRecommendations(
                                           NUM_RECOMMENDATIONS)
    for _, kRecommendation in pairs(pSettlementRecommendations) do

        local uiInstance = GetInstanceAt(kRecommendation.SettlingLocation)
        uiInstance.ImprovementRecommendationIcon:TrySetIcon(
            "ICON_UNITOPERATION_FOUND_CITY", 256) -- Update icon
        ...
        if kRecommendation.SettlingTooltip or (numReasons < 1) then
            ...
            if kRecommendation.SettlingTooltip then
                uiInstance.ImprovementRecommendationIcon:SetToolTipString(
                    kRecommendation.SettlingTooltip)
            else
                uiInstance.ImprovementRecommendationIcon:SetToolTipString(
                    Locale.Lookup("LOC_TOOLTIP_SETTLEMENT_RECOMMENDATION"))

            end
        else
            -- Update custom tooltip
            uiInstance.ImprovementRecommendationIcon:SetToolTipType(
                "SettlerRecommendationTooltip")
            uiInstance.ImprovementRecommendationIcon:SetToolTipCallback(
                function()
                    ...
                    if m_kSettlerTooltip["SettlingLocation"] ==
                        kRecommendation.SettlingLocation then
                        return
                    end
                    m_kSettlerTooltip["SettlingLocation"] =
                        kRecommendation.SettlingLocation -- Save last plot shown/
                    ...
                    for i = 0, numReasons - 1, 1 do -- C++ style 0 to n-1
                        local title = kRecommendation["SettleTitle" ..
                                          tostring(i)]
                        local details = kRecommendation["SettleExplanation" ..
                                            tostring(i)]
                        local isPositive =
                            kRecommendation["SettlePositive" .. tostring(i)]
                    ...
                    for _, kTip in ipairs(kTips) do
                        ...
                        ContextPtr:BuildInstanceForControl(
                            "RecommendationInstance", uiRow,
                            m_kSettlerTooltip.RecommendationStack)

                        uiRow.Title:SetText(Locale.Lookup(kTip.title))
                        uiRow.Explanation:SetText(Locale.Lookup(kTip.details))
                        uiRow.Icon:SetIcon(
                            kTip.isPositive and "ICON_THUMBS_UP" or
                                "ICON_THUMBS_DOWN")
                    end
                end)
        end
        ...
        table.insert(m_RecommendedSettlementPlots,
                     kRecommendation.SettlingLocation)
    end
end

-- ===========================================================================
function OnUnitSelectionChanged(player, unitId, locationX, locationY, locationZ,
    ...
    if pSelectedUnit then
        if pSelectedUnit:GetBuildCharges() > 0 then
            ...
            local pPlot = Map.GetPlotIndex(pSelectedUnit:GetX(),
                                           pSelectedUnit:GetY())
...
function LateInitialize()
    TTManager:GetTypeControlTable("SettlerRecommendationTooltip",
                                  m_kSettlerTooltip)
end
...
--	UI Callback
-- ===========================================================================
function OnInit(bHotload)
    if bHotload then Rebuild() end
    LateInitialize()
end

-- CUI =======================================================================
function CuiIsResourceImprovedByPlayer(resourceInfo, pPlot)
    if (pPlot:GetOwner() ~= Game.GetLocalPlayer()) then return false end
    if (table.count(resourceInfo.ImprovementCollection) > 0) then
        local currentImprovement =
            GameInfo.Improvements[pPlot:GetImprovementType()]
        if (currentImprovement ~= nil) then
            for _, improvement in ipairs(resourceInfo.ImprovementCollection) do
                if currentImprovement.ImprovementType ==
                    improvement.ImprovementType then return true end
            end
        end
    end
    return false
