...
function CanSelectUnit(playerID, unitID)
    ...
    if
        interfaceMode ~= InterfaceModeTypes.SELECTION and interfaceMode ~= InterfaceModeTypes.MAKE_TRADE_ROUTE and
            ...
            interfaceMode ~= InterfaceModeTypes.VIEW_MODAL_LENS
     then
        return false
    end
...
function UnitFlag.UpdateName(self)
    ...
    if pUnit ~= nil then
        ...
        if (GameConfiguration.IsAnyMultiplayer() and pPlayerCfg:IsHuman()) then
            nameString =
                Locale.Lookup(pPlayerCfg:GetCivilizationShortDescription()) ..
        ...
        if (idArchaeologyHomeCity ~= 0) then
            ...
            if (pCity ~= nil) then
                nameString =
                    nameString .. "[NEWLINE]" .. Locale.Lookup("LOC_UNITFLAG_ARCHAEOLOGY_HOME_CITY", pCity:GetName())
    ...
    if (self ~= nil) then
        ...
        if (pUnit ~= nil) then
            ...
            if unitX == -9999 or unitY == -9999 then
                UI.DataError(
                    "Unable to set a unit #" ..
                        tostring(pUnit:GetID()) ..
                            " (" ..
            ...
            if (flag ~= nil and flag.m_eVisibility == RevealedState.VISIBLE) then
                ...
                if (iFormationCount > 1 or numUnits > 1) then
                    if (iFormationCount < 2) then
                        ...
                        elseif
                            (formationClassString == "FORMATION_CLASS_CIVILIAN" or
                                formationClassString == "FORMATION_CLASS_SUPPORT")
                         then
                            flag.m_Instance.FlagRoot:SetOffsetVal(
...
function OnLensLayerOn(layerNum)
    if
        layerNum == g_UnitsMilitary or layerNum == g_UnitsReligious or layerNum == g_UnitsCivilian or
            layerNum == g_UnitsArcheology
     then
