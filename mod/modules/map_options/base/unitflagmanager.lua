...
local TXT_UNITFLAG_ARMY_SUFFIX = " " ..
                                     Locale.Lookup("LOC_UNITFLAG_ARMY_SUFFIX")
local TXT_UNITFLAG_CORPS_SUFFIX = " " ..
                                      Locale.Lookup("LOC_UNITFLAG_CORPS_SUFFIX")
local TXT_UNITFLAG_ARMADA_SUFFIX = " " ..
                                       Locale.Lookup(
                                           "LOC_UNITFLAG_ARMADA_SUFFIX")
local TXT_UNITFLAG_FLEET_SUFFIX = " " ..
                                      Locale.Lookup("LOC_UNITFLAG_FLEET_SUFFIX")
local TXT_UNITFLAG_ACTIVITY_ON_SENTRY = " " ..
                                            Locale.Lookup(
                                                "LOC_UNITFLAG_ACTIVITY_ON_SENTRY")
local TXT_UNITFLAG_ACTIVITY_ON_INTERCEPT =
    " " .. Locale.Lookup("LOC_UNITFLAG_ACTIVITY_ON_INTERCEPT")
local TXT_UNITFLAG_ACTIVITY_AWAKE = " " ..
                                        Locale.Lookup(
                                            "LOC_UNITFLAG_ACTIVITY_AWAKE")
local TXT_UNITFLAG_ACTIVITY_HOLD = " " ..
                                       Locale.Lookup(
                                           "LOC_UNITFLAG_ACTIVITY_HOLD")
local TXT_UNITFLAG_ACTIVITY_SLEEP = " " ..
                                        Locale.Lookup(
                                            "LOC_UNITFLAG_ACTIVITY_SLEEP")
local TXT_UNITFLAG_ACTIVITY_HEALING = " " ..
                                          Locale.Lookup(
                                              "LOC_UNITFLAG_ACTIVITY_HEALING")
local TXT_UNITFLAG_ACTIVITY_NO_ACTIVITY =
    " " .. Locale.Lookup("LOC_UNITFLAG_ACTIVITY_NO_ACTIVITY")
...
local m_MilitaryInstanceManager = InstanceManager:new("UnitFlag", "Anchor",
                                                      Controls.MilitaryFlags)
local m_CivilianInstanceManager = InstanceManager:new("UnitFlag", "Anchor",
                                                      Controls.CivilianFlags)
local m_SupportInstanceManager = InstanceManager:new("UnitFlag", "Anchor",
                                                     Controls.SupportFlags)
local m_TradeInstanceManager = InstanceManager:new("UnitFlag", "Anchor",
                                                   Controls.TradeFlags)
local m_NavalInstanceManager = InstanceManager:new("UnitFlag", "Anchor",
                                                   Controls.NavalFlags)
local m_AttentionMarkerIM = InstanceManager:new("AttentionMarkerInstance",
                                                "Root")
    ...
    if (self.m_InstanceManager ~= nil) then
        ...
        if (self.m_Instance ~= nil) then
            if (self.m_Instance.AirUnitInstance) then
                self.m_Instance.FlagRoot:DestroyChild(
                    self.m_Instance.AirUnitInstance.Root)
                self.m_Instance.AirUnitInstance = nil
            end

            -- Release any attention markers
            if (self.bHasAttentionMarker == true) then
                m_AttentionMarkerIM:ReleaseInstanceByParent(
                    self.m_Instance.FlagRoot)
            end
...
function OnUnitFlagClick(playerID, unitID)
    ...
    if (pUnit == nil) then
        print("Player clicked a unit flag for unit '" .. tostring(unitID) ..
                  "' but that unit doesn't exist.")
    ...
    if Game.GetLocalPlayer() ~= pUnit:GetOwner() then
        ...
        if (pSelectedUnit ~= nil) then
            ...
            if (UnitManager.CanStartOperation(pSelectedUnit,
                                              UnitOperationTypes.RANGE_ATTACK,
                                              nil, tParameters)) then
                UnitManager.RequestOperation(pSelectedUnit,
                                             UnitOperationTypes.RANGE_ATTACK,
                                             tParameters)
            elseif (UnitManager.CanStartOperation(pSelectedUnit,
...
function CanSelectUnit(playerID, unitID)
    ...
    if interfaceMode ~= InterfaceModeTypes.SELECTION and interfaceMode ~=
        InterfaceModeTypes.MAKE_TRADE_ROUTE and interfaceMode ~=
        InterfaceModeTypes.SPY_CHOOSE_MISSION and interfaceMode ~=
        InterfaceModeTypes.SPY_TRAVEL_TO_CITY and interfaceMode ~=
        InterfaceModeTypes.CITY_RANGE_ATTACK and interfaceMode ~=
        InterfaceModeTypes.DISTRICT_RANGE_ATTACK and interfaceMode ~=
...
function UnitFlag.SetInteractivity(self)
    ...
    self.m_Instance.HealthBarButton:RegisterCallback(Mouse.eLClick,
                                                     OnUnitFlagClick)
    self.m_Instance.HealthBarButton:RegisterCallback(Mouse.eRClick,
                                                     OnUnitFlagClick)
    ...
    if (pUnit ~= nil) then
        ...
        if airUnitCapacity > 0 then
            ...
            if not instance then
                instance = {}
                ContextPtr:BuildInstanceForControl("AirUnitInstance", instance,
                                                   self.m_Instance.FlagRoot)
            ...
            if (bHasAirUnits and tAirUnits ~= nil) then
                ...
                for i, unit in ipairs(tAirUnits) do
                    ...
                    if iconInfo.textureSheet then
                        unitEntry.UnitTypeIcon:SetTexture(
                            iconInfo.textureOffsetX, iconInfo.textureOffsetY,
                            iconInfo.textureSheet)
                    end

                    -- Update callback
                    unitEntry.Button:RegisterCallback(Mouse.eLClick,
                                                      OnUnitSelected)
...
function UnitFlag.SetColor(self)
    local primaryColor, secondaryColor =
        UI.GetPlayerColors(self.m_Player:GetID())
...
function UnitFlag.SetFlagUnitEmblem(self)
    ...
    if individual >= 0 then
        local individualType = GameInfo.GreatPersonIndividuals[individual]
                                   .GreatPersonIndividualType
        local iconModifier =
            GameInfo.GreatPersonIndividualIconModifiers[individualType]
...
function UnitFlag.UpdateFlagType(self)
    ...
    elseif (self.m_Style == FLAGSTYLE_RELIGION) then
        ...
        self.m_IsForceHide = not CuiSettings:GetBoolean(
                                 CuiSettings.SHOW_RELIGIONS) -- CUI: toggle religion flags
    ...
    elseif (self.m_Style == FLAGSTYLE_TRADE) then
        ...
        self.m_IsForceHide =
            not CuiSettings:GetBoolean(CuiSettings.SHOW_TRADERS) -- CUI: toggle trader flags
    else
...
function UnitFlag.UpdateHealth(self)
    ...
    if (maxDamage > 0) then
        healthPercent = math.max(math.min(
                                     (maxDamage - pUnit:GetDamage()) / maxDamage,
                                     1), 0)
    end
    ...
    if self.m_IsForceHide then
        ...
        if self.m_IsCurrentlyVisible then
            ...
            if self.m_IsInitialized then
                self.m_Instance.FlagRoot:RegisterEndCallback(
                    function()
                        self.m_Instance.Anchor:SetHide(
                            not self.m_IsCurrentlyVisible)
                    end)
                self.m_Instance.FlagRoot:SetToEnd()
    ...
    if (pUnit ~= nil) then
        if (pUnit:GetCombat() > 0) then
            ...
            if (iOwner ~= iOriginalOwner) then
                ...
                if (pOriginalOwner ~= nil and pOriginalOwner:GetInfluence() ~=
                    nil) then
                    local iLevyTurnCounter =
                        pOriginalOwner:GetInfluence():GetLevyTurnCounter()
                    if (iLevyTurnCounter >= 0 and iOwner ==
                        pOriginalOwner:GetInfluence():GetSuzerain()) then
                        return
                            (pOriginalOwner:GetInfluence():GetLevyTurnLimit() -
                                iLevyTurnCounter)
                    end
                end
            end
...
function UnitFlag.UpdateReligion(self)
    ...
    if pUnit ~= nil then
        ...
        if (religionType > 0 and pUnit:GetReligiousStrength() > 0) then
            ...
            self.m_Instance.ReligionIconBacking:LocalizeAndSetToolTip(
                religionName)
...
function UnitFlag.UpdateName(self)
    ...
    if pUnit ~= nil then
        ...
        if (GameConfiguration.IsAnyMultiplayer() and pPlayerCfg:IsHuman()) then
            nameString = Locale.Lookup(
                             pPlayerCfg:GetCivilizationShortDescription()) ..
                             " (" .. Locale.Lookup(pPlayerCfg:GetPlayerName()) ..
        ...
        if pUnitDef then
            ...
            if unitName ~= unitTypeName then
                nameString = nameString .. " " ..
                                 Locale.Lookup("LOC_UNIT_UNIT_TYPE_NAME_SUFFIX",
                                               unitTypeName)
            end
        end
    ...
    elseif (activityType == ActivityTypes.NO_ACTIVITY) then
      nameString = nameString .. TXT_UNITFLAG_ACTIVITY_NO_ACTIVITY;
        ...
        if (idArchaeologyHomeCity ~= 0) then
            local pCity = self.m_Player:GetCities()
                              :FindID(idArchaeologyHomeCity)
            if (pCity ~= nil) then
                nameString = nameString .. "[NEWLINE]" ..
                                 Locale.Lookup(
                                     "LOC_UNITFLAG_ARCHAEOLOGY_HOME_CITY",
                                     pCity:GetName())
                ...
                if (iGreatWorkIndex >= 0) then
                    ...
                    nameString = nameString .. "[NEWLINE]" ..
                                     Locale.Lookup(
                                         "LOC_UNITFLAG_ARCHAEOLOGY_ARTIFACT",
                                         GameInfo.GreatWorks[eGWType].Name,
        ...
        if (pUnit:GetReligiousStrength() > 0) then
            ...
            if (eReligion > 0) then
                nameString = nameString .. " (" ..
                                 Game.GetReligion():GetName(eReligion) .. ")"
            end
        end

        -- display levy status
        local iLevyTurnsRemaining = GetLevyTurnsRemaining(pUnit)
        if (iLevyTurnsRemaining >= 0 and
            PlayerConfigurations[pUnit:GetOriginalOwner()] ~= nil) then
            nameString = nameString .. "[NEWLINE]" ..
                             Locale.Lookup("LOC_UNITFLAG_LEVY_ACTIVE",
                                           PlayerConfigurations[pUnit:GetOriginalOwner()]:GetPlayerName(),
                                           iLevyTurnsRemaining)
        end
...
function CanRangeAttack(pCityOrDistrict)
    ...
    return CityManager.CanStartCommand(pCityOrDistrict,
...
function UnitFlag.SetPosition(self, worldX, worldY, worldZ)
    ...
    if (self ~= nil) then
        ...
        if (pUnit ~= nil) then
            ...
            if unitX == -9999 or unitY == -9999 then
                UI.DataError(
                    "Unable to set a unit #" .. tostring(pUnit:GetID()) .. " (" ..
                        tostring(pUnit:GetName()) ..
                        ") flag due to an invalid position: " .. unitX .. "," ..
                        unitY)
                return
            end
            ...
            if (pUnit ~= nil) then
                ...
                if (pCity ~= nil or pCityToTheRight ~= nil or pCityToTheLeft ~=
                    nil) then
                    if (pCity ~= nil) then
                        -- If the city can attack, offset the unit flag icon so that we can see the ranged attack action icon
                        local pDistrict =
                            pCity:GetDistricts():FindID(pCity:GetDistrictID())
                        if (pCity:GetOwner() == Game.GetLocalPlayer() and
                            CanRangeAttack(pDistrict)) then
                ...
                if (pPlot) then
                    ...
                    if (eImprovementType ~= -1) then
                        local kImprovementData =
                            GameInfo.Improvements[eImprovementType]
                        if (kImprovementData.AirSlots > 0 or
                            kImprovementData.WeaponSlots > 0) then
                            cityBannerZOffset = -15
                            cityBannerYOffset = 5
                        end
                    end
                    ...
                    if (eDistrictType ~= -1) then
                        if (GameInfo.Districts[eDistrictType].DistrictType ==
                            "DISTRICT_ENCAMPMENT") then
            ...
            if (cityBannerZOffset == 0) then
                ...
                if (pHeightOverride ~= nil) then
                    ...
                    self.m_Instance.Anchor:SetZoomOffsetNearVal(0, 0,
                                                                pHeightOverride)
                end
            end
    ...
    self.m_Instance.Anchor:SetWorldPositionVal(worldX + xOffset,
                                               worldY + yOffset,
                                               worldZ + zOffset)
...
function CreateUnitFlag(playerID, unitID, unitX, unitY)
    ...
    if (m_UnitFlagInstances[playerID] ~= nil and
        m_UnitFlagInstances[playerID][unitID] ~= nil) then return end
    ...
    if pUnit ~= nil and pUnit:GetUnitType() ~= -1 then
        if (pUnit:GetCombat() ~= 0 or pUnit:GetRangedCombat() ~= 0) then -- Need a simpler what to test if the unit is a combat unit or not.
            ...
            elseif "FORMATION_CLASS_SUPPORT" ==
                GameInfo.Units[pUnit:GetUnitType()].FormationClass then
...
-------------------------------------------------
function UnitSimPositionChanged(playerID, unitID, worldX, worldY, worldZ,
                                bVisible, bComplete)
    local flagInstance = GetUnitFlag(playerID, unitID)
    ...
    if (flagInstance ~= nil) then
        if (bComplete) then
            local plotX, plotY =
                UI.GetPlotCoordFromWorld(worldX, worldY, worldZ)
...
function OnCityVisibilityChanged(playerID, cityID, eVisibility)
    ...
    local pWestPlot = Map.GetAdjacentPlot(cityX, cityY,
    ...
    local pEastPlot = Map.GetAdjacentPlot(cityX, cityY,
            ...
            if (flag ~= nil and flag.m_eVisibility == RevealedState.VISIBLE) then
                ...
                if (iFormationCount > 1 or numUnits > 1) then
                    if (iFormationCount < 2) then
                        if (formationClassString ==
                            "FORMATION_CLASS_LAND_COMBAT") then
                            flag.m_Instance.FlagRoot:SetOffsetVal(
                                landCombatOffsetX + m_FlagOffsets[1][1],
                                m_FlagOffsets[1][2])
                            landCombatOffsetX =
                                landCombatOffsetX - multiSpacingX
                        elseif (formationClassString ==
                            "FORMATION_CLASS_CIVILIAN" or formationClassString ==
                            "FORMATION_CLASS_SUPPORT") then
                            flag.m_Instance.FlagRoot:SetOffsetVal(
                                civilianOffsetX + m_FlagOffsets[2][1],
                                m_FlagOffsets[2][2])
                            civilianOffsetX = civilianOffsetX + multiSpacingX
                        elseif (formationClassString == "FORMATION_CLASS_NAVAL") then
                            flag.m_Instance.FlagRoot:SetOffsetVal(
                                m_FlagOffsets[3][1], m_FlagOffsets[3][2])
                        elseif (formationClassString == "FORMATION_CLASS_AIR") then
                            flag.m_Instance.FlagRoot:SetOffsetVal(
                                m_FlagOffsets[3][1], m_FlagOffsets[3][2] * -1)
                        else
                        ...
                        if (formationClassString == "FORMATION_CLASS_CIVILIAN" or
                            formationClassString == "FORMATION_CLASS_SUPPORT") then
                            civilianOffsetX = civilianOffsetX + multiSpacingX
                        end
                        if (iFormationCount < 3) then
                            ...
                            if formationClassString ~=
                                ...
                                if DuoFlag then
                                    ...
                                    local flagParent =
                                        flag.m_Instance.Anchor:GetParent()
                                    if flagParent ==
                                        DuoFlag.m_Instance.Anchor:GetParent() then
                                        flagParent:AddChildAtIndex(
                                            flag.m_Instance.Anchor, 0)
                                    end
                                end
                                DuoFlag = flag

                                flag.m_Instance.Formation2:SetHide(false)
                                flag.m_Instance.Formation2:SetOffsetVal(
                                    m_LinkOffsets[1][1], m_LinkOffsets[1][2])
                            ...
                            flag.m_Instance.Formation3:SetHide(true)
                            if formationClassString ==
                                "FORMATION_CLASS_CIVILIAN" or
                                formationClassString ==
                                "FORMATION_CLASS_SUPPORT" then
                                ...
                                flag.m_Instance.Formation3:SetOffsetVal(
                                    m_LinkOffsets[2][1], m_LinkOffsets[2][2])
                        ...
                        flag.m_Instance.FlagRoot:SetOffsetVal(
                            m_FlagOffsets[formationIndex][1],
                            m_FlagOffsets[formationIndex][2])
                    end
                ...
                if flag.m_cacheMilitaryFormation ~= militaryFormation then
                    ...
                    elseif militaryFormation ==
                        MilitaryFormationTypes.ARMY_FORMATION then
                ...
                if (flag.m_Style == FLAGSTYLE_TRADE) then
                    flag.m_Instance.Anchor:SetHide(
                        not CuiSettings:GetBoolean(CuiSettings.SHOW_TRADERS))
                end

                -- CUI: toggle religion flags
                if (flag.m_Style == FLAGSTYLE_RELIGION) then
                    flag.m_Instance.Anchor:SetHide(
                        not CuiSettings:GetBoolean(CuiSettings.SHOW_RELIGIONS))
                end
                ...
                if pDistrict and not pDistrictInfo.CityCenter then
                    ...
                    if (bHasAirUnits and tAirUnits ~= nil) then
                        for _, unit in ipairs(tAirUnits) do
                            if unitOwner == unit:GetOwner() and unitID ==
                                unit:GetID() then
                                shouldHideFlag = true
                            end
                        end
                ...
                for i, pUnit in pPlayerUnits:Members() do
                    ...
                    if flag ~= nil then
                        ...
                        if targetPlayer ~= -1 and targetPlayer == idLocalPlayer then
                            m_AttentionMarkerIM:GetInstance(
                                flag.m_Instance.FlagRoot)
        ...
        if (pUnit ~= nil) then
            ...
            if (flag ~= nil) then
                ...
                if (flag.m_eVisibility == RevealedState.VISIBLE) then
                    ...
                    local szText
                    if (iDelta < 0) then
                        szText = Locale.Lookup(
                                     "LOC_WORLD_UNIT_DAMAGE_DECREASE_FLOATER",
                                     -iDelta)
                    else
                        szText = Locale.Lookup(
                                     "LOC_WORLD_UNIT_DAMAGE_INCREASE_FLOATER",
                                     -iDelta)
                    end

                    UI.AddWorldViewText(EventSubTypes.DAMAGE, szText,
                                        pUnit:GetX(), pUnit:GetY(), 0)
                end
            end
        end
    end
            ...
            if (pUnit ~= nil) then
                ...
                if (flag ~= nil) then
                    if (flag.m_eVisibility == RevealedState.VISIBLE) then
                        ...
                        if (abilityInfo ~= nil and
                            abilityInfo.ShowFloatTextWhenEarned) then
                            local sAbilityName =
                                GameInfo.UnitAbilities[eAbilityType].Name
                            if (sAbilityName ~= nil) then
                                local floatText = Locale.Lookup(sAbilityName)
                                UI.AddWorldViewText(EventSubTypes.DAMAGE,
                                                    floatText, pUnit:GetX(),
                                                    pUnit:GetY(), 0)
                            end
                        end
                    end
...
------------------------------------------------------------------
function OnObjectPairingChanged(eSubType, parentOwner, parentType, parentID,
                                childOwner, childType, childID)
    ...
    if (pLocalPlayerVis ~= nil) then
        ...
        for i, player in ipairs(players) do
            ...
            for ii, unit in playerUnits:Members() do
                ...
                if (m_UnitFlagInstances[playerID] == nil or
                    m_UnitFlagInstances[playerID][unitID] == nil) then
                ...
                if pLocalPlayerVis:IsVisible(locX, locY) then
                    OnUnitVisibilityChanged(playerID, unitID,
        ...
        for locX, ys in pairs(plotsToUpdate) do
            for locY, _ in pairs(ys) do UpdateIconStack(locX, locY) end
        end
...
function OnLensLayerOn(layerNum)
    if layerNum == g_UnitsMilitary or layerNum == g_UnitsReligious or layerNum ==
        g_UnitsCivilian or layerNum == g_UnitsArcheology then
