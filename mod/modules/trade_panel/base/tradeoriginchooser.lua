...
function Refresh()
    ...
    local originCity = Cities.GetCityInPlot(selectedUnit:GetX(),
                                            selectedUnit:GetY())
...
function AddCity(city)
    ...
    cityInstance.CityPopulation:SetText("[ICON_Citizen] " ..
                                            city:GetPopulation())
...
function CanTeleportToCity(city)
    ...
    local eOperation = UI.GetInterfaceModeParameter(
                           UnitOperationTypes.PARAM_OPERATION_TYPE)

    local pSelectedUnit = UI.GetHeadSelectedUnit()
    if (UnitManager.CanStartOperation(pSelectedUnit, eOperation, nil,
                                      tParameters)) then return true end
...
function OnUnitSelectionChanged(playerID, unitID, hexI, hexJ, hexK, bSelected,
    ...
    if m_AnimSupport:IsVisible() and playerID ~= -1 and playerID ==
        Game.GetLocalPlayer() then Close() end
end
...
function Open()
    LuaEvents.TradeOriginChooser_SetTradeUnitStatus(
        "LOC_HUD_UNIT_PANEL_CHOOSING_ORIGIN_CITY")
    m_AnimSupport:Show()
    Refresh()
end
...
function OnGameDebugReturn(context, contextTable)
    if context == RELOAD_CACHE_ID and contextTable["isVisible"] ~= nil and
        contextTable["isVisible"] then OnOpen() end
end
...
function CuiInit()
    ...
    Controls.ReplaceTitle:SetText(Locale.ToUpper(
                                      Locale.Lookup(
                                          "LOC_UNITOPERATION_MOVE_TO_DESCRIPTION")))
    -- reg
    Controls.ChangeOriginCityButton:RegisterCallback(Mouse.eLClick,
                                                     CuiOnChangeOriginCity)
