...
function RefreshHeader()
    ...
    if originCity then
        Controls.Header_OriginText:SetText(
            Locale.Lookup("LOC_ROUTECHOOSER_TO_DESTINATION", Locale.ToUpper(originCity:GetName()))
        )
    end
end
...
function RefreshTopPanel()
    ...
    if destinationCity ~= nil and originCity ~= nil then
        ...
        Controls.OriginResources:RegisterSizeChanged(
            function()
                ResizeResourceBackgroundColumns(
                    Controls.OriginResources,
        ...
        if (originMajorityReligion > 0) then
            ...
            if (pressureValue ~= 0) then
                ...
                destinationTooltipText = destinationTooltipText .. sourceText
                AddReligiousPressureResourceEntry(
                    GameInfo.Religions[originMajorityReligion],
                    pressureValue,
                    false,
                    sourceText,
        ...
        if
            originCity:GetID() == prevOriginComponentID.id and originCity:GetOwner() == prevOriginComponentID.player and
                destinationCity:GetID() == prevDestComponentID.id and
                destinationCity:GetOwner() == prevDestComponentID.player
         then
...
function OnTopGridSizeChanged()
    -- CUI
    Controls.BottomGrid:SetSizeY(
        Controls.RouteChooser:GetSizeY() - Controls.TopGrid:GetSizeY() - Controls.MidGrid:GetSizeY() + 2
    )
                ...
                for j, city in cities:Members() do
                    ...
                    if CheckTradeRoute(m_selectedUnit, city) then
                        ...
                        pathPlots, portalEntrances, portalExits =
                            tradeManager:GetTradeRoutePath(m_originCityOwner, m_originCityID, cityOwner, cityID)
...
function AddCityToDestinationStack(city)
    ...
    if (cityMajorityReligion > 0) then
        ...
        if (pressureValue ~= 0) then
            ...
            tooltipText = tooltipText .. sourceText
            AddReligiousPressureResourceEntry(
                GameInfo.Religions[cityMajorityReligion],
                pressureValue,
                true,
                sourceText,
...
function GetYieldForCityWithText(yieldIndex, city, originCity)
    ...
    if (originCity) then
        partialValue =
            tradeManager:CalculateOriginYieldFromPath(m_originCityOwner, m_originCityID, cityOwner, cityID, yieldIndex)
    else
        partialValue =
    ...
    if (partialValue > 0 and yieldInfo ~= nil) then
        ...
        sourceText =
            sourceText ..
            Locale.Lookup("LOC_ROUTECHOOSER_YIELD_SOURCE_BONUSES", partialValue, yieldInfo.IconString, yieldInfo.Name)
    end
...
function GetYieldsForCity(city, originCity)
    ...
    if (originCity) then
        routeYields =
            tradeManager:CalculateOriginYieldsFromPotentialRoute(m_originCityOwner, m_originCityID, cityOwner, cityID)
    else
        routeYields =
    ...
    if (originCity) then
        ...
        pathYields =
            tradeManager:CalculateDestinationYieldsFromPath(m_originCityOwner, m_originCityID, cityOwner, cityID)
    end
    ...
    if (originCity) then
        modifierYields =
            tradeManager:CalculateOriginYieldsFromModifiers(m_originCityOwner, m_originCityID, cityOwner, cityID)
    else
        modifierYields =
            tradeManager:CalculateDestinationYieldsFromModifiers(m_originCityOwner, m_originCityID, cityOwner, cityID)
    end
...
function GetReligiousPressureForCity(religionIndex, destinationCity, forOriginCity)
    ...
    local sourceText =
        ...
        pressureValue,
        pressureIconString,
        Game.GetReligion():GetName(religionIndex),
        cityName
    )
...
-- CUI ========================================================================
function CuiInit()
    Controls.FoodSortButton:RegisterCallback(
        Mouse.eLClick,
        function()
            CuiOnSortButtonClick(1)
        end
    )
    Controls.ProductionSortButton:RegisterCallback(
        Mouse.eLClick,
        function()
            CuiOnSortButtonClick(2)
        end
    )
    Controls.GoldSortButton:RegisterCallback(
        Mouse.eLClick,
        function()
            CuiOnSortButtonClick(3)
        end
    )
    Controls.ScienceSortButton:RegisterCallback(
        Mouse.eLClick,
        function()
            CuiOnSortButtonClick(4)
        end
    )
    Controls.CultureSortButton:RegisterCallback(
        Mouse.eLClick,
        function()
            CuiOnSortButtonClick(5)
        end
    )
    Controls.FaithSortButton:RegisterCallback(
        Mouse.eLClick,
        function()
...
function Initialize()
    ...
    Controls.BeginRouteButton:RegisterCallback(
        Mouse.eMouseEnter,
        function()
