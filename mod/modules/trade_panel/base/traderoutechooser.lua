...
local m_RouteChoiceIM = InstanceManager:new("RouteChoiceInstance", "Top",
                                            Controls.RouteChoiceStack)
...
function Refresh()
    ...
    local originCity = Cities.GetCityInPlot(m_selectedUnit:GetX(),
                                            m_selectedUnit:GetY())
...
function RefreshHeader()
    ...
    if originCity then
        Controls.Header_OriginText:SetText(
            Locale.Lookup("LOC_ROUTECHOOSER_TO_DESTINATION",
...
function RefreshTopPanel()
    ...
    if destinationCity ~= nil and originCity ~= nil then
        ...
        local oBackColor, oFrontColor =
            UI.GetPlayerColors(originCity:GetOwner())
        ...
        local backColor, frontColor = UI.GetPlayerColors(
        ...
        if destinationCity:GetTrade()
            :HasActiveTradingPost(originCity:GetOwner()) then
        ...
        local originYields_Values, originYields_SourceText,
              originYields_FromRouteBonus =
        ...
        for yieldIndex = 1, yieldCount, 1 do
            ...
            if (yieldValue ~= 0) then
                ...
                AddYieldResourceEntry(yieldInfo, yieldValue, sourceText,
                                      Controls.OriginResourceList)
                originReceivedResources = true
                bHasAnyFromRouteBonus = bHasAnyFromRouteBonus and true or
                                            originYields_FromRouteBonus[yieldIndex]
            else -- CUI: add empty
        ...
        if (destinationMajorityReligion > 0) then
            local pressureValue, sourceText =
                GetReligiousPressureForCity(destinationMajorityReligion,
            ...
            if (pressureValue ~= 0) then
                ...
                originTooltipText = originTooltipText .. sourceText
                AddReligiousPressureResourceEntry(
                    GameInfo.Religions[destinationMajorityReligion],
                    pressureValue, true, sourceText,
        ...
        Controls.OriginResources:RegisterSizeChanged(
            function()
                ResizeResourceBackgroundColumns(Controls.OriginResources,
                                                Controls.OriginResourcesLeftColumn,
                                                Controls.OriginResourcesMidColumn,
        ...
        local destinationYields_Values, destinationYields_SourceText,
              destinationYields_FromRouteBonus =
            ...
            if (yieldValue ~= 0) then
                ...
                if (destinationTooltipText ~= "") then
                    destinationTooltipText =
                        destinationTooltipText .. "[NEWLINE]"
                end
                destinationTooltipText = destinationTooltipText .. sourceText
                AddYieldResourceEntry(yieldInfo, yieldValue, sourceText,
                                      Controls.DestinationResourceList)
                destinationReceivedResources = true
            else -- CUI: add empty
                AddYieldResourceEntry(nil, nil, nil,
                                      Controls.DestinationResourceList)
            end
        end

        local originMajorityReligion = originCity:GetReligion()
                                           :GetMajorityReligion()
        if (originMajorityReligion > 0) then
            local pressureValue, sourceText =
                GetReligiousPressureForCity(originMajorityReligion,
            ...
            if (pressureValue ~= 0) then
                ...
                destinationTooltipText = destinationTooltipText .. sourceText
                AddReligiousPressureResourceEntry(
                    GameInfo.Religions[originMajorityReligion], pressureValue,
                    false, sourceText, Controls.DestinationReligiousPressure)
                destinationReceivedResources = true
            end
        end
        ...
        Controls.DestinationResources:RegisterSizeChanged(
            function()
                ResizeResourceBackgroundColumns(Controls.DestinationResources,
                                                Controls.DestinationResourcesLeftColumn,
                                                Controls.DestinationResourcesMidColumn,
                                                Controls.DestinationResourcesRightColumn)
            end)
        ...
        if originCity:GetID() == prevOriginComponentID.id and
            ...
            if tradeManager:CanStartRoute(originCity:GetOwner(),
                                          originCity:GetID(),
                                          destinationCity:GetOwner(),
                                          destinationCity:GetID()) then
                Controls.BeginRouteLabel:SetText(
                    Locale.Lookup("LOC_ROUTECHOOSER_REPEAT_ROUTE_BUTTON"))
            else
                HideConfirmGrid(true)
            end
        else
            Controls.BeginRouteLabel:SetText(
                Locale.Lookup("LOC_ROUTECHOOSER_BEGIN_ROUTE_BUTTON"))
        end
    else
        HideConfirmGrid(true)
    end
...
function OnTopGridSizeChanged()
    -- CUI
    Controls.BottomGrid:SetSizeY(Controls.RouteChooser:GetSizeY() -
                                     Controls.TopGrid:GetSizeY() -
                                     Controls.MidGrid:GetSizeY() + 2)
    Controls.BottomGrid:SetOffsetY(Controls.TopGrid:GetSizeY() +
                                       Controls.MidGrid:GetSizeY() - 2)
end
...
function RefreshFilters()
    ...
    for index, entry in ipairs(m_unfilteredDestinations) do
        ...
        if not pPlayerInfluence:CanReceiveInfluence() then
            ...
            local name = Locale.Lookup(
                             GameInfo.Civilizations[playerConfig:GetCivilizationTypeID()]
                                 .Name)
    ...
    for index, entry in ipairs(m_unfilteredDestinations) do
        ...
        if pPlayerInfluence:CanReceiveInfluence() then
            ...
            AddFilter(Locale.Lookup("LOC_ROUTECHOOSER_FILTER_CITYSTATES"),
                      FilterByCityStates)
            break
        end
    end
...
function GetFilterIndex(filterName)
    for index, filter in ipairs(m_filterList) do
        if filter.FilterText == filterName then return index end
    end
end
...
function AddFilter(filterName, filterFunction)
    ...
    if not GetFilterIndex(filterName) then
        table.insert(m_filterList,
                     {FilterText = filterName, FilterFunction = filterFunction})
    end
end
...
function FilterByResource(yieldIndex)
    ...
    table.sort(m_filteredDestinations,
               function(A, B) return cachedYields[A] > cachedYields[B] end)
end
...
function FilterByCityStates()
    ...
    for index, entry in ipairs(m_unfilteredDestinations) do
        local playerConfig = PlayerConfigurations[entry.owner]
        if playerConfig:GetCivilizationLevelTypeID() ~=
...
function AddTradeRoutePath(entry, color)
    ...
    local kVariations = {
        {"TradeRoute_Destination", entry.pathPlots[lastElement]}
    }
    ...
    for i, plot in pairs(entry.pathPlots) do
        ...
        if (pEntrance and pEntrance >= 0) then
            ...
            UILens.SetLayerHexesPath(m_TradeRoute, eLocalPlayer, pPathSegment,
                                     {}, color)
            pPathSegment = {}
        end
    end
...
function RefreshChooserPanel()
            ...
            for i, player in ipairs(players) do
                ...
                for j, city in cities:Members() do
                    ...
                    if CheckTradeRoute(m_selectedUnit, city) then
                        ...
                        local pathPlots = {}
                        pathPlots, portalEntrances, portalExits =
                            tradeManager:GetTradeRoutePath(m_originCityOwner,
                                                           m_originCityID,
                                                           cityOwner, cityID)
    ...
    for index, entryIndex in ipairs(m_filteredDestinations) do
        ...
        if (entry.owner ~= m_destinationCityOwner or entry.id ~=
            m_destinationCityID) then
    ...
    if destinationCity ~= nil then
        local entry = FindDestinationEntry(m_destinationCityOwner,
                                           m_destinationCityID)
...
function RefreshStack()
  ...
  if Controls.RouteChoiceScrollPanel:GetScrollBar():IsHidden() then
    ...
    if numberOfDestinations > 0 then
        Controls.StatusMessage:SetText(Locale.Lookup(
                                           "LOC_ROUTECHOOSER_SELECT_DESTINATION"))
    else
        Controls.StatusMessage:SetText(Locale.Lookup(
                                           "LOC_ROUTECHOOSER_NO_TRADE_ROUTES"))
    end
...
function AddCityToDestinationStack(city)
    ...
    if (questsManager ~= nil and Game.GetLocalPlayer() ~= nil) then
        ...
        if (tradeRouteQuestInfo ~= nil) then
            if (questsManager:HasActiveQuestFromPlayer(Game.GetLocalPlayer(),
                                                       city:GetOwner(),
                                                       tradeRouteQuestInfo.Index)) then
                local questTooltip = questTooltip .. "[NEWLINE]" ..
                                         tradeRouteQuestInfo.IconString ..
                                         questsManager:GetActiveQuestName(
                                             Game.GetLocalPlayer(),
                                             city:GetOwner(),
    ...
    local yields_Values, yields_SourceText, yields_FromRouteBonus =
        GetYieldsForCity(city, true)
    ...
    for yieldIndex = 1, yieldCount, 1 do
        ...
        if (yieldValue ~= 0) then
            ...
            AddYieldResourceEntry(yieldInfo, yieldValue, sourceText,
                                  cityEntry.ResourceList)
            bHasAnyFromRouteBonus = bHasAnyFromRouteBonus and true or
                                        yields_FromRouteBonus[yieldIndex]
        else -- CUI: add empty
    ...
    if (cityMajorityReligion > 0) then
        local pressureValue, sourceText =
            GetReligiousPressureForCity(cityMajorityReligion, city, true)
        if (pressureValue ~= 0) then
            if (tooltipText ~= "") then
                tooltipText = tooltipText .. "[NEWLINE]"
            end
            tooltipText = tooltipText .. sourceText
            AddReligiousPressureResourceEntry(
                GameInfo.Religions[cityMajorityReligion], pressureValue, true,
                sourceText, cityEntry.ReligiousPressure)
        end
    end
    ...
    cityEntry.BonusIconStack:RegisterSizeChanged(
        function() OnBonusIconStackSizeChanged(cityEntry) end)

    -- Resize resource background columns to fit parent
    cityEntry.ResourceInfoGrid:RegisterSizeChanged(
        function()
            ResizeResourceBackgroundColumns(cityEntry.ResourceInfoGrid,
                                            cityEntry.ResourceInfoLeftColumn,
                                            cityEntry.ResourceInfoMidColumn,
...
function OnBonusIconStackSizeChanged(pParentControl)
    ...
    pParentControl.CityName:SetTruncateWidth(
        pParentControl.NameContainer:GetSizeX() - iIconStackSizeX)
end

-- ===========================================================================
function ResizeResourceBackgroundColumns(parent, leftColumn, midColumn,
                                         rightColumn)
...
function AddYieldResourceEntry(yieldInfo, yieldValue, sourceText, stackControl)
    local entryInstance = {}
    ContextPtr:BuildInstanceForControl("ResourceEntryInstance", entryInstance,
                                       stackControl)
...
-- ===========================================================================
function AddReligiousPressureResourceEntry(religionInfo, pressureValue,
                                           forOriginCity, sourceText,
                                           stackControl)
    local entryInstance = {}
    ...
    local icon, text = FormatReligiousPressureText(religionInfo, pressureValue,
                                                   forOriginCity)
    entryInstance.ResourceEntryText:SetText(text)
    entryInstance.ResourceEntryStack:CalculateSize()
end
...
function CheckTradeRoute(unit, city)
    if city and unit then
        ...
        if (UnitManager.CanStartOperation(unit,
                                          UnitOperationTypes.MAKE_TRADE_ROUTE,
                                          nil, operationParams)) then
            return true
        end
    end
...
function RequestTradeRoute()
    ...
    if destinationCity and m_selectedUnit then
        ...
        if (UnitManager.CanStartOperation(m_selectedUnit,
                                          UnitOperationTypes.MAKE_TRADE_ROUTE,
                                          nil, operationParams)) then
            UnitManager.RequestOperation(m_selectedUnit,
                                         UnitOperationTypes.MAKE_TRADE_ROUTE,
                                         operationParams)
...
function GetYieldForCityWithText(yieldIndex, city, originCity)
    ...
    -- From route
    if (originCity) then
        partialValue = tradeManager:CalculateOriginYieldFromPotentialRoute(
                           m_originCityOwner, m_originCityID, cityOwner, cityID,
                           yieldIndex)
    else
    ...
    if (partialValue > 0 and yieldInfo ~= nil) then
        ...
        sourceText = sourceText ..
                         Locale.Lookup(
                             "LOC_ROUTECHOOSER_YIELD_SOURCE_DISTRICTS",
                             partialValue, yieldInfo.IconString, yieldInfo.Name,
                             city:GetName())
    end
    -- From path
    local bFromRouteBonus = false
    if (originCity) then
        partialValue = tradeManager:CalculateOriginYieldFromPath(
                           m_originCityOwner, m_originCityID, cityOwner, cityID,
                           yieldIndex)
    else
        partialValue = tradeManager:CalculateDestinationYieldFromPath(
                           m_originCityOwner, m_originCityID, cityOwner, cityID,
                           yieldIndex)
    end
    totalValue = totalValue + partialValue
    if (partialValue > 0 and yieldInfo ~= nil) then
        ...
        sourceText = sourceText ..
                         Locale.Lookup(
                             "LOC_ROUTECHOOSER_YIELD_SOURCE_TRADING_POSTS",
                             partialValue, yieldInfo.IconString, yieldInfo.Name)
    end
    -- From modifiers
    local resourceID = -1
    if (originCity) then
        partialValue = tradeManager:CalculateOriginYieldFromModifiers(
                           m_originCityOwner, m_originCityID, cityOwner, cityID,
                           yieldIndex, resourceID)
    else
        partialValue = tradeManager:CalculateDestinationYieldFromModifiers(
                           m_originCityOwner, m_originCityID, cityOwner, cityID,
                           yieldIndex, resourceID)
    end
    totalValue = totalValue + partialValue
    if (partialValue > 0 and yieldInfo ~= nil) then
        if (sourceText ~= "") then sourceText = sourceText .. "[NEWLINE]" end
        sourceText = sourceText ..
                         Locale.Lookup("LOC_ROUTECHOOSER_YIELD_SOURCE_BONUSES",
                                       partialValue, yieldInfo.IconString,
                                       yieldInfo.Name)
    end
...
function GetYieldsForCity(city, originCity)
    ...
    local routeYields = {}
    if (originCity) then
        routeYields = tradeManager:CalculateOriginYieldsFromPotentialRoute(
                          m_originCityOwner, m_originCityID, cityOwner, cityID)
    else
        routeYields = tradeManager:CalculateDestinationYieldsFromPotentialRoute(
                          m_originCityOwner, m_originCityID, cityOwner, cityID)
    end
    ...
    local bFromRouteBonus = false
    if (originCity) then
        pathYields = tradeManager:CalculateOriginYieldsFromPath(
                         m_originCityOwner, m_originCityID, cityOwner, cityID)
    else
        pathYields = tradeManager:CalculateDestinationYieldsFromPath(
                         m_originCityOwner, m_originCityID, cityOwner, cityID)
    end
    ...
    if (originCity) then
        modifierYields = tradeManager:CalculateOriginYieldsFromModifiers(
                             m_originCityOwner, m_originCityID, cityOwner,
                             cityID)
    else
        modifierYields = tradeManager:CalculateDestinationYieldsFromModifiers(
                             m_originCityOwner, m_originCityID, cityOwner,
                             cityID)
    end
        ...
        if yieldInfo ~= nil then
            ...
            if (routeValue > 0) then
                ...
                sourceText = sourceText ..
                                 Locale.Lookup(
                                     "LOC_ROUTECHOOSER_YIELD_SOURCE_DISTRICTS",
                                     routeValue, yieldInfo.IconString,
            ...
            if (pathValue > 0) then
                ...
                sourceText = sourceText ..
                                 Locale.Lookup(
                                     "LOC_ROUTECHOOSER_YIELD_SOURCE_TRADING_POSTS",
                                     pathValue, yieldInfo.IconString,
                                     yieldInfo.Name)
            end
            ...
            if (modifierValue > 0) then
                ...
                sourceText = sourceText ..
                                 Locale.Lookup(
                                     "LOC_ROUTECHOOSER_YIELD_SOURCE_BONUSES",
                                     modifierValue, yieldInfo.IconString,
                                     yieldInfo.Name)
            end
...
-- ===========================================================================
function GetReligiousPressureForCity(religionIndex, destinationCity,
                                     forOriginCity)
    ...
    local sourceText = Locale.Lookup(
                           "LOC_ROUTECHOOSER_RELIGIOUS_PRESSURE_SOURCE_MAJORITY_RELIGION",
                           pressureValue, pressureIconString,
...
function Open()
    LuaEvents.TradeRouteChooser_SetTradeUnitStatus(
        "LOC_HUD_UNIT_PANEL_CHOOSING_TRADE_ROUTE")
    ...
    if m_postOpenSelectPlayerID ~= -1 then
        ...
        local originCity = Cities.GetCityInPlot(m_selectedUnit:GetX(),
                                                m_selectedUnit:GetY())
        if originCity:GetID() == prevOriginComponentID.id and
            originCity:GetOwner() == prevOriginComponentID.player then
            TradeRouteSelected(prevDestComponentID.player,
                               prevDestComponentID.id)
        end
    end
...
function CheckNeedsToOpen()
    ...
    if UI.IsInGame() then
        ...
        if selectedUnit ~= nil then
            ...
            if selectedUnitInfo ~= nil and selectedUnitInfo.MakeTradeRoute ==
                true then
...
function OnInit(isReload)
    if isReload then LuaEvents.GameDebug_GetValues("TradeRouteChooser") end
end
...
function OnShutdown()
    ...
    LuaEvents.GameDebug_AddValue("TradeRouteChooser", "filterIndex",
                                 m_filterSelected)
    LuaEvents.GameDebug_AddValue("TradeRouteChooser", "destinationCityOwner",
                                 m_destinationCityOwner)
    LuaEvents.GameDebug_AddValue("TradeRouteChooser", "destinationCityID",
                                 m_destinationCityID)
end
...
function OnLocalPlayerTurnEnd()
    if (GameConfiguration.IsHotseat()) then Close() end
end
...
function CuiSortByYield(yieldID)
    ...
    m_filteredDestinations = {}
    for _, item in SortedTable(yieldValues,
                               function(t, a, b) return t[a].val > t[b].val end) do
...
function Initialize()
    ...
    LuaEvents.WorldInput_MakeTradeRouteDestination.Add(
        OnWorldInputMakeTradeRoute)
    ...
    Controls.BonusIconStack:RegisterSizeChanged(
        function() OnBonusIconStackSizeChanged(Controls) end)
