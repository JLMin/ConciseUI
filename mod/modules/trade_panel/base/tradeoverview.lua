...
local TRADE_TABS = {MY_ROUTES = 0, ROUTES_TO_CITIES = 1, AVAILABLE_ROUTES = 2}

local m_currentTab
local m_showMyBenefits = true
...
local m_HeaderInstanceIM = InstanceManager:new("HeaderInstance", "Top",
                                               Controls.BodyStack)
local m_SimpleButtonInstanceIM = InstanceManager:new("SimpleButtonInstance",
...
function ViewMyRoutes()
    ...
    Controls.ActiveRoutesLabel:SetText(Locale.Lookup(
                                           "LOC_TRADE_OVERVIEW_ACTIVE_ROUTES",
                                           routesActiveText, routesCapacity))

    -- Gather data and sort
    local routesSortedByPlayer = {}
    ...
    local haveAddedCityStateHeader = false
    ...
    local unusedRoutes = routesCapacity - routesActive
    if unusedRoutes > 0 then
        CreateUnusedRoutesHeader()

        local idleTradeUnits = GetIdleTradeUnits(Game.GetLocalPlayer())
...
function ViewRoutesToCities()
    ...
    -- Update Header
    Controls.HeaderLabel:SetText(Locale.ToUpper(
            ...
            for _, city in playerCities:Members() do
                ...
                for _, route in ipairs(outgoingRoutes) do
                    ...
                    if route.DestinationCityPlayer == Game.GetLocalPlayer() then
                        ...
                        table.insert(
                            routesSortedByPlayer[route.OriginCityPlayer], route)
                    end
                end
            end
        end
...
function ViewAvailableRoutes()
    ...
    -- Update Header
    Controls.HeaderLabel:SetText(Locale.ToUpper(
                                     "LOC_TRADE_OVERVIEW_AVAILABLE_ROUTES"))
    Controls.ActiveRoutesLabel:SetHide(true)

    -- Determine if a trade unit in a city can trade with any other player cities
    local pLocalPlayer = Players[localPlayerID]
    local pLocalPlayerCities = pLocalPlayer:GetCities()
    local hasTradeRouteWithPlayer = false
    local hasTradeRouteWithCityStates = false
    local players = Game.GetPlayers()
    for i, destinationPlayer in ipairs(players) do
        local playerHeader = nil
        hasTradeRouteWithPlayer = false

        local cities = destinationPlayer:GetCities()
            ...
            for cityID in pLocalPlayerCities:Members() do
                ...
                if originCity ~= nil then
                    if tradeManager:CanStartRoute(originCity:GetOwner(),
                        ...
                        local pPlayerInfluence =
                            destinationPlayer:GetInfluence()
                        ...
                        -- Add Route
                        AddRoute(pLocalPlayer, originCity, destinationPlayer,
                                 destinationCity, -1)
                    end
                end
            end
...
-- ===========================================================================
function SetMyRoutesTabSelected(isSelected)
...
-- ===========================================================================
function SetRoutesToCitiesTabSelected(isSelected)
...
-- ===========================================================================
function SetAvailableRoutesTabSelected(isSelected)
...
-- ===========================================================================
function AddChooseRouteButton(tradeUnit)
    local simpleButtonInstance = m_SimpleButtonInstanceIM:GetInstance()
    simpleButtonInstance.GridButton:SetText(
        Locale.Lookup("LOC_TRADE_OVERVIEW_CHOOSE_ROUTE"))
...
function AddProduceTradeUnitButton()
    local simpleButtonInstance = m_SimpleButtonInstanceIM:GetInstance()
    simpleButtonInstance.GridButton:SetText(
        Locale.Lookup("LOC_TRADE_OVERVIEW_PRODUCE_TRADE_UNIT"))
    simpleButtonInstance.GridButton:SetDisabled(true)
end

-- ===========================================================================
function AddRouteFromRouteInfo(routeInfo)
    ...
    local destinationCity = destinationPlayer:GetCities():FindID(
...
-- ===========================================================================
function AddRoute(originPlayer, originCity, destinationPlayer, destinationCity,
                  traderUnitID)
    local routeInstance = m_RouteInstanceIM:GetInstance()

    -- Update Route Label
    routeInstance.RouteLabel:SetText(
    ...
    for yieldIndex = 1, #yieldValues, 1 do
        ...
        if (yieldValue ~= 0) then
            ...
            if (yieldInfo.YieldType == "YIELD_FOOD") then
                resourceInstance.ResourceValueLabel:SetColorByName(
                    "ResFoodLabelCS")
            elseif (yieldInfo.YieldType == "YIELD_PRODUCTION") then
                resourceInstance.ResourceValueLabel:SetColorByName(
                    "ResProductionLabelCS")
    ...
    local influencingCity = nil
    local influencedCity = nil
    -- XOR used to show right benefits when origin/destination switch between tabs
    if m_showMyBenefits == not (m_currentTab == TRADE_TABS.ROUTES_TO_CITIES) then
    ...
    if destinationCity:GetTrade():HasActiveTradingPost(originPlayer) then
        ...
        routeInstance.TradingPostIndicator:LocalizeAndSetToolTip(
            "LOC_TRADE_OVERVIEW_TOOLTIP_TRADE_POST_ESTABLISHED")
    else
        routeInstance.TradingPostIndicator:SetAlpha(0.2)
        routeInstance.TradingPostIndicator:LocalizeAndSetToolTip(
            "LOC_TRADE_OVERVIEW_TOOLTIP_NO_TRADE_POST")
    end
    ...
    local textureOffsetX, textureOffsetY, textureSheet =
        IconManager:FindIconAtlas(originPlayerIconString, 30)
    local secondaryColor, primaryColor =
        UI.GetPlayerColors(originPlayer:GetID())
    routeInstance.OriginCivIcon:SetTexture(textureOffsetX, textureOffsetY,
                                           textureSheet)
    routeInstance.OriginCivIcon:LocalizeAndSetToolTip(
    ...
    local destinationPlayerConfig =
        PlayerConfigurations[destinationPlayer:GetID()]
    local destinationPlayerInfluence =
        Players[destinationPlayer:GetID()]:GetInfluence()
    if not destinationPlayerInfluence:CanReceiveInfluence() then
        ...
        if destinationPlayerConfig ~= nil then
            local iconString = "ICON_" ..
                                   destinationPlayerConfig:GetCivilizationTypeName()
            local textureOffsetX, textureOffsetY, textureSheet =
                IconManager:FindIconAtlas(iconString, 30)
            routeInstance.DestinationCivIcon:SetTexture(textureOffsetX,
                                                        textureOffsetY,
                                                        textureSheet)
            routeInstance.DestinationCivIcon:LocalizeAndSetToolTip(
                destinationPlayerConfig:GetCivilizationDescription())
        end

        local secondaryColor, primaryColor =
            UI.GetPlayerColors(destinationPlayer:GetID())
        ...
        if destinationPlayerConfig ~= nil then
            local secondaryColor, primaryColor =
                UI.GetPlayerColors(destinationPlayer:GetID())
            local leader = destinationPlayerConfig:GetLeaderTypeName()
            local leaderInfo = GameInfo.Leaders[leader]

            local iconString = GetCityStateIcon(leader, leaderInfo)
    ...
    if tradeUnitID and tradeUnitID ~= -1 then
        local tradeUnit = originPlayer:GetUnits():FindID(traderUnitID)
        if tradeUnit then
            if m_currentTab == TRADE_TABS.AVAILABLE_ROUTES then
                ...
                routeInstance.GridButton:RegisterCallback(Mouse.eLClick,
                    ...
                    LuaEvents.TradeOverview_SelectRouteFromOverview(
                        destinationPlayer:GetID(), destinationCity:GetID())
                end)
            else
                routeInstance.GridButton:RegisterCallback(Mouse.eLClick,
                                                          function()
...
-- ===========================================================================
function GetCityStateIcon(leaderName, leaderInfo)
    local iconString
...
function SetBenefitsFilter(showMyBenefits)
    ...
    if m_showMyBenefits then
        Controls.BenefitsLabel:SetText(Locale.Lookup(
                                           "LOC_TRADE_OVERVIEW_MY_BENEFITS"))
    else
        Controls.BenefitsLabel:SetText(Locale.Lookup(
                                           "LOC_TRADE_OVERVIEW_THEIR_BENEFITS"))
    end
...
function CreatePlayerHeader(player)
    local headerInstance = m_HeaderInstanceIM:GetInstance()

    local pPlayerConfig = PlayerConfigurations[player:GetID()]
    headerInstance.HeaderLabel:SetText(Locale.ToUpper(
                                           pPlayerConfig:GetPlayerName()))

    -- Determine are diplomatic visibility status
    local visibilityIndex = Players[Game.GetLocalPlayer()]:GetDiplomacy()
                                :GetVisibilityOn(player)
    ...
    local extraTourismModifier = Players[Game.GetLocalPlayer()]:GetCulture()
                                     :GetExtraTradeRouteTourismModifier()
    ...
    if hasTradeRoute then
        ...
        headerInstance.TourismBonusPercentage:SetColorByName(
            "TradeOverviewTextCS")
        headerInstance.TourismBonusIcon:SetTexture(0, 0, "Tourism_VisitingSmall")
        headerInstance.TourismBonusGrid:LocalizeAndSetToolTip(
            "LOC_TRADE_OVERVIEW_TOOLTIP_TOURISM_BONUS")

        headerInstance.VisibilityBonusCheckmark:SetHide(false)
        headerInstance.VisibilityBonusIcon:SetTexture(
            "Diplomacy_VisibilityIcons")
        headerInstance.VisibilityBonusIcon:SetVisState(
            math.min(math.max(visibilityIndex - 1, 0), 3))
        headerInstance.VisibilityBonusGrid:LocalizeAndSetToolTip(
            "LOC_TRADE_OVERVIEW_TOOLTIP_DIPLOMATIC_VIS_BONUS")
    else
        headerInstance.TourismBonusCheckmark:SetHide(true)
        headerInstance.TourismBonusPercentage:SetColorByName(
            "TradeOverviewTextDisabledCS")
        headerInstance.TourismBonusIcon:SetTexture(0, 0,
                                                   "Tourism_VisitingSmallGrey")
        headerInstance.TourismBonusGrid:LocalizeAndSetToolTip(
            "LOC_TRADE_OVERVIEW_TOOLTIP_NO_TOURISM_BONUS")

        headerInstance.VisibilityBonusCheckmark:SetHide(true)
        headerInstance.VisibilityBonusIcon:SetTexture(
            "Diplomacy_VisibilityIconsGrey")
        headerInstance.VisibilityBonusIcon:SetVisState(
            math.min(math.max(visibilityIndex, 0), 3))
        headerInstance.VisibilityBonusGrid:LocalizeAndSetToolTip(
            "LOC_TRADE_OVERVIEW_TOOLTIP_NO_DIPLOMATIC_VIS_BONUS")
    end
end
...
function CreateCityStateHeader()
    ...
    headerInstance.HeaderLabel:SetText(Locale.ToUpper(
                                           "LOC_TRADE_OVERVIEW_CITY_STATES"))
...
function CreateUnusedRoutesHeader()
    ...
    headerInstance.HeaderLabel:SetText(Locale.ToUpper(
                                           "LOC_TRADE_OVERVIEW_UNUSED_ROUTES"))
...
-- ===========================================================================
function GetYieldsFromCity(originCity, destinationCity)
    local tradeManager = Game.GetTradeManager()

    -- From route
    local routeYields = tradeManager:CalculateOriginYieldsFromPotentialRoute(
                            originCity:GetOwner(), originCity:GetID(),
                            destinationCity:GetOwner(), destinationCity:GetID())
    -- From path
    local pathYields = tradeManager:CalculateOriginYieldsFromPath(
                           originCity:GetOwner(), originCity:GetID(),
                           destinationCity:GetOwner(), destinationCity:GetID())
    -- From modifiers
    local modifierYields = tradeManager:CalculateOriginYieldsFromModifiers(
                               originCity:GetOwner(), originCity:GetID(),
                               destinationCity:GetOwner(),
...
-- ===========================================================================
function GetYieldsForDestinationCity(originCity, destinationCity)
    local tradeManager = Game.GetTradeManager()
    ...
    local routeYields =
        tradeManager:CalculateDestinationYieldsFromPotentialRoute(
            ...
            destinationCity:GetOwner(), destinationCity:GetID())
    -- From path
    local pathYields = tradeManager:CalculateDestinationYieldsFromPath(
                           originCity:GetOwner(), originCity:GetID(),
                           destinationCity:GetOwner(), destinationCity:GetID())
    -- From modifiers
    local modifierYields = tradeManager:CalculateDestinationYieldsFromModifiers(
                               originCity:GetOwner(), originCity:GetID(),
                               destinationCity:GetOwner(),
                               destinationCity:GetID())
...
function OnShutdown()
    ...
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "showMyBenefits",
                                 m_showMyBenefits)
end
...
function OnGameDebugReturn(context, contextTable)
    if context == RELOAD_CACHE_ID then
        if contextTable["showMyBenefits"] ~= nil and
            contextTable["showMyBenefits"] then
            SetBenefitsFilter(true)
        else
            SetBenefitsFilter(false)
        end
...
function OnUnitOperationStarted(ownerID, unitID, operationID)
    if m_AnimSupport.IsVisible() and operationID ==
        UnitOperationTypes.MAKE_TRADE_ROUTE then Refresh() end
end
...
function Initialize()
    ...
    Controls.RoutesToCitiesButton:RegisterCallback(Mouse.eLClick,
                                                   OnRoutesToCitiesButton)
    Controls.RoutesToCitiesButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
    Controls.AvailableRoutesButton:RegisterCallback(Mouse.eLClick,
                                                    OnAvailableRoutesButton)
    Controls.AvailableRoutesButton:RegisterCallback(Mouse.eMouseEnter,
                                                    function()
        UI.PlaySound("Main_Menu_Mouse_Over")
    end)
