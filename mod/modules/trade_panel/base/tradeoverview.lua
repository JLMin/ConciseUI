...
--    VARIABLES
-- ===========================================================================
local m_RouteInstanceIM = InstanceManager:new("RouteInstance", "Top", Controls.BodyStack)
local m_HeaderInstanceIM = InstanceManager:new("HeaderInstance", "Top", Controls.BodyStack)
local m_SimpleButtonInstanceIM = InstanceManager:new("SimpleButtonInstance", "Top", Controls.BodyStack)
...
function ViewMyRoutes()
    ...
    Controls.ActiveRoutesLabel:SetText(
        Locale.Lookup("LOC_TRADE_OVERVIEW_ACTIVE_ROUTES", routesActiveText, routesCapacity)
    )
...
function AddChooseRouteButton(tradeUnit)
    ...
    simpleButtonInstance.GridButton:RegisterCallback(
        Mouse.eLClick,
        function()
...
-- ===========================================================================
function AddRoute(originPlayer, originCity, destinationPlayer, destinationCity, traderUnitID)
    local routeInstance = m_RouteInstanceIM:GetInstance()

    -- Update Route Label
    routeInstance.RouteLabel:SetText(
        Locale.ToUpper(originCity:GetName()) ..
            " " .. Locale.ToUpper("LOC_TRADE_OVERVIEW_TO") .. " " .. Locale.ToUpper(destinationCity:GetName())
    )
    ...
    local originPlayerIconString = "ICON_" .. originPlayerConfig:GetCivilizationTypeName()
    ...
    local destinationPlayerConfig = PlayerConfigurations[destinationPlayer:GetID()]
    local destinationPlayerInfluence = Players[destinationPlayer:GetID()]:GetInfluence()
    if not destinationPlayerInfluence:CanReceiveInfluence() then
        ...
        if destinationPlayerConfig ~= nil then
            local iconString = "ICON_" .. destinationPlayerConfig:GetCivilizationTypeName()
    ...
    if tradeUnitID and tradeUnitID ~= -1 then
        ...
        if tradeUnit then
            if m_currentTab == TRADE_TABS.AVAILABLE_ROUTES then
                ...
                routeInstance.GridButton:RegisterCallback(
                    Mouse.eLClick,
                    function()
                        SelectUnit(tradeUnit)
                        LuaEvents.TradeOverview_SelectRouteFromOverview(
                            destinationPlayer:GetID(),
                            destinationCity:GetID()
                        )
                    end
                )
...
function CreatePlayerHeader(player)
    ...
    local visibilityIndex = Players[Game.GetLocalPlayer()]:GetDiplomacy():GetVisibilityOn(player)
...
function Initialize()
    ...
    Controls.MyRoutesButton:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    ...
    Controls.RoutesToCitiesButton:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    ...
    Controls.BenefitsButton:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
