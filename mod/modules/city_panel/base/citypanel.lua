...
function ViewMain(data)
    ...
    if (m_primaryColor == nil or m_secondaryColor == nil or m_primaryColor == 0 or m_secondaryColor == 0) then
        UI.DataError(
            "Couldn't find player colors for player - " .. (m_pPlayer and tostring(m_pPlayer:GetID()) or "nil")
        )
    end
    ...
    -- Name data
    Controls.CityName:SetText(
        (data.IsCapital and "[ICON_Capital]" or "") .. Locale.ToUpper(Locale.Lookup(data.CityName))
    )
        ...
        if cityCulture ~= nil then
            ...
            if (newGrowthPlot ~= -1 and newGrowthPlot ~= g_growthPlotId) then
                ...
                Controls.TurnsLeftDescription:SetText(
                    Locale.ToUpper(Locale.Lookup("LOC_HUD_CITY_TURNS_UNTIL_BORDER_GROWTH", turnsRemaining))
                )
...
function SetupCollapsibleToggle(pCheckBoxControl, pButtonControl, pAreaControl, kParentControls)
    ...
    if pButtonControl ~= nil then
        pButtonControl:RegisterCallback(
            Mouse.eLClick,
            function()
    ...
    if m_kTutorialDisabledControls ~= nil then
        for _, name in ipairs(m_kTutorialDisabledControls) do
            if Controls[name] ~= nil then
                ...
                UI.DataError(
                    "Tutorial requested the control '" ..
                        name .. "' be disabled in the city panel, but no such control exists in that context."
                )
            end
        end
    end
...
function Initialize()
    ...
    Controls.CultureIgnore:RegisterCallback(
        Mouse.eLClick,
        function()
            OnResetYieldToNormal(YieldTypes.CULTURE, "Culture")
        end
    )
    Controls.FaithIgnore:RegisterCallback(
        Mouse.eLClick,
        function()
            OnResetYieldToNormal(YieldTypes.FAITH, "Faith")
        end
    )
    Controls.FoodIgnore:RegisterCallback(
        Mouse.eLClick,
        function()
            OnResetYieldToNormal(YieldTypes.FOOD, "Food")
        end
    )
    Controls.GoldIgnore:RegisterCallback(
        Mouse.eLClick,
        function()
            OnResetYieldToNormal(YieldTypes.GOLD, "Gold")
        end
    )
    Controls.ProductionIgnore:RegisterCallback(
        Mouse.eLClick,
        function()
            OnResetYieldToNormal(YieldTypes.PRODUCTION, "Production")
        end
    )
    Controls.ScienceIgnore:RegisterCallback(
        Mouse.eLClick,
        function()
    ...
    if GameCapabilities.HasCapability("CAPABILITY_GOLD") then
        ...
        Controls.ProduceWithGoldCheck:RegisterCallback(
            Mouse.eMouseEnter,
            function()
    ...
    Controls.ChangeProductionCheck:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    ...
    Controls.ProduceWithFaithCheck:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    ...
    Controls.ToggleOverviewPanel:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
