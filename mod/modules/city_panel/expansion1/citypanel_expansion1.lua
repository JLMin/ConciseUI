...
function ViewMain(kData)
    ...
    if pCity ~= nil then
        ...
        Controls.BreakdownLabel:SetText(Locale.ToUpper(
                                            Locale.Lookup(
                                                "LOC_CULTURAL_IDENTITY_LOYALTY_SUBSECTION")))
        Controls.BreakdownNum:SetText(Round(currentLoyalty, 1))
        -- CUI: districts numbers
        Controls.ReligionIcon:SetIcon("ICON_BUILDINGS")
        Controls.ReligionLabel:SetText(Locale.ToUpper(
                                           Locale.Lookup("LOC_HUD_DISTRICTS")))
        Controls.ReligionNum:SetText(kData.DistrictsNum .. "/" ..
                                         kData.DistrictsPossibleNum)
