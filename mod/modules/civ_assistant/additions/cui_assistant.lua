...
local victoryIconInstance = InstanceManager:new("VictoryIconInstance", "Top",
                                                Controls.VictoryIconInstanceContainer)
local victoryLeaderInstance = InstanceManager:new("VictoryLeaderInstance",
                                                  "Top",
...
function UpdateVictoryToolTip(vType)
    ...
    for i, leader in ipairs(leaders) do
        local leaderInstance = victoryLeaderInstance:GetInstance(
                                   CuiVictoryTT.VictoryLeaderStack)
...
function SetVictoryLeaderInstance(vType, leader, instance)
    ...
    if shouldShowIcon then
        if vType == "SCIENCE" then
            text1 =
                "[ICON_SCIENCE]" .. leader.scienceY .. " (" .. leader.techs ..
                    ")"
            local progressText = ""
            local progress = leader.progresses
            if isExpansion2 then
                text2 = Locale.Lookup("LOC_CUI_DB_EXOPLANET_EXPEDITION",
                                      progress[1], progress[2], progress[3],
                                      progress[4], progress[5])
            else
                text2 = Locale.Lookup("LOC_CUI_DB_MARS_PROJECT", progress[1],
                                      progress[2], progress[3])
            end
        elseif vType == "CULTURE" then
            text1 = "[ICON_CULTURE]" .. leader.cultureY .. " ([ICON_TOURISM]" ..
                        leader.tourism .. ")"
            text2 = Locale.Lookup("LOC_CUI_DB_VISITING_TOURISTS",
                                  leader.visiter, leader.tourists)
        elseif vType == "DOMINATION" then
            text1 = "[ICON_STRENGTH]" .. leader.strength
            text2 =
                Locale.Lookup("LOC_CUI_DB_CAPITALS_CAPTURED", leader.capture)
        elseif vType == "RELIGION" then
            text1 = "[ICON_FAITH]" .. leader.faithY
            text2 = Locale.Lookup("LOC_CUI_DB_CIVS_CONVERTED", leader.convert,
                                  leader.totalCiv)
        elseif vType == "DIPLOMATIC" then
            text1 =
                "[ICON_FAVOR] " .. leader.favor .. " (+" .. leader.favorPT ..
                    ")"
            text2 = Locale.Lookup("LOC_CUI_DB_DIPLOMATIC_POINT", leader.current,
                                  leader.total)
        end
    end

    instance.Icon:SetTexture(CuiLeaderTexture(leader.leaderIcon, 45,
                                              shouldShowIcon))
