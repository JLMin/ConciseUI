    ...
    for _, city in pPlayerCities:Members() do
        ...
        for i, projectInfo in ipairs(EARTH_SATELLITE_PROJECT_INFOS) do
            ...
            if pPlayerStats:GetNumProjectsAdvanced(projectInfo.Index) == 0 then
                projectProgress = pBuildQueue:GetProjectProgress(
                                      projectInfo.Index)
            end
            if projectProgress ~= 0 then
                local progress = Round((projectProgress / projectCost), 2) * 100
                progresses[1] = progress == 100 and "[ICON_CheckmarkBlue]" or
                                    (tostring(progress) .. "%")
                pValue[1] = progress
            end
        end
        ...
        if progresses[1] == "[ICON_CheckmarkBlue]" then
            for i, projectInfo in ipairs(MOON_LANDING_PROJECT_INFOS) do
                local projectCost =
                    pBuildQueue:GetProjectCost(projectInfo.Index)
                local projectProgress = projectCost
                if pPlayerStats:GetNumProjectsAdvanced(projectInfo.Index) == 0 then
                    projectProgress = pBuildQueue:GetProjectProgress(
                                          projectInfo.Index)
                ...
                if projectProgress ~= 0 then
                    local progress = Round((projectProgress / projectCost), 2) *
                                         100
                    progresses[2] =
                        progress == 100 and "[ICON_CheckmarkBlue]" or
                            (tostring(progress) .. "%")
                    pValue[2] = progress
                end
            end
...
function GetGSScienceCustomData(playerID) -- science victory Gathering Storm
    ...
    if progresses[4] == "[ICON_CheckmarkBlue]" then
        if localPlayer then
            ...
            local lightYearsPerTurn = pPlayer:GetStats()
                                          :GetScienceVictoryPointsPerTurn()
            local totalLightYears = localPlayer:GetStats()
                                        :GetScienceVictoryPointsTotalNeeded()
            ...
            if progress >= 100 then
                progresses[5] =
                    "[ICON_Checkmark][ICON_Checkmark][ICON_Checkmark][ICON_Checkmark][ICON_Checkmark]"
            else
                progresses[5] = lightYears .. "([COLOR_ModStatusGreen]+" ..
                                    lightYearsPerTurn .. "[ENDCOLOR]) / " ..
                                    totalLightYears
            end
            pValue[5] = progress
        end
...
function GetCultureCustomData(playerID)
    ...
    for i, player in ipairs(Players) do
        if i ~= playerID and IsAliveAndMajor(i) and player:GetTeam() ~=
            pPlayer:GetTeam() then
...
function GetMilitaryCustomData(playerID)
    ...
    for _, city in pPlayer:GetCities():Members() do
        ...
        if (playerID ~= originalOwnerID and pOriginalOwner:IsMajor() and
            city:IsOriginalCapital()) then capitals = capitals + 1 end
    end
    data.cities = cities
        ...
        if IsAliveAndMajor(i) then
            ...
            if playerReligionType ~= -1 then
                ...
                if otherReligion ~= nil then
                    local otherReligionType =
                        otherReligion:GetReligionInMajorityOfCities()
