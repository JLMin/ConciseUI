...
function AttachToTopPanel()
    if not isAttached then
        ...
        ContextPtr:BuildInstanceForControl("CuiNotes", notesButtonInstance,
                                           buttonStack)
        ContextPtr:BuildInstanceForControl("Pip", pipInstance, buttonStack)

        notesButtonInstance.NotesButton:RegisterCallback(Mouse.eLClick,
                                                         OnToggleNoteScreen)
        notesButtonInstance.NotesButton:SetToolTipString(
            Locale.Lookup("LOC_CUI_NOTES"))

        local x, y, sheet = IconManager:FindIconAtlas(
                                "ICON_CIVIC_DIPLOMATIC_SERVICE", 42)
