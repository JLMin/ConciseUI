...
local TXT_PLAYER_DISCONNECTED_CHAT = Locale.Lookup("LOC_MP_PLAYER_DISCONNECTED_CHAT")
local TXT_PLAYER_KICKED_CHAT = Locale.Lookup("LOC_MP_PLAYER_KICKED_CHAT")

-- ===========================================================================
--    MEMBERS
-- ===========================================================================
local m_gossipIM = InstanceManager:new("GossipMessageInstance", "Root", Controls.GossipStack)
local m_statusIM = InstanceManager:new("StatusMessageInstance", "Root", Controls.DefaultStack)
local m_kGossip = {}
...
function AddGossip(eGossipType, message, optionalDisplayTime)
    ...
    if kGossipData then
        ...
        UI.DataError(
            "The gossip type '" ..
                tostring(eGossipType) .. "' could not be found in the database; icon will not be set."
        )
    end
    ...
    local verticalSpace = uiInstance.Message:GetSizeY() + GOSSIP_VERTICAL_PADDING
    ...
    uiInstance.Anim:RegisterEndCallback(
        function()
            OnGossipEndAnim(kEntry, uiInstance)
        end
    )
    uiInstance.Button:RegisterCallback(
        Mouse.eLClick,
        function()
    ...
    uiInstance.ExpandButton:RegisterCallback(
        Mouse.eLClick,
        function()
...
function OnMultplayerPlayerConnected(playerID)
    ...
    if (ContextPtr:IsHidden() == false and GameConfiguration.IsNetworkMultiplayer()) then
        ...
        local statusMessage = Locale.Lookup(pPlayerConfig:GetPlayerName()) .. " " .. TXT_PLAYER_CONNECTED_CHAT
    ...
    ContextPtr:SetInputHandler(
        function(pInputStruct)
            ...
            if uiMsg == KeyEvents.KeyUp then
                ...
                if key == Keys.F then
                    OnStatusMessage("The rain in Spain will dry quickly in Madrid.", 7, type, subType)
                    return true
                end
                if key == Keys.G then
