...
--	MEMBERS
-- ===========================================================================
local m_civicsIM = InstanceManager:new("CivicListInstance", "TopContainer",
                                       Controls.CivicStack)
...
function AddAvailableCivic(playerID, kData)
    ...
    if (cachedModifiers) then
        for _, tModifier in ipairs(cachedModifiers) do
            ...
            if (tIconData) then
                hideDescriptionIcon = hideDescriptionIcon or
                                          tIconData.HideDescriptionIcon
                table.insert(extraUnlocks,
                             {IconData = tIconData, ModifierTable = tModifier})
            end
        end
    end

    -- Handle overflow for unlock icons
    numUnlockables = PopulateUnlockablesForCivic(playerID, kData.ID, unlockIM,
                                                 nil, callback,
                                                 hideDescriptionIcon)

    -- Initialize extra icons
    for _, tUnlock in pairs(extraUnlocks) do
        tUnlock.IconData:Initialize(kItemInstance.UnlockStack,
...
function OnChooseCivic(civicHash)
    ...
    if (Game.GetLocalPlayer() >= 0) then
        ...
        UI.RequestPlayerOperation(Game.GetLocalPlayer(),
                                  PlayerOperations.PROGRESS_CIVIC, tParameters)
...
function RealizeSize()
    ...
    Controls.ChooseCivicList:SetSizeY(screenY -
                                          Controls.ChooseCivicList:GetOffsetY() -
                                          30)
...
function Initialize()
    ...
    Controls.Title:SetText(Locale.ToUpper(
                               Locale.Lookup("LOC_CIVICS_CHOOSER_CHOOSE_CIVIC")))
    Controls.OpenTreeButton:SetText(Locale.Lookup(
                                        "LOC_CIVICS_CHOOSER_OPEN_CIVICS_TREE"))
