            ...
            if cityPlots ~= nil and next(cityPlots) ~= nil then
                ...
                for _, plotID in pairs(cityPlots) do
                    ...
                    for _, budType in ipairs(budTypes) do
                        ...
                        if building.IsWonder and
                            buds:HasBuilding(building.Index) then
                            local name = building.Name
...
function GetResourceData()
    ...
    for resource in GameInfo.Resources() do
        ...
        local amount = localPlayer:GetResources():GetResourceAmount(
                           resource.Index)
        ...
        elseif resource.ResourceClassType == "RESOURCECLASS_STRATEGIC" then
            ...
            if isExpansion2 then
                ...
                local stockpileCap = playerResources:GetResourceStockpileCap(
                                         resource.ResourceType)

                local stockpileAmount = playerResources:GetResourceAmount(
                                            resource.ResourceType)
                ...
                local accumulationPerTurn =
                    playerResources:GetResourceAccumulationPerTurn(
                        resource.ResourceType)
                local importPerTurn = playerResources:GetResourceImportPerTurn(
                                          resource.ResourceType)
                local bonusPerTurn = playerResources:GetBonusResourcePerTurn(
                                         resource.ResourceType)
                local perTurnAmount = accumulationPerTurn + importPerTurn +
                                          bonusPerTurn

                local unitConsumptionPerTurn =
                    playerResources:GetUnitResourceDemandPerTurn(
                        resource.ResourceType)
                local powerConsumptionPerTurn =
                    playerResources:GetPowerResourceDemandPerTurn(
                        resource.ResourceType)
                local totalConsumptionPerTurn =
                    unitConsumptionPerTurn + powerConsumptionPerTurn
                ...
                table.insert(r_strategic,
                             {Icon = icon, Amount = amount, Order = order})
            end
        end
    end
    ...
    local possibleResources = DealManager.GetPossibleDealItems(localPlayerID,
                                                               localPlayerID,
    ...
    if (possibleResources ~= nil) then
        for i, resource in ipairs(possibleResources) do
            ...
            if (resourceDesc ~= nil and resourceDesc.ResourceClassType ==
...
function GetPlayerBasicData()
    ...
    for _, pPlayer in ipairs(aliveMajors) do
        ...
        playerData[playerID] = {
            ...
            isHuman = GameConfiguration.IsAnyMultiplayer() and
