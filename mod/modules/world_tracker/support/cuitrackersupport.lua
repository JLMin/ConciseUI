...
function GetResourceData()
  ...
  for resource in GameInfo.Resources() do
    ...
    if resource.ResourceClassType == "RESOURCECLASS_LUXURY" then
      if amount > 0 then
        table.insert(r_luxury, {Icon = icon, Amount = amount, CanTrade = false});
      end
    elseif resource.ResourceClassType == "RESOURCECLASS_STRATEGIC" then
      local order = 0;
      if     resource.ResourceType == "RESOURCE_HORSES"   then order = 1;
      elseif resource.ResourceType == "RESOURCE_IRON"     then order = 2;
      elseif resource.ResourceType == "RESOURCE_NITER"    then order = 3;
      elseif resource.ResourceType == "RESOURCE_COAL"     then order = 4;
      elseif resource.ResourceType == "RESOURCE_OIL"      then order = 5;
      elseif resource.ResourceType == "RESOURCE_ALUMINUM" then order = 6;
      elseif resource.ResourceType == "RESOURCE_URANIUM"  then order = 7;
      end
      ...
      if isExpansion2 then
        ...
        table.insert(r_strategic, {Icon     = icon,
                                   Amount   = totalAmount,
                                   Cap      = stockpileCap,
...
function GetBorderData()
  ...
  for _, pData in pairs(playerData) do
    ...
    if playerID ~= localPlayerID then
      if (pData.isMet or pData.isHuman) then
        local openTo   = false;
        ...
        if openTo or openFrom then
          table.insert(leaders, {Icon = pData.leaderIcon, OpenTo = openTo, OpenFrom = openFrom});
          active = true;
        end
      end
    end
  ...
  for _, pData in pairs(playerData) do
    ...
    if playerID ~= localPlayerID then
      if (pData.isMet or pData.isHuman) then
        ...
        table.insert(leaders, {Icon = pData.leaderIcon, IsTraded = routeNum > 0, RouteNum = routeNum});
        --
      end
    end
  end
