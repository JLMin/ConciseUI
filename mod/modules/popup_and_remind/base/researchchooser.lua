...
--    CONSTANTS
-- ===========================================================================
local RELOAD_CACHE_ID = "ResearchChooser" -- Must be unique (usually the same as the file name)
local SIZE_ICON_LARGE = 38
local SIZE_ICON_SMALL = 30
...
function GetData()
    ...
    for kTech in GameInfo.Technologies() do
        ...
        if
            iTech == m_currentID or iTech == m_lastCompletedID or
                (iTech ~= m_currentID and pPlayerTechs:CanResearch(iTech))
         then
...
function AddAvailableResearch(playerID, kData)
    ...
    kItemInstance.Top:RegisterCallback(
        Mouse.eMouseEnter,
        function()
...
function Initialize()
    ...
    Controls.IconButton:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    ...
    if (HasCapability("CAPABILITY_TECH_TREE")) then
        ...
        Controls.OpenTreeButton:RegisterCallback(
            Mouse.eLClick,
            function()
