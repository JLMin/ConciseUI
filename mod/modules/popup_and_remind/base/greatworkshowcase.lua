...
function UpdateGreatWork()
    ...
    local greatWorkCreationDate =
        ...
        m_CityBldgs:GetTurnFromIndex(m_GreatWorkIndex),
    ...
    Controls.CreatedDate:SetText(
        Locale.Lookup("LOC_GREAT_WORKS_CREATED_TIME", greatWorkTypeName, greatWorkCreationDate)
    )
    Controls.CreatedPlace:SetText(
        Locale.Lookup("LOC_GREAT_WORKS_CREATED_PLACE", greatWorkCreationBuilding, greatWorkCreationCity)
    )
...
function OnGameDebugReturn(context, data)
    if context == RELOAD_CACHE_ID and data["isHidden"] ~= nil and not data["isHidden"] then
        if data["isGallery"] then
            OnViewGreatWork(
                Cities.GetCityInPlot(Map.GetPlotIndex(data["cityX"], data["cityY"])),
                data["buildingID"],
                data["greatWorkIndex"]
            )
        else
...
function Initialize()
    ...
    Controls.ViewGreatWorks:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
