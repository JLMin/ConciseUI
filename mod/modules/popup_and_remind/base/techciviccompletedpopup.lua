...
local m_unlockIM = InstanceManager:new("UnlockInstance", "Top", Controls.UnlockStack)
...
function ShowCivicCompletedPopup(player, civic, quote, audio)
    ...
    Controls.UnlockCountLabel:SetText(
        Locale.Lookup("LOC_RESEARCH_COMPLETE_UNLOCKED_BY_CIVIC", m_unlockIM.m_iAllocatedInstances)
    )
    ...
    if quote then
        ...
        if audio then
            ...
            Controls.QuoteButton:RegisterCallback(
                Mouse.eLClick,
                function()
                    UI.PlaySound(audio)
                end
    ...
    if isCivicUnlockGovernmentType then
        ...
        Controls.ChangeGovernmentButton:RegisterCallback(
            Mouse.eMouseEnter,
            function()
...
function ShowTechCompletedPopup(player, tech, quote, audio)
    ...
    Controls.UnlockCountLabel:SetText(
        Locale.Lookup("LOC_RESEARCH_COMPLETE_UNLOCKED_BY_TECH", m_unlockIM.m_iAllocatedInstances)
    )
...
function AddCompletedPopup(player, civic, tech, isByUser)
    ...
    if player == Game.GetLocalPlayer() and isNotBlockedByTutorial and (not GameConfiguration.IsNetworkMultiplayer()) then
        ...
        if civic then
            ...
            if civicInfo == nil then
                UI.DataError(
                    "Cannot show civic popup because GameInfo.Civics[" .. tostring(civic) .. "] doesn't have data."
                )
                return
            end
            ...
            if techInfo == nil then
                UI.DataError(
                    "Cannot add popup because GameInfo.Technologies[" .. tostring(tech) .. "] doesn't have data."
                )
                return
            end
