...
local m_LargeMomentIM = InstanceManager:new("LargeMoment", "Root", Controls.TimelineStack)

local m_NewSmallMomentIM = InstanceManager:new("NewSmallMoment", "Root")
local m_NewLargeMomentIM = InstanceManager:new("NewLargeMoment", "Root", Controls.TimelineStack)

local m_LargeIllustrationIM = InstanceManager:new("LargeIllustration", "Root")
local m_SmallMomentStackIM = InstanceManager:new("SmallMomentStack", "Root", Controls.TimelineStack)

local m_EraLabelIM = InstanceManager:new("EraLabel", "Root", Controls.TimelineStack)
local m_TimelinePaddingIM = InstanceManager:new("TimelinePadding", "Root", Controls.TimelineStack)
...
function DebugMomentData(momentData)
    local debugInfo =
        "(Turn = " ..
        momentData.Turn ..
            ", GameEra = " .. momentData.GameEra .. ", ActingPlayer = " .. momentData.ActingPlayer .. ", ExtraData = "
...
function ShowNewTimelineMoment(popupData)
    ...
    Controls.ModalScreenTitle:SetText(
        Locale.ToUpper(Locale.Lookup("LOC_HISTORY_NEW_MOMENT", pPlayerConfig:GetCivilizationDescription()))
    )
end
            ...
            if momentID then
                ...
                UI.DataError(
                    "Moment Notification received, but is missing 'MomentID' variant. PlayerID=" ..
                        tostring(playerID) .. ", NotificationID=" .. tostring(notificationID)
                )
            end
        end
    end
...
function DisplayTimeline(showAnim)
    ...
    local pPlayerConfig = PlayerConfigurations[localPlayerID]
    Controls.ModalScreenTitle:SetText(
        Locale.ToUpper(Locale.Lookup("LOC_HISTORY_TIMELINE_TITLE", pPlayerConfig:GetCivilizationDescription()))
    )
    ...
    if momentInfo then
        ...
        if (momentScore ~= 0) then
            instance.Effect:SetText(
                Locale.Lookup("LOC_HISTORY_MOMENT_EFFECTS", momentDate, momentData.Turn, "+" .. momentScore)
            )
        else
            ...
            if illustrations then
                for _, dataPair in ipairs(momentData.ExtraData) do
                    ...
                    if dataType and dataValue then
                        ...
                        UI.DataError(
                            "Malformed ExtraData in Moment { ID='" ..
                                momentData.ID ..
                                    "', Type='" .. momentData.Type .. "' }, expected DataType and DataValue fields"
                        )
                    end
                end
            else
        ...
        UI.DataError(
            "No data was found on Moments table for Moment { ID='" ..
                tostring(momentData.ID) .. "', Type='" .. tostring(momentData.Type) .. "' }"
        )
    end
end
...
function AddIllustration(instance, texture, dataType, momentType)
    if instance.Illustrations then
        ...
        UI.DataError(
            "Moment '" ..
                momentType ..
                    "' attempted to load illustration '" ..
                        texture ..
...
function RealizeStackSize()
    ...
    local bgSize = math.max(stackSizeX + TIMELINE_STACK_X_OFFSET_SCROLL, m_ScreenWidth)
    ...
    Controls.TimelineStack:SetOffsetX(
        shouldScroll and TIMELINE_STACK_X_OFFSET_SCROLL or TIMELINE_STACK_X_OFFSET_NO_SCROLL
    )
...
function Show()
    if ContextPtr:IsHidden() then
        ...
        ContextPtr:ChangeParent(
            ContextPtr:LookUpControl("/InGame/" .. (m_isOpenFromEndGame and "AdditionalUserInterfaces" or "Screens"))
        )
...
function CacheMomentIllustrations()
    for illustration in GameInfo.MomentIllustrations() do
        ...
        local illustrationType = m_CachedIllustrations[illustration.MomentIllustrationType]
