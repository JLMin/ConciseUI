...
local unitGroupIM = InstanceManager:new("UnitGroupInstance", "Top",
                                        Controls.UnitGroupStack)
local UNIT_GROUP = {
...
function GetUnitExpenses()
    ...
    for i, pUnit in pUnits:Members() do
        ...
        if (unitMilitaryFormation == MilitaryFormationTypes.CORPS_FORMATION) then
            unitMaintenance =
                UnitManager.GetUnitCorpsMaintenance(pUnitInfo.Hash)
    ...
    if isExpansion2 then
        ...
        for resource in GameInfo.Resources() do
            if (resource.ResourceClassType ~= nil and resource.ResourceClassType ~=
                "RESOURCECLASS_BONUS" and resource.ResourceClassType ~=
                "RESOURCECLASS_LUXURY" and resource.ResourceClassType ~=
                "RESOURCECLASS_ARTIFACT") then
                local unitConsumptionPerTurn =
                    pPlayerResources:GetUnitResourceDemandPerTurn(
                        resource.ResourceType)
                if unitConsumptionPerTurn > 0 then
                    local iconName = "[ICON_" .. resource.ResourceType .. "]"
                    local cost = "[COLOR_RED]" .. -unitConsumptionPerTurn ..
                                     "[ENDCOLOR]"
    ...
    local totalCost =
        Locale.Lookup("LOC_HUD_REPORTS_TOTAL_EXPENSES_PER_TURN") .. "  " ..
            goldText .. resourcesText
    return totalCost
end
...
function AddUnitToUnitList(pUnit, stackControl)
    local unitEntry = {}
    ContextPtr:BuildInstanceForControl("UnitEntryInstance", unitEntry,
                                       stackControl)

    -- unit icon
    local iconInfo, iconShadowInfo = GetUnitIcon(pUnit, 38, true)
    if iconInfo.textureSheet then
        unitEntry.UnitTypeIcon:SetTexture(iconInfo.textureOffsetX,
                                          iconInfo.textureOffsetY,
    ...
    if showDetails then
        ...
        elseif activityType ~= ActivityTypes.ACTIVITY_AWAKE and
            pUnit:GetFortifyTurns() > 0 then
        ...
        for _, promotion in ipairs(promotionList) do
            ...
            local promotionInst = {}
            ContextPtr:BuildInstanceForControl("PromotionInstance",
                                               promotionInst,
                                               unitEntry.PromotionsStack)
            local descriptionStr = Locale.Lookup(promotionDef.Name)
            descriptionStr = descriptionStr .. "[NEWLINE]" ..
                                 Locale.Lookup(promotionDef.Description)
            promotionInst.Top:SetToolTipString(descriptionStr)
        end

        -- upgrade
        unitEntry.UpgradeButton:SetDisabled(true)
        local bCanStart = UnitManager.CanStartCommand(pUnit,
                                                      UnitCommandTypes.UPGRADE,
                                                      true)
        if bCanStart then
            local bCanStartNow, tResults =
                UnitManager.CanStartCommand(pUnit, UnitCommandTypes.UPGRADE,
                                            false, true)
            local toolTipString = ""
            if (tResults ~= nil) then
                if (tResults[UnitCommandResults.UNIT_TYPE] ~= nil) then
                    local upgradeUnitName =
                        GameInfo.Units[tResults[UnitCommandResults.UNIT_TYPE]]
                            .Name
                    toolTipString = Locale.Lookup(upgradeUnitName)
                    local upgradeCost = pUnit:GetUpgradeCost()
                    if (upgradeCost ~= nil) then
                        toolTipString = Locale.Lookup(
                                            "LOC_UNITOPERATION_UPGRADE_INFO",
                                            upgradeUnitName, upgradeCost)
                    end
                    toolTipString = toolTipString ..
                                        AddUpgradeResourceCost(pUnit)
                end
                if (tResults[UnitOperationResults.ACTION_NAME] ~= nil and
                    tResults[UnitOperationResults.ACTION_NAME] ~= "") then
                    toolTipString = Locale.Lookup(
                                        tResults[UnitOperationResults.ACTION_NAME])
                end
                if (tResults[UnitOperationResults.ADDITIONAL_DESCRIPTION] ~= nil) then
                    for i, v in ipairs(
                                    tResults[UnitOperationResults.ADDITIONAL_DESCRIPTION]) do
                ...
                if not bCanStartNow then
                    if (tResults[UnitOperationResults.FAILURE_REASONS] ~= nil) then
                        ...
                        for i, v in ipairs(
                                        tResults[UnitOperationResults.FAILURE_REASONS]) do
                            toolTipString =
                                toolTipString .. "[NEWLINE]" .. "[COLOR:Red]" ..
            ...
            if bCanStartNow then
                ...
                unitEntry.UpgradeButton:RegisterCallback(Mouse.eLClick,
                                                         function()
...
function SetUnitEntryStatusIcon(unitEntry, icon)
    local textureOffsetX, textureOffsetY, textureSheet =
        IconManager:FindIconAtlas(icon, 22)
    unitEntry.UnitStatusIcon:SetTexture(textureOffsetX, textureOffsetY,
                                        textureSheet)
...
function AddUpgradeResourceCost(pUnit)
    if isExpansion2 then
        ...
        if (GameInfo.Units_XP2 ~= nil) then
            local upgradeResource, upgradeResourceCost =
                pUnit:GetUpgradeResourceCost()
            if (upgradeResource ~= nil and upgradeResource >= 0) then
                local resourceName = Locale.Lookup(
                                         GameInfo.Resources[upgradeResource]
                                             .Name)
                local resourceIcon = "[ICON_" ..
                                         GameInfo.Resources[upgradeResource]
                                             .ResourceType .. "]"
                toolTipString = "[NEWLINE]" ..
                                    Locale.Lookup(
                                        "LOC_UNITOPERATION_UPGRADE_RESOURCE_INFO",
                                        upgradeResourceCost, resourceIcon,
                                        resourceName)
            end
        end
