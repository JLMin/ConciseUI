...
local g_playerTarget = {
    targetType = ChatTargetTypes.CHATTARGET_PLAYER,
    targetID = Game.GetLocalPlayer()
}
...
local defaultIcons = {
    {icon = "ICON_MAP_PIN_STRENGTH"}, {icon = "ICON_MAP_PIN_RANGED"},
    {icon = "ICON_MAP_PIN_BOMBARD"}, {icon = "ICON_MAP_PIN_DISTRICT"},
    {icon = "ICON_MAP_PIN_CHARGES"}, {icon = "ICON_MAP_PIN_DEFENSE"},
    {icon = "ICON_MAP_PIN_MOVEMENT"}, {icon = "ICON_MAP_PIN_NO"},
    {icon = "ICON_MAP_PIN_PLUS"}, {icon = "ICON_MAP_PIN_CIRCLE"},
    {icon = "ICON_MAP_PIN_TRIANGLE"}, {icon = "ICON_MAP_PIN_SUN"},
...
local sendToChatNotVisibleTTStr = Locale.Lookup(
                                      "LOC_MAP_PIN_SEND_TO_CHAT_NOT_VISIBLE_TT")
...
function MapPinIsVisibleToChatTarget(mapPinVisibility, chatPlayerTarget)
    ...
    elseif (mapPinVisibility == ChatTargetTypes.CHATTARGET_TEAM) then
        ...
        elseif (chatPlayerTarget.targetType == ChatTargetTypes.CHATTARGET_PLAYER and
            chatPlayerTarget.targetID ~= NO_PLAYERTARGET_ID) then
    ...
    elseif (mapPinVisibility >= 0) then
        ...
        if (chatPlayerTarget.targetType == ChatTargetTypes.CHATTARGET_PLAYER and
            mapPinVisibility == chatPlayerTarget.targetID) then
            return true
        end
    end
...
function PopulateIconOptions()
    ...
    if CuiSettings:GetBoolean(CuiSettings.SHOW_DISTRICTS) then
        ...
        for _, item in ipairs(districtIcons) do
            CuiCreateIconEntry(districtStack, item.name, item.icon,
                               item.tooltip, count)
            count = count + 1
        end
    end
    ...
    if CuiSettings:GetBoolean(CuiSettings.SHOW_WONDERS) then
        ...
        for _, item in ipairs(wonderIcons) do
            CuiCreateIconEntry(wonderStack, item.name, item.icon, item.tooltip,
                               count)
            count = count + 1
        end
    end
...
function RequestMapPin(hexX, hexY)
    ...
    if (pMapPin ~= nil) then
        ...
        if GameConfiguration.IsAnyMultiplayer() then
            MapPinVisibilityToPlayerTarget(pMapPin:GetVisibility(),
                                           g_playerTarget)
...
function ShowHideSendToChatButton()
    ...
    local showSendButton = GameConfiguration.IsNetworkMultiplayer() and
                               not privatePin
    ...
    if (showSendButton) then
        local chatVisible = MapPinIsVisibleToChatTarget(editPin:GetVisibility(),
                                                        g_cachedChatPanelTarget)
...
function OnOk()
    if (not ContextPtr:IsHidden()) then
        ...
        if (editPin ~= nil) then
            ...
            local newMapPinVisibility = PlayerTargetToMapPinVisibility(
                                            g_playerTarget)
...
function OnSendToChatButton()
    ...
    if (editPinCfg ~= nil) then
        ...
        LuaEvents.MapPinPopup_SendPinToChat(editPinCfg:GetPlayerID(),
                                            editPinCfg:GetID())
    end
end
...
function OnMapPinPlayerInfoChanged(playerID)
    PlayerTarget_OnPlayerInfoChanged(playerID, Controls.VisibilityPull, nil,
                                     nil, g_visibilityTargetEntries,
                                     g_playerTarget, true)
end
...
function CuiGetWonderIcons()
    ...
    for item in GameInfo.Buildings() do
        if item.IsWonder then table.insert(icons, CuiGetIconByItem(item)) end
    end
...
function CuiCreateIconEntry(instance, name, icon, tooltip, index)
    local controlTable = {}
    local newIconEntry = {}
    ContextPtr:BuildInstanceForControl("IconOptionInstance", controlTable,
                                       instance)
    ...
    if (tooltip ~= nil) then
        local finalTT =
            ToolTipHelper.GetToolTip(tooltip, Game.GetLocalPlayer()) or
                Locale.Lookup(tooltip)
        controlTable.IconOptionButton:SetToolTipString(finalTT)
    end

    table.insert(g_iconPulldownOptions,
                 {name = name, icon = icon, tooltip = tooltip})
