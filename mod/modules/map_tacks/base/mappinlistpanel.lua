...
local PlayerMapPinListTTStr = Locale.Lookup(
                                  "LOC_MAP_PIN_LIST_REMOTE_PIN_TOOLTIP")
local RemoteMapPinListTTStr = Locale.Lookup(
                                  "LOC_MAP_PIN_LIST_REMOTE_PIN_TOOLTIP")
...
function GetMapPinListEntry(iPlayerID, mapPinID)
    ...
    if (mapPinEntry == nil) then
        mapPinEntry = {}
        ContextPtr:BuildInstanceForControl("MapPinListEntry", mapPinEntry,
                                           Controls.MapPinEntryStack)
        ...
        if (iPlayerID == Game.GetLocalPlayer()) then
            ...
            mapPinEntry.EditMapPin:RegisterCallback(Mouse.eLClick,
                                                    OnMapPinEntryEdit)
        else
            mapPinEntry.EditMapPin:SetHide(true)
        end

        mapPinEntry.MapPinListButton:RegisterCallback(Mouse.eLClick,
                                                      OnMapPinEntryLeftClick)
...
function BuildMapPinList()
    ...
    -- CUI
    Controls.ShowDistricts:SetCheck(CuiSettings:GetBoolean(
                                        CuiSettings.SHOW_DISTRICTS))
    Controls.ShowWonders:SetCheck(CuiSettings:GetBoolean(
                                      CuiSettings.SHOW_WONDERS))
...
function CuiOnToggleMapPin()
    ...
    local pinListPanel = ContextPtr:LookUpControl(
                             "/InGame/MinimapPanel/MapPinListPanel")
    pinListPanel:SetHide(false)
    local pinListButton = ContextPtr:LookUpControl(
                              "/InGame/MinimapPanel/MapPinListButton")
    pinListButton:SetSelected(true)

    -- map options panel and button
    local optionsPanel = ContextPtr:LookUpControl(
                             "/InGame/MinimapPanel/MapOptionsPanel")
    optionsPanel:SetHide(true)
    local optionsButton = ContextPtr:LookUpControl(
                              "/InGame/MinimapPanel/MapOptionsButton")
    ...
    local lensButton = ContextPtr:LookUpControl(
                           "/InGame/MinimapPanel/LensButton")
...
function CuiInit()
    ...
    Controls.ShowDistricts:RegisterCallback(Mouse.eLClick,
                                            CuiOnShowDistrictsClick)
