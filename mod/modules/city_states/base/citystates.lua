...
--    MEMBERS
-- ===========================================================================
local m_CityStateRowIM = InstanceManager:new("CityStateRowInstance", "CityStateBase", Controls.CityStateStack)
local m_CityStateColumnIM = InstanceManager:new("CityStateIconInstance", "Top", Controls.CityStateIconStack)
local m_BonusCityHeaderIM = InstanceManager:new("BonusCityHeaderInstance", "Top", Controls.BonusStack)
local m_BonusItemIM = InstanceManager:new("BonusItemOnInstance", "Top", Controls.BonusStack)
local m_EnvoysBonusCityHeaderIM = InstanceManager:new("BonusCityHeaderInstance", "Top", Controls.EnvoysBonusStack)
local m_EnvoysBonusItemIM = InstanceManager:new("BonusItemOnInstance", "Top", Controls.EnvoysBonusStack)
local m_InfluenceRowIM = InstanceManager:new("InfluenceRowInstance", "Top", Controls.InfluenceStack)
...
local m_RelationshipsCityStatesIM =
    InstanceManager:new("RelationshipIcon", "Background", Controls.RelationshipsCityStatesStack)
-- CUI: icon instance
...
function LookAtCityState(iPlayerID)
    ...
    if pPlayerCities:GetCount() > 1 then
        ...
        UI.DataError(
            "The CityState player " ..
                tostring(iPlayerID) ..
                    " has " .. tostring(pPlayerCities:GetCount()) .. " cities, but should only have 1."
        )
    end
...
function GetBonusText(playerID, envoyTokenNum)
    ...
    if (leader == "LEADER_MINOR_CIV_SCIENTIFIC" or leaderInfo.InheritFrom == "LEADER_MINOR_CIV_SCIENTIFIC") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_SCIENTIFIC_TRAIT_SMALL_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_SCIENTIFIC_TRAIT_MEDIUM_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_SCIENTIFIC_TRAIT_LARGE_INFLUENCE_BONUS")
        else
            UI.DataError("Unknown envoy number for city-state type bonus: " .. tostring(envoyTokenNum))
        end
    elseif (leader == "LEADER_MINOR_CIV_RELIGIOUS" or leaderInfo.InheritFrom == "LEADER_MINOR_CIV_RELIGIOUS") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_RELIGIOUS_TRAIT_SMALL_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_RELIGIOUS_TRAIT_MEDIUM_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_RELIGIOUS_TRAIT_LARGE_INFLUENCE_BONUS")
        else
            UI.DataError("Unknown envoy number for city-state type bonus: " .. tostring(envoyTokenNum))
        end
    elseif (leader == "LEADER_MINOR_CIV_TRADE" or leaderInfo.InheritFrom == "LEADER_MINOR_CIV_TRADE") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_TRADE_TRAIT_SMALL_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_TRADE_TRAIT_MEDIUM_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_TRADE_TRAIT_LARGE_INFLUENCE_BONUS")
        else
            UI.DataError("Unknown envoy number for city-state type bonus: " .. tostring(envoyTokenNum))
        end
    elseif (leader == "LEADER_MINOR_CIV_CULTURAL" or leaderInfo.InheritFrom == "LEADER_MINOR_CIV_CULTURAL") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_CULTURAL_TRAIT_SMALL_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_CULTURAL_TRAIT_MEDIUM_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_CULTURAL_TRAIT_LARGE_INFLUENCE_BONUS")
        else
            UI.DataError("Unknown envoy number for city-state type bonus: " .. tostring(envoyTokenNum))
        end
    elseif (leader == "LEADER_MINOR_CIV_MILITARISTIC" or leaderInfo.InheritFrom == "LEADER_MINOR_CIV_MILITARISTIC") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_MILITARISTIC_TRAIT_SMALL_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_MILITARISTIC_TRAIT_MEDIUM_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_MILITARISTIC_TRAIT_LARGE_INFLUENCE_BONUS")
        else
            UI.DataError("Unknown envoy number for city-state type bonus: " .. tostring(envoyTokenNum))
        end
    elseif (leader == "LEADER_MINOR_CIV_INDUSTRIAL" or leaderInfo.InheritFrom == "LEADER_MINOR_CIV_INDUSTRIAL") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_INDUSTRIAL_TRAIT_SMALL_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_INDUSTRIAL_TRAIT_MEDIUM_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS then
            bonusDetailsText = Locale.Lookup("LOC_MINOR_CIV_INDUSTRIAL_TRAIT_LARGE_INFLUENCE_BONUS")
        else
            UI.DataError("Unknown envoy number for city-state type bonus: " .. tostring(envoyTokenNum))
        end
    else
        ...
        if (leader ~= nil and leader == leaderTraitPairInfo.LeaderType) then
            ...
            if (traitInfo ~= nil) then
                ...
                text =
                    text ..
                    "[COLOR:SuzerainDark]" ..
                        Locale.Lookup("LOC_CITY_STATES_SUZERAIN_UNIQUE_BONUS", name) .. "[ENDCOLOR] "
    ...
    if (player ~= nil) then
        for resourceInfo in GameInfo.Resources() do
            ...
            if (player:GetResources():HasResource(resource) or player:GetResources():HasExportedResource(resource)) then
                ...
                resourceIcons =
                    resourceIcons ..
                    amount .. " [ICON_" .. resourceInfo.ResourceType .. "] " .. Locale.Lookup(resourceInfo.Name)
            end
        end
    end
    if (resourceIcons ~= "") then
...
function GetRelationships(cityStateID)
    ...
    for _, playerID in ipairs(PlayerManager.GetAliveMajorIDs()) do
        ...
        local diploStateID = pPlayer:GetDiplomaticAI():GetDiplomaticStateIndex(cityStateID)
        if diploStateID ~= -1 then
            ...
            civEntry.HasMet =
                playerID == Game.GetLocalPlayer() or Players[Game.GetLocalPlayer()]:GetDiplomacy():HasMet(playerID)
...
function GetRelationshipPipAtlasPieces(kCityState)
    ...
    local tooltip = Locale.Lookup(DIPLO_PIP_INFO[kCityState.DiplomaticState].Tooltip)
    ...
    local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas(iconName, 23)
    ...
    elseif kCityState.Type == "INDUSTRIAL" then
        ...
        UI.DataError(
            "WARNING: Unknown type '" ..
                kCityState.Type .. "' for getting the City-State icon (at size " .. tostring(size) .. ")"
        )
        return 0, 0, "", ""
    end

    local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas(iconName, size)
...
function Close()
    ...
    if (localPlayerID ~= -1) then
        ...
        if
            (localPlayer ~= nil and localPlayer:GetInfluence() ~= nil and
                not localPlayer:GetInfluence():IsGivingTokensConsidered())
         then
...
function RealizeListHeader()
    ...
    if m_mode == MODE.SendEnvoys then
        ...
        header =
            header ..
            " " .. Locale.Lookup("LOC_CITY_STATES_SEND_ENVOY_AMOUNT", (m_kPlayerData.EnvoyTokens - sum)) .. " " -- HACK: space on end due to textcontrol bug, see below
    end
    ...
    for iPlayer, inst in pairs(m_uiCityStateRows) do
        ...
        if m_kCityStates[iPlayer].isAlive then
            ...
            if m_kCityStates[iPlayer].CanReceiveTokensFrom or not m_kCityStates[iPlayer].isAtWar then
                ...
                local tooltip = Locale.Lookup("LOC_CITY_STATES_CURRENTLY_AT_WAR")
...
function AddCityStateRow(kCityState)
    ...
    kInst.NameButton:SetColor(
        Mouse.eLClick,
        function()
            OpenSingleViewCityState(kCityState.iPlayer)
        end
    )
    kInst.NameButton:RegisterCallback(
        Mouse.eLClick,
        function()
            OpenSingleViewCityState(kCityState.iPlayer)
        end
    ...
    kInst.BonusImageSuzerainOff:SetToolTipString(
        kCityState.Bonuses["Suzerain"].Title .. "[NEWLINE]" .. kCityState.Bonuses["Suzerain"].Details
    )
...
function ViewCityState(iPlayer)
    ...
    if kCityState == nil then
        UI.DataError(
            "Attempt to show details for CityState player #" .. tostring(iPlayer) .. ", but that doesn't exist!"
        )
    ...
    if GameCapabilities.HasCapability("CAPABILITY_MILITARY") then
        ...
        if kCityState.isAtWar then
            ...
            if not kCityState.CanMakePeaceWith then
                if
                    (GlobalParameters.DIPLOMACY_WAR_LAST_FOREVER == 1 or
                        GlobalParameters.DIPLOMACY_WAR_LAST_FOREVER == true)
                 then
        ...
        Controls.PeaceWarButton:RegisterCallback(
            Mouse.eLClick,
            function()
        ...
        Controls.LevyMilitaryButton:RegisterCallback(
            Mouse.eLClick,
            function()
    ...
    Controls.InfluencedByValue:SetText(
        Locale.Lookup("LOC_CITY_STATES_CIVILIZATIONS", table.count(kCityState.Influence))
    )
    ...
    elseif m_mode == MODE.InfluencedBy then
        ...
        for iOtherPlayer, influence in pairs(kCityState.Influence) do
            local kItem = AddInfluenceRow(kCityState.iPlayer, iOtherPlayer, influence, largestAmount)
    ...
    elseif m_mode == MODE.Relationships then
        ...
        UI.DataError(
            "City-States in an unhandled mode '" .. tostring(m_mode) .. "' when attempting to view a single City-State."
        )
        return
    end
end
    ...
    for _, kRelationship in pairs(kRelationships) do
        if kRelationship.HasMet then
            ...
            if #Teams[kRelationship.TeamID] > 1 then
                local teamRibbonName = TEAM_RIBBON_PREFIX .. tostring(kRelationship.TeamID)
            ...
            if DIPLO_PIP_INFO[kRelationship.DiploState] ~= nil then
                local iconName = DIPLO_PIP_INFO[kRelationship.DiploState].IconName
                if iconName == nil or iconName == "" then
                    print(
                        "WARNING: Unexpected DiplomaticState when obtain PIP art. value:" .. kCityState.DiplomaticState
                    )
                    iconName = "ICON_RELATIONSHIP_NEUTRAL"
                end
                local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas(iconName, 23)
    ...
    if context == RELOAD_CACHE_ID then
        ...
        if contextTable["isHidden"] ~= nil and not contextTable["isHidden"] then
            ...
            elseif m_mode == MODE.SendEnvoys then
                OnOpenSendEnvoys()
            elseif
                m_mode == MODE.EnvoySent or m_mode == MODE.InfluencedBy or m_mode == MODE.Quests or
                    m_mode == MODE.Relationships
             then
                m_kScreenSlideAnim.Show()
                Refresh()
...
function Initialize()
    ...
    -- Check:
    if
        NUM_ENVOY_TOKENS_FOR_FIRST_BONUS == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS or
            NUM_ENVOY_TOKENS_FOR_SECOND_BONUS == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS or
            NUM_ENVOY_TOKENS_FOR_FIRST_BONUS == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS
     then
