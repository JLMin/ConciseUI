...
--	CONSTANTS
-- ===========================================================================
local COLOR_ICON_BONUS_OFF = UI.GetColorValueFromHexLiteral(0xff606060)
local COLOR_ICON_BONUS_ON = UI.GetColorValueFromHexLiteral(0xff999900)
local COLOR_TEXT_BONUS_OFF = UI.GetColorValueFromHexLiteral(0xff606060)
local COLOR_TEXT_BONUS_ON = UI.GetColorValueFromHexLiteral(0xffb0b0b0)
local FONT_SIZE_SINGLE_DIGIT_ENVOYS = 40
local FONT_SIZE_TWO_DIGIT_ENVOYS = 26
local FONT_SIZE_THREE_DIGIT_ENVOYS = 18
local MIN_ENVOY_TOKENS_SUZERAIN = 3 -- TODO: expose via game core
local NUM_ENVOY_TOKENS_FOR_FIRST_BONUS = 1
local NUM_ENVOY_TOKENS_FOR_SECOND_BONUS = 3
local NUM_ENVOY_TOKENS_FOR_THIRD_BONUS = 6
...
DIPLO_PIP_INFO["DIPLO_STATE_PATRON"] = {
    IconName = "ICON_RELATIONSHIP_GOOD",
    Tooltip = "LOC_CITY_STATES_DIPLO_GOOD"
}
DIPLO_PIP_INFO["DIPLO_STATE_AWARE"] = {
    IconName = "ICON_RELATIONSHIP_NEUTRAL",
    Tooltip = "LOC_CITY_STATES_DIPLO_AWARE"
}
DIPLO_PIP_INFO["DIPLO_STATE_WAR_WITH_MAJOR"] =
    {IconName = "ICON_RELATIONSHIP_WAR", Tooltip = "LOC_CITY_STATES_DIPLO_WAR"}
...
local REPORT_CONTAINER_SIZE_PADDING = -18

-- ===========================================================================
--	MEMBERS
-- ===========================================================================
local m_CityStateRowIM = InstanceManager:new("CityStateRowInstance",
                                             "CityStateBase",
                                             Controls.CityStateStack)
local m_CityStateColumnIM = InstanceManager:new("CityStateIconInstance", "Top",
                                                Controls.CityStateIconStack)
local m_BonusCityHeaderIM = InstanceManager:new("BonusCityHeaderInstance",
                                                "Top", Controls.BonusStack)
local m_BonusItemIM = InstanceManager:new("BonusItemOnInstance", "Top",
                                          Controls.BonusStack)
local m_EnvoysBonusCityHeaderIM = InstanceManager:new("BonusCityHeaderInstance",
                                                      "Top",
                                                      Controls.EnvoysBonusStack)
local m_EnvoysBonusItemIM = InstanceManager:new("BonusItemOnInstance", "Top",
                                                Controls.EnvoysBonusStack)
local m_InfluenceRowIM = InstanceManager:new("InfluenceRowInstance", "Top",
                                             Controls.InfluenceStack)
local m_QuestsIM = InstanceManager:new("QuestInstance", "Top",
                                       Controls.QuestsStack)
local m_RelationshipsButtonIM = InstanceManager:new("RelationshipIcon",
                                                    "Background",
                                                    Controls.RelationshipsButtonStack)
local m_RelationshipsCivsIM = InstanceManager:new("RelationshipIcon",
                                                  "Background",
                                                  Controls.RelationshipsCivsStack)
local m_RelationshipsCityStatesIM = InstanceManager:new("RelationshipIcon",
                                                        "Background",
                                                        Controls.RelationshipsCityStatesStack)
-- CUI: icon instance
local m_SuzerainIM = InstanceManager:new("CuiSuzerainInstance", "Top",
                                         Controls.SuzerainStack)

local m_kScreenSlideAnim -- Controls overall look/feel of animation.
local m_kCityStates = {} -- City states, key is player num of city state
local m_kLastCityStates -- Last set before confirming a city state change.
local m_kPlayerData = {} -- Holds live player data that hasn't been solidifed in the game core.
local m_kEnvoyChanges = {} -- Table (player,deltas) for envoy tokens about to be given out
local m_uiCityStateRows = {} -- Instances holding the city state rows
local m_mode -- How the screen should act.
local m_iCurrentCityState = -1 -- Player # of the city state currently active
local m_iTurnsOfPeace = Game.GetGameDiplomacy():GetMinPeaceDuration()
local m_iTurnsOfWar = Game.GetGameDiplomacy():GetMinPeaceDuration()
...
function GetCityStatesMetNum()
    ...
    for _, kCityState in pairs(m_kCityStates) do
        if kCityState.isHasMet then total = total + 1 end
    end
    return total
end
...
-- ===========================================================================
function LookAtCityState(iPlayerID)
    local pPlayer = Players[iPlayerID]
    local pPlayerCities = pPlayer:GetCities()
    if pPlayerCities:GetCount() > 1 then
        -- Might change in the future; currently city states will just raze
        UI.DataError(
            "The CityState player " .. tostring(iPlayerID) .. " has " ..
                tostring(pPlayerCities:GetCount()) ..
    ...
    if cameraXOffset < 0.1 or cameraXOffset > 0.9 then
        UI.DataError(
            "Got a whack offset for camera 'centering' of viewable space for the City States: " ..
                tostring(cameraXOffset))
        cameraXOffset = 0.33
    end

    for _, pCity in pPlayerCities:Members() do
        local locX = pCity:GetX()
        local locY = pCity:GetY()
...
-- ===========================================================================
function GetBonusText(playerID, envoyTokenNum)
    ...
    if leaderInfo == nil or leaderInfo.InheritFrom == nil then
        UI.DataError(
            "Cannot determine the type of city state bonus for player #: " ..
                tostring(playerID))
        return "UNKNOWN"
    end
    ...
    elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_THIRD_BONUS then
        ...
        UI.DataError("Unknown envoy number for city-state type bonus:" ..
                         tostring(envoyTokenNum))
    end

    local bonusDetailsText = ""
    if (leader == "LEADER_MINOR_CIV_SCIENTIFIC" or leaderInfo.InheritFrom ==
        "LEADER_MINOR_CIV_SCIENTIFIC") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup(
    ...
    elseif (leader == "LEADER_MINOR_CIV_RELIGIOUS" or leaderInfo.InheritFrom ==
        "LEADER_MINOR_CIV_RELIGIOUS") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup(
    ...
    elseif (leader == "LEADER_MINOR_CIV_TRADE" or leaderInfo.InheritFrom ==
        "LEADER_MINOR_CIV_TRADE") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup(
                                   "LOC_MINOR_CIV_TRADE_TRAIT_SMALL_INFLUENCE_BONUS")
        elseif envoyTokenNum == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS then
            bonusDetailsText = Locale.Lookup(
    ...
    elseif (leader == "LEADER_MINOR_CIV_CULTURAL" or leaderInfo.InheritFrom ==
        "LEADER_MINOR_CIV_CULTURAL") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup(
    ...
    elseif (leader == "LEADER_MINOR_CIV_MILITARISTIC" or leaderInfo.InheritFrom ==
        "LEADER_MINOR_CIV_MILITARISTIC") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup(
    ...
    elseif (leader == "LEADER_MINOR_CIV_INDUSTRIAL" or leaderInfo.InheritFrom ==
        "LEADER_MINOR_CIV_INDUSTRIAL") then
        if envoyTokenNum == NUM_ENVOY_TOKENS_FOR_FIRST_BONUS then
            bonusDetailsText = Locale.Lookup(
...
-- ===========================================================================
function GetSuzerainBonusText(playerID)

    local leader = PlayerConfigurations[playerID]:GetLeaderTypeName()
    local leaderInfo = GameInfo.Leaders[leader]
    if leaderInfo == nil then
        UI.DataError(
            "GetSuzerainBonusText, cannot determine the type of city state suzerain bonus for player #: " ..
                tostring(playerID))
        return "UNKNOWN"
    end
    ...
    for leaderTraitPairInfo in GameInfo.LeaderTraits() do
        if (leader ~= nil and leader == leaderTraitPairInfo.LeaderType) then
            ...
            if (traitInfo ~= nil) then
                local name =
                    PlayerConfigurations[playerID]:GetCivilizationShortDescription()
                text = text .. "[COLOR:SuzerainDark]" ..
                           Locale.Lookup(
                               "LOC_CITY_STATES_SUZERAIN_UNIQUE_BONUS", name) ..
                           "[ENDCOLOR] "
    ...
    text = text .. "[NEWLINE][NEWLINE]" ..
               Locale.Lookup("LOC_CITY_STATES_SUZERAIN_DIPLOMATIC_BONUS")
    ...
    if (player ~= nil) then
        for resourceInfo in GameInfo.Resources() do
            ...
            if (player:GetResources():HasResource(resource) or
                player:GetResources():HasExportedResource(resource)) then
                local amount =
                    player:GetResources():GetResourceAmount(resource) +
                        player:GetResources()
                            :GetExportedResourceAmount(resource)
                if (resourceIcons ~= "") then
                    resourceIcons = resourceIcons .. comma_separator
                end
                resourceIcons = resourceIcons .. amount .. " [ICON_" ..
    ...
    if (resourceIcons ~= "") then
        ...
        text = text .. " " ..
                   Locale.Lookup(
                       "LOC_CITY_STATES_SUZERAIN_NO_RESOURCES_AVAILABLE")
    end
...
-- ===========================================================================
function GetQuests(playerID)
    local kQuests = {}
    local questsManager = Game.GetQuestsManager()
    local localPlayerID = Game.GetLocalPlayer()
    if questsManager ~= nil then
        for questInfo in GameInfo.Quests() do
            if questsManager:HasActiveQuestFromPlayer(localPlayerID, playerID,
                                                      questInfo.Index) then
                kQuests[questInfo.Index] =
                    {
                        Description = questsManager:GetActiveQuestDescription(
                            localPlayerID, playerID, questInfo.Index),
                        Name = questsManager:GetActiveQuestName(localPlayerID,
                                                                playerID,
                                                                questInfo.Index),
                        Reward = questsManager:GetActiveQuestReward(
                            localPlayerID, playerID, questInfo.Index),
...
-- ===========================================================================
function GetRelationships(cityStateID)
    local kRelationships = {}

    -- Civ relationships
    kRelationships.CivRelationships = {}
    for _, playerID in ipairs(PlayerManager.GetAliveMajorIDs()) do
        local pPlayer = Players[playerID]
        local diploStateID = pPlayer:GetDiplomaticAI():GetDiplomaticStateIndex(
                                 cityStateID)
        if diploStateID ~= -1 then
            ...
            civEntry.DiploState = GameInfo.DiplomaticStates[diploStateID]
                                      .StateType
            civEntry.DiploTooltip = GetRelationshipDiplomaticTooltip(
                                        cityStateID, playerID, diploStateID)
            civEntry.HasMet = playerID == Game.GetLocalPlayer() or
                                  Players[Game.GetLocalPlayer()]:GetDiplomacy()
                                      :HasMet(playerID)
    ...
    for _, playerID in ipairs(PlayerManager.GetAliveMinorIDs()) do
        ...
        local diploStateID = pPlayer:GetDiplomaticAI():GetDiplomaticStateIndex(
                                 cityStateID)
        if diploStateID ~= -1 then
            local cityStateEntry = {}

            local pPlayerConfig = PlayerConfigurations[playerID]
            cityStateEntry.PlayerIcon = "ICON_" ..
                                            pPlayerConfig:GetCivilizationTypeName()
            cityStateEntry.PlayerName =
                pPlayerConfig:GetCivilizationShortDescription()
            cityStateEntry.TeamID = pPlayerConfig:GetTeam()
            cityStateEntry.DiploState = GameInfo.DiplomaticStates[diploStateID]
                                            .StateType
            cityStateEntry.DiploTooltip =
                GetRelationshipDiplomaticTooltip(cityStateID, playerID,
                                                 diploStateID)
...
function GetRelationshipDiplomaticTooltip(cityStateID, playerID, diploStateID)
    ...
    local pCityStateConfig = PlayerConfigurations[cityStateID]
    local pPlayerConfig = PlayerConfigurations[playerID]
    tooltip = Locale.Lookup("LOC_CITY_STATES_AT_WAR_WTIH",
                            pCityStateConfig:GetCivilizationShortDescription(),
                            pPlayerConfig:GetCivilizationShortDescription())
...
-- ===========================================================================
function GetBonusIconAtlasPieces(kCityState, size)
    local iconName = ""
    if kCityState.Type == "SCIENTIFIC" then
        iconName = "ICON_ENVOY_BONUS_SCIENCE"
    elseif kCityState.Type == "RELIGIOUS" then
        iconName = "ICON_ENVOY_BONUS_FAITH"
    elseif kCityState.Type == "TRADE" then
        iconName = "ICON_ENVOY_BONUS_GOLD"
    elseif kCityState.Type == "CULTURE" then
        iconName = "ICON_ENVOY_BONUS_CULTURE"
    elseif kCityState.Type == "MILITARISTIC" then
        iconName = "ICON_ENVOY_BONUS_MILITARY"
    elseif kCityState.Type == "INDUSTRIAL" then
        iconName = "ICON_ENVOY_BONUS_PRODUCTION"
    end
...
-- ===========================================================================
function GetRelationshipPipAtlasPieces(kCityState)
    local iconName = DIPLO_PIP_INFO[kCityState.DiplomaticState].IconName
    local tooltip = Locale.Lookup(DIPLO_PIP_INFO[kCityState.DiplomaticState]
                                      .Tooltip)
    if iconName == nil or iconName == "" then
        print(
            "WARNING: Unexpected DiplomaticState when obtain PIP art. value:" ..
                kCityState.DiplomaticState)
        iconName = "ICON_RELATIONSHIP_NEUTRAL"
    end
    local textureOffsetX, textureOffsetY, textureSheet =
        IconManager:FindIconAtlas(iconName, 23)
...
-- ===========================================================================
function GetTypeTooltip(kCityState)
    local toolTip
    if kCityState.Type == "SCIENTIFIC" then
        toolTip = "LOC_CITY_STATES_TYPE_SCIENTIFIC"
    elseif kCityState.Type == "RELIGIOUS" then
        toolTip = "LOC_CITY_STATES_TYPE_RELIGIOUS"
    elseif kCityState.Type == "TRADE" then
        toolTip = "LOC_CITY_STATES_TYPE_TRADE"
    elseif kCityState.Type == "CULTURE" then
        toolTip = "LOC_CITY_STATES_TYPE_CULTURAL"
    elseif kCityState.Type == "MILITARISTIC" then
        toolTip = "LOC_CITY_STATES_TYPE_MILITARISTIC"
    elseif kCityState.Type == "INDUSTRIAL" then
        toolTip = "LOC_CITY_STATES_TYPE_INDUSTRIAL"
    else
        UI.DataError("WARNING: Unknown type '" .. kCityState.Type ..
                         "' for getting the City-State tooltip.")
        return 0, 0, "", ""
    end
    return toolTip
end
...
-- ===========================================================================
function GetTypeAtlasPieces(kCityState, size)
    local iconName
    if kCityState.Type == "SCIENTIFIC" then
        iconName = "ICON_CITYSTATE_SCIENCE"
    elseif kCityState.Type == "RELIGIOUS" then
        iconName = "ICON_CITYSTATE_FAITH"
    elseif kCityState.Type == "TRADE" then
        iconName = "ICON_CITYSTATE_TRADE"
    elseif kCityState.Type == "CULTURE" then
        iconName = "ICON_CITYSTATE_CULTURE"
    elseif kCityState.Type == "MILITARISTIC" then
        iconName = "ICON_CITYSTATE_MILITARISTIC"
    elseif kCityState.Type == "INDUSTRIAL" then
        iconName = "ICON_CITYSTATE_INDUSTRIAL"
    else
        UI.DataError("WARNING: Unknown type '" .. kCityState.Type ..
                         "' for getting the City-State icon (at size " ..
                         tostring(size) .. ")")
        return 0, 0, "", ""
    end

    local textureOffsetX, textureOffsetY, textureSheet =
        IconManager:FindIconAtlas(iconName, size)
...
--	Returns the texture name and UV for a bonus icon type.
-- ===========================================================================
function GetTypeName(kCityState)
    if kCityState.Type == "SCIENTIFIC" then
        return Locale.Lookup("LOC_CITY_STATES_TYPE_SCIENTIFIC")
    elseif kCityState.Type == "RELIGIOUS" then
        return Locale.Lookup("LOC_CITY_STATES_TYPE_RELIGIOUS")
    elseif kCityState.Type == "TRADE" then
        return Locale.Lookup("LOC_CITY_STATES_TYPE_TRADE")
    elseif kCityState.Type == "CULTURE" then
        return Locale.Lookup("LOC_CITY_STATES_TYPE_CULTURAL")
    elseif kCityState.Type == "MILITARISTIC" then
        return Locale.Lookup("LOC_CITY_STATES_TYPE_MILITARISTIC")
    elseif kCityState.Type == "INDUSTRIAL" then
        return Locale.Lookup("LOC_CITY_STATES_TYPE_INDUSTRIAL")
    end
...
-- ===========================================================================
function RealizeEnvoyToken(total, control)
    if total < 10 then
...
function RealizeListHeader()
    ...
    if m_mode == MODE.SendEnvoys then
        header = Locale.ToUpper(Locale.Lookup("LOC_CITY_STATES_SEND_ENVOYS",
                                              (m_kPlayerData.EnvoyTokens - sum)))
        header = header .. " " ..
                     Locale.Lookup("LOC_CITY_STATES_SEND_ENVOY_AMOUNT",
                                   (m_kPlayerData.EnvoyTokens - sum)) .. " " -- HACK: space on end due to textcontrol bug, see below
    end
    ...
    local envoyDetails = Locale.Lookup(
                             "LOC_TOP_PANEL_INFLUENCE_TOOLTIP_POINTS_THRESHOLD",
                             envoysPerThreshold, influenceThreshold)
    Controls.EnvoyDetails:SetText(envoyDetails)

    local sTooltip = ""
    if (currentEnvoys > 0) then
        sTooltip = sTooltip ..
                       Locale.Lookup("LOC_TOP_PANEL_INFLUENCE_TOOLTIP_ENVOYS",
                                     currentEnvoys)
    ...
    sTooltip = sTooltip ..
                   Locale.Lookup(
                       "LOC_TOP_PANEL_INFLUENCE_TOOLTIP_POINTS_BALANCE",
                       influenceBalance)
    sTooltip = sTooltip .. "[NEWLINE]"
    sTooltip = sTooltip ..
                   Locale.Lookup("LOC_TOP_PANEL_INFLUENCE_TOOLTIP_POINTS_RATE",
                                 influenceRate)
    sTooltip = sTooltip .. "[NEWLINE][NEWLINE]"
    sTooltip = sTooltip ..
...
function RealizeEnvoyChangeButtons()
    ...
    local totalLeft = (m_kPlayerData.EnvoyTokens - sum)
    local isMoreDisabled = not m_isLocalPlayerTurn or (totalLeft == 0)
    local isLessDisabled = not m_isLocalPlayerTurn or (sum <= 0)

    -- Likely going away so even if warring envoys can still be sent.
    for iPlayer, inst in pairs(m_uiCityStateRows) do
        inst.EnvoyMoreButton:SetDisabled(
            isMoreDisabled or (not m_kCityStates[iPlayer].CanReceiveTokensFrom))
        inst.EnvoyLessButton:SetDisabled(
            isLessDisabled or (not m_kCityStates[iPlayer].CanReceiveTokensFrom))
        ...
        if m_kCityStates[iPlayer].isAlive then
            ...
            if m_kCityStates[iPlayer].CanReceiveTokensFrom or
                not m_kCityStates[iPlayer].isAtWar then
                inst.EnvoyMoreButton:SetToolTipString(
                    Locale.Lookup("LOC_CITY_STATES_ADD_AN_ENVOY"))
                inst.EnvoyLessButton:SetToolTipString(
                    Locale.Lookup("LOC_CITY_STATES_REMOVE_AN_ENVOY"))
                inst.Envoy:SetToolTipString(nil)
            else
                local tooltip =
                    Locale.Lookup("LOC_CITY_STATES_CURRENTLY_AT_WAR")
            ...
            local tooltip = Locale.Lookup("LOC_CITY_STATES_DESTROYED_LONG")
            inst.EnvoyMoreButton:SetToolTipString(tooltip)
            inst.EnvoyLessButton:SetToolTipString(tooltip)
            inst.Envoy:SetToolTipString(tooltip)
        end
...
function OnLessEnvoyTokens(iPlayer)
    ...
    m_uiCityStateRows[iPlayer].EnvoyLessButton:SetDisabled(
        m_kEnvoyChanges[iPlayer] == 0)

    UI.PlaySound("UI_Click_Sweetener_Metal_Button_Small")

    RealizeEnvoyChangeButtons()
    RealizeEnvoyToken(m_kCityStates[iPlayer].Tokens + m_kEnvoyChanges[iPlayer],
                      m_uiCityStateRows[iPlayer].EnvoyCount)
    RealizeListHeader()
end

-- ===========================================================================
function OnMoreEnvoyTokens(iPlayer)
    ...
    if pLocalPlayer ~= nil then
        for cityStatePlayerID, numTokens in pairs(m_kEnvoyChanges) do
            for i = 1, numTokens, 1 do
                local parameters = {}
                parameters[PlayerOperations.PARAM_PLAYER_ONE] =
                    cityStatePlayerID
                UI.RequestPlayerOperation(playerID,
                                          PlayerOperations.GIVE_INFLUENCE_TOKEN,
                                          parameters)
            end
        end
        m_kEnvoyChanges = {}
        ...
        UI.DataError(
            "Unable to get a valid local player when confirming envoy tokens.")
    end
end
...
-- ===========================================================================
function OnChangeWarPeaceStatus(kCityState)
    ...
    if pLocalPlayer ~= nil then
        if kCityState.isAtWar then
            ...
            UI.RequestPlayerOperation(localPlayerID,
                                      PlayerOperations.DIPLOMACY_MAKE_PEACE,
                                      parameters)
        else
            LuaEvents.CityStates_ConfirmWarDialog(localPlayerID, iPlayer,
                                                  WarTypes.SURPRISE_WAR)
        end
    else
        UI.DataError(
            "Could not get local player to declare war on city state '" ..
                kCityState.Name .. "'.")
    end
end
...
-- ===========================================================================
function OnLevyMilitary(kCityState)
    local iPlayer = kCityState.iPlayer
    local localPlayerID = Game.GetLocalPlayer()
    local pLocalPlayer = Players[localPlayerID]
    ...
    if pLocalPlayer ~= nil then
        ...
        UI.RequestPlayerOperation(localPlayerID, PlayerOperations.LEVY_MILITARY,
                                  parameters)
        UI.PlaySound("UI_Levy_Military")
        Controls.LevyMilitaryButton:SetDisabled(true)
    else
        UI.DataError(
            "Could not get local player to levy military from city state '" ..
                kCityState.Name .. "'.")
    end
end

-- ===========================================================================
function AddCityStateRow(kCityState)

    local kInst = m_CityStateRowIM:GetInstance()
    local textureOffsetX
    local textureOffsetY
    local textureSheet
    local questToolTip = Locale.Lookup("LOC_CITY_STATES_QUESTS")
    local numQuests = 0
    local cityStateName = Locale.ToUpper(
                              Locale.Lookup(kCityState.Name) ..
                                  (kCityState.isAlive and "" or "(" ..
                                      Locale.Lookup("LOC_CITY_STATES_DESTROYED") ..
                                      ")"))

    -- Set name, truncate if necessary
    kInst.NameLabel:SetText(cityStateName)
    local targetSize = (kInst.NameButton:GetSizeX() - 12)
    ...
    textureOffsetX, textureOffsetY, textureSheet, tooltip =
        GetRelationshipPipAtlasPieces(kCityState)
    ...
    if numQuests > 0 then
        ...
        kInst.CuiCityStateQuest:SetString(
            Locale.Lookup("LOC_NOTIFICATION_CITYSTATE_QUEST_COMPLETED_MESSAGE"))
    ...
    textureOffsetX, textureOffsetY, textureSheet =
        GetBonusIconAtlasPieces(kCityState, 26)

    kInst.BonusImage1:SetTexture(
        kCityState.isBonus1 and "CityState_BonusSlotOn" or
            "CityState_BonusSlotOff")
    kInst.BonusImage1:SetToolTipString(kCityState.Bonuses[1].Title ..
                                           "[NEWLINE]" ..
                                           kCityState.Bonuses[1].Details)

    kInst.BonusIcon1:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
    kInst.BonusIcon1:SetColor(
        kCityState.isBonus1 and kCityState.ColorSecondary or
            COLOR_ICON_BONUS_OFF)
    kInst.BonusText1:SetColor(kCityState.isBonus1 and COLOR_TEXT_BONUS_ON or
                                  COLOR_TEXT_BONUS_OFF)
    kInst.BonusImage3:SetTexture(
        kCityState.isBonus3 and "CityState_BonusSlotOn" or
            "CityState_BonusSlotOff")
    kInst.BonusImage3:SetToolTipString(kCityState.Bonuses[3].Title ..
                                           "[NEWLINE]" ..
                                           kCityState.Bonuses[3].Details)
    kInst.BonusIcon3:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
    kInst.BonusIcon3:SetColor(
        kCityState.isBonus3 and kCityState.ColorSecondary or
            COLOR_ICON_BONUS_OFF)
    kInst.BonusText3:SetColor(kCityState.isBonus3 and COLOR_TEXT_BONUS_ON or
                                  COLOR_TEXT_BONUS_OFF)
    kInst.BonusImage6:SetTexture(
        kCityState.isBonus6 and "CityState_BonusSlotOn" or
            "CityState_BonusSlotOff")
    kInst.BonusImage6:SetToolTipString(kCityState.Bonuses[6].Title ..
                                           "[NEWLINE]" ..
                                           kCityState.Bonuses[6].Details)
    kInst.BonusIcon6:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
    kInst.BonusIcon6:SetColor(
        kCityState.isBonus6 and kCityState.ColorSecondary or
            COLOR_ICON_BONUS_OFF)
    kInst.BonusText6:SetColor(kCityState.isBonus6 and COLOR_TEXT_BONUS_ON or
                                  COLOR_TEXT_BONUS_OFF)
    ...
    kInst.BonusImageSuzerainOff:SetToolTipString(
        kCityState.Bonuses["Suzerain"].Title .. "[NEWLINE]" ..
            kCityState.Bonuses["Suzerain"].Details)
    kInst.BonusImageSuzerainOn:SetToolTipString(
        kCityState.Bonuses["Suzerain"].Title .. "[NEWLINE]" ..
            kCityState.Bonuses["Suzerain"].Details)
    kInst.BonusIconSuzerain:SetColor(kCityState.isBonusSuzerain and
                                         kCityState.ColorSecondary or
                                         COLOR_ICON_BONUS_OFF)
    kInst.BonusTextSuzerain:SetColor(kCityState.isBonusSuzerain and
                                         kCityState.ColorSecondary or
                                         COLOR_TEXT_BONUS_OFF)
    kInst.BonusTextSuzerain:SetText(kCityState.SuzerainTokensNeeded)
    kInst.SuzerainLabel:SetColor(kCityState.isBonusSuzerain and
                                     kCityState.ColorSecondary or
                                     COLOR_ICON_BONUS_OFF)
    kInst.Suzerain:SetColor(kCityState.isBonusSuzerain and
                                kCityState.ColorSecondary or
                                COLOR_ICON_BONUS_OFF)
...
function ViewList()
    ...
    local cui_SuzerainList = {
        SCIENTIFIC = {
            idx = 1,
            icon = "ICON_ENVOY_BONUS_SCIENCE",
            color = "",
            num = 0
        },
        CULTURE = {
            idx = 2,
            icon = "ICON_ENVOY_BONUS_CULTURE",
            color = "",
            num = 0
        },
        RELIGIOUS = {
            idx = 3,
            icon = "ICON_ENVOY_BONUS_FAITH",
            color = "",
            num = 0
        },
        ...
        INDUSTRIAL = {
            idx = 5,
            icon = "ICON_ENVOY_BONUS_PRODUCTION",
            color = "",
            num = 0
        },
        MILITARISTIC = {
            idx = 6,
            icon = "ICON_ENVOY_BONUS_MILITARY",
            color = "",
            num = 0
        }
    }
    ...
    for iPlayer, kCityState in pairs(m_kCityStates) do
        if kCityState.isHasMet then
            ...
            if kCityState.isBonusSuzerain then
                ...
                cui_SuzerainList[kCityState.Type].color =
                    kCityState.ColorSecondary
                cui_SuzerainList[kCityState.Type].num =
                    cui_SuzerainList[kCityState.Type].num + 1
            end
            --
            local kInst = AddCityStateRow(kCityState)
            m_uiCityStateRows[iPlayer] = kInst
        end
    end
    -- CUI: ui setup
    m_SuzerainIM:ResetInstances()
    Controls.Totals:SetText(Locale.Lookup("LOC_CUI_CSP_ENVOYS_SUZERAIN",
                                          cui_Envoys, cui_Suzerain))
    for _, item in SortedTable(cui_SuzerainList,
                               function(t, a, b) return t[a].idx < t[b].idx end) do
        local cui_sInst = m_SuzerainIM:GetInstance()
        cui_sInst.SuzerainImage:SetTexture(
            IconManager:FindIconAtlas(item.icon, 26))
        cui_sInst.SuzerainImage:SetColor(
            item.num > 0 and item.color or COLOR_ICON_BONUS_OFF)
        cui_sInst.SuzerainNumber:SetText(item.num)
        cui_sInst.SuzerainNumber:SetColor(
            item.num > 0 and item.color or COLOR_ICON_BONUS_OFF)
    end
    --
    ...
    if m_mode == MODE.SendEnvoys then
        ...
        for iPlayer, kCityState in pairs(m_kCityStates) do
            ...
            if kCityState.isBonus1 then
                local kHeader = m_BonusCityHeaderIM:GetInstance()
                kHeader.CityName:SetText(Locale.ToUpper(kCityState.Name))

                local kItem = m_BonusItemIM:GetInstance()
                local textureOffsetX, textureOffsetY, textureSheet =
                    GetBonusIconAtlasPieces(kCityState, 50)
                kItem.Icon:SetTexture(textureOffsetX, textureOffsetY,
                                      textureSheet)
                kItem.Icon:SetColor(kCityState.ColorSecondary)
                kItem.Title:SetColor(kCityState.ColorSecondary)
                kItem.Title:SetText(
                    kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_FIRST_BONUS].Title)
                kItem.Details:SetText(
                    kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_FIRST_BONUS].Details)

                if kCityState.isBonus3 then
                    kItem = m_BonusItemIM:GetInstance()
                    kItem.Icon:SetTexture(textureOffsetX, textureOffsetY,
                                          textureSheet) -- Same as above
                    kItem.Icon:SetColor(kCityState.ColorSecondary)
                    kItem.Title:SetColor(kCityState.ColorSecondary)
                    kItem.Title:SetText(
                        kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_SECOND_BONUS]
                            .Title)
                    kItem.Details:SetText(
                        kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_SECOND_BONUS]
                            .Details)
                ...
                if kCityState.isBonus6 then
                    ...
                    kItem.Title:SetColor(kCityState.ColorSecondary)
                    kItem.Title:SetText(
                        kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_THIRD_BONUS]
                            .Title)
                    kItem.Details:SetText(
                        kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_THIRD_BONUS]
                            .Details)
                ...
                if kCityState.isBonusSuzerain then
                    kItem = m_BonusItemIM:GetInstance()
                    textureOffsetX, textureOffsetY, textureSheet =
                        IconManager:FindIconAtlas("ICON_ENVOY_BONUS_SUZERAIN",
                                                  50)
                    kItem.Icon:SetTexture(textureOffsetX, textureOffsetY,
                                          textureSheet)
                    kItem.Icon:SetColor(kCityState.ColorSecondary)
                    kItem.Title:SetColor(kCityState.ColorSecondary)
                    TruncateStringWithTooltip(kItem.Title,
                                              MAX_BEFORE_TRUNC_SUZERAIN,
                                              Locale.Lookup(
        ...
        --[[ CUI: hide bonus area
    local bonusAreaY = Controls.BonusArea:GetSizeY();
...
-- ===========================================================================
function ColorizeBonusItem(isHaveBonus, pInst, kCityState)
...
function ViewCityState(iPlayer)
    ...
    for _, kCityState in pairs(m_kCityStates) do
        if kCityState.isHasMet then
            local kInst = m_CityStateColumnIM:GetInstance()
            kInst.Icon:SetIcon("ICON_" .. kCityState.CivType)
            kInst.Icon:SetColor(kCityState.ColorSecondary)
            ...
            kInst.IconButton:RegisterCallback(Mouse.eLClick,
                                              OpenSingleViewCityState)

            textureOffsetX, textureOffsetY, textureSheet, tooltip =
                GetRelationshipPipAtlasPieces(kCityState)
            kInst.DiplomacyPip:SetTexture(textureOffsetX, textureOffsetY,
                                          textureSheet)
            if GameCapabilities.HasCapability("CAPABILITY_MILITARY") then
                kInst.DiplomacyPip:SetToolTipString(tooltip)
            end
    ...
    local kCityState = m_kCityStates[iPlayer]
    if kCityState == nil then
        UI.DataError("Attempt to show details for CityState player #" ..
                         tostring(iPlayer) .. ", but that doesn't exist!")
    ...
    Controls.CityStateTypeIcon:SetToolTipString(
        Locale.Lookup(GetTypeTooltip(kCityState)))
    Controls.CityStateName:SetText(Locale.ToUpper(kCityState.Name))

    local textureOffsetX, textureOffsetY, textureSheet, tooltip =
        GetRelationshipPipAtlasPieces(kCityState)
    Controls.DiplomacyPip:SetTexture(textureOffsetX, textureOffsetY,
                                     textureSheet)
    if GameCapabilities.HasCapability("CAPABILITY_MILITARY") then
    ...
    if GameCapabilities.HasCapability("CAPABILITY_MILITARY") then
        ...
        if kCityState.isAtWar then
            Controls.PeaceWarButton:SetText(
                Locale.Lookup("LOC_CITY_STATES_MAKE_PEACE"))
            Controls.PeaceWarButton:SetDisabled(not kCityState.CanMakePeaceWith)
            if not kCityState.CanMakePeaceWith then
                if (GlobalParameters.DIPLOMACY_WAR_LAST_FOREVER == 1 or
                    GlobalParameters.DIPLOMACY_WAR_LAST_FOREVER == true) then
                    warPeaceTooltip = warPeaceTooltip ..
                                          Locale.Lookup(
                                              "LOC_CITY_STATES_TURNS_WAR_NO_PEACE")
                else
                    if kCityState.SuzerainID ~= -1 and
                        pLocalPlayerDiplomacy:IsAtWarWith(kCityState.SuzerainID) then
                        warPeaceTooltip =
                            warPeaceTooltip ..
                                Locale.Lookup(
                                    "LOC_CITY_STATES_SUZERAIN_WAR_NO_PEACE")
                    else
                        warPeaceTooltip =
                            warPeaceTooltip ..
                                Locale.Lookup("LOC_CITY_STATES_TURNS_WAR",
                                              m_iTurnsOfWar +
                                                  kCityState.iTurnChanged -
            ...
            Controls.PeaceWarButton:SetText(
                Locale.Lookup("LOC_CITY_STATES_DECLARE_WAR_BUTTON"))
            Controls.PeaceWarButton:SetDisabled(not kCityState.CanDeclareWarOn)
            warPeaceTooltip = warPeaceTooltip ..
                                  Locale.Lookup(
                                      "LOC_CITY_STATES_DECLARE_WAR_DETAILS")
            if not kCityState.CanDeclareWarOn then
                if HasTrait("TRAIT_CIVILIZATION_FACES_OF_PEACE", localPlayerID) then
                    warPeaceTooltip = Locale.Lookup(
                                          "LOC_CIVILIZATION_NOT_ABLE_TO_DECLARE_SURPRISE_WAR")
                else
                    warPeaceTooltip = warPeaceTooltip .. " " ..
                                          Locale.Lookup(
                                              "LOC_CITY_STATES_TURNS_PEACE",
                                              m_iTurnsOfPeace +
                                                  kCityState.iTurnChanged -
                                                  Game.GetCurrentGameTurn())
                end
        ...
        if kCityState.HasLevyActive and kCityState.IsLocalPlayerSuzerain then
            local levyTooltip = Locale.Lookup(
                                    "LOC_CITY_STATES_MILITARY_ALREADY_LEVIED")
            Controls.LevyMilitaryButton:SetToolTipString(levyTooltip)
        else
            local levyTooltip = Locale.Lookup(
                                    "LOC_CITY_STATES_LEVY_MILITARY_DETAILS",
                                    kCityState.LevyMilitaryCost,
    ...
    Controls.InfluencedByValue:SetText(Locale.Lookup(
                                           "LOC_CITY_STATES_CIVILIZATIONS",
                                           table.count(kCityState.Influence)))
    ...
    RefreshRelationshipStack(kCityState.Relationships.CivRelationships,
                             m_RelationshipsButtonIM)
    ...
    if m_mode == MODE.EnvoySent then
        ...
        local kHeader = m_EnvoysBonusCityHeaderIM:GetInstance()
        kHeader.CityName:SetText(Locale.ToUpper(
                                     Locale.Lookup("LOC_CITY_STATES_BONUSES",
                                                   kCityState.Name)))

        -- At least the simplest of bonuses?
        local kItem = m_EnvoysBonusItemIM:GetInstance()
        local textureOffsetX, textureOffsetY, textureSheet =
            GetBonusIconAtlasPieces(kCityState, 50)
        kItem.Icon:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
        kItem.Title:SetText(kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_FIRST_BONUS]
                                .Title)
        kItem.Details:SetText(
            kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_FIRST_BONUS].Details)
        ...
        kItem.Title:SetText(
            kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_SECOND_BONUS].Title)
        kItem.Details:SetText(
            kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_SECOND_BONUS].Details)
        ColorizeBonusItem(kCityState.isBonus3, kItem, kCityState)

        kItem = m_EnvoysBonusItemIM:GetInstance()
        kItem.Icon:SetTexture(textureOffsetX, textureOffsetY, textureSheet) -- Same as above
        kItem.Title:SetText(kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_THIRD_BONUS]
                                .Title)
        kItem.Details:SetText(
            kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_THIRD_BONUS].Details)
        ColorizeBonusItem(kCityState.isBonus6, kItem, kCityState)

        kItem = m_EnvoysBonusItemIM:GetInstance()
        textureOffsetX, textureOffsetY, textureSheet =
            IconManager:FindIconAtlas("ICON_ENVOY_BONUS_SUZERAIN", 50)
        kItem.Icon:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
        TruncateStringWithTooltip(kItem.Title, MAX_BEFORE_TRUNC_SUZERAIN,
                                  Locale.Lookup(
                                      kCityState.Bonuses["Suzerain"].Title))
        kItem.Details:SetText(kCityState.Bonuses["Suzerain"].Details)
    ...
    elseif m_mode == MODE.InfluencedBy then
        ...
        for iOtherPlayer, influence in pairs(kCityState.Influence) do
            local kItem = AddInfluenceRow(kCityState.iPlayer, iOtherPlayer,
                                          influence, largestAmount)
            kSortTable[tostring(kItem.GetTopControl())] =
                {influence = influence} -- Store for sorting.
        end
    ...
    elseif m_mode == MODE.Quests then
        ...
        for _, kQuest in pairs(kCityState.Quests) do
            local kItem = m_QuestsIM:GetInstance()
...
function OnSingleViewStackSizeChanged()
    Controls.ReportTabContainer:SetParentRelativeSizeY(
        REPORT_CONTAINER_SIZE_PADDING - Controls.SingleViewStack:GetSizeY() -
            Controls.SingleViewStack:GetOffsetY())
end

-- ===========================================================================
function AddInfluenceRow(cityStateID, playerID, influence, largestInfluence)
    local kItem = m_InfluenceRowIM:GetInstance()
    ...
    local civName = "LOCAL_CITY_STATES_UNKNOWN"
...
-- ===========================================================================
function ViewRelationships(kCityState)
    -- Setup the buttons and what (sub) areas are shown/hidden.
    Controls.EnvoysSentButton:SetSelected(false)
    Controls.InfluencedByButton:SetSelected(false)
    Controls.QuestsButton:SetSelected(false)
    ...
    RefreshRelationshipStack(kCityState.Relationships.CivRelationships,
                             m_RelationshipsCivsIM)

    RefreshRelationshipStack(kCityState.Relationships.CityStateRelationships,
                             m_RelationshipsCityStatesIM)

    Controls.RelationshipsScroll:CalculateSize()
end

-- ===========================================================================
function RefreshRelationshipStack(kRelationships, StackIM)

    StackIM:ResetInstances()

    for _, kRelationship in pairs(kRelationships) do
        if kRelationship.HasMet then
            local instance = StackIM:GetInstance()
            ...
            if #Teams[kRelationship.TeamID] > 1 then
                local teamRibbonName = TEAM_RIBBON_PREFIX ..
                                           tostring(kRelationship.TeamID)
            ...
            if DIPLO_PIP_INFO[kRelationship.DiploState] ~= nil then
                local iconName = DIPLO_PIP_INFO[kRelationship.DiploState]
                                     .IconName
                if iconName == nil or iconName == "" then
                    print(
                        "WARNING: Unexpected DiplomaticState when obtain PIP art. value:" ..
                ...
                instance.DiplomacyPip:SetTexture(textureOffsetX, textureOffsetY,
                                                 textureSheet)
                if GameCapabilities.HasCapability("CAPABILITY_MILITARY") then
                    instance.DiplomacyPip:SetToolTipString(
                        kRelationship.DiploTooltip)
                end
...
function GetData()
    ...
    local pLocalPlayer = Players[localPlayerID]
    local isCanGiveInfluence = false
    local pLocalPlayerInfluence = pLocalPlayer:GetInfluence()
    ...
    m_kPlayerData = {EnvoyTokens = envoyTokensAvailable}

    local isNewBonusAchieved = false

    -- Every player that is in this game...
    for i, pPlayer in ipairs(PlayerManager.GetAliveMinors()) do
        local iPlayer = pPlayer:GetID()
        local isCanReceiveInfluence = false
        local envoyTokens = 0
        local envoyTokensMostReceived = 0
        local kInfluence = {}
        local suzerainID = -1
        ...
        if pPlayerInfluence ~= nil then
            ...
            envoyTokens = pPlayerInfluence:GetTokensReceived(
                              pLocalPlayer:GetID())
            ...
            for _, iInfluencePlayer in ipairs(PlayerManager.GetAliveMajorIDs()) do
                local tokensReceived = pPlayerInfluence:GetTokensReceived(
                                           iInfluencePlayer)
        ...
        if iPlayer ~= pLocalPlayer:GetID() and isCanReceiveInfluence then
            ...
            if suzerainID ~= -1 then
                if (suzerainID == localPlayerID) then
                    local pPlayerConfig = PlayerConfigurations[suzerainID]
            ...
            local cityStateType = GetCityStateType(iPlayer)

            local iPlayerDiploState = pPlayer:GetDiplomaticAI()
                                          :GetDiplomaticStateIndex(localPlayerID)
            local diplomaticState = nil
            if iPlayerDiploState ~= -1 then
                diplomaticState = GameInfo.DiplomaticStates[iPlayerDiploState]
                                      .StateType
            end
            ...
            local kCityState = {
                ...
                Bonuses = {},
                CanDeclareWarOn = pLocalPlayer:GetDiplomacy():CanDeclareWarOn(
                    iPlayer),
                CanLevyMilitary = pLocalPlayer:GetInfluence():CanLevyMilitary(
                    iPlayer),
                CanMakePeaceWith = pLocalPlayer:GetDiplomacy():CanMakePeaceWith(
                    iPlayer),
                CanReceiveTokensFrom = pLocalPlayer:GetInfluence()
                    :CanGiveTokensToPlayer(iPlayer),
                ...
                iScore = pPlayer:GetDiplomaticAI():GetDiplomaticScore(
                    localPlayerID),
                iState = iPlayerDiploState,
                iTurnChanged = pLocalPlayer:GetDiplomacy():GetAtWarChangeTurn(
                    iPlayer),
                iVisibility = pLocalPlayer:GetDiplomacy():GetVisibilityOn(
                    iPlayer),
                iGameScore = pPlayer:GetScore(),
                LevyMilitaryCost = pLocalPlayer:GetInfluence()
                    :GetLevyMilitaryCost(iPlayer),
                LevyMilitaryTurnLimit = pPlayer:GetInfluence()
                    :GetLevyTurnLimit(),
                HasLevyActive = (pPlayer:GetInfluence():GetLevyTurnCounter() >=
                    0),
            ...
            elseif not kCityState.isBonusSuzerain then
                kCityState.SuzerainTokensNeeded =
                    kCityState.SuzerainTokensNeeded + 1
            end

            -- Obtain bonus text:
            local title, details = GetBonusText(iPlayer,
                                                NUM_ENVOY_TOKENS_FOR_FIRST_BONUS)
            kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_FIRST_BONUS] =
                {Title = title, Details = details}
            if kCityState.isBonus1 then
                if m_kLastCityStates ~= nil and
                    not m_kLastCityStates[iPlayer].isBonus1 then
                    isNewBonusAchieved = true
                end
            end
            title, details = GetBonusText(iPlayer,
                                          NUM_ENVOY_TOKENS_FOR_SECOND_BONUS)
            kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_SECOND_BONUS] =
                {Title = title, Details = details}
            if kCityState.isBonus3 then
                if m_kLastCityStates ~= nil and
                    not m_kLastCityStates[iPlayer].isBonus3 then
                    isNewBonusAchieved = true
                end
            end
            title, details = GetBonusText(iPlayer,
                                          NUM_ENVOY_TOKENS_FOR_THIRD_BONUS)
            kCityState.Bonuses[NUM_ENVOY_TOKENS_FOR_THIRD_BONUS] =
                {Title = title, Details = details}
            if kCityState.isBonus6 then
                if m_kLastCityStates ~= nil and
                    not m_kLastCityStates[iPlayer].isBonus6 then
                    isNewBonusAchieved = true
                end
            end
            ...
            if kCityState.isBonusSuzerain then
                if m_kLastCityStates ~= nil and
                    not m_kLastCityStates[iPlayer].isBonusSuzerain then
                    isNewBonusAchieved = true
                end
            end
...
-- ===========================================================================
function GetCityStateType(playerID)

    local cityStateType = ""
    local leader = PlayerConfigurations[playerID]:GetLeaderTypeName()
    local leaderInfo = GameInfo.Leaders[leader]
    if leaderInfo == nil or leaderInfo.InheritFrom == nil then
        UI.DataError("Cannot determine leader type for player #" ..
                         tostring(iPlayer))
        cityStateType = "unknown"
    elseif (leader == "LEADER_MINOR_CIV_SCIENTIFIC" or leaderInfo.InheritFrom ==
        "LEADER_MINOR_CIV_SCIENTIFIC") then
...
function OnShutdown()
    ...
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "m_iCurrentCityState",
                                 m_iCurrentCityState)
end
    ...
    if context == RELOAD_CACHE_ID then
        ...
        if contextTable["isHidden"] ~= nil and not contextTable["isHidden"] then
            ...
            elseif m_mode == MODE.SendEnvoys then
                OnOpenSendEnvoys()
            elseif m_mode == MODE.EnvoySent or m_mode == MODE.InfluencedBy or
                m_mode == MODE.Quests or m_mode == MODE.Relationships then
                m_kScreenSlideAnim.Show()
                Refresh()
            end
        end
...
function OnInterfaceModeChanged(eOldMode, eNewMode)
    if eNewMode == InterfaceModeTypes.VIEW_MODAL_LENS then Close() end
end
...
function Initialize()
    ...
    if NUM_ENVOY_TOKENS_FOR_FIRST_BONUS == NUM_ENVOY_TOKENS_FOR_SECOND_BONUS or
        ...
        UI.DataError(
            "At least 2 city state bonuses have the same value, this will cause issues!")
    end
    ...
    Controls.Title:SetText(
        Locale.ToUpper(Locale.Lookup("LOC_CITY_STATES_TITLE")))
    ...
    Controls.InfluencedByButton:RegisterCallback(Mouse.eLClick,
                                                 OnInfluencedByClick)
    Controls.QuestsButton:RegisterCallback(Mouse.eLClick, OnQuestsClick)
    Controls.RelationshipsButton:RegisterCallback(Mouse.eLClick,
