        ...
        if unitInfo.Spy then
            ...
            if operationType == -1 then
                ...
                if operationInfo then
                    ...
                    spyEntry.Turns = " [" ..
                                         Locale.Lookup(
                                             "LOC_ESPIONAGEOVERVIEW_MORE_TURNS",
                                             turnsRemaining) .. "]"
                end
            end
    ...
    for i, player in ipairs(players) do
        ...
        for i = 0, numCapturedSpies - 1, 1 do
            ...
            if spy and spy.OwningPlayer == Game.GetLocalPlayer() then
                ...
                local capturingPlayerConfig =
                    PlayerConfigurations[player:GetID()]
                local capureingPlayerName =
                    Locale.Lookup(capturingPlayerConfig:GetPlayerName())
                if capureingPlayerName then
                    spyEntry.Name = Locale.ToUpper(spy.Name)
                    spyEntry.Rank = " (" ..
                                        Locale.Lookup(
                                            GetSpyRankNameByLevel(spy.Level)) ..
                                        ")"
                    spyEntry.Action = Locale.Lookup(
                                          "LOC_ESPIONAGEOVERVIEW_CAPTURED") ..
                                          " '" .. capureingPlayerName .. "'"
    ...
    if playerDiplomacy then
        ...
        for i = 0, numSpiesOffMap - 1, 1 do
            ...
            if spy and spy.ReturnTurn ~= -1 then
                ...
                local travelTurnsRemaining =
                    spy.ReturnTurn - Game.GetCurrentGameTurn()
                if targetCity and travelTurnsRemaining then
                    ...
                    spyEntry.Rank = " (" ..
                                        ...
                                        ")"
                    spyEntry.Action = Locale.Lookup(
                                          "LOC_ESPIONAGEOVERVIEW_TRANSIT_TO",
                                          targetCity:GetName())
                    spyEntry.Turns = "[" ..
                                         Locale.Lookup(
                                             "LOC_ESPIONAGEOVERVIEW_MORE_TURNS",
...
function RefreshSpyInfo()
    ...
    if spyCapacity > 0 then
        ...
        local sSummary = ""
        sSummary = sSummary ..
                       Locale.Lookup("LOC_CUI_SI_SPY_AVAILABLE", spyAvailable) ..
                       "[NEWLINE]"
        sSummary = sSummary ..
                       Locale.Lookup("LOC_CUI_SI_SPY_CAPACITY", spyCapacity)
...
function AttachToTopPanel()
    if not isAttached then
        local infoStack = ContextPtr:LookUpControl(
                              "/InGame/TopPanel/InfoStack/StaticInfoStack")
