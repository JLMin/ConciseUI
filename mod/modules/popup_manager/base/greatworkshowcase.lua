...
local LOC_BACK_TO_GREAT_WORKS = Locale.Lookup(
                                    "LOC_GREAT_WORKS_BACK_TO_GREAT_WORKS")
...
function UpdateGalleryData()
            ...
            for buildingInfo in GameInfo.Buildings() do
                ...
                if (pCityBldgs:HasBuilding(buildingIndex)) then
                    ...
                    for index = 0, numSlots - 1 do
                        local greatWorkIndex =
                            pCityBldgs:GetGreatWorkInSlot(buildingIndex, index)
                        if greatWorkIndex ~= -1 then
                            table.insert(m_GreatWorks, {
    ...
    local canCycleGreatWorks = m_GalleryIndex ~= -1 and
                                   table.count(m_GreatWorks) > 1
...
function UpdateGreatWork()
    ...
    local greatWorkCreator = Locale.Lookup(
                                 m_CityBldgs:GetCreatorNameFromIndex(
                                     m_GreatWorkIndex))
    local greatWorkCreationDate = Calendar.MakeDateStr(
                                      m_CityBldgs:GetTurnFromIndex(
                                          m_GreatWorkIndex),
    ...
    if greatWorkObjectType == GREAT_WORK_MUSIC_TYPE then
        ...
        Controls.MusicName:SetText(Locale.ToUpper(
    ...
    elseif greatWorkObjectType == GREAT_WORK_WRITING_TYPE then
        ...
        Controls.WritingName:SetText(Locale.ToUpper(
                                         Locale.Lookup(greatWorkInfo.Name)))
        ...
        if (quoteKey ~= nil) then
            ...
            Controls.WritingDeco:SetOffsetY(
                Controls.WritingName:GetSizeY() + -20)
        end
        Controls.WritingDetails:SetHide(false)
    elseif greatWorkObjectType == GREAT_WORK_ARTIFACT_TYPE or
        greatWorkObjectType == GREAT_WORK_RELIC_TYPE then
        if greatWorkObjectType == GREAT_WORK_ARTIFACT_TYPE then
            ...
            Controls.GreatWorkName:SetText(
                Locale.ToUpper(Locale.Lookup(greatWorkInfo.Name)))
        ...
        Controls.GreatWorkName:SetText(Locale.ToUpper(
                                           Locale.Lookup(greatWorkInfo.Name)))
    ...
    Controls.CreatedBy:SetText(Locale.Lookup("LOC_GREAT_WORKS_CREATED_BY",
                                             greatWorkCreator))
    Controls.CreatedDate:SetText(Locale.Lookup("LOC_GREAT_WORKS_CREATED_TIME",
                                               greatWorkTypeName,
                                               greatWorkCreationDate))
    Controls.CreatedPlace:SetText(Locale.Lookup("LOC_GREAT_WORKS_CREATED_PLACE",
                                                greatWorkCreationBuilding,
                                                greatWorkCreationCity))

    Controls.GreatWorkHeader:SetText(m_isGallery and "" or LOC_NEW_GREAT_WORK)
    Controls.ViewGreatWorks:SetText(m_isGallery and LOC_BACK_TO_GREAT_WORKS or
                                        LOC_VIEW_GREAT_WORKS)
...
function OnGreatWorkCreated(playerID, creatorID, cityX, cityY, buildingID,
                            greatWorkIndex)
    ...
    if CuiSettings:GetBoolean(CuiSettings.POPUP_CREATWORK) then
        DisplayGreatWorkCreated(playerID, creatorID, cityX, cityY, buildingID,
                                greatWorkIndex, false)
    end
end
function OnShowRelicCreated(playerID, creatorID, cityX, cityY, buildingID,
                            greatWorkIndex)
    -- CUI
...
function DisplayGreatWorkCreated(playerID, creatorID, cityX, cityY, buildingID,
                                 greatWorkIndex, showRelics)
    ...
    if m_City ~= nil then
        ...
        m_GreatWorkType =
            m_CityBldgs:GetGreatWorkTypeFromIndex(m_GreatWorkIndex)
...
function OnPreviousGreatWork()
    ...
    if numGreatWorks > 1 then
        ...
        OnViewGreatWork(greatWorkData.City, greatWorkData.Building,
                        greatWorkData.Index)
    end
end
...
function OnShutdown()
    ...
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "playerID",
                                 Game.GetLocalPlayer())
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "cityX",
                                 m_City ~= nil and m_City:GetX() or -1)
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "cityY",
                                 m_City ~= nil and m_City:GetY() or -1)
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "buildingID", m_BuildingID)
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "greatWorkIndex",
                                 m_GreatWorkIndex)
end
function OnGameDebugReturn(context, data)
    if context == RELOAD_CACHE_ID and data["isHidden"] ~= nil and
        not data["isHidden"] then
        if data["isGallery"] then
            OnViewGreatWork(Cities.GetCityInPlot(
                                Map.GetPlotIndex(data["cityX"], data["cityY"])),
                            data["buildingID"], data["greatWorkIndex"])
        else
            OnGreatWorkCreated(data["playerID"], nil, data["cityX"],
                               data["cityY"], data["buildingID"],
                               data["greatWorkIndex"])
        end
    end
end
...
function Initialize()
    ...
    Controls.PreviousGreatWork:RegisterCallback(Mouse.eLClick,
                                                OnPreviousGreatWork)
