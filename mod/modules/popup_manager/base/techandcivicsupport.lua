...
function GetUnlockablesForCivic_Cached(civicType, playerId)
    ...
    if m_kCivicsData == nil or table.count(m_kCivicsData) == 0 then
        m_kCivicsData = {}
        for i = 0, GameDefines.MAX_PLAYERS, 1 do m_kCivicsData[i] = {} end
    end
...
function GetUnlockablesForTech_Cached(techType, playerId, playerUnlockables)
    ...
    if m_kTechsData == nil or table.count(m_kTechsData) == 0 then
        m_kTechsData = {}
        for i = 0, GameDefines.MAX_PLAYERS - 1, 1 do m_kTechsData[i] = {} end
    end
...
function GetUnlockIcon(typeName)
    ...
    if (typeInfo) then
        ...
        if (typeInfo.Kind == "KIND_POLICY") then
            ...
            elseif (slotType == "SLOT_WILDCARD" or slotType ==
                "SLOT_GREAT_PERSON") then
...
function ResetOverflowArrow(kItemInstance)
    if kItemInstance == nil then return end
    if kItemInstance.PageTurnerImage == nil then return end
...
function HandleOverflow(numUnlockables, kItemInstance, numMaxVisible,
                        numMaxPerPage)
    ...
    kItemInstance.UnlockPageTurner:RegisterCallback(Mouse.eLClick, function()
...
--
-- ===========================================================================
function PopulateUnlockablesForCivic(playerID, civicID, kItemIM, kGovernmentIM,
                                     callback, hideDescriptionIcon)

    local kCivicData = GameInfo.Civics[civicID]
    if kCivicData == nil then
        UI.DataError(
            "Unable to find a civic type in the database with an ID value of #" ..
                tostring(civicID))
        return
    end
    ...
    if (unlockables and #unlockables > 0) then
        for i, v in ipairs(unlockables) do
            ...
            if (kGovernmentIM and typeInfo and typeInfo.Kind ==
                "KIND_GOVERNMENT") then

                local unlock = kGovernmentIM:GetInstance()

                local government = governmentData[typeName]
                if (government) then
                    unlock.MilitaryPolicyLabel:SetText(
                        tostring(government.NumSlotMilitary))
                    unlock.EconomicPolicyLabel:SetText(
                        tostring(government.NumSlotEconomic))
                    unlock.DiplomaticPolicyLabel:SetText(
                        tostring(government.NumSlotDiplomatic))
                    unlock.WildcardPolicyLabel:SetText(
                        tostring(government.NumSlotWildcard))
                ...
                unlock.GovernmentInstanceGrid:RegisterCallback(Mouse.eLClick,
                                                               callback)

                if (not IsTutorialRunning()) then
                    unlock.GovernmentInstanceGrid:RegisterCallback(
                ...
                unlockIcon.Icon:SetHide(not unlockIcon.Icon:SetIcon(
                                            "ICON_" .. typeName)) -- Hide if an icon isn't found with that type.

                local textureOffsetX, textureOffsetY, textureSheet =
                    IconManager:FindIconAtlas(iconName, 38)
                if textureSheet ~= nil then
                    unlockIcon.UnlockIcon:SetTexture(textureOffsetX,
                                                     textureOffsetY,
                                                     textureSheet)
                end
                ...
                if callback ~= nil then
                    unlockIcon.UnlockIcon:RegisterCallback(Mouse.eLClick,
                                                           callback)
                else
                ...
                if (not IsTutorialRunning()) then
                    unlockIcon.UnlockIcon:RegisterCallback(Mouse.eRClick,
                                                           function()
                        LuaEvents.OpenCivilopedia(civilopediaKey)
                    end)
                end
    ...
    if (kCivicData.Description and hideDescriptionIcon ~= true) then
        ...
        local textureOffsetX, textureOffsetY, textureSheet =
            IconManager:FindIconAtlas("ICON_TECHUNLOCK_13", 38)
        if textureSheet ~= nil then
            unlockIcon.UnlockIcon:SetTexture(textureOffsetX, textureOffsetY,
                                             textureSheet)
        end
...
function PopulateUnlockablesForTech(playerID, techID, instanceManager, callback)
    ...
    if kTechData == nil then
        UI.DataError(
            "Unable to find a tech type in the database with an ID value of #" ..
                tostring(techID))
        return
    end
...
function GetActiveData(kData)
    for _, data in ipairs(kData) do
        if data.IsCurrent or data.IsLastCompleted then return data end
    end
    return nil
end
...
function GetUnlockIM(kControl)
    ...
    if unlockIM ~= nil then
        ...
        -- Create
        unlockIM = InstanceManager:new("UnlockIconInstance", "UnlockIcon",
                                       kControl.UnlockStack)
...
function RealizeMeterAndBoosts(kControl, kData)
    ...
    if kData.Boostable then
        ...
        if kData.BoostTriggered then
            boostString = Locale.Lookup("LOC_TECH_HAS_BEEN_BOOSTED") ..
                              boostString -- Same whether tech/civic
            kControl.IconHasBeenBoosted:SetToolTipString(boostString)
            progress = math.clamp(progress, 0, 1.0)
        else
            boostString = Locale.Lookup("LOC_TECH_CAN_BE_BOOSTED") ..
                              boostString -- Same whether tech/civic
            kControl.IconCanBeBoosted:SetToolTipString(boostString)
            local boostAmount = math.min((kData.Progress + kData.BoostAmount),
                                         1.0)
            kControl.BoostMeter:SetPercent(boostAmount)
        end

        TruncateStringWithTooltip(kControl.BoostLabel,
                                  MAX_BEFORE_TRUNC_BOOST_MSG,
                                  Locale.Lookup(kData.TriggerDesc))
    end

    -- CUI: boost reminder
    if kControl.MainPanel ~= nil and kData.Boostable then
        local boostAmount = kData.Progress + kData.BoostAmount
        local couldRemind = (not kData.BoostTriggered) and (boostAmount >= 1)
        if kData.TechType ~= nil then
            local techRemind = true -- "GetBoolean("TECK_REMINDER)
            if techRemind and couldRemind then
                -- todo
            end
        elseif kData.CivicType ~= nil then
            local civicsRemind = true -- "GetBoolean("TECK_REMINDER)
    ...
    --
    if kData.IsLastCompleted then progress = 1.0 end

    kControl.IconCanBeBoosted:SetHide((not (kData.Boostable and
                                          not kData.BoostTrigger)) or
                                          kData.IsLastCompleted)
    kControl.IconHasBeenBoosted:SetHide((not kData.BoostTriggered) or
                                            kData.IsLastCompleted)
    kControl.ProgressMeter:SetPercent(progress)
    kControl.BoostLabel:SetHide((not kData.Boostable) or kData.IsLastCompleted)
    kControl.BoostMeter:SetHide(
        (not kData.Boostable) or kData.IsLastCompleted or (kData.BoostTriggered))
end
...
function RealizeIcon(kIconControl, typeName, size)
    ...
    local textureOffsetX, textureOffsetY, textureSheet =
        IconManager:FindIconAtlas(textureString, size)
    if textureSheet ~= nil then
        kIconControl:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
    else
        UI.DataError(
            "Missing icon '" .. tostring(textureString) .. "' at size " ..
                tostring(size))
    end
end
...
function RealizeTurnsLeft(kControl, kData)
    ...
    if kData ~= nil then
        ...
        if isLastCompleted and not isRepeatable then
            kControl.TurnsLeft:SetText(Locale.Lookup(
                                           "LOC_RESEARCH_CHOOSER_JUST_COMPLETED"))
        else
...
function RealizeCurrentResearch(playerID, kData, kControl)
    ...
    if kControl == nil then kControl = Controls end
    ...
    if kData ~= nil then
        ...
        if (not IsTutorialRunning()) then
            kControl.TitleButton:RegisterCallback(Mouse.eRClick, function()
                LuaEvents.OpenCivilopedia(techType)
            end)
        end

        kControl.MainPanel:RegisterMouseEnterCallback(
            function() kControl.MainGearAnim:Play() end)
        kControl.MainPanel:RegisterMouseExitCallback(
            function() kControl.MainGearAnim:Stop() end)

        RealizeMeterAndBoosts(kControl, kData)
        RealizeIcon(kControl.Icon, kData.TechType, SIZE_ICON_RESEARCH_LARGE)

        numUnlockables = PopulateUnlockablesForTech(playerID, kData.ID,
                                                    techUnlockIM, nil)
        if numUnlockables ~= nil and kControl ~= nil then
            HandleOverflow(numUnlockables, kControl, MAX_ICONS_BEFORE_OVERFLOW,
                           MAX_ICONS_BEFORE_OVERFLOW - 1)
        end
    ...
    if kData ~= nil then
        ...
        if (civicModifiers) then
            for _, tModifier in ipairs(civicModifiers) do
                ...
                if (tIconData) then
                    hideDescriptionIcon =
                        hideDescriptionIcon or tIconData.HideDescriptionIcon
                    table.insert(extraUnlocks, {
                        IconData = tIconData,
        ...
        numUnlockables = numUnlockables +
                             PopulateUnlockablesForCivic(playerID, kData.ID,
                                                         unlockIM, nil, nil,
                                                         hideDescriptionIcon)

        -- Initialize extra icons
        for _, tUnlock in pairs(extraUnlocks) do
            tUnlock.IconData:Initialize(kControl.UnlockStack,
                                        tUnlock.ModifierTable)
