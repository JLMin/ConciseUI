...
--	CONSTANTS / MEMBERS
-- ===========================================================================
local RELOAD_CACHE_ID = "TechCivicCompletedPopup"
local m_unlockIM = InstanceManager:new("UnlockInstance", "Top",
                                       Controls.UnlockStack)
local m_isDisabledByTutorial = false
local m_kCurrentData = nil
local m_kPopupData = {}
local m_isCivicData = false
local m_quote_audio
...
function ShowCivicCompletedPopup(player, civic, quote, audio)
    ...
    if civicInfo == nil then
        UI.DataError("Cannot show civic popup because GameInfo.Civics[" ..
                         tostring(civic) .. "] doesn't have data.")
        return
    end
    ...
    -- Update Header
    Controls.HeaderLabel:SetText(Locale.Lookup(
                                     "LOC_RESEARCH_COMPLETE_CIVIC_COMPLETE"))
    ...
    local textureOffsetX, textureOffsetY, textureSheet =
        IconManager:FindIconAtlas(icon, 160)
    if textureSheet ~= nil then
        Controls.ResearchIcon:SetTexture(textureOffsetX, textureOffsetY,
                                         textureSheet)
    end
    ...
    Controls.CivicMsgLabel:SetText(Locale.Lookup(
                                       "LOC_UI_CIVIC_PROGRESS_COMPLETE_BLURB",
                                       civicInfo.Name))
    ...
    Controls.UnlockCountLabel:SetText(Locale.Lookup(
                                          "LOC_RESEARCH_COMPLETE_UNLOCKED_BY_CIVIC",
    ...
    if isCivicUnlockGovernmentType then
        Controls.ChangeGovernmentButton:SetText(
            Locale.Lookup("LOC_GOVT_GOVERNMENT_UNLOCKED"))
        Controls.ChangeGovernmentButton:ClearCallback(Mouse.eLClick)
        Controls.ChangeGovernmentButton:RegisterCallback(Mouse.eLClick,
                                                         OnChangeGovernment)
        Controls.ChangeGovernmentButton:RegisterCallback(Mouse.eMouseEnter,
                                                         function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end)
    else
        Controls.ChangeGovernmentButton:SetText(
            Locale.Lookup("LOC_GOVT_CHANGE_POLICIES"))
        Controls.ChangeGovernmentButton:ClearCallback(Mouse.eLClick)
        Controls.ChangeGovernmentButton:RegisterCallback(Mouse.eLClick,
                                                         OnChangePolicy)
        Controls.ChangeGovernmentButton:RegisterCallback(Mouse.eMouseEnter,
...
-- ===========================================================================
function ShowTechCompletedPopup(player, tech, quote, audio)
    local techInfo = GameInfo.Technologies[tech]
    if techInfo == nil then
        UI.DataError("Cannot show popup because GameInfo.Technologies[" ..
                         tostring(tech) .. "] doesn't have data.")
        return
    end
    ...
    -- Update Header
    Controls.HeaderLabel:SetText(Locale.Lookup(
                                     "LOC_RESEARCH_COMPLETE_TECH_COMPLETE"))
    ...
    Controls.UnlockCountLabel:SetText(Locale.Lookup(
                                          "LOC_RESEARCH_COMPLETE_UNLOCKED_BY_TECH",
                                          m_unlockIM.m_iAllocatedInstances))
...
-- ===========================================================================
function AddCompletedPopup(player, civic, tech, isByUser)
    local isNotBlockedByTutorial = (not m_isDisabledByTutorial)

    if player == Game.GetLocalPlayer() and isNotBlockedByTutorial and
        (not GameConfiguration.IsNetworkMultiplayer()) then
        ...
        if civic then
            ...
            if civicInfo == nil then
                UI.DataError(
                    "Cannot show civic popup because GameInfo.Civics[" ..
                        tostring(civic) .. "] doesn't have data.")
                return
            end
            civicType = civicInfo.CivicType
            results = DB.Query(
                          "SELECT Quote, QuoteAudio from CivicQuotes where CivicType = ? ORDER BY RANDOM() LIMIT 1",
                          civicType)
        else
            local techInfo = GameInfo.Technologies[tech]
            if techInfo == nil then
                UI.DataError(
                    "Cannot add popup because GameInfo.Technologies[" ..
                        tostring(tech) .. "] doesn't have data.")
                return
            end
...
function RealizeNextPopup()
    ...
    if m_kCurrentData == nil then
        if (table.count(m_kPopupData) < 1) then
            UI.DataError(
                "Attempt to realize the next WorldBuiltPopup but there is no data.")
            Close()
        end
    ...
    if m_isCivicData then
        ShowCivicCompletedPopup(m_kCurrentData.player, m_kCurrentData.civic,
                                m_kCurrentData.quote, m_kCurrentData.audio)
    else
        ShowTechCompletedPopup(m_kCurrentData.player, m_kCurrentData.tech,
                               m_kCurrentData.quote, m_kCurrentData.audio)
    end
...
function TryClose()
    ...
    if m_kCurrentData == nil then
        UI.DataError(
            "Attempting to TryClosing the techcivic completed popup but it appears to have no data in it.")
        Close()
        return
    end

    if m_kCurrentData.civicType and string.len(m_kCurrentData.civicType) > 0 then
        LuaEvents.TechCivicCompletedPopup_CivicShown(m_kCurrentData.player,
                                                     m_kCurrentData.civicType)
    else
        LuaEvents.TechCivicCompletedPopup_TechShown(m_kCurrentData.player,
                                                    m_kCurrentData.techType)
    end
...
function OnShutdown()
    ...
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "m_kCurrentData",
                                 m_kCurrentData)
end
...
function OnGameDebugReturn(context, contextTable)
    ...
    if m_kCurrentData ~= nil then
        AddCompletedPopup(m_kCurrentData.player, m_kCurrentData.civic,
                          m_kCurrentData.tech, m_kCurrentData.isByUser)
    end
...
function Initialize()
    ...
    LuaEvents.TutorialUIRoot_DisableTechAndCivicPopups.Add(
        OnDisableTechAndCivicPopups)
    LuaEvents.TutorialUIRoot_EnableTechAndCivicPopups.Add(
        OnEnableTechAndCivicPopups)
    LuaEvents.NotificationPanel_ShowTechDiscovered.Add(
        OnNotificationPanel_ShowTechDiscovered)
    LuaEvents.NotificationPanel_ShowCivicDiscovered.Add(
        OnNotificationPanel_ShowCivicDiscovered)
