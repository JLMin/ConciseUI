...
function GetFormattedOperationDetailText(operation, spy, city)
    ...
    local sOperationDetails = UnitManager.GetOperationDetailText(eOperation,
                                                                 spy,
                                                                 Map.GetPlot(
                                                                     city:GetX(),
                                                                     city:GetY()))
    if operation.OperationType == "UNITOPERATION_SPY_GREAT_WORK_HEIST" then
        outputString = Locale.Lookup(
                           "LOC_SPYMISSIONDETAILS_UNITOPERATION_SPY_GREAT_WORK_HEIST",
                           sOperationDetails)
    elseif operation.OperationType == "UNITOPERATION_SPY_SIPHON_FUNDS" then
        outputString = Locale.Lookup("LOC_CUI_EP_SIPHON_FUNDS",
                                     sOperationDetails) -- CUI
    elseif operation.OperationType == "UNITOPERATION_SPY_FOMENT_UNREST" then
        outputString = Locale.Lookup(
                           "LOC_SPYMISSIONDETAILS_UNITOPERATION_SPY_FOMENT_UNREST",
                           sOperationDetails)
    elseif operation.OperationType == "UNITOPERATION_SPY_FABRICATE_SCANDAL" then
        outputString = Locale.Lookup(
                           "LOC_SPYMISSIONDETAILS_UNITOPERATION_SPY_FABRICATE_SCANDAL",
                           sOperationDetails)
    elseif operation.OperationType == "UNITOPERATION_SPY_NEUTRALIZE_GOVERNOR" then
        outputString = Locale.Lookup(
                           "LOC_SPYMISSIONDETAILS_UNITOPERATION_SPY_NEUTRALIZE_GOVERNOR",
                           sOperationDetails)
    elseif sOperationDetails ~= "" then
        ...
        outputString = Locale.Lookup("LOC_SPYMISSIONDETAILS_" ..
...
-- ===========================================================================
function RefreshMissionStats(parentControl, operation, result, spy, city,
                             targetPlot)
    ...
    if operation.Hash ~= UnitOperationTypes.SPY_COUNTERSPY then
        local resultProbability = UnitManager.GetResultProbability(eOperation,
                                                                   spy,
                                                                   targetPlot)
        if resultProbability["ESPIONAGE_SUCCESS_UNDETECTED"] then
            local probability =
                resultProbability["ESPIONAGE_SUCCESS_UNDETECTED"]

            -- Add ESPIONAGE_SUCCESS_MUST_ESCAPE
            if resultProbability["ESPIONAGE_SUCCESS_MUST_ESCAPE"] then
                probability = probability +
                                  resultProbability["ESPIONAGE_SUCCESS_MUST_ESCAPE"]
            end
            ...
            if probability > 85 then
                parentControl.ProbabilityLabel:SetColorByName(
                    "OperationChance_Green")
            elseif probability > 65 then
                parentControl.ProbabilityLabel:SetColorByName(
                    "OperationChance_YellowGreen")
            elseif probability > 45 then
                parentControl.ProbabilityLabel:SetColorByName(
                    "OperationChance_Yellow")
            elseif probability > 25 then
                parentControl.ProbabilityLabel:SetColorByName(
                    "OperationChance_Orange")
            else
                parentControl.ProbabilityLabel:SetColorByName(
                    "OperationChance_Red")
            end
    ...
    if operation.Hash == UnitOperationTypes.SPY_COUNTERSPY then
        ...
        parentControl.MissionDistrictName:SetText(
            Locale.Lookup(kDistrictInfo.Name))
    ...
    elseif result and result[UnitOperationResults.PLOTS] then
        for i, districtPlotID in ipairs(result[UnitOperationResults.PLOTS]) do
            ...
            for i, district in districts:Members() do
                local districtPlot = Map.GetPlot(district:GetX(),
                                                 district:GetY())
        ...
        parentControl.MissionDistrictName:SetText(
            Locale.Lookup("LOC_DISTRICT_CITY_CENTER_NAME"))
...
function GetMissionDescriptionString(mission, noloot, withloot)
    if mission.LootInfo >= 0 then
        return Locale.Lookup(withloot, GetMissionLootString(mission),
                             mission.CityName)
    end
...
function GetMissionOutcomeDetails(mission)
    ...
    if kOpDef ~= nil and kOpDef.Hash == UnitOperationTypes.SPY_COUNTERSPY then
        ...
        outcomeDetails.Description = Locale.Lookup(
                                         "LOC_ESPIONAGEOVERVIEW_MISSIONOUTCOME_SUCCESS_COUNTERSPY",
                                         mission.CityName)
    ...
    elseif mission.InitialResult == EspionageResultTypes.SUCCESS_MUST_ESCAPE then
        ...
        if mission.EscapeResult == EspionageResultTypes.FAIL_MUST_ESCAPE then
            ...
            outcomeDetails.Description =
                GetMissionDescriptionString(mission,
        ...
        elseif mission.EscapeResult == EspionageResultTypes.KILLED then
            ...
            outcomeDetails.SpyStatus = Locale.ToUpper(
                                           "LOC_ESPIONAGEOVERVIEW_SPYKILLED")
        elseif mission.EscapeResult == EspionageResultTypes.CAPTURED then
            ...
            outcomeDetails.SpyStatus = Locale.ToUpper(
                                           "LOC_ESPIONAGEOVERVIEW_SPYCAUGHT")
        end
...
function GetMissionLootString(mission)
    ...
    elseif operationInfo.Hash == UnitOperationTypes.SPY_SIPHON_FUNDS then
        if mission.LootInfo <= 0 then
            lootString = Locale.Lookup(
                             "LOC_ESPIONAGEOVERVIEW_MISSIONOUTCOME_NO_GOLD")
        else
            lootString = Locale.Lookup(
                             "LOC_ESPIONAGEOVERVIEW_MISSIONOUTCOME_GOLD",
                             mission.LootInfo)
        end
    end
...
function CanMissionBeRenewed(mission)
    ...
    if kOperationInfo.Hash == UnitOperationTypes.SPY_LISTENING_POST or
        kOperationInfo.Hash == UnitOperationTypes.SPY_COUNTERSPY then
        return true
    end
