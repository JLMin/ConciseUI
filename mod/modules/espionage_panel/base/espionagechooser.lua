...
-- Instance managers
local m_RouteChoiceIM = InstanceManager:new("DestinationInstance",
                                            "DestinationButton",
                                            Controls.DestinationStack)
local m_MissionStackIM = InstanceManager:new("MissionInstance", "MissionButton",
                                             Controls.MissionStack)
...
CUI_AVAILABLE_DISTRICTS[1] = {
    button = "CityCenterSortButton",
    dType = "DISTRICT_CITY_CENTER"
}
CUI_AVAILABLE_DISTRICTS[2] = {
    button = "CampusSortButton",
    dType = "DISTRICT_CAMPUS"
}
CUI_AVAILABLE_DISTRICTS[3] = {
    button = "CommercialSortButton",
    dType = "DISTRICT_COMMERCIAL_HUB"
}
CUI_AVAILABLE_DISTRICTS[4] = {
    button = "IndustrialSortButton",
    dType = "DISTRICT_INDUSTRIAL_ZONE"
}
CUI_AVAILABLE_DISTRICTS[5] = {
    button = "NeighborhoodSortButton",
    dType = "DISTRICT_NEIGHBORHOOD"
}
CUI_AVAILABLE_DISTRICTS[6] = {
    button = "TheaterSortButton",
    dType = "DISTRICT_THEATER"
}
CUI_AVAILABLE_DISTRICTS[7] = {
    button = "SpacePortSortButton",
    dType = "DISTRICT_SPACEPORT"
}
...
function Refresh()
    ...
    if m_city == nil and m_currentChooserMode ==
        EspionageChooserModes.MISSION_CHOOSER then
        UI.DataError(
...
function RefreshTop()
    if m_currentChooserMode == EspionageChooserModes.DESTINATION_CHOOSER then
        -- DESTINATION_CHOOSER
        Controls.Title:SetText(Locale.ToUpper(
                                   "LOC_ESPIONAGECHOOSER_PANEL_HEADER"))
        ...
        -- MISSION_CHOOSER
        Controls.Title:SetText(Locale.ToUpper(
                                   "LOC_ESPIONAGECHOOSER_CHOOSE_MISSION"))
        ...
        if playerDiplomacy then
            local boostedTurnsRemaining =
                playerDiplomacy:GetSourceTurnsRemaining(m_city)
            if boostedTurnsRemaining > 0 then
                TruncateStringWithTooltip(Controls.ActiveBoostLabel,
                                          MAX_BEFORE_TRUNC_CHOOSE_NEXT,
                                          Locale.Lookup(
                                              "LOC_ESPIONAGECHOOSER_GAIN_SOURCES_ACTIVE",
...
function RefreshBottom()
    if m_currentChooserMode == EspionageChooserModes.DESTINATION_CHOOSER then
        ...
        if m_city then
            ...
            Controls.MissionBackground:SetParentRelativeSizeY(
                DESTINATION_CHOOSER_MISSIONSCROLLPANEL_RELATIVE_SIZE_Y) -- CUI
            RefreshMissionList()
        ...
        Controls.MissionBackground:SetParentRelativeSizeY(
            MISSION_CHOOSER_MISSIONSCROLLPANEL_RELATIVE_SIZE_Y) -- CUI
        RefreshMissionList()
    end
end
        ...
        for operation in GameInfo.UnitOperations() do
            if operation.OperationType == "UNITOPERATION_SPY_COUNTERSPY" then
                ...
                local canStart, results =
                    UnitManager.CanStartOperation(m_spy, operation.Hash,
                                                  cityPlot, false, true)
                if canStart then
                    -- Check the CanStartOperation results for a target district plot
                    for i, districtPlotID in
                        ipairs(results[UnitOperationResults.PLOTS]) do
            ...
            if operation.CategoryInUI == "OFFENSIVESPY" then
                ...
                if canStart then
                    ...
                    if results and results[UnitOperationResults.FAILURE_REASONS] then
                        AddDisabledOffensiveOperation(operation, results,
                                                      cityPlot)
                    end
                end
            end
...
function AddCounterspyOperation(operation, districtPlotID)
    ...
    for i, district in cityDistricts:Members() do
        ...
        if districtPlot:GetIndex() == districtPlotID then
            ...
            -- Update mission info
            missionInstance.MissionName:SetText(
                Locale.Lookup(operation.Description))
            missionInstance.MissionDetails:SetText(
                Locale.Lookup("LOC_ESPIONAGECHOOSER_COUNTERSPY",
    ...
    missionInstance.SelectorBrace:SetColor(
        UI.GetColorValueFromHexLiteral(0x00FFFFFF))
end
...
function AddOffensiveOperation(operation, result, targetCityPlot)
    ...
    RefreshMissionStats(missionInstance, operation, result, m_spy, m_city,
                        targetCityPlot)
...
function AddDisabledOffensiveOperation(operation, result, targetCityPlot)
    local missionInstance = AddOffensiveOperation(operation, result,
                                                  targetCityPlot)
...
function OnCounterspySelected(districtPlot)
    ...
    UnitManager.RequestOperation(m_spy, UnitOperationTypes.SPY_COUNTERSPY,
                                 tParameters)
end
...
function OnMissionSelected(operation, instance)
    ...
    for i = 1, m_MissionStackIM.m_iCount, 1 do
        ...
        if otherInstances then
            otherInstances.SelectorBrace:SetColor(
                UI.GetColorValue("COLOR_CLEAR"))
        end
    end
...
function UpdateCityBanner(city)
    ...
    TruncateStringWithTooltip(Controls.CityName, 195,
                              Locale.ToUpper(city:GetName()))
    Controls.BannerBase:SetHide(false)
    ...
    if m_currentChooserMode == EspionageChooserModes.DESTINATION_CHOOSER then
        ...
        Controls.BannerBase:SetToolTipString(
            Locale.Lookup("LOC_ESPIONAGECHOOSER_TRAVEL_TIME_TOOLTIP",
                          travelTime, establishTime))
    else
...
function AddPlayerCities(player)
    ...
    for j, city in playerCities:Members() do
        ...
        if localPlayerVis:IsRevealed(city:GetX(), city:GetY()) then
            if CuiHasDistrict(city) then AddDestination(city) end
        end
    end
end
...
function AddDestination(city)
    ...
    if city:IsCapital() and Players[city:GetOwner()]:IsMajor() then
        TruncateStringWithTooltip(destinationInstance.CityName, 185,
                                  "[ICON_Capital] " ..
                                      Locale.ToUpper(city:GetName()))
    else
        TruncateStringWithTooltip(destinationInstance.CityName, 185,
    ...
    destinationInstance.BannerBase:SetToolTipString(
        Locale.Lookup("LOC_ESPIONAGECHOOSER_TRAVEL_TIME_TOOLTIP", travelTime,
                      establishTime))
    ...
    -- Set button callback
    destinationInstance.DestinationButton:RegisterCallback(Mouse.eLClick,
                                                           function()
...
function AddDistrictIcons(kParentControl, pCity)
    ...
    if kParentControl[DISTRICT_IM] == nil then
        kParentControl[DISTRICT_IM] = InstanceManager:new(
                                          "CityDistrictInstance",
                                          "DistrictIcon",
    ...
    for _, item in pairs(CUI_AVAILABLE_DISTRICTS) do
        ...
        if pDistrict then
            kDistrictInst = AddDistrictIcon(kParentControl[DISTRICT_IM], pCity,
                                            pDistrict)
        else
...
function RefreshDistrictIcon(city, districtType, districtIcon)
    ...
    for i, district in cityDistricts:Members() do
        if district:IsComplete() then
            ...
            if (replaces) then
                currentDistrictType =
                    GameInfo.Districts[replaces.ReplacesDistrictType]
                        .DistrictType
            end
...
function OnConfirmPlacement()
    ...
    if m_city:GetOwner() == Game.GetLocalPlayer() and spyCity:GetID() ==
        m_city:GetID() and m_city:GetOwner() == spyCity:GetOwner() then
        m_currentChooserMode = EspionageChooserModes.MISSION_CHOOSER
        Refresh()
    else
...
function OnInterfaceModeChanged(oldMode, newMode)
    if oldMode == InterfaceModeTypes.SPY_CHOOSE_MISSION and newMode ~=
        InterfaceModeTypes.SPY_TRAVEL_TO_CITY then
        if m_AnimSupport:IsVisible() then Close() end
    end
    if oldMode == InterfaceModeTypes.SPY_TRAVEL_TO_CITY and newMode ~=
        InterfaceModeTypes.SPY_CHOOSE_MISSION then
        if m_AnimSupport:IsVisible() then Close() end
    end
...
function OnShutdown()
    ...
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "currentChooserMode",
                                 m_currentChooserMode)
end
...
function CuiCivCheck(player)
    ...
    if player:IsMajor() then
        if (player:GetID() == localPlayer:GetID() or player:GetTeam() == -1 or
            localPlayer:GetTeam() == -1 or player:GetTeam() ~=
...
function CuiRefreshFilters()
    ...
    for i, player in ipairs(players) do
        ...
        if pPlayerInfluence:CanReceiveInfluence() then
            ...
            CuiAddFilter(Locale.Lookup("LOC_ROUTECHOOSER_FILTER_CITYSTATES"),
                         CuiFilterByCityStates)
            break
        end
    end
