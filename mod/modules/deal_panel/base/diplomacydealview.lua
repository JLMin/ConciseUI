        ...
        if
         ...
         then
            ...
            if (DealManager.AreWorkingDealsEqual(ms_LocalPlayer:GetID(), ms_OtherPlayer:GetID())) then
                ...
                local pDeal =
                    DealManager.GetWorkingDeal(DealDirection.OUTGOING, ms_LocalPlayer:GetID(), ms_OtherPlayer:GetID())
    ...
    if (not ms_OtherPlayerIsHuman) then
        ...
        if
            (pDeal ~= nil and not ms_bIsDemand and pDeal:IsValid() and
                not DealManager.HasPendingDeal(ms_LocalPlayer:GetID(), ms_OtherPlayer:GetID()))
         then
            local iItemsFromLocal = pDeal:GetItemCount(ms_LocalPlayer:GetID(), ms_OtherPlayer:GetID())
            local iItemsFromOther = pDeal:GetItemCount(ms_OtherPlayer:GetID(), ms_LocalPlayer:GetID())
...
function PopulateAvailableCities(player, iconList)
    ...
    local pForDeal = DealManager.GetWorkingDeal(DealDirection.OUTGOING, ms_LocalPlayer:GetID(), ms_OtherPlayer:GetID())
    local possibleItems =
        DealManager.GetPossibleDealItems(player:GetID(), GetOtherPlayer(player):GetID(), DealItemTypes.CITIES, pForDeal)
                ...
                if (type == DealItemTypes.AGREEMENTS) then
                    ...
                    if
                        (info.DiplomaticActionType == "DIPLOACTION_JOINT_WAR" and
                            pDealItem:GetFromPlayerID() == ms_OtherPlayer:GetID())
                     then
...
function OnShow()
    ...
    iAvailableItemCount =
        iAvailableItemCount + PopulatePlayerAvailablePanel(Controls.TheirInventoryStack, ms_OtherPlayer)
