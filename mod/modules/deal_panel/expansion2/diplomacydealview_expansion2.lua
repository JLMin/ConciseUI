...
function OnClickAvailableResource(player, resourceType)
    ...
    local pResourceDef =
        GameInfo.Resource_Consumption[pBaseResourceDef.ResourceType]

    local pDeal = DealManager.GetWorkingDeal(DealDirection.OUTGOING,
                                             Game.GetLocalPlayer(),
                                             GetOtherPlayer():GetID())
    if (pDeal ~= nil) then
            ...
            for i, pDealItem in ipairs(dealItems) do
                if pDealItem:GetValueType() == resourceType then
                    ...
                    if (pResourceDef ~= nil and pResourceDef.Accumulate) then
                        ...
                        local newValue =
                            clip(oldValue + cuiSPC, nil,
                                 pDealItem:GetMaxAmount())
...
function CreateGroupTypes()
    ...
    AvailableDealItemGroupTypes.FAVOR =
        table.count(AvailableDealItemGroupTypes) + 1
...
function PopulatePlayerAvailablePanel(rootControl, player)
    ...
    iAvailableItemCount = iAvailableItemCount +
                              BASE_PopulatePlayerAvailablePanel(rootControl,
                                                                player)
...
function OnClickAvailableOneTimeFavor(player, iAddAmount)
    ...
    if (pDeal ~= nil) then
        ...
        -- Already there?
        local dealItems = pDeal:FindItemsByType(DealItemTypes.FAVOR,
                                                DealItemSubTypes.NONE,
                                                player:GetID())
        local pDealItem
        if (dealItems ~= nil) then
