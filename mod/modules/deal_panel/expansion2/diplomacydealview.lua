...
g_OtherPlayer = nil
g_AvailableGroups = {}
g_IconOnlyIM = InstanceManager:new("IconOnly", "SelectButton",
                                   Controls.IconOnlyContainer)
g_IconAndTextIM = InstanceManager:new("IconAndText", "SelectButton",
...
function ProposeWorkingDeal(bIsAutoPropose)
    ...
    if (not DealManager.HasPendingDeal(g_LocalPlayer:GetID(),
...
function RequestEqualizeWorkingDeal()
    if (not DealManager.HasPendingDeal(g_LocalPlayer:GetID(),
        ...
        DealManager.SendWorkingDeal(DealProposalAction.EQUALIZE,
                                    g_LocalPlayer:GetID(), g_OtherPlayer:GetID())
    end
end
    ...
    if (pDeal ~= nil) then
        ...
        if (pDealItem ~= nil) then
            ...
            if (g_ValueEditDealItemControlTable ~= nil) then
                ...
                g_ValueEditDealItemControlTable.ValueText:LocalizeAndSetText(
...
function UpdateDealStatus()
    ...
    local bHasPendingDeal = DealManager.HasPendingDeal(g_LocalPlayer:GetID(),
                                                       g_OtherPlayer:GetID())
...
function OnProposeOrAcceptDeal()
    ...
    if (ms_LastIncomingDealProposalAction == DealProposalAction.PENDING or
        ...
        if (ms_LastIncomingDealProposalAction == DealProposalAction.ACCEPTED or
            ...
            -- Any adjustments?
            if (DealManager.AreWorkingDealsEqual(g_LocalPlayer:GetID(),
                                                 g_OtherPlayer:GetID())) then
                -- Yes, we can accept
                ...
                if DealAgreementTypes.JOINT_WAR and pJointWarItem then
                    ...
                    if (targetPlayerID >= 0) then
                        LuaEvents.DiplomacyActionView_ConfirmWarDialog(
                            g_LocalPlayer:GetID(), targetPlayerID, iWarType,
                            sendDealAndContinue)
                    else
                        UI.DataError(
...
function DetachValueEdit(itemID)
    ...
    if (itemID == g_ValueEditDealItemID) then ClearValueEdit() end
    ...
    if (pDeal ~= nil) then
        ...
        if (pDealItem ~= nil) then
            ...
            if (pDealItem:HasPossibleValues() or pDealItem:HasPossibleAmounts()) then
                -- Yes
                g_ValueEditDealItemControlTable =
...
function OnClickAvailableOneTimeGold(player, iAddAmount)
    ...
    if ((ms_bIsDemand == true or ms_bIsGift == true) and ms_InitiatedByPlayerID ==
        ms_OtherPlayerID) then
...
function IsAutoPropose()
    if (not ms_OtherPlayerIsHuman) then
        ...
        if (pDeal ~= nil and not ms_bIsDemand and pDeal:IsValid() and
            not DealManager.HasPendingDeal(g_LocalPlayer:GetID(),
                                           g_OtherPlayer:GetID())) then
            local iItemsFromLocal = pDeal:GetItemCount(g_LocalPlayer:GetID(),
                                                       g_OtherPlayer:GetID())
            local iItemsFromOther = pDeal:GetItemCount(g_OtherPlayer:GetID(),
                                                       g_LocalPlayer:GetID())
...
function UpdateProposalButtons(bDealValid)
    ...
    local bDealIsPending = DealManager.HasPendingDeal(g_LocalPlayer:GetID(),
                                                      g_OtherPlayer:GetID())
    ...
    if (bDealValid and (not bDealIsPending or not ms_OtherPlayerIsHuman)) then
            ...
            if (not ms_bIsDemand) then
                if (not ms_OtherPlayerIsHuman) then
                    ...
                    elseif ms_bIsGift then -- Incoming gift
                        ...
                        if (UpdateProposalButtonsForGift(iItemsFromLocal,
                                                         iItemsFromOther)) then
                    ...
                    if (ms_LastIncomingDealProposalAction ==
                        ...
                        if (DealManager.AreWorkingDealsEqual(
                            g_LocalPlayer:GetID(), g_OtherPlayer:GetID())) then
                            Controls.AcceptDeal:LocalizeAndSetText(
                                "LOC_DIPLOMACY_DEAL_ACCEPT_DEAL")
                        else
                            Controls.AcceptDeal:LocalizeAndSetText(
        ...
        local pDeal = DealManager.GetWorkingDeal(DealDirection.OUTGOING,
                                                 ...
                                                 g_OtherPlayer:GetID())
        if (pDeal ~= nil) then
            iItemsFromLocal = pDeal:GetItemCount(g_LocalPlayer:GetID(),
                                                 g_OtherPlayer:GetID())
            iItemsFromOther = pDeal:GetItemCount(g_OtherPlayer:GetID(),
                                                 g_LocalPlayer:GetID())
        end
    ...
    if (bDealIsPending and ms_OtherPlayerIsHuman) then
        ...
        elseif ms_bIsGift then -- Incoming gift
            Controls.RefuseDeal:LocalizeAndSetText(
                "LOC_DIPLOMACY_DEAL_REFUSE_GIFT")
        else
            Controls.RefuseDeal:LocalizeAndSetText(
        ...
        if (ms_InitiatedByPlayerID == ms_OtherPlayerID) then
            if (not bDealValid) then
                ...
                elseif ms_bIsGift then -- Incoming gift
                    Controls.AcceptDeal:LocalizeAndSetText(
                        "LOC_DIPLOMACY_DEAL_ACCEPT_GIFT")
                    Controls.RefuseDeal:LocalizeAndSetText(
                        "LOC_DIPLOMACY_DEAL_REFUSE_GIFT")
                else
                    Controls.RefuseDeal:LocalizeAndSetText(
    ...
    if (possibleValues ~= nil) then
        for i, entry in ipairs(possibleValues) do
            ...
            if (entry.SubType == DealAgreementTypes.RESEARCH_AGREEMENT) then
                ...
                local iTurns = g_LocalPlayer:GetDiplomacy()
                                   :ComputeResearchAgreementTurns(g_OtherPlayer,
                                                                  eTech)
            ...
            if agreementType == DealAgreementTypes.ALLIANCE then
                local allianceLevel = g_LocalPlayer:GetDiplomacy()
                                          :GetAllianceLevel(g_OtherPlayer)
        ...
        for i, entry in ipairs(possibleResources) do
            ...
            if (resourceDesc ~= nil) then
                ...
                if (entry.MaxAmount > 0 and resourceDesc.ResourceClassType ==
                    ...
                    if (resource ~= nil and resource.ResourceClassType ==
                        "RESOURCECLASS_LUXURY") then
                        -- their inventory
                    ...
                    elseif (resource ~= nil and resource.ResourceClassType ==
                        ...
                        if entry.IsValid then
                            ...
                            if player ~= g_LocalPlayer then
                                addTooltip =
                                    addTooltip ..
                                        Locale.Lookup(
                                            "LOC_DEAL_PLAYER_HAS_NO_CAP_ROOM")
                                icon.SelectButton:SetToolTipString(addTooltip)
                            else
                                addTooltip =
                                    addTooltip ..
                                        Locale.Lookup(
                                            "LOC_DEAL_AI_HAS_NO_CAP_ROOM")
                                icon.SelectButton:SetToolTipString(addTooltip)
                            end
            ...
            if (pDealItem:GetFromPlayerID() == player:GetID()) then
                ...
                if (type == DealItemTypes.AGREEMENTS) then
                    ...
                    if (info.DiplomaticActionType == "DIPLOACTION_JOINT_WAR" and
                        pDealItem:GetFromPlayerID() == g_OtherPlayer:GetID()) then
                        icon.SelectButton:SetDisabled(true)
                        icon.SelectButton:SetToolTipString(
                            Locale.Lookup(
...
function OnDiplomacyIncomingDeal(eFromPlayer, eToPlayer, eAction)
    if (eFromPlayer == ms_OtherPlayerID) then
        ...
        if (pDeal ~= nil) then
            ...
            DealManager.CopyIncomingToOutgoingWorkingDeal(g_LocalPlayer:GetID(),
                                                          g_OtherPlayer:GetID())
...
function OnShow()
    ...
    iAvailableItemCount = iAvailableItemCount +
                              PopulatePlayerAvailablePanel(
                                  Controls.MyInventoryStack, g_LocalPlayer)
    iAvailableItemCount = iAvailableItemCount +
                              PopulatePlayerAvailablePanel(
                                  Controls.TheirInventoryStack, g_OtherPlayer)
...
function CuiEditGroupSetup(player, editGroup, groupType)
    ...
    elseif groupType == "FAVOR" then
        ...
        end, function() OnClickAvailableOneTimeFavor(player, -100) end)
        CuiRegCallback(editGroup.Edit10,
                       function()
            OnClickAvailableOneTimeFavor(player, 10)
        end, function() OnClickAvailableOneTimeFavor(player, -10) end)
        CuiRegCallback(editGroup.Edit1,
