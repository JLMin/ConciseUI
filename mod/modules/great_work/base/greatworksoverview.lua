...
function UpdateGreatWorks()
    ...
    if table.count(m_GreatWorkYields) > 0 then
        ...
        Controls.ProvidingLabel:SetOffsetX(
            Controls.TotalResources:GetOffsetX() + Controls.TotalResources:GetSizeX() + PADDING_PROVIDING_LABEL
        )
...
function PopulateGreatWorkSlot(instance, pCity, pCityBldgs, pBuildingInfo)
    ...
    if numGreatWorks == 0 then
        if themeDescription ~= nil then
            instance.ThemingLabel:SetText(
                Locale.Lookup("LOC_GREAT_WORKS_THEME_BONUS_PROGRESS", numThemedGreatWorks, numSlots)
            )
    ...
    if (textureSheet == nil or textureSheet == "") then
        UI.DataError(
            'Could not find slot type icon in GetGreatWorkIcon: icon="' ..
                greatWorkIcon .. '", iconSize=' .. tostring(SIZE_GREAT_WORK_ICON)
        )
    end
...
function PopulateGreatWork(instance, pCityBldgs, pBuildingInfo, slotIndex, greatWorkIndex, slotType)
    ...
    if greatWorkIndex == -1 then
        ...
        instance.EmptySlot:SetToolTipString(
            GetGreatWorkTooltip(pCityBldgs, greatWorkIndex, greatWorkType, pBuildingInfo)
        )
        ...
        if bAllowMove and srcGreatWorkObjectType == GREAT_WORK_ARTIFACT_TYPE then
            if not IsBuildingFull(pCityBldgs, buildingIndex) then
                instance.GreatWorkDraggable:RegisterCallback(
                    Drag.eDragDisabled,
                    function()
        ...
        if
            bAllowMove and srcGreatWorkObjectType == "GREATWORKOBJECT_SCULPTURE" or
...
function GetGreatWorkTooltip(pCityBldgs, greatWorkIndex, greatWorkType, pBuildingInfo)
    ...
    if bIsThemeable then
        ...
        if pCityBldgs:IsBuildingThemedCorrectly(GameInfo.Buildings[pBuildingInfo.BuildingType].Index) then
            ...
            elseif pBuildingInfo.BuildingType == "BUILDING_MUSEUM_ARTIFACT" then
                local artifactEraName =
                    Locale.Lookup("LOC_" .. greatWorkInfo.GreatWorkObjectType .. "_" .. greatWorkInfo.EraType)
                if firstGreatWork == greatWorkIndex then
                    ...
                    local firstArtifactEraName =
                        Locale.Lookup(
                        "LOC_" ..
                            firstGreatWorkObjectType ..
                                "_" .. GameInfo.GreatWorks[firstGreatWorkObjectTypeID].EraType .. "_PLURAL"
                    )

                    if greatWorkInfo.EraType ~= GameInfo.GreatWorks[firstGreatWorkObjectTypeID].EraType then
                        strThemeTooltip =
                            Locale.Lookup("LOC_GREAT_WORKS_MISMATCHED_ERA", artifactEraName, firstArtifactEraName)
                    else
                        local greatWorkPlayer = Game.GetGreatWorkPlayer(greatWorkIndex)
                        local greatWorks = GetGreatWorksInBuilding(pCityBldgs, pBuildingInfo)
...
function MoveGreatWork(kSrcInstance, kDestInstance)
    if kSrcInstance ~= nil and kDestInstance ~= nil then
        ...
        if
            kSrcInstance[DATA_FIELD_CITY_ID] == kDestInstance[DATA_FIELD_CITY_ID] and
                kSrcInstance[DATA_FIELD_BUILDING_ID] == kDestInstance[DATA_FIELD_BUILDING_ID] and
                kSrcInstance[DATA_FIELD_SLOT_INDEX] == kDestInstance[DATA_FIELD_SLOT_INDEX]
         then
...
function Initialize()
    ...
    Controls.ModalScreenClose:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    ...
    Controls.ViewGallery:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    ...
    Controls.ViewGreatWork:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
