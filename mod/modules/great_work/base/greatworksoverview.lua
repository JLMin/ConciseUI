...
local m_GreatWorkSlotsIM = InstanceManager:new("GreatWorkSlot", "TopControl",
                                               Controls.GreatWorksStack)
local m_TotalResourcesIM = InstanceManager:new("AgregateResource", "Resource",
                                               Controls.TotalResources)
...
function UpdateGreatWorks()
        ...
        if pCity ~= nil and pCity:GetOwner() == m_LocalPlayerID then
            ...
            for buildingInfo in GameInfo.Buildings() do
                ...
                if (pCityBldgs:HasBuilding(buildingIndex)) then
                    local numSlots = pCityBldgs:GetNumGreatWorkSlots(
                                         buildingIndex)
    ...
    for _, item in ipairs(cui_tmpBuildings) do
        local instance = m_GreatWorkSlotsIM:GetInstance()
        local greatWorks = PopulateGreatWorkSlot(instance, item.City,
                                                 item.CityBldgs, item.Info)
        table.insert(m_GreatWorkBuildings, {
            Instance = instance,
            Type = item.Type,
            Index = item.Index,
            CityBldgs = item.CityBldgs
        })
        numDisplaySpaces = numDisplaySpaces +
                               item.CityBldgs:GetNumGreatWorkSlots(item.Index)
        numGreatWorks = numGreatWorks + greatWorks
    end
    --
    ...
    if table.count(m_GreatWorkYields) > 0 then
        ...
        Controls.ProvidingLabel:SetOffsetX(
            Controls.TotalResources:GetOffsetX() +
                Controls.TotalResources:GetSizeX() + PADDING_PROVIDING_LABEL)
...
function PopulateGreatWorkSlot(instance, pCity, pCityBldgs, pBuildingInfo)
    ...
    instance.BuildingName:SetText(Locale.ToUpper(
                                      Locale.Lookup(pBuildingInfo.Name)))
    ...
    if (greatWorkIM == nil) then
        greatWorkIM = InstanceManager:new("GreatWork", "TopControl",
                                          instance.GreatWorks)
    ...
    if (numSlots ~= nil and numSlots > 0) then
        for _ = 0, numSlots - 1 do
            ...
            local greatWorkIndex = pCityBldgs:GetGreatWorkInSlot(buildingIndex,
                                                                 index)
            local greatWorkSlotType = pCityBldgs:GetGreatWorkSlotType(
                                          buildingIndex, index)
            local greatWorkSlotString =
                GameInfo.GreatWorkSlotTypes[greatWorkSlotType].GreatWorkSlotType

            PopulateGreatWork(instance, pCityBldgs, pBuildingInfo, index,
                              greatWorkIndex, greatWorkSlotString)
            index = index + 1
            instanceCache[index] = instance
            if greatWorkIndex ~= -1 then
                numGreatWorks = numGreatWorks + 1
                local greatWorkType = pCityBldgs:GetGreatWorkTypeFromIndex(
                                          greatWorkIndex)
                local greatWorkInfo = GameInfo.GreatWorks[greatWorkType]
                if firstGreatWork == nil then
                    firstGreatWork = greatWorkInfo
                end
                if greatWorkInfo ~= nil and
                    GreatWorkFitsTheme(pCityBldgs, pBuildingInfo,
                                       greatWorkIndex, greatWorkInfo) then
        ...
        if firstGreatWork ~= nil and themeDescription ~= nil then
            ...
            local textureOffsetX, textureOffsetY, textureSheet =
                IconManager:FindIconAtlas(slotTypeIcon, SIZE_SLOT_TYPE_ICON)
            if (textureSheet == nil or textureSheet == "") then
                UI.DataError(
                    "Could not find slot type icon in PopulateGreatWorkSlot: icon=\"" ..
                        slotTypeIcon .. "\", iconSize=" ..
                        tostring(SIZE_SLOT_TYPE_ICON))
            else
                for i = 0, numSlots - 1 do
                    local slotIndex = index - i
                    instanceCache[slotIndex].SlotTypeIcon:SetTexture(
    ...
    if (numSlots > 1) then
        ...
        local finalPadding = ((MAX_NUM_SLOTS - numSlots) * paddingRange /
                                 slotRange) + MIN_PADDING_SLOTS
    ...
    if (themeBonusIM == nil) then
        themeBonusIM = InstanceManager:new("Resource", "Resource",
    ...
    if numGreatWorks == 0 then
        if themeDescription ~= nil then
            instance.ThemingLabel:SetText(
                Locale.Lookup("LOC_GREAT_WORKS_THEME_BONUS_PROGRESS",
                              numThemedGreatWorks, numSlots))
    ...
    for row in GameInfo.Yields() do
        local yieldValue = pCityBldgs:GetBuildingYieldFromGreatWorks(row.Index,
                                                                     buildingIndex)
        if yieldValue > 0 then
            AddYield(themeBonusIM:GetInstance(), Locale.Lookup(row.Name),
                     YIELD_FONT_ICONS[row.YieldType], yieldValue)
        end
    end

    local regularTourism = pCityBldgs:GetBuildingTourismFromGreatWorks(false,
                                                                       buildingIndex)
    local religionTourism = pCityBldgs:GetBuildingTourismFromGreatWorks(true,
                                                                        buildingIndex)
    local totalTourism = regularTourism + religionTourism

    if totalTourism > 0 then
        AddYield(themeBonusIM:GetInstance(), LOC_TOURISM,
                 YIELD_FONT_ICONS[DATA_FIELD_TOURISM_YIELD], totalTourism)
    end
...
function GreatWorkFitsTheme(pCityBldgs, pBuildingInfo, greatWorkIndex,
                            greatWorkInfo)
    local firstGreatWork =
        GetFirstGreatWorkInBuilding(pCityBldgs, pBuildingInfo)
    if firstGreatWork < 0 then return false end

    local firstGreatWorkObjectTypeID = pCityBldgs:GetGreatWorkTypeFromIndex(
                                           firstGreatWork)
    local firstGreatWorkObjectType =
        GameInfo.GreatWorks[firstGreatWorkObjectTypeID].GreatWorkObjectType

    if pCityBldgs:IsBuildingThemedCorrectly(
        GameInfo.Buildings[pBuildingInfo.BuildingType].Index) then
        return true
    else
        if pBuildingInfo.BuildingType == "BUILDING_MUSEUM_ART" then
            ...
            if firstGreatWork == greatWorkIndex then
                return true
            elseif not IsFirstGreatWorkByArtist(greatWorkIndex, pCityBldgs,
                                                pBuildingInfo) then
                return false
            else
                return firstGreatWorkObjectType ==
                           greatWorkInfo.GreatWorkObjectType
            end
        elseif pBuildingInfo.BuildingType == "BUILDING_MUSEUM_ARTIFACT" then
            ...
            if firstGreatWork == greatWorkIndex then
                ...
                if greatWorkInfo.EraType ~=
                    GameInfo.GreatWorks[firstGreatWorkObjectTypeID].EraType then
                    return false
                else
                    local greatWorkPlayer =
                        Game.GetGreatWorkPlayer(greatWorkIndex)
                    local greatWorks = GetGreatWorksInBuilding(pCityBldgs,
                                                               pBuildingInfo)
...
function GetGreatWorkIcon(greatWorkInfo)
    ...
    local textureOffsetX, textureOffsetY, textureSheet =
        IconManager:FindIconAtlas(greatWorkIcon, SIZE_GREAT_WORK_ICON)
    if (textureSheet == nil or textureSheet == "") then
        UI.DataError(
            "Could not find slot type icon in GetGreatWorkIcon: icon=\"" ..
                greatWorkIcon .. "\", iconSize=" ..
...
function PopulateGreatWork(instance, pCityBldgs, pBuildingInfo, slotIndex,
                           greatWorkIndex, slotType)
    ...
    if (textureSheet == nil or textureSheet == "") then
        UI.DataError(
            "Could not find slot type icon in PopulateGreatWork: icon=\"" ..
                slotTypeIcon .. "\", iconSize=" .. tostring(SIZE_SLOT_TYPE_ICON))
    else
        instance.SlotTypeIcon:SetTexture(textureOffsetX, textureOffsetY,
                                         textureSheet)
    end
    ...
    if greatWorkIndex == -1 then
        ...
        for row in GameInfo.GreatWork_ValidSubTypes() do
            if slotType == row.GreatWorkSlotType then
                ...
                validWorks = validWorks ..
                                 Locale.Lookup("LOC_" .. row.GreatWorkObjectType)
            end
        end

        instance.EmptySlot:ClearCallback(Mouse.eLClick)
        instance.EmptySlot:SetToolTipString(
            Locale.Lookup("LOC_GREAT_WORKS_EMPTY_TOOLTIP", validWorks))
    else
        ...
        local srcGreatWorkType = pCityBldgs:GetGreatWorkTypeFromIndex(
                                     srcGreatWork)
        local srcGreatWorkObjectType = GameInfo.GreatWorks[srcGreatWorkType]
                                           .GreatWorkObjectType
        ...
        local greatWorkType = pCityBldgs:GetGreatWorkTypeFromIndex(
                                  greatWorkIndex)
        local textureOffsetX, textureOffsetY, textureSheet =
            GetGreatWorkIcon(GameInfo.GreatWorks[greatWorkType])
        instance.GreatWorkIcon:SetTexture(textureOffsetX, textureOffsetY,
                                          textureSheet)

        instance.EmptySlot:SetToolTipString(
            GetGreatWorkTooltip(pCityBldgs, greatWorkIndex, greatWorkType,
                                pBuildingInfo))
        ...
        if bAllowMove and srcGreatWorkObjectType == GREAT_WORK_ARTIFACT_TYPE then
            if not IsBuildingFull(pCityBldgs, buildingIndex) then
                instance.GreatWorkDraggable:RegisterCallback(Drag.eDragDisabled,
                                                             function()
                    ShowCannotMoveMessage(
                        Locale.Lookup(
                            "LOC_GREAT_WORKS_ARTIFACT_LOCKED_FROM_MOVE"))
                end)
                bAllowMove = false
            end
        end
        ...
        if bAllowMove and srcGreatWorkObjectType == "GREATWORKOBJECT_SCULPTURE" or
            ...
            local iTurnsBeforeMove = GlobalParameters.GREATWORK_ART_LOCK_TIME or
                                         DEFAULT_LOCK_TURNS
            ...
            if iTurnsToWait > 0 then
                instance.GreatWorkDraggable:RegisterCallback(Drag.eDragDisabled,
                                                             function()
                    ShowCannotMoveMessage(
                        Locale.Lookup("LOC_GREAT_WORKS_LOCKED_FROM_MOVE",
                                      iTurnsToWait))
                end)
                bAllowMove = false
            end
        ...
        if bAllowMove then
            ...
            instance.GreatWorkDraggable:RegisterCallback(Drag.eDown,
                                                         function(kDragStruct)
                OnClickGreatWork(kDragStruct, pCityBldgs, buildingIndex,
                                 greatWorkIndex, slotIndex)
            end)
            instance.GreatWorkDraggable:RegisterCallback(Drag.eDrop, function(
                kDragStruct) OnGreatWorkDrop(kDragStruct, instance) end)
            instance.GreatWorkDraggable:RegisterCallback(Drag.eDrag, function(
                kDragStruct) OnGreatWorkDrag(kDragStruct, instance) end)
        else
...
function OnGreatWorkDrop(kDragStruct, kInstance)
    if m_uiSelectedDropTarget ~= nil then
        if m_uiSelectedDropTarget == Controls.ViewGreatWork then
            ...
            local kSelectedDropInstance =
                m_kControlToInstanceMap[m_uiSelectedDropTarget]
...
function OnGreatWorkDrag(kDragStruct, kInstance)
    ...
    local uiBestDropTarget = uiDragControl:GetBestOverlappingControl(
...
function HighlightDropTarget(uiBestDropTarget)
    for _, uiDropTarget in ipairs(m_kViableDropTargets) do
        if uiDropTarget == Controls.ViewGreatWork then
            Controls.ViewGreatWork:SetSelected(
                uiBestDropTarget == Controls.ViewGreatWork)
        else
            local pDropInstance = m_kControlToInstanceMap[uiDropTarget]
            if pDropInstance ~= nil then
                pDropInstance.EmptySlotHighlight:SetHide(
                    uiDropTarget ~= uiBestDropTarget)
            end
        end
    end
end
...
function GetGreatWorkTooltip(pCityBldgs, greatWorkIndex, greatWorkType,
                             pBuildingInfo)
    local greatWorkTypeName
    local greatWorkInfo = GameInfo.GreatWorks[greatWorkType]
    local greatWorkCreator = Locale.Lookup(
                                 pCityBldgs:GetCreatorNameFromIndex(
                                     greatWorkIndex))

    local bIsThemeable = GetThemeDescription(pBuildingInfo.BuildingType) ~= nil
    local strBasicTooltip = GreatWorksSupport_GetBasicTooltip(greatWorkIndex,
                                                              bIsThemeable)

    local buildingName = Locale.Lookup(
                             GameInfo.Buildings[pBuildingInfo.BuildingType].Name)

    if greatWorkInfo.EraType ~= nil then
        greatWorkTypeName = Locale.Lookup(
                                "LOC_" .. greatWorkInfo.GreatWorkObjectType ..
                                    "_" .. greatWorkInfo.EraType)
    else
        greatWorkTypeName = Locale.Lookup(
                                "LOC_" .. greatWorkInfo.GreatWorkObjectType)
    end
    ...
    if bIsThemeable then
        ...
        local firstGreatWork = GetFirstGreatWorkInBuilding(pCityBldgs,
                                                           pBuildingInfo)
        if firstGreatWork < 0 then return strBasicTooltip end

        local firstGreatWorkObjectTypeID =
            pCityBldgs:GetGreatWorkTypeFromIndex(firstGreatWork)
        local firstGreatWorkObjectType =
            GameInfo.GreatWorks[firstGreatWorkObjectTypeID].GreatWorkObjectType

        if pCityBldgs:IsBuildingThemedCorrectly(
            GameInfo.Buildings[pBuildingInfo.BuildingType].Index) then
            strThemeTooltip = Locale.Lookup("LOC_GREAT_WORKS_ART_MATCHED_THEME",
                                            buildingName)
        else
            if pBuildingInfo.BuildingType == "BUILDING_MUSEUM_ART" then

                if firstGreatWork == greatWorkIndex then
                    strThemeTooltip = Locale.Lookup(
                                          "LOC_GREAT_WORKS_ART_THEME_SINGLE",
                                          Locale.Lookup(
                                              "LOC_" .. firstGreatWorkObjectType))
                elseif not IsFirstGreatWorkByArtist(greatWorkIndex, pCityBldgs,
                                                    pBuildingInfo) then
                    ...
                    local nTurnCreated =
                        Game.GetGreatWorkDataFromIndex(greatWorkIndex)
                            .TurnCreated
                    local greatWorkName = Locale.Lookup(greatWorkInfo.Name)
                    local greatWorkCreationDate =
                        Calendar.MakeDateStr(nTurnCreated,
                                             GameConfiguration.GetCalendarType(),
                                             GameConfiguration.GetGameSpeedType(),
                                             false)
                    strBasicTooltip = Locale.Lookup(
                                          "LOC_GREAT_WORKS_TOOLTIP_DUPLICATE",
                                          greatWorkName, greatWorkTypeName,
                                          greatWorkCreator,
                                          greatWorkCreationDate)
                    strThemeTooltip = Locale.Lookup(
                                          "LOC_GREAT_WORKS_ART_THEME_DUPLICATE_ARTIST")
                elseif firstGreatWorkObjectType ==
                    greatWorkInfo.GreatWorkObjectType then
                    strThemeTooltip = Locale.Lookup(
                                          "LOC_GREAT_WORKS_ART_THEME_DUAL",
                                          Locale.Lookup(
                                              "LOC_" .. firstGreatWorkObjectType))
                else
                    strThemeTooltip = Locale.Lookup(
                                          "LOC_GREAT_WORKS_MISMATCHED_THEME",
                                          greatWorkTypeName, Locale.Lookup(
                                              "LOC_" .. firstGreatWorkObjectType ..
                                                  "_PLURAL"))
                end
            elseif pBuildingInfo.BuildingType == "BUILDING_MUSEUM_ARTIFACT" then
                ...
                local artifactEraName = Locale.Lookup(
                                            "LOC_" ..
                                                greatWorkInfo.GreatWorkObjectType ..
                                                "_" .. greatWorkInfo.EraType)
                if firstGreatWork == greatWorkIndex then
                    ...
                    local firstArtifactEraName =
                        Locale.Lookup(
                            "LOC_" .. firstGreatWorkObjectType .. "_" ..
                                GameInfo.GreatWorks[firstGreatWorkObjectTypeID]
                                    .EraType .. "_PLURAL")

                    if greatWorkInfo.EraType ~=
                        GameInfo.GreatWorks[firstGreatWorkObjectTypeID].EraType then
                        strThemeTooltip =
                            Locale.Lookup("LOC_GREAT_WORKS_MISMATCHED_ERA",
                                          artifactEraName, firstArtifactEraName)
                    else
                        local greatWorkPlayer =
                            Game.GetGreatWorkPlayer(greatWorkIndex)
                        local greatWorks =
                            GetGreatWorksInBuilding(pCityBldgs, pBuildingInfo)
                        ...
                        if table.count(duplicates) > 0 then
                            strThemeTooltip =
                                Locale.Lookup(
                                    "LOC_GREAT_WORKS_DUPLICATE_ARTIFACT_CIVS",
...
function AddYield(instance, yieldName, yieldIcon, yieldValue)
    ...
    if bFoundYield == false then
        table.insert(m_GreatWorkYields,
                     {Name = yieldName, Icon = yieldIcon, Value = yieldValue})
    end
    instance.Resource:SetText(yieldIcon .. yieldValue)
    instance.Resource:SetToolTipString(yieldName)
end

function OnClickGreatWork(kDragStruct, pCityBldgs, buildingIndex,
                          greatWorkIndex, slotIndex)
    ...
    Controls.PlacingName:SetText(Locale.ToUpper(
                                     Locale.Lookup(
                                         GameInfo.GreatWorks[greatWorkType].Name)))
    local textureOffsetX, textureOffsetY, textureSheet =
        GetGreatWorkIcon(GameInfo.GreatWorks[greatWorkType])
    ...
    for _, destination in ipairs(m_GreatWorkBuildings) do
        ...
        for index = 0, numSlots - 1 do
            if CanMoveGreatWork(pCityBldgs, buildingIndex, slotIndex, dstBldgs,
                                dstBuilding, index) then
                ...
                if slotInstance then
                    ...
                    m_kControlToInstanceMap[slotInstance.TopControl] =
                        slotInstance
                end
            end
        end
...
function CanMoveWorkAtAll(srcBldgs, srcBuilding, srcSlot)
    ...
    if (srcGreatWorkObjectType == GREAT_WORK_ARTIFACT_TYPE) then
        if not IsBuildingFull(srcBldgs, srcBuilding) then return false end
    end
...
function IsBuildingFull(pBuildings, buildingIndex)
    ...
    for index = 0, numSlots - 1 do
        local greatWorkIndex = pBuildings:GetGreatWorkInSlot(buildingIndex,
                                                             index)
...
function CanMoveToSlot(destBldgs, destBuilding)
    ...
    -- Don't allow moving artifacts if the museum is not full
    local srcGreatWorkType =
        m_GreatWorkSelected.CityBldgs:GetGreatWorkTypeFromIndex(
            m_GreatWorkSelected.Index)
    local srcGreatWorkObjectType = GameInfo.GreatWorks[srcGreatWorkType]
                                       .GreatWorkObjectType
...
-- ===========================================================================
function CanMoveGreatWork(srcBldgs, srcBuilding, srcSlot, dstBldgs, dstBuilding,
                          dstSlot)
    ...
    local dstSlotTypeString = GameInfo.GreatWorkSlotTypes[dstSlotType]
                                  .GreatWorkSlotType

    for row in GameInfo.GreatWork_ValidSubTypes() do
        -- Ensure source great work can be placed into destination slot
        if dstSlotTypeString == row.GreatWorkSlotType and srcGreatWorkObjectType ==
            row.GreatWorkObjectType then
            if dstGreatWork == -1 then
                ...
                local srcSlotType = srcBldgs:GetGreatWorkSlotType(srcBuilding,
                                                                  srcSlot)
                local srcSlotTypeString =
                    GameInfo.GreatWorkSlotTypes[srcSlotType].GreatWorkSlotType

                local dstGreatWorkType =
                    dstBldgs:GetGreatWorkTypeFromIndex(dstGreatWork)
                local dstGreatWorkObjectType =
                    GameInfo.GreatWorks[dstGreatWorkType].GreatWorkObjectType

                for row in GameInfo.GreatWork_ValidSubTypes() do
                    if srcSlotTypeString == row.GreatWorkSlotType and
                        dstGreatWorkObjectType == row.GreatWorkObjectType then
...
function MoveGreatWork(kSrcInstance, kDestInstance)
    if kSrcInstance ~= nil and kDestInstance ~= nil then
        ...
        if kSrcInstance[DATA_FIELD_CITY_ID] == kDestInstance[DATA_FIELD_CITY_ID] and
            kSrcInstance[DATA_FIELD_BUILDING_ID] ==
            kDestInstance[DATA_FIELD_BUILDING_ID] and
            kSrcInstance[DATA_FIELD_SLOT_INDEX] ==
        ...
        if destGreatWorkType == -1 then
            ...
            local textureOffsetX, textureOffsetY, textureSheet =
                GetGreatWorkIcon(GameInfo.GreatWorks[destGreatWorkType])
            kSrcInstance.GreatWorkIcon:SetHide(false)
            kSrcInstance.GreatWorkIcon:SetTexture(textureOffsetX,
        ...
        if sourceGreatWorkType == -1 then
            ...
            local textureOffsetX, textureOffsetY, textureSheet =
                GetGreatWorkIcon(GameInfo.GreatWorks[sourceGreatWorkType])
            kDestInstance.GreatWorkIcon:SetHide(false)
            kDestInstance.GreatWorkIcon:SetTexture(textureOffsetX,
                                                   textureOffsetY, textureSheet)
        end
        ...
        tParameters[PlayerOperations.PARAM_CITY_SRC] =
            kSrcInstance[DATA_FIELD_CITY_ID]
        tParameters[PlayerOperations.PARAM_CITY_DEST] =
            kDestInstance[DATA_FIELD_CITY_ID]
        tParameters[PlayerOperations.PARAM_BUILDING_SRC] =
            kSrcInstance[DATA_FIELD_BUILDING_ID]
        tParameters[PlayerOperations.PARAM_BUILDING_DEST] =
            kDestInstance[DATA_FIELD_BUILDING_ID]
        tParameters[PlayerOperations.PARAM_GREAT_WORK_INDEX] =
            kSrcInstance[DATA_FIELD_GREAT_WORK_INDEX]
        tParameters[PlayerOperations.PARAM_SLOT] =
            kDestInstance[DATA_FIELD_SLOT_INDEX]
        UI.RequestPlayerOperation(Game.GetLocalPlayer(),
                                  PlayerOperations.MOVE_GREAT_WORK, tParameters)
...
function OnGreatWorkMoved(fromCityOwner, fromCityID, toCityOwner, toCityID,
                          buildingID, greatWorkType)
    if (not ContextPtr:IsHidden() and
        (fromCityOwner == Game.GetLocalPlayer() or toCityOwner ==
            Game.GetLocalPlayer())) then
        m_during_move = true
        UpdateData()
        m_during_move = false
    end
...
function OnGameDebugReturn(context, contextTable)
    if context == RELOAD_CACHE_ID and contextTable["isHidden"] ~= nil and
        not contextTable["isHidden"] then Open() end
end
...
function CuiSetLockMask(instance, srcBldgs, srcBuilding, srcSlot)
    ...
    if (srcGreatWorkObjectType == "GREATWORKOBJECT_ARTIFACT") then
        ...
        for index = 0, numSlots - 1 do
            local greatWorkIndex = srcBldgs:GetGreatWorkInSlot(srcBuilding,
                                                               index)
            if (greatWorkIndex == -1) then
...
function CuiOnSortButtonClick()
    local IsSortByCity = CuiSettings:ReverseAndGetBoolean(
                             CuiSettings.SORT_BY_CITY)
    if IsSortByCity then
        Controls.SortGreatWork:SetText(Locale.Lookup(
                                           "LOC_CUI_GW_SORT_BY_BUILDING"))
    else
...
function CuiInit()
    ...
    Controls.ThemeHelper:SetToolTipString(
        Locale.Lookup("LOC_CUI_GW_THEMING_HELPER_TOOLTIP"))
end
