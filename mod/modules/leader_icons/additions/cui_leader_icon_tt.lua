...
function GetPlayerData(playerID)
    ...
    data.government = (pGovernment ~= -1) and
                          Locale.Lookup(GameInfo.Governments[pGovernment].Name) or
    ...
    data.gold = math.floor(pPlayer:GetTreasury():GetGoldYield() -
                               pPlayer:GetTreasury():GetTotalMaintenance())
...
function GetRelationShip(tPlayerID, allianceData)
    ...
    if (eState == DiplomaticStates.WAR) then
        local bValidAction, tResults = Players[lPlayerID]:GetDiplomacy()
                                           :IsDiplomaticActionValid(
                                               "DIPLOACTION_PROPOSE_PEACE_DEAL",
                                               tPlayerID, true)
        data.atWar = true
        data.peaceDeal = bValidAction
        data.warTurns = Game.GetGameDiplomacy():GetMinPeaceDuration() +
                            lPlayer:GetDiplomacy():GetAtWarChangeTurn(tPlayerID) -
    ...
    local modSum = 0
    local modifiers = tPlayer:GetDiplomaticAI()
                          :GetDiplomaticModifiers(lPlayerID)
    if modifiers then
        for _, mod in ipairs(modifiers) do modSum = modSum + mod.Score end
    end
    ...
    if allianceData.isAlliance then
        ...
        if (GameInfo.DiplomaticStates[iState].StateType ==
            ...
            local iTheirDenounceTurn = Players[tPlayerID]:GetDiplomacy()
                                           :GetDenounceTurn(lPlayerID)
        ...
        elseif (GameInfo.DiplomaticStates[iState].StateType ==
            ...
            local iFriendshipTurn = lPlayerDiplomacy:GetDeclaredFriendshipTurn(
                                        tPlayerID)
            iRemainingTurns = iFriendshipTurn +
                                  Game.GetGameDiplomacy():GetDenounceTimeLimit() -
                                  Game.GetCurrentGameTurn()
        end
...
function GetResourceList(otherPlayerID)
    ...
    if (isOther) then
        ...
        -- local player
        local localForDeal = DealManager.GetWorkingDeal(DealDirection.OUTGOING,
                                                        localPlayerID,
                                                        otherPlayerID)
        local localPossibleResources = DealManager.GetPossibleDealItems(
                                           localPlayerID, otherPlayerID,
                                           DealItemTypes.RESOURCES, localForDeal)
        local localResources = Players[localPlayerID]:GetResources()

        -- other player
        local otherForDeal = DealManager.GetWorkingDeal(DealDirection.OUTGOING,
                                                        otherPlayerID,
                                                        localPlayerID)
        local otherPossibleResources = DealManager.GetPossibleDealItems(
                                           otherPlayerID, localPlayerID,
            ...
            for _, entry in ipairs(otherPossibleResources) do
                ...
                if (resource ~= nil and
                    (resource.ResourceClassType == "RESOURCECLASS_LUXURY" or
                        resource.ResourceClassType == "RESOURCECLASS_STRATEGIC")) then
                    local logic_AI = entry.MaxAmount > 1 and
                                         not localResources:HasResource(
                                             resource.Index) and
                                         not Players[otherPlayerID]:IsHuman()
                    local logic_Human = entry.MaxAmount > 0 and
                                            not localResources:HasResource(
                                                resource.Index) and
                                            Players[otherPlayerID]:IsHuman()
                    if logic_AI or logic_Human then
                        local icon = "ICON_" .. resource.ResourceType
                        local amount = logic_AI and (entry.MaxAmount - 1) or
                                           entry.MaxAmount
        ...
        if (localPossibleResources ~= nil) then
            ...
            for _, entry in ipairs(localPossibleResources) do
                ...
                if (resource ~= nil and
                    (resource.ResourceClassType == "RESOURCECLASS_LUXURY" or
                        resource.ResourceClassType == "RESOURCECLASS_STRATEGIC")) then
                    if (entry.MaxAmount > 0 and
                        not otherResources:HasResource(resource.Index)) then
                        local icon = "ICON_" .. resource.ResourceType
            ...
            for _, entry in ipairs(otherPossibleResources) do
                ...
                if resource ~= nil then
                    if resource.ResourceClassType == "RESOURCECLASS_LUXURY" then
                        local logic_AI =
                            entry.MaxAmount > 1 and
                                not localResources:HasResource(resource.Index) and
                                not Players[otherPlayerID]:IsHuman()
                        local logic_Human =
                            entry.MaxAmount > 0 and
                                not localResources:HasResource(resource.Index) and
                                Players[otherPlayerID]:IsHuman()
                        if logic_AI or logic_Human then
                            local icon = "ICON_" .. resource.ResourceType
                            local amount =
                                logic_AI and (entry.MaxAmount - 1) or
                                    entry.MaxAmount
                            table.insert(otherOffer,
                                         {Icon = icon, Amount = amount})
                        end
                    elseif resource.ResourceClassType ==
...
function GetGrievanceTooltip(otherPlayerID)
    ...
    local grievancePerTurn = kGameDiplomacy:GetGrievanceChangePerTurn(
                                 otherPlayerID, localPlayerID)
    local tooltip = ""
    ...
    if totalGrievances == 0 then
        ...
        local txt = "[COLOR_" .. color .. "]" .. math.abs(totalGrievances) ..
                        "[ENDCOLOR]"
        tooltip = Locale.Lookup("LOC_CUI_DB_GRIEVANCES", txt) .. " (" ..
                      grievancePerTurn .. ")"
    end
...
function UpdateLeaderIconTooltip(tControl, playerID)
    ...
    if pData.leaderTypeName ~= nil then
        ...
        if (textureSheet == nil or textureSheet == "") then
            UI.DataError(
                "Could not find icon in UpdateLeaderTooltip: icon=\"" .. "ICON_" ..
                    leaderTypeName .. "\", iconSize=55")
        else
            CuiLeaderIconTT.Icon:SetTexture(textureOffsetX, textureOffsetY,
                                            textureSheet)
        end
        -- update description
        local desc = ""
        if GameConfiguration.IsAnyMultiplayer() and Players[playerID]:IsHuman() then
            desc = Locale.Lookup("LOC_DIPLOMACY_DEAL_PLAYER_PANEL_TITLE",
                                 pData.leaderName, pData.civDesc) .. "[NEWLINE]" ..
                       " (" .. pData.playerName .. ")"
            CuiLeaderIconTT.Desc:SetOffsetY(2)
        else
            desc = Locale.Lookup("LOC_DIPLOMACY_DEAL_PLAYER_PANEL_TITLE",
                                 pData.leaderName, pData.civDesc)
        ...
        local sGovernment = Locale.Lookup("LOC_TOOLTIP_UNLOCKS_GOVERNMENT",
                                          pData.government)
        ...
        local pMilitary = "[ICON_Strength] " .. "[COLOR_Military]" ..
                              pData.military .. "[ENDCOLOR]"
        CuiLeaderIconTT.Strength:SetText(pMilitary)

        local pScience = "[ICON_Science] " .. "[COLOR_Science]+" ..
                             pData.science .. "[ENDCOLOR]"
        CuiLeaderIconTT.Science:SetText(pScience)

        local pCulture = "[ICON_Culture] " .. "[COLOR_Culture]+" ..
                             pData.culture .. "[ENDCOLOR]"
        CuiLeaderIconTT.Culture:SetText(pCulture)

        local pTourism = "[ICON_Tourism] " .. "[COLOR_Tourism]+" ..
                             pData.tourism .. "[ENDCOLOR]"
        CuiLeaderIconTT.Tourism:SetText(pTourism)

        local pFaith = "[ICON_Faith] " .. "[COLOR_FaithDark]+" .. pData.faith ..
                           "[ENDCOLOR]"
        CuiLeaderIconTT.Faith:SetText(pFaith)

        -- 2nd section, (A) gold  and favor
        local pGold = "[ICON_Gold] " .. pData.balance .. " ( " ..
                          TwoColorNumber(pData.gold, "GoldDark", "Civ6Red") ..
                          " )"
        CuiLeaderIconTT.Gold:SetText(pGold)
        if isExpansion2 then
            local pFavor = "[ICON_Favor] " .. pData.favor .. " ( " ..
                               TwoColorNumber(pData.favorPT, "GoldDark",
                                              "Civ6Red") .. " )"
            CuiLeaderIconTT.GoldAndFavor:SetText(
                Locale.Lookup("LOC_CUI_DB_GOLD_AND_FAVOR"))
            CuiLeaderIconTT.Favor:SetText(pFavor)
        else
            CuiLeaderIconTT.GoldAndFavor:SetText(
                Locale.Lookup("LOC_CUI_DB_GOLD"))
        end
        ...
        if isOther then
            ...
            for _, item in ipairs(otherOffer) do
                local icon = resourcesInstance:GetInstance(
                                 CuiLeaderIconTT.OtherStack)
...
function UpdateRelationShipTooltip(tControl, playerID, allianceData)
    ...
    local relationship = Locale.Lookup("LOC_CUI_DB_RELATIONSHIP", data.level) ..
                             " (" ..
                             TwoColorNumber(data.modifier, "ModStatusGreen",
                                            "Civ6Red") .. ")"
    ...
    if data.isAlliance or data.turns ~= 0 then
        states = data.tooltip .. "[NEWLINE][" ..
                     Locale.Lookup("LOC_ESPIONAGEPOPUP_TURNS_REMAINING",
                                   data.turns) .. "]"
    else
        if data.atWar then
            if data.peaceDeal then
                states = data.tooltip .. "[NEWLINE]" ..
                             Locale.Lookup("LOC_CUI_DB_PEACE_DEAL_AVAILABLE")
            else
                states = data.tooltip .. "[NEWLINE]" ..
                             Locale.Lookup(
                                 "LOC_CUI_DB_PEACE_DEAL_NOT_AVAILABLE",
                                 data.warTurns)
            end
        else
            states = data.tooltip
...
function SetLeaderIconToolTip(tControl, playerID)
    if playerID == Game.GetLocalPlayer() or
        localPlayer:GetDiplomacy():HasMet(playerID) then
