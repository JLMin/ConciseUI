...
function CuiGetAllianceData(otherPlayerID)
    ...
    if (isExpansion1 or isExpansion2) and localPlayerDiplomacy then
        ...
        if allianceType ~= -1 then
            local allianceName = Locale.Lookup(
                                     GameInfo.Alliances[allianceType].Name)
            local allianceLevel = localPlayerDiplomacy:GetAllianceLevel(
                                      otherPlayerID)
            allianceData.isAlliance = true
            allianceData.tooltip = Locale.Lookup(
                                       "LOC_DIPLOMACY_ALLIANCE_FLAG_TT",
                                       allianceName, allianceLevel)
            allianceData.remainingTurns =
                localPlayerDiplomacy:GetAllianceTurnsUntilExpiration(
                    otherPlayerID)
        end
    end
