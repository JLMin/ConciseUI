...
function SetStage(stageNum, beginCongress)
    ...
    elseif stageNum == 4 then
        ...
        if prevIsInSession then
            Controls.Title:SetText(
                Locale.Lookup(
                    prevIsEmergencySession and "LOC_WORLD_CONGRESS_SPECIAL_SESSION_END" or "LOC_WORLD_CONGRESS_END"
                )
            )
        else
    ...
    Controls.EmptyLabel:SetShow(
        m_ReviewTab ~= REVIEW_TAB_AVAILABLE_PROPOSALS and m_kReviewResolutionIM.m_iAllocatedInstances == 0 and
...
function SetPhase(phaseNum)
    ...
    if phaseNum == 1 then
        Controls.Description:SetText(
            Locale.Lookup(
                m_CurrentStage == 1 and "LOC_WORLD_CONGRESS_VOTE_NORMAL_SESSION_DESCRIPTION" or
    ...
    elseif phaseNum == 2 then
        Controls.Description:SetText(
            Locale.Lookup(
                m_CurrentStage == 1 and "LOC_WORLD_CONGRESS_SUBMIT_PROPOSALS_CONFIRM" or
...
function CanMoveToNextPhase()
    for _, kVoteData in pairs(m_kResolutionVotes) do
        ...
        if
            (kVoteData.A.votes + kVoteData.B.votes) <= 0 or not choiceData or
                (choiceData.choice < 0 or choiceData.target < 0)
         then
            return false
        end
    end
...
function UpdateWorkingFavor()
    ...
    if m_kResolutionsTitle then
        ...
        m_kResolutionsTitle.Cost:SetToolTipString(
            Locale.Lookup("LOC_WORLD_CONGRESS_TT_RESOLUTIONS_FAVOR_COST_WITH_REFUND", totalCost)
        )
    end
...
function UpdateNavButtons()
    ...
    if m_CurrentStage == 4 then
        ...
        Controls.PassButton:SetShow(
            hasChoices and m_HasSpecialSessionNotification and m_ReviewTab == REVIEW_TAB_AVAILABLE_PROPOSALS
        )
        ...
        Controls.AvailableProposalsButton:SetToolTipString(
            hasProposals and "" or Locale.Lookup("LOC_WORLD_CONGRESS_NO_AVAILABLE_PROPOSALS_TT")
        )
    ...
    if Controls.AcceptButton:IsVisible() then
        ...
        Controls.AcceptButton:SetToolTipString(
            Controls.AcceptButton:IsDisabled() and Locale.Lookup("LOC_WORLD_CONGRESS_TT_SELECT_ONE") or ""
        )
        Controls.AcceptButton:SetText(
            Locale.Lookup(
                m_HasSpecialSessionNotification and "LOC_WORLD_CONGRESS_ADD_PROPOSALS" or "LOC_WORLD_CONGRESS_SUBMIT"
            )
        )
    end
end
...
function PopulateLeaderStack()
    ...
    for _, pPlayer in ipairs(aPlayers) do
        ...
        if hasMetPlayer or (GameConfiguration.IsAnyMultiplayer() and pPlayerConfig:IsHuman()) then
            ...
            local icon =
                (isLocalPlayer or pDiplomacy:HasMet(playerID)) and "ICON_" .. leaderName or "ICON_LEADER_DEFAULT"
...
function AddBGElement(aPlayers, aPlayersIndex, parentControl)
    ...
    if playerID == localPlayerID or pDiplomacy:HasMet(playerID) then
        ...
        instance.Top:SetTexture(
            m_IsInSession and (m_IsEmergencySession and "WC_BGBannerTop_Special" or "WC_BGBannerTop_Normal") or
                "WC_BGBannerTop_NoSession"
        )
        instance.Center:SetTexture(
            m_IsInSession and (m_IsEmergencySession and "WC_BGBanner_Special" or "WC_BGBanner_Normal") or
        ...
        if type(i) == "number" then -- There's a "Stage" key kResolutions
            if m_kProposalTitleIM.m_iAllocatedInstances == 0 then
                ...
                m_kResolutionsTitle.Title:SetText(
                    Locale.ToUpper(Locale.Lookup("LOC_WORLD_CONGRESS_RESULTS_WORLD_EVENT_RESOLUTIONS"))
                )
            ...
            instance.Vote1.UpButton:RegisterCallback(
                Mouse.eLClick,
                OnVoteResolution(1, UP_VOTE, kVoteData.A, kCostData)
            )
            instance.Vote1.DownButton:RegisterCallback(
                Mouse.eLClick,
                OnVoteResolution(1, DOWN_VOTE, kVoteData.A, kCostData)
            )

            UpdateVotingWidget(instance.Vote2, kVoteData.B, kCostData, TestResolutionVote, 2)
            instance.Vote2.UpButton:RegisterCallback(
                Mouse.eLClick,
                OnVoteResolution(2, UP_VOTE, kVoteData.B, kCostData)
            )
            instance.Vote2.DownButton:RegisterCallback(
                Mouse.eLClick,
                OnVoteResolution(2, DOWN_VOTE, kVoteData.B, kCostData)
            )
            ...
            if table.count(kResolutionData.FavoredPlayerIDs) > 0 then
                ...
                instance.FavoredContainer:SetToolTipString(
                    Locale.Lookup("LOC_WORLD_CONGRESS_PREFERRED_VOTES_TT", favoredTT)
                )
            ...
            if table.count(kResolutionData.DisfavoredPlayerIDs) > 0 then
                ...
                instance.DisfavoredContainer:SetToolTipString(
                    Locale.Lookup("LOC_WORLD_CONGRESS_PREFERRED_VOTES_TT", disfavotedTT)
                )
...
function EvaluateTiebroken(kEvaluationData)
    ...
    return kEvaluationData.aVotes == kEvaluationData.bVotes and
        "[NEWLINE]" .. Locale.Lookup("LOC_WORLD_CONGRESS_PREVIOUS_TOOLTIP_WAS_TIE") or
        ""
end
...
function EvaluateNeckAndNeck(kEvaluationData)
    ...
    if voteMargin <= GlobalParameters.WORLD_CONGRESS_NEARLY_TIED_RANGE / 100 then
        return "[NEWLINE]" ..
            (kEvaluationData.aVotes > kEvaluationData.bVotes and
                Locale.Lookup("LOC_WORLD_CONGRESS_PREVIOUS_TOOLTIP_NECK_AND_NECK_A") or
        ...
        majorityString =
            ...
            "[NEWLINE]" ..
                Locale.Lookup(
                    ...
                    GetVisiblePlayerName(kEvaluationData.biggestAVoter)
                )
    end
        ...
        majorityString =
            ...
            "[NEWLINE]" ..
                Locale.Lookup(
                    ...
                    GetVisiblePlayerName(kEvaluationData.biggestBVoter)
                )
    end
...
function PopulateChoicePulldown(kResolutionChoice, kVoteData)
    ...
    if kResolutionData.TargetType == "PlayerType" then
        ...
        instance.PlayerPulldown:GetButton():RegisterCallback(
            Mouse.eLClick,
            function()
        ...
        if possibleTargets and table.count(possibleTargets) > 0 then
            ...
            UI.DataError(
                "Resolution choice of Type '" ..
                    kResolution.Type .. "' has no PossibleTargets -  aborting WorldCongress"
            )
        end
        ...
        instance.Pulldown:GetButton():RegisterCallback(
            Mouse.eLClick,
            function()
...
function OnVoteResolution(choice, direction, kVoteData, kCostData, isTest)
    return function()
        ...
        if nextVotes < 1 or nextVotes > table.count(kCostData) or nextVotes > kCostData.MaxVotes then
            return
        end
...
function UpdateVotingWidget(instance, kVoteData, kCostData, TestVoteCallback, resolutionChoice)
    ...
    local voteTooltip =
        Locale.Lookup(
        "LOC_WORLD_CONGRESS_TT_VOTE_COST",
        kVoteData.voteDirection < 0 and "[ICON_VOTE_DOWN]" or "[ICON_VOTE_UP]",
        kVoteData.votes,
        kVoteData.cost
    )
    ...
    if nextVotes < table.count(kCostData) then
        ...
        if cost ~= 0 then
            if instance.UpButton:IsDisabled() then
                ...
                tooltip =
                    Locale.Lookup(
                    kVoteData.voteDirection >= 0 and "LOC_WORLD_CONGRESS_TT_VOTE_UP" or
    ...
    instance.UpCost:SetText(
        cost == 0 and " " or Locale.Lookup("LOC_WORLD_CONGRESS_FAVOR", cost * (kVoteData.voteDirection < 0 and -1 or 1))
    )
    ...
    if prevVotes < table.count(kCostData) then
        ...
        if cost ~= 0 then
            if instance.DownButton:IsDisabled() then
                ...
                tooltip =
                    Locale.Lookup(
                    kVoteData.voteDirection >= 0 and "LOC_WORLD_CONGRESS_TT_VOTE_DOWN" or
    ...
    if kVoteData.votes == 0 then
        if not kVoteData.voteBlocker or not kVoteData.voteBlocker.NoUpvote then
            instance.UpButton:SetToolTipString(
                Locale.Lookup(
                    resolutionChoice and "LOC_WORLD_CONGRESS_TT_NO_VOTING_DOWN" or
        ...
        if not kVoteData.voteBlocker or not kVoteData.voteBlocker.NoDownvote then
            ...
            if bIsTarget then
                ...
                instance.DownButton:SetToolTipString(
                    Locale.Lookup(resolutionChoice and "" or "LOC_WORLD_CONGRESS_TT_PLEASE_VOTE_AGAINST")
                )
            end
        end
    end
...
function PopulateProposals()
    ...
    local kSortedCategories =
        GetSortedProposalCategories(Game.GetWorldCongress():GetProposals(Game.GetLocalPlayer()).Proposals)
...
function UpdateSenderTitle(instance, titleInstance, kProposals, kCategoryProposals, kProposalDef, pPlayerConfig)
    ...
    instance.Cost:SetToolTipString(
        Locale.Lookup(
            (not m_IsInSession and m_ReviewTab == REVIEW_TAB_AVAILABLE_PROPOSALS) and
    ...
    for _, kProposal in pairs(kProposals) do
        ...
        if not kVoteData then
            kVoteData = {
                ...
                voteBlocker = kProposal.VotingRestriction and GameInfo.VotingBlockers[kProposal.VotingRestriction] or
                    nil,
        ...
        if canVote or (not m_IsInSession and m_ReviewTab == REVIEW_TAB_AVAILABLE_PROPOSALS) then
            ...
            if pPlayerConfig then
                ...
                pIconManager:UpdateIconSimple(
                    leaderIcon,
                    kProposal.Target,
                    isUniqueLeader[kProposal.Target] or false,
                ...
                instance.LeaderIcon.TargetIcon:SetColor(
                    kProposal.TypeName == "WC_EMERGENCY_REQUEST_AID" and COLOR_GREEN or COLOR_RED
                )
                instance.LeaderIcon.TargetIcon:SetTexture(
                    kProposal.TypeName == "WC_EMERGENCY_REQUEST_AID" and "Emergency_TargetAid44" or "Emergency_Target44"
                )
            ...
            if not m_IsInSession and m_ReviewTab == REVIEW_TAB_AVAILABLE_PROPOSALS then
                instance.ExpandButton:RegisterCallback(
                    Mouse.eLClick,
                    function()
                ...
                instance.SelectBox:RegisterCallback(
                    Mouse.eLClick,
                    OnToggleProposal(instance, kProposal, kProposalCategory)
                )
            else
                ...
                instance.Vote.DownButton:RegisterCallback(
                    Mouse.eLClick,
                    OnVoteProposal(DOWN_VOTE, kVoteData, kCostData)
                )
        ...
        if instance.TurnsLeft then
            if not m_IsInSession then
                ...
                instance.TurnsLeft:SetToolTipString(
                    Locale.Lookup("LOC_WORLD_CONGRESS_TURNS_LEFT_TT", emergency.TurnsLeft)
                )
            else
...
function GetProposalVoteKey(kProposal)
    return tostring(kProposal.TypeName) ..
        "_" .. kProposal.Target .. "_" .. (kProposal.Sender ~= nil and kProposal.Sender or "NONE")
end
...
function UpdateAvailableProposal(kVoteData, kProposalCategory)
    ...
    instance.Root:SetTexture(
        not kVoteData.disabled and kVoteData.votes == 0 and "WC_ProposalFrame_Normal" or "WC_ProposalFrame_Selected"
    )

    instance.SelectCheck:SetTexture(
        not kVoteData.disabled and kVoteData.votes == 0 and "WC_ProposalCheck_Off" or "WC_ProposalCheck_On"
    )
    ...
    if not isTargetSelected then
        instance.SelectBox:SetDisabled(
            kVoteData.disabled or (kVoteData.votes == 0 and m_WorkingFavor < kProposalCategory.FavorCost)
        )
        ...
        elseif instance.SelectBox:IsSelected() then
            instance.SelectBox:SetToolTipString(
                Locale.Lookup("LOC_WORLD_CONGRESS_TT_PROPOSAL_REFUND", kProposalCategory.FavorCost)
            )
        else
            instance.SelectBox:SetToolTipString(
                Locale.Lookup("LOC_WORLD_CONGRESS_TT_PROPOSAL_COST", kProposalCategory.FavorCost)
            )
        end
    end
...
function UpdateProposal(kVoteData)
    ...
    kVoteData.instance.Root:SetTexture(
        canVote and kVoteData.votes == 0 and "WC_ProposalFrameOpen_Normal" or "WC_ProposalFrameOpen_Selected"
    )
end
...
function GetSortedProposalCategories(kProposals)
    ...
    if kProposals ~= nil then
        ...
        table.sort(
            kSortedCategories,
            function(a, b)
                return a.kData.Sort < b.kData.Sort
            end
        )
    end
    ...
    if m_CurrentStage == 1 then
        ...
        for i, kResolutionData in pairs(kResolutions) do
            if type(i) == "number" then -- There's a "Stage" key kResolutions
                ...
                local aTT =
                    kResolutionChoice.choice == 1 and Locale.Lookup("LOC_WORLD_CONGRESS_A_VOTES_TT_SUMMARY", aVotes) or
                    ""

                local bVotes = kResolutionChoice.choice ~= 1 and kVoteData.B.votes or 0
                local bTT =
                    kResolutionChoice.choice ~= 1 and Locale.Lookup("LOC_WORLD_CONGRESS_B_VOTES_TT_SUMMARY", bVotes) or
                    ""
                ...
                if kResolutionData.TargetType == "PlayerType" then
                    ...
                    if pPlayerConfig then
                        ...
                        local pDiplomacy = Players[localPlayerID]:GetDiplomacy()
                        local icon =
                            (isLocalPlayer or pDiplomacy:HasMet(playerID)) and "ICON_" .. playerTypeName or
                            "ICON_LEADER_DEFAULT"
                        pIconManager:UpdateIconSimple(
                            icon,
                            playerID,
                ...
                instance.ChoiceLabel:SetText(
                    Locale.Lookup(
                        "LOC_WORLD_CONGRESS_PREVIEW_PROPOSAL_OUTCOME",
                        kResolutionChoice.choice == 1 and Locale.Lookup("LOC_WORLD_CONGRESS_CHOICE_A") or
                            Locale.Lookup("LOC_WORLD_CONGRESS_CHOICE_B")
                    )
                )
                instance.Description:SetText(
                    Locale.Lookup(
                        kResolutionChoice.choice == 1 and kResolution.Effect1Description or
                ...
                instance.Cost:SetToolTipString(
                    Locale.Lookup("LOC_WORLD_CONGRESS_TT_PROPOSAL_COST", kVoteData.A.cost + kVoteData.B.cost)
                )
                instance.TurnsLeft:SetText("")
    ...
    for _, kProposal in pairs(kProposals) do
        ...
        if not m_IsInSession or kVoteData then
            ...
            if pPlayerConfig then
                ...
                local isLocalPlayer = kProposal.Target == localPlayerID
                local icon =
                    (isLocalPlayer or pDiplomacy:HasMet(kProposal.Target)) and "ICON_" .. playerTypeName or
                    "ICON_LEADER_DEFAULT"
                pIconManager:UpdateIconSimple(
                    icon,
                    kProposal.Target,
            ...
            instance.Cost:SetToolTipString(
                cost ~= nil and Locale.Lookup("LOC_WORLD_CONGRESS_TT_PROPOSAL_COST", cost) or ""
            )
            ...
            if not m_IsInSession then
                instance.EmergencyButton:RegisterCallback(
                    Mouse.eLClick,
                    function()
                        local emergency =
                            Game.GetEmergencyManager():GetSingleEmergency(kProposal.Target, kProposal.EmergencyType)
...
function UpdateResolutionChoice(kResolutionChoice)
    ...
    kResolutionChoice.instance.Choice1Slot:SetTexture(
        hasTarget and kResolutionChoice.choice == 1 and "WC_ProposalSlot_On" or "WC_ProposalSlot_Off"
    )
    kResolutionChoice.instance.Choice2Slot:SetTexture(
        hasTarget and kResolutionChoice.choice == 2 and "WC_ProposalSlot_On" or "WC_ProposalSlot_Off"
    )
    ...
    kResolutionChoice.instance.Root:SetTexture(
        isSelected and "WC_ProposalFrameOpen_Selected" or "WC_ProposalFrameOpen_Normal"
    )
...
function PopulateEmergencyProposals()
    ...
    local kSortedCategories =
        GetSortedProposalCategories(Game.GetWorldCongress():GetEmergencies(Game.GetLocalPlayer()).Proposals)
    for _, kSorted in ipairs(kSortedCategories) do
        local proposalType = kSorted.type
        local kProposalDef = kSorted.kData
        local kProposalCategory = kSorted.kCategory
        ...
        local updateFn = function()
            UpdateCategoryTitle(titleInstance, kCategoryProposals, kProposalDef, kProposalCategory.FavorCost)
        end
...
function PopulateReview()
    ...
    for i, kResolutionData in pairs(kReviewData.Resolutions) do
        if type(kResolutionData) == "table" then -- There's a "Stage" key in kResolutions
            ...
            instance.ChoiceLabel:SetText(
                Locale.Lookup("LOC_WORLD_CONGRESS_REVIEW_PROPOSAL_OUTCOME", kResolutionData.ChosenLabel)
            )
            ...
            if table.count(kResolutionData.PlayerSelections) > 0 then
                ...
                rejectedOutcome.ChoiceLabel:SetText(
                    Locale.Lookup("LOC_WORLD_CONGRESS_REVIEW_PROPOSAL_REJECTED_OUTCOME", kResolutionData.RejectedLabel)
                )
                ...
                for _, kData in pairs(kResolutionData.PlayerSelections) do
                    ...
                    if kResolutionData.TargetType == "PlayerType" then
                        ...
                        local text, icon =
                            targetPlayerID ~= -1 and GetPulldownNameAndIcons(kResolutionData, targetPlayerID) or "",
                            ""
                    ...
                    voterInstance.ChoiceLabel:SetText(
                        Locale.Lookup("LOC_WORLD_CONGRESS_REVIEW_VOTER_OUTCOME", kData.OptionChosenLabel)
                    )
                end
...
function PopulateActiveEffects()
    ...
    for i, kResolutionData in pairs(kResolutions) do
        if type(kResolutionData) == "table" then -- There's a "Stage" key in kResolutions
            ...
            instance.TurnsLeft:SetToolTipString(
                Locale.Lookup("LOC_WORLD_CONGRESS_TURNS_LEFT_TT", pCongressData.TurnsLeft + 1)
            )
...
function PopulateReviewProposals(kProposals, kProposalIM)
    ...
    for _, kProposal in pairs(kProposals) do
        ...
        -- Title + Target Icon
        local proposalResult =
            upVotes > downVotes and "LOC_WORLD_CONGRESS_PROPOSAL_PASSED" or "LOC_WORLD_CONGRESS_PROPOSAL_FAILED"
        local pIconManager = LeaderIcon:AttachInstance(instance.LeaderIcon)
        ...
        if pPlayerConfig then
            ...
            local icon =
                (isLocalPlayer or pDiplomacy:HasMet(kProposal.TargetPlayer)) and "ICON_" .. playerTypeName or
                "ICON_LEADER_DEFAULT"
            pIconManager:UpdateIconSimple(
                icon,
                kProposal.TargetPlayer,
        ...
        if table.count(kProposal.PlayerVotes) > 0 then
            ...
            for _, kData in pairs(kProposal.PlayerVotes) do
                ...
                if pPlayerConfig then
                    local playerTypeName = pPlayerConfig:GetLeaderTypeName()
                    local icon =
                        (playerID == localPlayerID or pDiplomacy:HasMet(playerID)) and "ICON_" .. playerTypeName or
                        "ICON_LEADER_DEFAULT"
                ...
                local downTT =
                    Locale.Lookup("LOC_WORLD_CONGRESS_REVIEW_DOWN_VOTES_PLAYER_TT", playerName, kData.Votes * -1)
                instance.UpVoteIcon:SetToolTipString(upTT)
                instance.UpVoteLabel:SetToolTipString(upTT)
                instance.DownVoteIcon:SetToolTipString(downTT)
                instance.DownVoteLabel:SetToolTipString(downTT)
...
function OnAccept()
    ...
    elseif not m_IsInSession and m_ReviewTab == REVIEW_TAB_AVAILABLE_PROPOSALS then
        ...
        m_kPopupDialog:ShowOkCancelDialog(
            dialogText,
            function()
                Controls.AcceptButton:SetDisabled(true)
                for _, kVoteData in pairs(m_kProposalVotes) do
...
function ShowPopup(delayShow, fromHotLoad)
    ...
    if ContextPtr:IsHidden() or fromHotLoad then
        UIManager:QueuePopup(
            ContextPtr,
            PopupPriority.WorldCongressPopup,
            {AlwaysVisibleInQueue = true, DelayShow = delayShow}
        )
...
-- ===========================================================================
function AssignActivePulldown(pulldown)
...
function LateInitialize()
    ...
    Controls.LastResultsButton:RegisterCallback(
        Mouse.eLClick,
        function()
            OnWorldCongressResults(REVIEW_TAB_RESULTS)
        end
    )
    Controls.CurrentEffectsButton:RegisterCallback(
        Mouse.eLClick,
        function()
            OnWorldCongressResults(REVIEW_TAB_CURRENT_EFFECTS)
        end
    )
    Controls.AvailableProposalsButton:RegisterCallback(
        Mouse.eLClick,
        function()
            OnWorldCongressResults(REVIEW_TAB_AVAILABLE_PROPOSALS)
        end
...
function Initialize()
    ...
    Events.WorldCongressStage1.Add(
        function(i)
            OnWorldCongressStageChange(i, 1)
        end
    )
    Events.WorldCongressStage2.Add(
        function(i)
