...
--	CONSTANTS
-- ===========================================================================
local LEADERTEXT_PADDING_X = 40
local LEADERTEXT_PADDING_Y = 40
local SELECTION_PADDING_Y = 20

local OVERVIEW_MODE = 0
local CONVERSATION_MODE = 1
local CINEMA_MODE = 2
local DEAL_MODE = 3
local SIZE_BUILDING_ICON = 32
local SIZE_UNIT_ICON = 32
...
local TEAM_RIBBON_SIZE = 53
local TEAM_RIBBON_SMALL_SIZE = 30
local TEAM_RIBBON_PREFIX = "ICON_TEAM_RIBBON_"

local VOICEOVER_SUPPORT = {
    "KUDOS", "WARNING", "DECLARE_WAR_FROM_HUMAN", "DECLARE_WAR_FROM_AI",
    "FIRST_MEET", "DEFEAT", "ENRAGED"
}
...
--	VARIABLES
-- ===========================================================================
local ms_PlayerPanelIM = InstanceManager:new("PlayerPanel", "Root")
local ms_DiplomacyRibbonIM = InstanceManager:new("DiplomacyRibbonVert", "Root")
local ms_DiplomacyRibbonLeaderIM = InstanceManager:new("DiplomacyRibbonLeader",
                                                       "Root")
local ms_IconOnlyIM = InstanceManager:new("IconOnly", "Icon")
...
local ms_IntelPanelIM = InstanceManager:new("IntelPanel", "Panel")
local ms_IntelTabButtonIM = InstanceManager:new("IntelTabButtonInstance",
                                                "Button")

-- Intel panel instances
local ms_IntelOverviewIM = InstanceManager:new("IntelOverviewInstance", "Top")
local ms_IntelGossipIM = InstanceManager:new("IntelGossipHistoryPanel", "Top")
local ms_IntelAccessLevelIM =
    InstanceManager:new("IntelAccessLevelPanel", "Top")
local ms_IntelRelationshipIM = InstanceManager:new("IntelRelationshipPanel",
                                                   "Top")
local ms_IntelTabAnchorIM = InstanceManager:new("IntelTabAnchorInstance",
                                                "Anchor")

-- Intel overview row instances
local ms_IntelOverviewDividerIM = InstanceManager:new(
                                      "IntelOverviewDividerInstance",
                                      "TheDivider")
local ms_IntelOverviewGossipIM = InstanceManager:new(
                                     "IntelOverviewGossipInstance", "Top")
local ms_IntelOverviewAccessLevelIM = InstanceManager:new(
                                          "IntelOverviewAccessLevelInstance",
                                          "Top")
local ms_IntelOverviewGovernmentIM = InstanceManager:new(
                                         "IntelOverviewGovernmentInstance",
                                         "Top")
local ms_IntelOverviewAgendasIM = InstanceManager:new(
                                      "IntelOverviewAgendasInstance", "Top")
local ms_IntelOverviewAgendaEntryIM = InstanceManager:new(
                                          "IntelOverviewAgendaEntryInstance",
                                          "Top")
local ms_IntelOverviewAgreementsIM = InstanceManager:new(
                                         "IntelOverviewAgreementsInstance",
                                         "Top")
local ms_IntelOverviewOurRelationshipIM =
    InstanceManager:new("IntelOverviewOurRelationshipInstance", "Top")
local ms_IntelOverviewOtherRelationshipsIM =
    InstanceManager:new("IntelOverviewOtherRelationshipsInstance", "Top")
local ms_IntelOverviewAnchorIM = InstanceManager:new(
                                     "IntelOverviewAnchorInstance", "Anchor")

local ms_IntelRelationshipReasonIM = InstanceManager:new(
                                         "IntelRelationshipReasonEntry",
                                         "Background")
local ms_RelationshipIconsIM = InstanceManager:new("RelationshipIcon",
                                                   "Background")

local ms_IntelGossipHistoryPanelEntryIM =
    InstanceManager:new("IntelGossipHistoryPanelEntry", "Background")

local ms_ConversationSelectionIM = InstanceManager:new(
                                       "ConversationSelectionInstance",
                                       "SelectionButton",
                                       Controls.ConversationSelectionStack)

local ms_uniqueIconIM = InstanceManager:new("IconInfoInstance", "Top",
                                            Controls.FeaturesStack)
local ms_uniqueTextIM = InstanceManager:new("TextInfoInstance", "Top",
                                            Controls.FeaturesStack)
...
local m_LeaderCoordinates = {}
local m_lastLeaderPlayedMusicFor = -1
local m_bCurrentMusicIsModder = false
local ms_LastDealResponseAnimation = nil

-- VOICEOVER SUPPORT
local m_voiceoverText = ""
local m_cinemaMode = false
local m_currentLeaderAnim = ""
local m_currentSceneEffect = ""
...
-- ===========================================================================
function GetStatementMood(fromPlayer, inputMood)
...
function GetVisStateFromDiplomaticState(iState)
    ...
    if (iVisState ~= nil) then return iVisState end
...
function UpdateSelectedPlayer(allowDeadPlayer)
    ...
    if (allowDeadPlayer == nil) then allowDeadPlayer = false end

    -- Have we met them and are they in the ribbon (alive) or allowing dead players (for defeat messages)
    if (ms_LocalPlayer:GetDiplomacy():HasMet(ms_SelectedPlayerID) and
        (allowDeadPlayer == true or
            ms_LeaderIDToRibbonEntry[ms_SelectedPlayerID] ~= nil)) then
...
-- ===========================================================================
function CreatePlayerPanel(rootControl)
...
-- ===========================================================================
function CreateDiplomacyRibbon(rootControl)
...
function CreatePanels()
    ...
    ms_DiplomacyRibbon =
        CreateDiplomacyRibbon(Controls.DiplomacyRibbonContainer)
end
...
function IsWarChoice(key)
    local isWar = key == "CHOICE_DECLARE_SURPRISE_WAR" or key ==
                      "CHOICE_DECLARE_FORMAL_WAR" or key ==
                      "CHOICE_DECLARE_HOLY_WAR" or key ==
                      "CHOICE_DECLARE_LIBERATION_WAR" or key ==
                      "CHOICE_DECLARE_RECONQUEST_WAR" or key ==
                      "CHOICE_DECLARE_PROTECTORATE_WAR" or key ==
                      "CHOICE_DECLARE_COLONIAL_WAR" or key ==
                      "CHOICE_DECLARE_TERRITORIAL_WAR"
    return isWar
end
...
function GetWarType(key)
    ...
    if (key == "CHOICE_DECLARE_LIBERATION_WAR") then
        return WarTypes.LIBERATION_WAR
    end
    if (key == "CHOICE_DECLARE_RECONQUEST_WAR") then
        return WarTypes.RECONQUEST_WAR
    end
    if (key == "CHOICE_DECLARE_PROTECTORATE_WAR") then
        return WarTypes.PROTECTORATE_WAR
    end
    if (key == "CHOICE_DECLARE_COLONIAL_WAR") then
        return WarTypes.COLONIAL_WAR
    end
    if (key == "CHOICE_DECLARE_TERRITORIAL_WAR") then
        return WarTypes.TERRITORIAL_WAR
    end
...
function GetGoldCost(key)
    ...
    if (key == "CHOICE_DIPLOMATIC_DELEGATION") then
        szActionString = "DIPLOACTION_DIPLOMATIC_DELEGATION"
    end
    if (key == "CHOICE_RESIDENT_EMBASSY") then
        szActionString = "DIPLOACTION_RESIDENT_EMBASSY"
    end
    if (key == "CHOICE_OPEN_BORDERS") then
        szActionString = "DIPLOACTION_OPEN_BORDERS"
    end
...
function IsPeaceChoice(key)
    local isPeace = (key == "CHOICE_MAKE_PEACE")
    return isPeace
end

function CanInitiateDiplomacyStatement()
    return
        ms_LocalPlayerID ~= ms_SelectedPlayerID and ms_SelectedPlayerID >= 0 and
            not GameConfiguration.IsPaused()
end
...
function OnSelectInitialDiplomacyStatement(key)
    ...
    if CanInitiateDiplomacyStatement() then
        ...
        if (key == "CHOICE_DECLARE_SURPRISE_WAR") then
            DiplomacyManager.RequestSession(ms_LocalPlayerID,
                                            ms_SelectedPlayerID,
        ...
        elseif (key == "CHOICE_MAKE_PEACE") then
            ...
            if (not DealManager.HasPendingDeal(ms_LocalPlayerID,
                                               ms_SelectedPlayerID)) then
                DealManager.ClearWorkingDeal(DealDirection.OUTGOING,
                                             ms_LocalPlayerID,
                                             ms_SelectedPlayerID)
        ...
        elseif (key == "CHOICE_OPEN_BORDERS") then
            DiplomacyManager.RequestSession(ms_LocalPlayerID,
                                            ms_SelectedPlayerID, "OPEN_BORDERS")
...
function OnSelectConversationDiplomacyStatement(key)
    ...
    if (key == "CHOICE_EXIT") then
        ...
        elseif (key == "CHOICE_MAKE_PEACE") then
            DiplomacyManager.AddStatement(ms_ActiveSessionID,
                                          Game.GetLocalPlayer(), "MAKE_PEACE")
        elseif (key == "CHOICE_MAKE_DEAL") then
            DiplomacyManager.AddStatement(ms_ActiveSessionID,
                                          Game.GetLocalPlayer(), "MAKE_DEAL")
        elseif (key == "CHOICE_MAKE_DEMAND") then
            DiplomacyManager.AddStatement(ms_ActiveSessionID,
                                          Game.GetLocalPlayer(), "MAKE_DEMAND")
        else
            if (key == "CHOICE_POSITIVE") then
                DiplomacyManager.AddResponse(ms_ActiveSessionID,
                                             Game.GetLocalPlayer(), "POSITIVE")
            else
...
function ApplyStatement(handler, statementTypeName, statementSubTypeName,
                        toPlayer, kStatement)
    ...
    local kParsedStatement = handler.ExtractStatement(handler,
                                                      statementTypeName,
                                                      statementSubTypeName,
                                                      kStatement.FromPlayer,
                                                      eFromPlayerMood,
                                                      kStatement.Initiator)
    handler.RemoveInvalidSelections(kParsedStatement, ms_LocalPlayerID,
                                    ms_OtherPlayerID)
    ...
    if (kParsedStatement.StatementText ~= nil) then
        leaderstr = Locale.Lookup(DiplomacyManager.FindTextKey(
                                      kParsedStatement.StatementText,
                                      kStatement.FromPlayer,
                                      kStatement.FromMood, toPlayer))
        local reasonStrKey = DiplomacyManager.FindReasonTextKey(
                                 kParsedStatement.ReasonText,
                                 kStatement.FromPlayer, kStatement.AiReason,
                                 kStatement.AiModifier)
        if (reasonStrKey ~= nil) then
            reasonStr = Locale.Lookup(reasonStrKey)
            local agendaStr = DiplomacyManager.FindReasonAgendaTextKey(
                                  kStatement.FromPlayer, toPlayer,
                                  kStatement.AiReason, kStatement.AiModifier)
    ...
    if (kParsedStatement.Selections ~= nil) then
        for _, selection in ipairs(kParsedStatement.Selections) do
            local instance = ms_ConversationSelectionIM:GetInstance()
            instance.SelectionText:SetText(Locale.Lookup(selection.Text))

            local texth = math.max(instance.SelectionText:GetSizeY() +
                                       SELECTION_PADDING_Y, 45)
            ...
            if (selection.IsDisabled == nil or selection.IsDisabled == false) then
                ...
                instance.SelectionButton:RegisterCallback(Mouse.eMouseEnter,
                                                          function()
                ...
                if (selection.FailureReasons ~= nil) then
                    instance.SelectionButton:SetToolTipString(
                        Locale.Lookup(selection.FailureReasons[1]))
                end
            end
        end
    end
    ...
    local ePlayerMood = DiplomacySupport_GetPlayerMood(ms_SelectedPlayer,
                                                       ms_LocalPlayerID)

    if (ms_currentViewMode == CONVERSATION_MODE) then
        LeaderSupport_QueueAnimationSequence(ms_OtherLeaderName,
                                             kParsedStatement.LeaderAnimation,
                                             ePlayerMood)
    ...
    elseif (ms_currentViewMode == DEAL_MODE) then
        if (ePlayerMood == DiplomacyMoodTypes.HAPPY) then
            LeaderSupport_QueueAnimationSequence(
                ms_SelectedPlayerLeaderTypeName, "HAPPY_IDLE")
        elseif (ePlayerMood == DiplomacyMoodTypes.NEUTRAL) then
            LeaderSupport_QueueAnimationSequence(
                ms_SelectedPlayerLeaderTypeName, "NEUTRAL_IDLE")
        elseif (ePlayerMood == DiplomacyMoodTypes.UNHAPPY) then
            LeaderSupport_QueueAnimationSequence(
                ms_SelectedPlayerLeaderTypeName, "UNHAPPY_IDLE")
        end
    end

    -- Leader icon
    local leaderIconController = CivilizationIcon:AttachInstance(
                                     Controls.LeaderResponseIcon)
    leaderIconController:UpdateIconFromPlayerID(kStatement.FromPlayer)

    -- Leader name
    local leaderDesc =
        PlayerConfigurations[kStatement.FromPlayer]:GetLeaderName()
    Controls.LeaderResponseName:SetText(Locale.ToUpper(
                                            Locale.Lookup(
...
function PopulateStatementList(options, rootControl, isSubList)
    local buttonIM
    local stackControl
    local selectionText = "[SIZE_16]" -- Resetting the string size for the new button instance
    if (isSubList) then
    ...
    for _, selection in ipairs(options) do
        local instance = buttonIM:GetInstance(stackControl)
        local selectionText = selectionText .. Locale.Lookup(selection.Text)
        local callback
        local tooltipString = nil
        if (selection.Key ~= nil) then
            callback = function()
                OnSelectInitialDiplomacyStatement(selection.Key)
            end

            local pActionDef =
                GameInfo.DiplomaticActions[selection.DiplomaticActionType]
            instance.Button:SetToolTipString(
                GetStatementButtonTooltip(pActionDef))

            -- If costs gold add text
            local iCost = GetGoldCost(selection.Key)
            if iCost > 0 then
                local szGoldString = Locale.Lookup("LOC_DIPLO_CHOICE_GOLD_INFO",
                                                   iCost)
            ...
            if (IsWarChoice(selection.Key)) then
                ...
                local iWarmongerPoints =
                    ms_LocalPlayer:GetDiplomacy():ComputeDOWWarmongerPoints(
                        ms_SelectedPlayerID, eWarType)
                local szWarmongerLevel =
                    ms_LocalPlayer:GetDiplomacy():GetWarmongerLevel(
                        -iWarmongerPoints)
                local szWarmongerString =
                    Locale.Lookup("LOC_DIPLO_CHOICE_WARMONGER_INFO",
                                  szWarmongerLevel)
                ...
                callback = function()
                    LuaEvents.DiplomacyActionView_ConfirmWarDialog(
                        ms_LocalPlayerID, ms_SelectedPlayerID, eWarType)
                end
            end

            -- If denounce statement change callback to prompt first.
            if (selection.Key == "CHOICE_DENOUNCE") then
                local denounceFn = function()
                    OnSelectInitialDiplomacyStatement(selection.Key)
                end
                callback = function()
                    local playerConfig =
                        PlayerConfigurations[ms_SelectedPlayer:GetID()]
                    if (playerConfig ~= nil) then

                        selectedCivName =
                            Locale.Lookup(
                                playerConfig:GetCivilizationShortDescription())
                        m_PopupDialog:Reset()
                        m_PopupDialog:AddText(
                            Locale.Lookup("LOC_DENOUNCE_POPUP_BODY",
                                          selectedCivName))
                        m_PopupDialog:AddButton(Locale.Lookup("LOC_CANCEL"), nil)
                        m_PopupDialog:AddButton(
                            Locale.Lookup("LOC_DIPLO_CHOICE_DENOUNCE"),
                            denounceFn, nil, nil, "PopupButtonInstanceRed")
                        m_PopupDialog:Open()
            ...
            instance.Button:SetDisabled(not g_bIsLocalPlayerTurn or
        ...
        local wasTruncated = TruncateString(instance.ButtonText,
                                            MAX_BEFORE_TRUNC_BUTTON_INST,
                                            selectionText)
        if wasTruncated then
            local finalTooltipString = selectionText
            if tooltipString ~= nil then
                finalTooltipString = finalTooltipString .. "[NEWLINE]" ..
                                         tooltipString
            end
        ...
        if selection.Tooltip then
            ...
            instance.Button:SetToolTipString(
                currentTooltipString .. Locale.Lookup(selection.Tooltip))
        end
...
function ShouldAddTopLevelStatementOption(uiGroup) return true end

-- ===========================================================================
function GetInitialStatementOptions(parsedStatements, rootControl)
    local discussOptions = {}
    local warOptions = {}
    local topOptions = {}
...
-- ===========================================================================
function AddStatmentOptions(rootControl)

    g_ActionListIM:ResetInstances()
    g_SubActionListIM:ResetInstances()

    if (ms_LocalPlayerID ~= -1 and ms_SelectedPlayerID ~= -1) then
        local useStatementType = (ms_LocalPlayerID ~= ms_SelectedPlayerID) and
                                     "GREETING" or "NO_TARGET"
        ...
        local kParsedStatement = handler.ExtractStatement(handler,
                                                          ...
                                                          "NONE",
                                                          ms_LocalPlayerID,
                                                          DiplomacyMoodTypes.ANY,
                                                          DiplomacyInitiatorTypes.HUMAN)
        handler.RemoveInvalidSelections(kParsedStatement, ms_LocalPlayerID,
                                        ms_SelectedPlayerID)
        ...
        if kParsedStatement and kParsedStatement.Selections then
            local topOptions = GetInitialStatementOptions(
                                   kParsedStatement.Selections, rootControl)
...
-- ===========================================================================
function OnActivateIntelRelationshipPanel(relationshipInstance)
    ...
    local iState = selectedPlayerDiplomaticAI:GetDiplomaticStateIndex(
                       ms_LocalPlayerID)
    local kStateEntry = GameInfo.DiplomaticStates[iState]
    local eState = kStateEntry.Hash
    intelSubPanel.RelationshipText:LocalizeAndSetText(
        Locale.ToUpper(kStateEntry.Name))
    ...
    intelSubPanel.RelationshipIcon:SetOffsetX(
        relationshipPercent * intelSubPanel.RelationshipBar:GetSizeX())
    intelSubPanel.RelationshipIcon:SetVisState(
        GetVisStateFromDiplomaticState(iState))
        ...
        for i, tip in ipairs(toolTips) do
            ...
            if (score ~= 0) then
                ...
                if (score > 0) then
                    ...
                    relationshipReason.Score:SetText(
                        "[COLOR_Civ6Red]" .. scoreText .. "[ENDCOLOR]")
                end

                if (text == "LOC_TOOLTIP_DIPLOMACY_UNKNOWN_REASON") then
                    relationshipReason.Text:SetText(
                        "[COLOR_Grey]" .. Locale.Lookup(text) .. "[ENDCOLOR]")
                else
    ...
    if GameCapabilities.HasCapability("CAPABILITY_DIPLOMACY_RELATIONSHIP_INFO") then
        ...
        intelSubPanel.AdvisorIcon:SetTexture(
            IconManager:FindIconAtlas("ADVISOR_GENERIC", 32))
        ...
        advisorTextlower = advisorTextlower ..
        ...
        advisorTextlower = advisorTextlower ..
                               Locale.Lookup(
                                   "LOC_DIPLOMACY_ADVISOR_TRADE_ROUTE",
                                   selectedCivName)
        advisorTextlower = advisorTextlower .. "[NEWLINE]"
        if (not ms_SelectedPlayer:GetDiplomacy():HasOpenBordersFrom(
            ms_LocalPlayer:GetID())) then
            advisorTextlower = advisorTextlower ..
                                   Locale.Lookup(
                                       "LOC_DIPLOMACY_ADVISOR_OPEN_BORDERS",
                                       selectedCivName)
            advisorTextlower = advisorTextlower .. "[NEWLINE]"
        end
        if (not ms_LocalPlayer:GetDiplomacy():HasDelegationAt(
            ms_SelectedPlayer:GetID()) and
            not ms_LocalPlayer:GetDiplomacy():HasEmbassyAt(
                ms_SelectedPlayer:GetID())) then
            advisorTextlower = advisorTextlower ..
                                   Locale.Lookup(
                                       "LOC_DIPLOMACY_ADVISOR_DELEGATION_EMBASSY")
            advisorTextlower = advisorTextlower .. "[NEWLINE]"
        end
        advisorTextlower = advisorTextlower ..
                               Locale.Lookup(
                                   "LOC_DIPLOMACY_ADVISOR_POSITIVE_AGENDA",
                                   selectedCivName)
        advisorTextlower = advisorTextlower .. "[NEWLINE]"
        ...
        --		advisorTextRaise = advisorTextRaise .. "[NEWLINE]";
        advisorTextRaise = advisorTextRaise ..
                               Locale.Lookup(
                                   "LOC_DIPLOMACY_ADVISOR_NEGATIVE_AGENDA",
                                   selectedCivName)
        advisorTextRaise = advisorTextRaise .. "[NEWLINE]"
        advisorTextRaise = advisorTextRaise ..
                               Locale.Lookup(
                                   "LOC_DIPLOMACY_ADVISOR_DENOUNCE_THEM")
        advisorTextRaise = advisorTextRaise .. "[NEWLINE]"
        advisorTextRaise = advisorTextRaise ..
                               Locale.Lookup(
                                   "LOC_DIPLOMACY_ADVISOR_REJECT_PROMISE")
        advisorTextRaise = advisorTextRaise .. "[NEWLINE]"
        advisorTextRaise = advisorTextRaise ..
                               Locale.Lookup(
                                   "LOC_DIPLOMACY_ADVISOR_BREAK_PROMISE")
        advisorTextRaise = advisorTextRaise .. "[NEWLINE]"
...
-- ===========================================================================
function OnActivateIntelAccessLevelPanel(accessLevelInstance)
    ...
    local iAccessLevel = localPlayerDiplomacy:GetVisibilityOn(
                             ms_SelectedPlayerID)
    ...
    for row in GameInfo.DiplomaticVisibilitySources() do
        if (localPlayerDiplomacy:IsVisibilitySourceActive(ms_SelectedPlayerID,
                                                          row.Index)) then
            if (row.Description ~= nil) then
                if (#accessContributionText > 0) then
                    accessContributionText =
                        accessContributionText .. "[NEWLINE]"
                end
                accessContributionText =
                    accessContributionText .. Locale.Lookup(row.Description)
            end
        end
    end
    ...
    intelSubPanel.AccessLevelText:LocalizeAndSetText(
        Locale.ToUpper(GameInfo.Visibilities[iAccessLevel].Name))
    ...
    for row in GameInfo.Gossips() do
        if (row.VisibilityLevel == iAccessLevel) then
            if (row.Description ~= nil) then
                ...
                szInfoSharedText = szInfoSharedText .. "   " ..
    ...
    if GameCapabilities.HasCapability("CAPABILITY_DIPLOMACY_ACCESS_LEVEL_INFO") then
        ...
        for row in GameInfo.Gossips() do
            if (row.VisibilityLevel == iAccessLevel + 1) then
                if (row.Description ~= nil) then
                    if (iNumAdded > 0) then
                        szNextAccessLevelText =
                            szNextAccessLevelText .. "[NEWLINE]"
                    end
                    szNextAccessLevelText =
                        szNextAccessLevelText .. "   " ..
                            Locale.Lookup(row.Description)
        ...
        local advisorText = ""
        for row in GameInfo.DiplomaticVisibilitySources() do
            if (not localPlayerDiplomacy:IsVisibilitySourceActive(
                ms_SelectedPlayerID, row.Index)) then
                if (row.ActionDescription ~= nil) then
                    advisorText = advisorText ..
                                      Locale.Lookup(row.ActionDescription) ..
                                      "[NEWLINE]"
                end
            end
        end
...
-- ===========================================================================
function OnActivateIntelGossipHistoryPanel(gossipInstance)
...
-- ===========================================================================
function AutoSizeGrid(gridControl, labelControl, padding, minSize)
    local sizeY = labelControl:GetSizeY() + padding
    if (sizeY < minSize) then sizeY = minSize end
    gridControl:SetSizeY(sizeY)
end

-- ===========================================================================
function GetSelectedPlayerID() return ms_SelectedPlayerID end

-- ===========================================================================
function AddIntelPanel(rootControl)

    -- Reset panel instance
    ms_IntelPanel = nil
    ms_IntelPanelIM:ResetInstances()

    if (ms_LocalPlayerID ~= -1 and ms_SelectedPlayerID ~= -1 and
        ms_LocalPlayerID ~= ms_SelectedPlayerID) then
...
-- ===========================================================================
function PopulateIntelPanels(tabContainer)
    AddIntelOverview()
    AddIntelGossip()
    AddIntelAccessLevel()

    -- Don't add this tab if the civ in question is human-controlled, it makes no sense
    local tPlayer = Players[ms_SelectedPlayerID]
    if (tPlayer and not tPlayer:IsHuman()) then AddIntelRelationship() end
end
...
-- ===========================================================================
function SetIntelPanelHeader(header)
    if ms_IntelPanel == nil then return end

    ms_IntelPanel.IntelHeader:SetText(header)
end

-- ===========================================================================
function AddIntelTab(tabPanelIM, buttonTooltip, headerText, buttonIcon)
    -- Create tab panel
    tabPanelIM:ResetInstances()
    local tabPanelInstance = tabPanelIM:GetInstance(
                                 ms_IntelPanel.IntelPanelContainer)

    -- Create tab button
    local tabButtonInstance = ms_IntelTabButtonIM:GetInstance(
                                  ms_IntelPanel.IntelTabButtonStack)
    tabButtonInstance.Button:RegisterCallback(Mouse.eLClick, function()
        ShowPanel(tabPanelInstance:GetTopControl())
    end)
...
function AddIntelOverview()
    ...
    local uiOverviewInst = AddIntelTab(ms_IntelOverviewIM, Locale.Lookup(
                                           "LOC_DIPLOMACY_INTEL_OVERVIEW_COLON_TOOLTIP"),
                                       Locale.ToUpper(
                                           "LOC_DIPLOMACY_INTEL_REPORT_OVERVIEW"),
                                       "ICON_OVERVIEW")
...
-- ===========================================================================
function AddIntelOverviewDivider(uiOverviewInst)
    ms_IntelOverviewDividerIM:GetInstance(uiOverviewInst.IntelOverviewStack)
end

-- ===========================================================================
function PopulateIntelOverview(overviewInstance)
...
-- ===========================================================================
function GetOverviewAnchor(parent)
    return ms_IntelOverviewAnchorIM:GetInstance(parent)
end

-- ===========================================================================
function GetTabAnchor(parent) return ms_IntelTabAnchorIM:GetInstance(parent) end

-- ===========================================================================
function AddOverviewGossip(overviewInstance)
    ms_IntelOverviewGossipIM:ResetInstances()
    local overviewGossipInst = ms_IntelOverviewGossipIM:GetInstance(
                                   overviewInstance.IntelOverviewStack)

    -- Determine if there is any gossip in the last two turns
    local gossipThisTurn = 0
    local gossipStringTable =
        Game.GetGossipManager():GetRecentVisibleGossipStrings(
            Game.GetCurrentGameTurn() - 1, ms_LocalPlayerID, ms_SelectedPlayerID)
    for i, gossip in pairs(gossipStringTable) do
        gossipThisTurn = gossipThisTurn + 1
    end

    if (gossipThisTurn > 0) then
        overviewGossipInst.GossipText:SetText(
            Locale.Lookup("LOC_DIPLOMACY_GOSSIP_ITEM_COUNT", gossipThisTurn))
    else
        overviewGossipInst.GossipText:SetText(
            Locale.Lookup("LOC_DIPLOMACY_GOSSIP_ITEM_NONE_THIS_TURN"))
    end
end

-- ===========================================================================
function AddOverviewAccessLevel(overviewInstance)
    ms_IntelOverviewAccessLevelIM:ResetInstances()
    local overviewAccessLevelInst = ms_IntelOverviewAccessLevelIM:GetInstance(
                                        overviewInstance.IntelOverviewStack)
    ...
    overviewAccessLevelInst.AccessLevelText:LocalizeAndSetText(
        GameInfo.Visibilities[iAccessLevel].Name)
end

-- ===========================================================================
function AddOverviewGovernment(overviewInstance)
    ms_IntelOverviewGovernmentIM:ResetInstances()
    local overviewGovernmentInst = ms_IntelOverviewGovernmentIM:GetInstance(
                                       overviewInstance.IntelOverviewStack)

    -- What Government does the selected player have?
    local eSelectedPlayerGovernment = ms_SelectedPlayer:GetCulture()
                                          :GetCurrentGovernment()
    if eSelectedPlayerGovernment ~= -1 then
        overviewGovernmentInst.GovernmentText:LocalizeAndSetText(
            GameInfo.Governments[eSelectedPlayerGovernment].Name)
    elseif ms_SelectedPlayer:GetCulture():IsInAnarchy() then
        local iAnarchyTurns =
            ms_SelectedPlayer:GetCulture():GetAnarchyEndTurn() -
                Game.GetCurrentGameTurn()
        overviewGovernmentInst.GovernmentText:LocalizeAndSetText(
            "LOC_GOVERNMENT_ANARCHY_TURNS", iAnarchyTurns)
    else
        overviewGovernmentInst.GovernmentText:LocalizeAndSetText(
            "LOC_DIPLOMACY_GOVERNMENT_NONE")
    end
end

function GetIntelOverviewAgendas() return ms_IntelOverviewAgendasIM end

function GetIntelOverviewAgendaEntries() return ms_IntelOverviewAgendaEntryIM end

-- ===========================================================================
function AddOverviewAgendas(overviewInstance)
    ms_IntelOverviewAgendasIM:ResetInstances()
    local overviewAgendasInst = ms_IntelOverviewAgendasIM:GetInstance(
                                    overviewInstance.IntelOverviewStack)
    ...
    if (PlayerConfigurations[ms_SelectedPlayerID]:IsHuman()) then
        ...
        local leader =
            PlayerConfigurations[ms_SelectedPlayerID]:GetLeaderTypeName()
        ...
        for row in GameInfo.HistoricalAgendas() do
            if (row.LeaderType == leader) then
                ...
                if (agenda) then
                    local historicalAgenda =
                        ms_IntelOverviewAgendaEntryIM:GetInstance(
                            overviewAgendasInst.OverviewAgendasStack)
                    historicalAgenda.Text:LocalizeAndSetText(
                        GameInfo.Agendas[agendaType].Name)
                    historicalAgenda.Text:LocalizeAndSetToolTip(
                        GameInfo.Agendas[agendaType].Description)
            ...
            if (bRevealRandom) then
                ...
                for i, agendaType in ipairs(kAgendaTypes) do
                    local randomAgenda =
                        ms_IntelOverviewAgendaEntryIM:GetInstance(
                            overviewAgendasInst.OverviewAgendasStack)
                    randomAgenda.Text:LocalizeAndSetText(
                        GameInfo.Agendas[agendaType].Name)
                    randomAgenda.Text:LocalizeAndSetToolTip(
                        GameInfo.Agendas[agendaType].Description)
                end
            else
                -- Otherwise, display that how many hidden agendas there are, and incentivize player to gain visibility to see them!
                local hiddenAgenda = ms_IntelOverviewAgendaEntryIM:GetInstance(
                                         overviewAgendasInst.OverviewAgendasStack)
                hiddenAgenda.Text:LocalizeAndSetText(
                    "LOC_DIPLOMACY_HIDDEN_AGENDAS", numRandomAgendas,
                    numRandomAgendas > 1)
                hiddenAgenda.Text:LocalizeAndSetToolTip(
                    "LOC_DIPLOMACY_HIDDEN_AGENDAS_TT")
            end
        elseif (numRandomAgendas == 0) then
            local noRandomAgendas = ms_IntelOverviewAgendaEntryIM:GetInstance(
                                        overviewAgendasInst.OverviewAgendasStack)
            noRandomAgendas.Text:LocalizeAndSetText(
                "LOC_DIPLOMACY_RANDOM_AGENDA_NONE")
...
-- ===========================================================================
function AddOverviewAgreements(overviewInstance)
    ms_IntelOverviewAgreementsIM:ResetInstances()
    local overviewAgreementsInst = ms_IntelOverviewAgreementsIM:GetInstance(
                                       overviewInstance.IntelOverviewStack)
    ...
    if (localPlayerDiplomacy:IsFightingAnyJointWarWith(ms_SelectedPlayer:GetID())) then
        AddAgreementEntry(overviewAgreementsInst, "ICON_DIPLOACTION_JOINT_WAR",
                          "LOC_DIPLOACTION_JOINT_WAR_NAME")
    end
...
function AddAgreementEntry(agreementInstance, icon, tooltip)
    local agreement =
        ms_IconOnlyIM:GetInstance(agreementInstance.AgreementStack)
    agreement.Icon:SetIcon(icon)
    agreement.Icon:SetToolTipString(Locale.Lookup(tooltip))
end

-- ===========================================================================
function AddOverviewOurRelationship(overviewInstance)
    ms_IntelOverviewOurRelationshipIM:ResetInstances()
    local overviewOurRelationshipInst =
        ms_IntelOverviewOurRelationshipIM:GetInstance(
            overviewInstance.IntelOverviewStack)
    ...
    if (PlayerConfigurations[ms_SelectedPlayerID]:IsHuman()) then
        ...
        local iState = selectedPlayerDiplomaticAI:GetDiplomaticStateIndex(
                           ms_LocalPlayerID)
        local relationshipString = Locale.Lookup(
                                       GameInfo.DiplomaticStates[iState].Name)
        -- Add team name to relationship text for our own teams
        if Players[ms_LocalPlayerID]:GetTeam() ==
            Players[ms_SelectedPlayerID]:GetTeam() then
            relationshipString = "(" ..
                                     Locale.Lookup("LOC_WORLD_RANKINGS_TEAM",
                                                   Players[ms_LocalPlayerID]:GetTeam()) ..
                                     ") " .. relationshipString
        end
        ...
        if (GameInfo.DiplomaticStates[iState].StateType ==
            "DIPLO_STATE_DENOUNCED") then
            local szDenounceTooltip
            local iRemainingTurns
            local iOurDenounceTurn = localPlayerDiplomacy:GetDenounceTurn(
                                         ms_SelectedPlayerID)
            local iTheirDenounceTurn =
                Players[ms_SelectedPlayerID]:GetDiplomacy():GetDenounceTurn(
                    ms_LocalPlayerID)
            ...
            if (iOurDenounceTurn >= iTheirDenounceTurn) then
                iRemainingTurns = 1 + iOurDenounceTurn +
                                      Game.GetGameDiplomacy():GetDenounceTimeLimit() -
                                      Game.GetCurrentGameTurn() +
                                      iPlayerOrderAdjustment
                szDenounceTooltip = Locale.Lookup(
                                        "LOC_DIPLOMACY_DENOUNCED_TOOLTIP",
                ...
                iRemainingTurns = 1 + iTheirDenounceTurn +
                                      Game.GetGameDiplomacy():GetDenounceTimeLimit() -
                                      Game.GetCurrentGameTurn() +
                                      iPlayerOrderAdjustment
                szDenounceTooltip = Locale.Lookup(
            ...
            szDenounceTooltip = szDenounceTooltip .. " [" ..
                                    Locale.Lookup(
                                        "LOC_ESPIONAGEPOPUP_TURNS_REMAINING",
                                        iRemainingTurns) .. "]"
            overviewOurRelationshipInst.RelationshipText:SetToolTipString(
                szDenounceTooltip)
        elseif (GameInfo.DiplomaticStates[iState].StateType ==
            "DIPLO_STATE_DECLARED_FRIEND") then
            local szFriendTooltip
            local iFriendshipTurn =
                localPlayerDiplomacy:GetDeclaredFriendshipTurn(
                    ms_SelectedPlayerID)
            local iRemainingTurns = iFriendshipTurn +
                                        Game.GetGameDiplomacy():GetDenounceTimeLimit() -
                                        Game.GetCurrentGameTurn()
            szFriendTooltip = Locale.Lookup(
                                  "LOC_DIPLOMACY_DECLARED_FRIENDSHIP_TOOLTIP",
                                  PlayerConfigurations[ms_LocalPlayerID]:GetCivilizationShortDescription(),
                                  PlayerConfigurations[ms_SelectedPlayerID]:GetCivilizationShortDescription(),
                                  iRemainingTurns)
            overviewOurRelationshipInst.RelationshipText:SetToolTipString(
        ...
        overviewOurRelationshipInst.RelationshipIcon:SetVisState(
            GetVisStateFromDiplomaticState(iState))
    end
...
-- ===========================================================================
function AddOverviewOtherRelationships(overviewInstance)
    ms_IntelOverviewOtherRelationshipsIM:ResetInstances()
    local overviewOtherRelationshipsInst =
        ms_IntelOverviewOtherRelationshipsIM:GetInstance(
            overviewInstance.IntelOverviewStack)
    ...
    for _, pPlayer in ipairs(aPlayers) do
        ...
        if (pPlayer:IsMajor() and playerID ~= ms_LocalPlayerID and playerID ~=
            ms_SelectedPlayer:GetID() and
            selectedPlayerDiplomacy:HasMet(playerID)) then
            local playerConfig = PlayerConfigurations[playerID]
            local leaderTypeName = playerConfig:GetLeaderTypeName()
            if (leaderTypeName ~= nil) then
                local relationshipIcon =
                    ms_RelationshipIconsIM:GetInstance(
                        overviewOtherRelationshipsInst.RelationshipsStack)
                local iPlayerDiploState =
                    pPlayer:GetDiplomaticAI():GetDiplomaticStateIndex(
                        ms_SelectedPlayer:GetID())
                local kRelationship =
                    GameInfo.DiplomaticStates[iPlayerDiploState]
                local isRelationHidden = true
                ...
                if (kRelationship.Hash ~= DiplomaticStates.NEUTRAL) then
                    local isHuman = not (ms_SelectedPlayer:IsAI() or
                                        pPlayer:IsAI())
                    local relationType = kRelationship.StateType
                    local isValid = (isHuman and
                                        Relationship.IsValidWithHuman(
                                            relationType)) or
                                        (not isHuman and
                                            Relationship.IsValidWithAI(
                                                relationType))
                    if isValid then
                        relationshipIcon.Status:SetToolTipString(
                            Locale.Lookup(kRelationship.Name))
                ...
                if (pLocalPlayerDiplomacy:HasMet(playerID)) then
                    relationshipIcon.Icon:SetTexture(
                        IconManager:FindIconAtlas(
                            "ICON_" .. playerConfig:GetLeaderTypeName(), 32))
                    -- Tool tip
                    local leaderDesc = playerConfig:GetLeaderName()
                    relationshipIcon.Background:LocalizeAndSetToolTip(
                        "LOC_DIPLOMACY_DEAL_PLAYER_PANEL_TITLE", leaderDesc,
                        playerConfig:GetCivilizationDescription())

                    -- Show team ribbon for ourselves and civs we've met
                    local teamID = playerConfig:GetTeam()
                    if #Teams[teamID] > 1 then
                        local teamRibbonName =
                            TEAM_RIBBON_PREFIX .. tostring(teamID)
                        relationshipIcon.TeamRibbon:SetIcon(teamRibbonName,
                                                            TEAM_RIBBON_SMALL_SIZE)
                        relationshipIcon.TeamRibbon:SetHide(false)
                        relationshipIcon.TeamRibbon:SetColor(
                            GetTeamColor(teamID))
                    else
                    ...
                    relationshipIcon.Icon:SetTexture(
                        IconManager:FindIconAtlas("ICON_LEADER_DEFAULT", 32))
                    relationshipIcon.Background:LocalizeAndSetToolTip(
                        "LOC_DIPLOPANEL_UNMET_PLAYER")
                    relationshipIcon.TeamRibbon:SetHide(true)
                end
            end
        end
...
function AddIntelGossip()
    local gossipInstance = AddIntelTab(ms_IntelGossipIM, Locale.Lookup(
                                           "LOC_DIPLOMACY_INTEL_GOSSIP_COLON_TOOLTIP"),
                                       Locale.ToUpper(
                                           "LOC_DIPLOMACY_INTEL_REPORT_GOSSIP"),
                                       "ICON_GOSSIP")
...
function AddIntelAccessLevel()
    local accessLevelInstance = AddIntelTab(ms_IntelAccessLevelIM,
                                            Locale.Lookup(
                                                "LOC_DIPLOMACY_INTEL_ACCESS_LEVEL_COLON_TOOLTIP"),
                                            Locale.ToUpper(
                                                "LOC_DIPLOMACY_INTEL_REPORT_ACCESS_LEVEL"),
                                            "ICON_ACCESS_LEVEL")
...
function AddIntelRelationship()
    local relationshipInstance = AddIntelTab(ms_IntelRelationshipIM,
                                             Locale.Lookup(
                                                 "LOC_DIPLOMACY_INTEL_OUR_RELATIONSHIP_TOOLTIP"),
                                             Locale.ToUpper(
                                                 "LOC_DIPLOMACY_INTEL_REPORT_RELATIONSHIP"),
                                             "ICON_RELATIONSHIP")
...
function ShowOverviewPanel()
    local overviewInstance = ms_IntelOverviewIM:GetAllocatedInstance()
...
-- ===========================================================================
function ShowPanel(panelInstance)
    if ms_IntelPanel == nil then return end

    -- Show panel
    if panelInstance then panelInstance:SetHide(false) end

    -- Hide other panels
    local intelPanelList = ms_IntelPanel.IntelPanelContainer:GetChildren()
    for i, child in ipairs(intelPanelList) do
        if child ~= panelInstance then child:SetHide(true) end
    end
    ...
    if panelInstance.m_ButtonInstance ~= nil then
        for i = 1, ms_IntelTabButtonIM.m_iCount, 1 do
            local buttonInstance = ms_IntelTabButtonIM:GetAllocatedInstance(i)
...
-- ===========================================================================
function PopulatePlayerPanel(rootControl, player)
    if (player ~= nil) then
        ...
        rootControl.RootOptionStack:RegisterSizeChanged(
            OnRootOptionStackSizeChanged)
    end
end

-- ===========================================================================
function ShowOptionStack(showSubOptions)
    if not m_LiteMode then
        ...
        ms_PlayerPanel.IntelContainer:SetSizeY(
            ms_PlayerPanel.ContentContainer:GetSizeY())
    end
end
...
function OnRootOptionStackSizeChanged()
    if ms_PlayerPanel ~= nil then
        ...
        local fillSize = ms_PlayerPanel.ContentContainer:GetSizeY() -
                             ms_PlayerPanel.RootOptionStack:GetSizeY()
        ms_PlayerPanel.IntelContainer:SetSizeY(fillSize)
    end
end

-- ===========================================================================
function PopulatePlayerPanelHeader(rootControl, player)

    if (player ~= nil) then
        local playerConfig = PlayerConfigurations[player:GetID()]
        if (playerConfig ~= nil) then
            -- Set the civ icon
            local civIconController = CivilizationIcon:AttachInstance(
                                          rootControl.CivIcon)
            civIconController:UpdateIconFromPlayerID(player:GetID())

            -- Set the leader name
            local leaderDesc = playerConfig:GetLeaderName()
            rootControl.PlayerNameText:LocalizeAndSetText(
                Locale.ToUpper(Locale.Lookup(leaderDesc)))
            rootControl.CivNameText:LocalizeAndSetText(
...
-- ===========================================================================
function PopulateLeader(leaderIcon, player, isUniqueLeader)
    ...
    if (player ~= nil and player:IsMajor()) then
        ...
        if (playerConfig ~= nil) then
            ...
            if (leaderTypeName ~= nil) then
                ...
                leaderIcon:RegisterCallback(Mouse.eMouseEnter, function()
                    UI.PlaySound("Main_Menu_Mouse_Over")
                end)
                leaderIcon:RegisterCallback(Mouse.eLClick, OnPlayerSelected)

                -- The selection background
                leaderIcon.Controls.SelectedBackground:SetHide(
                    playerID ~= ms_SelectedPlayerID)

                -- CUI: use advenced tooltip
                local allianceData = CuiGetAllianceData(playerID)
                LuaEvents.CuiLeaderIconToolTip(leaderIcon.Controls.Portrait,
                                               playerID)
                LuaEvents.CuiRelationshipToolTip(
                    leaderIcon.Controls.Relationship, playerID, allianceData)
                --
            end
        end
    end
...
-- ===========================================================================
function SetConversationMode(player)
...
-- ===========================================================================
function ScrollToNode(playerID)
    local percent = 0
    ...
    if (m_LeaderCoordinates ~= nil) and (m_LeaderCoordinates[playerID] ~= nil) then
        ...
        local size = (scrollHeight /
                         ms_DiplomacyRibbon.LeaderRibbonScroll:GetRatio()) -
                         scrollHeight
...
function SelectPlayer(playerID, mode, refresh, allowDeadPlayer)
    if (mode == nil) then mode = ms_currentViewMode end
    if (refresh == nil) then refresh = false end

    if (allowDeadPlayer == nil) then allowDeadPlayer = false end

    local isDifferentPlayer = false
    if (ms_SelectedPlayerID ~= playerID or mode ~= ms_currentViewMode or refresh ==
        true) then
    ...
    if (ms_SelectedPlayerID == ms_LocalPlayerID) then
        ...
        if (playerConfig ~= nil) then
            ...
            local leaderDesc = playerConfig:GetLeaderName()
            Controls.PlayerNameText:LocalizeAndSetText(
                Locale.ToUpper(Locale.Lookup(leaderDesc)))
            Controls.CivNameText:LocalizeAndSetText(
                Locale.ToUpper(Locale.Lookup(
                                   playerConfig:GetCivilizationDescription())))
...
function SetUniqueCivLeaderData()
    ...
    for i, v in ipairs(CivUniqueBuildings) do
        table.insert(uniqueBuildings, v)
    end
...
function OnPlayerSelected(playerID)
    if (HasCapability("CAPABILITY_DIPLOMACY") or
        (playerID == Game.GetLocalPlayer() and
            HasCapability("CAPABILITY_DIPLOMACY_VIEW_SELF"))) then
        ResetPlayerPanel()
...
-- ===========================================================================
function PopulateDiplomacyRibbon(diplomacyRibbon)
    ...
    if (ms_LocalPlayer ~= nil) then
        ...
        table.sort(aPlayers, function(a, b)
            return pLocalPlayerDiplomacy:GetMetTurn(a:GetID()) <
                       pLocalPlayerDiplomacy:GetMetTurn(b:GetID())
        end)
        --
        for _, pPlayer in ipairs(aPlayers) do
            if (pPlayer:GetID() ~= ms_LocalPlayerID and
                pLocalPlayerDiplomacy:HasMet(pPlayer:GetID())) then
                local leaderIcon, leaderInstance =
                    LeaderIcon:GetInstance(ms_DiplomacyRibbonLeaderIM,
                                           diplomacyRibbon.Leaders)
                ...
                currentCoordinateY = currentCoordinateY +
                                         coordinateOffsetIncrement
                local leaderName =
                    PlayerConfigurations[pPlayer:GetID()]:GetLeaderTypeName()
                if (isUniqueLeader[leaderName] ~= nil) then
                    PopulateLeader(leaderIcon, pPlayer,
                                   isUniqueLeader[leaderName])
                else
                    PopulateLeader(leaderIcon, pPlayer)
                end
            end
        ...
        if (diplomacyRibbon.Leaders:GetSizeY() >
            diplomacyRibbon.LeaderRibbonScroll:GetSizeY()) then
            diplomacyRibbon.Root:SetOffsetX(15)
        end
        local offsetX = DIPLOMACY_RIBBON_OFFSET +
                            diplomacyRibbon.Root:GetOffsetX()
...
function SetupPlayers()
    ...
    if (ms_LocalPlayerID == -1) then
        ...
        ms_LocalPlayerLeaderID =
            PlayerConfigurations[ms_LocalPlayerID]:GetLeaderTypeID()
    end
    ...
    if (ms_OtherPlayerID ~= -1) then
        ...
        local sessionID = DiplomacyManager.FindOpenSessionID(ms_LocalPlayerID,
...
function ShowLeader(player)
    ...
    if (leaderName ~= ms_showingLeaderName) then
        ...
        Events.ShowLeaderScreen(leaderName,
                                player:GetID() == Game.GetLocalPlayer())
    ...
    if (not ms_bIsViewInitialized) then
        ...
        if not m_LiteMode then
            if m_eventID == 0 then
                ...
                UI.DataError(
                    "Attempt to obtain lock ID but m_eventID already has ID: " ..
                        tostring(m_eventID))
            end
        end
...
function OnOpenDiplomacyActionView(otherPlayerID)
    ...
    if (HasCapability("CAPABILITY_DIPLOMACY") or
        (otherPlayerID == Game.GetLocalPlayer() and
            HasCapability("CAPABILITY_DIPLOMACY_VIEW_SELF"))) then
...
-- ===========================================================================
function OnLeaderAnimationComplete(animationName)

    -- Getting this a little late?
    if (not ms_bIsViewInitialized) then return end
...
-- ===========================================================================
function OnSetDealAnimation(animationName, useMood)

    if (ms_currentViewMode == DEAL_MODE) then
        if (useMood ~= nil and useMood == true) then
            local ePlayerMood = DiplomacySupport_GetPlayerMood(
                                    ms_SelectedPlayer, ms_LocalPlayerID)
        ...
        LeaderSupport_QueueAnimationSequence(ms_SelectedPlayerLeaderTypeName,
                                             animationName)
    end
end
...
function ShowCinemaMode()
    ...
    local ePlayerMood = DiplomacySupport_GetPlayerMood(ms_SelectedPlayer,
                                                       ms_LocalPlayerID)
    LeaderSupport_QueueAnimationSequence(ms_OtherLeaderName,
                                         m_currentLeaderAnim, ePlayerMood)
...
function OnLeaderLoaded()
    ...
    if (m_lastLeaderPlayedMusicFor ~= ms_OtherLeaderID) then bDoAudio = true end
    ...
    if (bDoAudio == true) then
        ...
        if (UI.GetCivilizationSoundSwitchValueByLeader(ms_LocalPlayerLeaderID) ==
            -1) then UI.PauseModCivMusic() end
        ...
        if (m_lastLeaderPlayedMusicFor ~= ms_OtherLeaderID) then
            ...
            if (UI.GetCivilizationSoundSwitchValueByLeader(ms_OtherLeaderID) ==
                -1) then
                ...
                if (UI.GetCivilizationSoundSwitchValueByLeader(
                    m_lastLeaderPlayedMusicFor) ~=
                    UI.GetCivilizationSoundSwitchValueByLeader(ms_OtherLeaderID)) then
                    ...
                    UI.SetSoundSwitchValue("LEADER_SCREEN_CIVILIZATION",
                                           UI.GetCivilizationSoundSwitchValueByLeader(
                                               ms_OtherLeaderID))
                    UI.SetSoundSwitchValue("Game_Location",
...
-- ===========================================================================
function GetStatementHandler(statementTypeName)
    local handler = StatementHandlers[statementTypeName]
    if (handler == nil) then handler = DefaultHandlers end
    return handler
end

-- ===========================================================================
function OnDiplomacyMakePeace(eActingPlayer, eReactingPlayer)
    local localPlayer = Game.GetLocalPlayer()
    if (ms_SelectedPlayerID ~= -1 and
        (localPlayer == eActingPlayer or localPlayer == eReactingPlayer) and
        (ms_SelectedPlayerID == eActingPlayer or ms_SelectedPlayerID ==
            eReactingPlayer)) then
...
function HasNextQueuedSession(sessionID)
    if (ms_ActiveSessionID ~= nil and ms_ActiveSessionID == sessionID and
        not m_isInHotload) then
    ...
    if (ms_ActiveSessionID ~= nil) then
        ...
        if (ms_currentViewMode == OVERVIEW_MODE) then
            ...
            if (sessionInfo ~= nil) then
                if (sessionInfo.FromPlayer == Game.GetLocalPlayer() or
                    sessionInfo.ToPlayer == Game.GetLocalPlayer()) then
...
function SetDefaultHandlers(forStatement)
    ...
    StatementHandlers[forStatement].ExtractStatement =
        DiplomacySupport_ExtractStatement
    StatementHandlers[forStatement].ParseStatement =
        DiplomacySupport_ParseStatement
    StatementHandlers[forStatement].ParseStatementSelection =
        DiplomacySupport_ParseStatementSelection
    StatementHandlers[forStatement].RemoveInvalidSelections =
        DiplomacySupport_RemoveInvalidSelections
    StatementHandlers[forStatement].ApplyStatement = ApplyStatement
    StatementHandlers[forStatement].OnSelectionButtonClicked =
...
-------------------------------------------------------------------------------
function MakeDeal_ApplyStatement(handler, statementTypeName,
                                 statementSubTypeName, toPlayer, kStatement)

    -- Initial statement or the ackknowledgement of the initial statement?
    if ((kStatement.DealAction == DealProposalAction.ADJUSTED and
        not ms_bShowingDeal) or (statementSubTypeName == "NONE" and
        (kStatement.ResponseType == DiplomacyResponseTypes.INITIAL or
            kStatement.ResponseType == DiplomacyResponseTypes.ACKNOWLEDGE or
            not ms_bShowingDeal))) then

        ApplyStatement(handler, statementTypeName, statementSubTypeName,
                       toPlayer, kStatement)
    ...
    elseif (kStatement.RespondingToDealAction ~= DealProposalAction.EQUALIZE and
        kStatement.RespondingToDealAction ~= DealProposalAction.INSPECT and
        (statementSubTypeName == "HUMAN_ACCEPT_DEAL" or statementSubTypeName ==
            "HUMAN_REFUSE_DEAL" or statementSubTypeName == "AI_ACCEPT_DEAL" or
            statementSubTypeName == "AI_REFUSE_DEAL")) then
        ...
        if (kParsedStatement.LeaderAnimation ~= nil) then
            ...
            if (kStatement.RespondingToDealAction == DealProposalAction.EQUALIZE or
                kStatement.RespondingToDealAction == DealProposalAction.INSPECT) then
                -- We don't want to repeat the last animation
                if (ms_LastDealResponseAnimation ~= nil and
                    kParsedStatement.LeaderAnimation ==
                    ms_LastDealResponseAnimation) then
                    bPlay = false
                end
            end
        ...
        local leaderstr = Locale.Lookup(DiplomacyManager.FindTextKey(
                                            kParsedStatement.StatementText,
                                            kStatement.FromPlayer,
                                            kStatement.FromMood, toPlayer))
        LuaEvents.DiploPopup_DealUpdated(ms_OtherPlayerID,
                                         kStatement.DealAction, leaderstr)
    end
end
...
-------------------------------------------------------------------------------
function MakeDemand_ApplyStatement(handler, statementTypeName,
                                   statementSubTypeName, toPlayer, kStatement)

    ms_currentViewMode = DEAL_MODE

    if (statementSubTypeName == "NONE" and
        (kStatement.ResponseType == DiplomacyResponseTypes.INITIAL or
            kStatement.ResponseType == DiplomacyResponseTypes.ACKNOWLEDGE or
            not ms_bShowingDeal)) then
        ...
        ms_bShowingDeal = true
        UI.PlaySound("UI_Diplomacy_Menu_Change")
    elseif (statementSubTypeName == "HUMAN_ACCEPT_DEAL" or statementSubTypeName ==
        "HUMAN_REFUSE_DEAL" or statementSubTypeName == "AI_ACCEPT_DEAL" or
        statementSubTypeName == "AI_REFUSE_DEAL") then
...
-- ===========================================================================
function OnTalkToLeader(playerID) OnOpenDiplomacyActionView(playerID) end
...
function CloseFocusedState()
    ...
    elseif (ms_currentViewMode == CINEMA_MODE) then
        ...
        if (Controls.BlackFadeAnim:IsStopped()) then StartFadeOut() end
    elseif (ms_currentViewMode == DEAL_MODE) then
...
function OnDiplomacyStatement(fromPlayer, toPlayer, kVariants)
    ...
    if (toPlayer == localPlayer or fromPlayer == localPlayer) then
        ...
        local statementTypeName = DiplomacyManager.GetKeyName(
                                      kVariants.StatementType)
        if (statementTypeName ~= nil) then
            local statementSubTypeName =
                DiplomacyManager.GetKeyName(kVariants.StatementSubType)
            ...
            if (ms_ActiveSessionID == nil) then
                ...
                local pOtherPlayerConfig =
                    PlayerConfigurations[ms_OtherPlayerID]

                ms_OtherLeaderName = pOtherPlayerConfig:GetLeaderTypeName()
                ms_OtherCivilizationID =
                    pOtherPlayerConfig:GetCivilizationTypeID()
                ms_OtherLeaderID = pOtherPlayerConfig:GetLeaderTypeID()

                -- Check to see if the session is valid.
                if (handler.TestValid == nil or
                    handler.TestValid(ms_ActiveSessionID, ms_OtherPlayerID)) then
                    InitializeView()
                    SetupPlayers()
                    PopulateDiplomacyRibbon(ms_DiplomacyRibbon)
                else
            ...
            if (statementTypeName == "MAKE_DEAL") then
                ...
                if UI.IsPlayersLeaderAnimated(ms_OtherPlayerID) then
                    ...
                    local eFromPlayerMood =
                        GetStatementMood(kVariants.FromPlayer,
                                         kVariants.FromPlayerMood)
                    local kParsedStatement =
                        handler.ExtractStatement(handler, statementTypeName,
                                                 statementSubTypeName,
                                                 kVariants.FromPlayer,
                                                 eFromPlayerMood,
                                                 kVariants.Initiator)
                    for _, voiceoverAnimationName in ipairs(VOICEOVER_SUPPORT) do
                        if (voiceoverAnimationName ==
                            kParsedStatement.LeaderAnimation) then
            ...
            if (handler.ApplyStatement ~= nil) then
                handler.ApplyStatement(handler, statementTypeName,
                                       statementSubTypeName, toPlayer, kVariants)
...
function ResetPlayerPanel()
    ...
    if (ms_PlayerPanel ~= nil) then ShowOptionStack(false) end
end
...
function Close()
    ...
    if m_PopupDialog:IsOpen() then
        UI.DataError(
            "Closing DiplomacyActionView but it's popup dialog was open.")
        m_PopupDialog:Close()
    end

    local isCleanExit = UninitializeView()
    ...
    if ms_LocalPlayer then
        UI.SetSoundSwitchValue("Game_Location", UI.GetNormalEraSoundSwitchValue(
                                   ms_LocalPlayer:GetID()))
    end
    ...
    if isCleanExit then LuaEvents.DiplomacyActionView_ShowIngameUI() end
end
...
-- ===========================================================================
function SetButtonSelected(buttonControl, isSelected)
...
function OnLocalPlayerTurnBegin()
    g_bIsLocalPlayerTurn = true
    if (not ContextPtr:IsHidden()) then OnForceClose() end
end
...
function OnPlayerDefeat(player, defeat, eventID)
    ...
    if (localPlayer and localPlayer >= 0) then -- Check to see if there is any local player
        ...
        if (localPlayer == player) then Close() end
    end
end
...
-- ===========================================================================
function OnBlockingPopupShown() OnForceClose() end
...
function OnUserRequestClose()
    ...
    if (not ContextPtr:IsHidden() and ms_currentViewMode ~= DEAL_MODE) then
        ...
        m_PopupDialog:AddButton(Locale.Lookup("LOC_YES"), OnQuitYes, nil, nil,
...
function OnShutdown()
    ...
    LuaEvents.GameDebug_AddValue("DiplomacyActionView", "isHidden",
                                 ContextPtr:IsHidden())
    LuaEvents.GameDebug_AddValue("DiplomacyActionView", "otherPlayerID",
                                 ms_OtherPlayerID)
...
function OnGameDebugReturn(context, contextTable)
    if context == "DiplomacyActionView" and contextTable["isHidden"] ~= nil and
        not contextTable["isHidden"] then
...
function LateInitialize()
    ...
    LuaEvents.DiplomacyRibbon_OpenDiplomacyActionView.Add(
        OnOpenDiplomacyActionView)
...
if GameCapabilities.HasCapability("CAPABILITY_DIPLOMACY") then Initialize() end
