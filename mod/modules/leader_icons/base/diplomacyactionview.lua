...
local ms_ConversationSelectionIM =
    InstanceManager:new("ConversationSelectionInstance", "SelectionButton", Controls.ConversationSelectionStack)
...
function UpdateSelectedPlayer(allowDeadPlayer)
    ...
    if
        (ms_LocalPlayer:GetDiplomacy():HasMet(ms_SelectedPlayerID) and
            (allowDeadPlayer == true or ms_LeaderIDToRibbonEntry[ms_SelectedPlayerID] ~= nil))
     then
...
function IsWarChoice(key)
    local isWar =
        key == "CHOICE_DECLARE_SURPRISE_WAR" or key == "CHOICE_DECLARE_FORMAL_WAR" or key == "CHOICE_DECLARE_HOLY_WAR" or
        key == "CHOICE_DECLARE_LIBERATION_WAR" or
        key == "CHOICE_DECLARE_RECONQUEST_WAR" or
        key == "CHOICE_DECLARE_PROTECTORATE_WAR" or
        key == "CHOICE_DECLARE_COLONIAL_WAR" or
        key == "CHOICE_DECLARE_TERRITORIAL_WAR"
    return isWar
end
    ...
    if CanInitiateDiplomacyStatement() then
        ...
        elseif (key == "CHOICE_MAKE_PEACE") then
            ...
            if (not DealManager.HasPendingDeal(ms_LocalPlayerID, ms_SelectedPlayerID)) then
                ...
                local pDeal =
                    DealManager.GetWorkingDeal(DealDirection.OUTGOING, ms_LocalPlayer:GetID(), ms_SelectedPlayerID)
                if (pDeal ~= nil) then
...
function ApplyStatement(handler, statementTypeName, statementSubTypeName, toPlayer, kStatement)
    ...
    if (kParsedStatement.Selections ~= nil) then
        for _, selection in ipairs(kParsedStatement.Selections) do
            ...
            if (selection.IsDisabled == nil or selection.IsDisabled == false) then
                ...
                instance.SelectionButton:RegisterCallback(
                    Mouse.eMouseEnter,
                    function()
    ...
    Controls.LeaderResponseName:SetText(
        ...
        if (selection.Key ~= nil) then
            ...
            if (IsWarChoice(selection.Key)) then
                ...
                local iWarmongerPoints =
                    ms_LocalPlayer:GetDiplomacy():ComputeDOWWarmongerPoints(ms_SelectedPlayerID, eWarType)
...
function OnActivateIntelRelationshipPanel(relationshipInstance)
        ...
        for i, tip in ipairs(toolTips) do
            ...
            if (score ~= 0) then
                local relationshipReason =
                    ms_IntelRelationshipReasonIM:GetInstance(intelSubPanel.RelationshipReasonStack)
    ...
    if GameCapabilities.HasCapability("CAPABILITY_DIPLOMACY_RELATIONSHIP_INFO") then
        ...
        if
            (not ms_LocalPlayer:GetDiplomacy():HasDelegationAt(ms_SelectedPlayer:GetID()) and
                not ms_LocalPlayer:GetDiplomacy():HasEmbassyAt(ms_SelectedPlayer:GetID()))
         then
...
function OnActivateIntelGossipHistoryPanel(gossipInstance)
    ...
    local gossipStringTable =
        gossipManager:GetRecentVisibleGossipStrings(earliestTurn, ms_LocalPlayerID, ms_SelectedPlayerID)
...
function AddIntelTab(tabPanelIM, buttonTooltip, headerText, buttonIcon)
    ...
    tabButtonInstance.Button:RegisterCallback(
        Mouse.eLClick,
        function()
...
function AddIntelOverview()
    ...
    local uiOverviewInst =
        AddIntelTab(
        ms_IntelOverviewIM,
        Locale.Lookup("LOC_DIPLOMACY_INTEL_OVERVIEW_COLON_TOOLTIP"),
        Locale.ToUpper("LOC_DIPLOMACY_INTEL_REPORT_OVERVIEW"),
        "ICON_OVERVIEW"
    )
            ...
            if (row.LeaderType == leader) then
                ...
                if (agenda) then
                    local historicalAgenda =
                        ms_IntelOverviewAgendaEntryIM:GetInstance(overviewAgendasInst.OverviewAgendasStack)
...
function AddOverviewOurRelationship(overviewInstance)
    ...
    local overviewOurRelationshipInst =
        ms_IntelOverviewOurRelationshipIM:GetInstance(overviewInstance.IntelOverviewStack)
    ...
    if (PlayerConfigurations[ms_SelectedPlayerID]:IsHuman()) then
        ...
        if (GameInfo.DiplomaticStates[iState].StateType == "DIPLO_STATE_DENOUNCED") then
            ...
            szDenounceTooltip =
                szDenounceTooltip .. " [" .. Locale.Lookup("LOC_ESPIONAGEPOPUP_TURNS_REMAINING", iRemainingTurns) .. "]"
...
function AddOverviewOtherRelationships(overviewInstance)
    ...
    local overviewOtherRelationshipsInst =
        ms_IntelOverviewOtherRelationshipsIM:GetInstance(overviewInstance.IntelOverviewStack)
    ...
    for _, pPlayer in ipairs(aPlayers) do
        ...
        if
            (pPlayer:IsMajor() and playerID ~= ms_LocalPlayerID and playerID ~= ms_SelectedPlayer:GetID() and
                selectedPlayerDiplomacy:HasMet(playerID))
         then
            local playerConfig = PlayerConfigurations[playerID]
            local leaderTypeName = playerConfig:GetLeaderTypeName()
            if (leaderTypeName ~= nil) then
                local relationshipIcon =
                    ms_RelationshipIconsIM:GetInstance(overviewOtherRelationshipsInst.RelationshipsStack)
                ...
                if (kRelationship.Hash ~= DiplomaticStates.NEUTRAL) then
                    ...
                    local isValid =
                        (isHuman and Relationship.IsValidWithHuman(relationType)) or
                ...
                if (pLocalPlayerDiplomacy:HasMet(playerID)) then
                    relationshipIcon.Icon:SetTexture(
                        IconManager:FindIconAtlas("ICON_" .. playerConfig:GetLeaderTypeName(), 32)
                    )
                    -- Tool tip
                    local leaderDesc = playerConfig:GetLeaderName()
                    relationshipIcon.Background:LocalizeAndSetToolTip(
                        "LOC_DIPLOMACY_DEAL_PLAYER_PANEL_TITLE",
                        leaderDesc,
...
function AddIntelGossip()
    local gossipInstance =
        AddIntelTab(
        ms_IntelGossipIM,
        Locale.Lookup("LOC_DIPLOMACY_INTEL_GOSSIP_COLON_TOOLTIP"),
        Locale.ToUpper("LOC_DIPLOMACY_INTEL_REPORT_GOSSIP"),
        "ICON_GOSSIP"
    )
...
function AddIntelAccessLevel()
    local accessLevelInstance =
        AddIntelTab(
        ms_IntelAccessLevelIM,
        Locale.Lookup("LOC_DIPLOMACY_INTEL_ACCESS_LEVEL_COLON_TOOLTIP"),
        Locale.ToUpper("LOC_DIPLOMACY_INTEL_REPORT_ACCESS_LEVEL"),
        "ICON_ACCESS_LEVEL"
    )
...
function AddIntelRelationship()
    local relationshipInstance =
        AddIntelTab(
        ms_IntelRelationshipIM,
...
function OnPlayerSelected(playerID)
    if
        (HasCapability("CAPABILITY_DIPLOMACY") or
            (playerID == Game.GetLocalPlayer() and HasCapability("CAPABILITY_DIPLOMACY_VIEW_SELF")))
     then
        ResetPlayerPanel()
        ...
        for _, pPlayer in ipairs(aPlayers) do
            if (pPlayer:GetID() ~= ms_LocalPlayerID and pLocalPlayerDiplomacy:HasMet(pPlayer:GetID())) then
                local leaderIcon, leaderInstance =
                    LeaderIcon:GetInstance(ms_DiplomacyRibbonLeaderIM, diplomacyRibbon.Leaders)
...
function OnOpenDiplomacyActionView(otherPlayerID)
    if
        (HasCapability("CAPABILITY_DIPLOMACY") or
            (otherPlayerID == Game.GetLocalPlayer() and HasCapability("CAPABILITY_DIPLOMACY_VIEW_SELF")))
     then
    ...
    if (bDoAudio == true) then
        ...
        if (m_lastLeaderPlayedMusicFor ~= ms_OtherLeaderID) then
            ...
            if (UI.GetCivilizationSoundSwitchValueByLeader(ms_OtherLeaderID) == -1) then
                ...
                if
                    (UI.GetCivilizationSoundSwitchValueByLeader(m_lastLeaderPlayedMusicFor) ~=
...
function OnDiplomacyMakePeace(eActingPlayer, eReactingPlayer)
    ...
    if
        (ms_SelectedPlayerID ~= -1 and (localPlayer == eActingPlayer or localPlayer == eReactingPlayer) and
            (ms_SelectedPlayerID == eActingPlayer or ms_SelectedPlayerID == eReactingPlayer))
     then
...
function MakeDeal_ApplyStatement(handler, statementTypeName, statementSubTypeName, toPlayer, kStatement)
    ...
    if
        ((kStatement.DealAction == DealProposalAction.ADJUSTED and not ms_bShowingDeal) or
    ...
    elseif
        (kStatement.RespondingToDealAction ~= DealProposalAction.EQUALIZE and
            kStatement.RespondingToDealAction ~= DealProposalAction.INSPECT and
            (statementSubTypeName == "HUMAN_ACCEPT_DEAL" or statementSubTypeName == "HUMAN_REFUSE_DEAL" or
                statementSubTypeName == "AI_ACCEPT_DEAL" or
                statementSubTypeName == "AI_REFUSE_DEAL"))
     then
        ...
        if (kParsedStatement.LeaderAnimation ~= nil) then
            ...
            if
                (kStatement.RespondingToDealAction == DealProposalAction.EQUALIZE or
                    kStatement.RespondingToDealAction == DealProposalAction.INSPECT)
             then
                -- We don't want to repeat the last animation
                if
                    (ms_LastDealResponseAnimation ~= nil and
                        kParsedStatement.LeaderAnimation == ms_LastDealResponseAnimation)
                 then
                    bPlay = false
                end
            end
