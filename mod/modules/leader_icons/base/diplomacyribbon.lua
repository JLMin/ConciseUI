...
--	MEMBERS
-- ===========================================================================
local m_kLeaderIM = InstanceManager:new("LeaderInstance", "LeaderContainer",
...
function AddLeader(iconName, playerID, kProps)
    ...
    leaderIcon:RegisterCallback(Mouse.eLClick,
...
function FinishAddingLeader(playerID, uiLeader, kProps)
    ...
    local isHideScore = isMasked or
                            (not (Game.IsVictoryEnabled("VICTORY_SCORE") or
                                (not HasCapability("VICTORY_SCORE"))))
    local isHideMilitary = isMasked or
                               (not Game.IsVictoryEnabled("VICTORY_CONQUEST") or
                                   not GameCapabilities.HasCapability(
                                       "CAPABILITY_DISPLAY_TOP_PANEL_YIELDS"))
    ...
    local isHideCulture = isMasked or
                              (not HasCapability("CAPABILITY_CULTURE") or
                                  not GameCapabilities.HasCapability(
                                      "CAPABILITY_DISPLAY_TOP_PANEL_YIELDS"))
    local isHideGold = isMasked or (not HasCapability("CAPABILITY_GOLD") or
                           not GameCapabilities.HasCapability(
                               "CAPABILITY_DISPLAY_TOP_PANEL_YIELDS"))
    local isHideFaith = isMasked or (not HasCapability("CAPABILITY_RELIGION") or
                            not GameCapabilities.HasCapability(
                                "CAPABILITY_DISPLAY_TOP_PANEL_YIELDS"))
...
function UpdateLeaders()
    ...
    if localPlayerID ~= -1 then
        ...
        table.sort(kPlayers, function(a, b)
            return localDiplomacy:GetMetTurn(a:GetID()) <
                       localDiplomacy:GetMetTurn(b:GetID())
        end)

        AddLeader("ICON_" ..
                      PlayerConfigurations[localPlayerID]:GetLeaderTypeName(),
                  localPlayerID, {}) -- First, add local player.
    ...
    for _, pPlayer in ipairs(kPlayers) do
        ...
        if (playerID ~= localPlayerID) then
            ...
            local isHumanMP = (GameConfiguration.IsAnyMultiplayer() and
                                  pPlayerConfig:IsHuman())
            if (isMet or isHumanMP) then
                ...
                AddLeader(iconName, playerID,
                          {isMasked = isMasked, isUnique = isUnique})
            end
        end
    end
...
function RealizeSize(additionalElementsWidth)
    ...
    for _, uiLeader in ipairs(m_uiLeadersByID) do
        ...
        if m_ribbonStats == RibbonHUDStats.SHOW then
            m_leaderInstanceHeight = math.max(
                                         uiLeader.LeaderContainer:GetSizeY(),
                                         m_leaderInstanceHeight)
        else
    ...
    -- Obtain controls
    local uiPartialScreenHookRoot = ContextPtr:LookUpControl(
                                        "/InGame/PartialScreenHooks/RootContainer")
    local uiPartialScreenHookBar = ContextPtr:LookUpControl(
                                       "/InGame/PartialScreenHooks/ButtonStack")
    local uiLaunchBar =
        ContextPtr:LookUpControl("/InGame/LaunchBar/ButtonStack")

    if (uiLaunchBar ~= nil) then
        launchBarWidth = math.max(uiLaunchBar:GetSizeX() + WORLD_TRACKER_OFFSET,
                                  MIN_LEFT_HOOKS)
    end
    ...
    if m_leadersMet > 0 then
        ...
        if (m_leadersMet > g_maxNumLeaders) then
            bgSize = g_maxNumLeaders * (SIZE_LEADER + paddingLeader) +
                         additionalElementsWidth + BG_PADDING_EDGE
        else
            bgSize = m_leadersMet * (SIZE_LEADER + paddingLeader) +
                         additionalElementsWidth + BG_PADDING_EDGE
        end
        ...
        size = g_maxNumLeaders * (SIZE_LEADER + paddingLeader) + PADDING_EDGE +
...
function UpdateScroll(deltaTime)
    ...
    Controls.LeaderScroll:SetScrollValue(
        start + (destination - start) * m_scrollPercent)
end
...
function OnDiplomacySessionClosed(sessionID)
    ...
    if (localPlayerID ~= -1) then
        ...
        if (diplomacyInfo ~= nil and
            (diplomacyInfo.FromPlayer == localPlayerID or diplomacyInfo.ToPlayer ==
                localPlayerID)) then UpdateLeaders() end
    end
end
...
function UpdateStatValues(playerID, uiLeader)
    ...
    if uiLeader.Military:IsVisible() then
        local military = Round(Players[playerID]:GetStats()
                                   :GetMilitaryStrengthWithoutTreasury())
    ...
    elseif m_ribbonStats == RibbonHUDStats.FOCUS or m_ribbonStats ==
        RibbonHUDStats.HIDE then
...
function OnTurnBegin(playerID)
    ...
    if (uiLeader ~= nil) then
        ...
        local localPlayerID = Game.GetLocalPlayer()
        if (localPlayerID == PlayerTypes.NONE or localPlayerID ==
            PlayerTypes.OBSERVER) then return end
        -- Update the approripate animation (alpha vs slide) based on what mode is being used.
        if m_ribbonStats == RibbonHUDStats.SHOW then
            if (not (playerID == localPlayerID or
                Players[localPlayerID]:GetDiplomacy():HasMet(playerID))) then
                uiLeader.LeaderContainer:SetSizeVal(63, 63)
            end
            local pSize = uiLeader.LeaderContainer:GetSize()
            uiLeader.ActiveLeaderAndStats:SetSizeVal(
                pSize.x + LEADER_ART_OFFSET_X, pSize.y + LEADER_ART_OFFSET_Y)
...
function OnChatReceived(fromPlayer, stayOnScreen)
    ...
    if stayOnScreen then
        ...
        instance.ChatIndicatorFade:RegisterEndCallback(
            function()
                Controls.ChatIndicatorWaitTimer:RegisterEndCallback(
                    function()
                        instance.ChatIndicatorFade:RegisterEndCallback(
                            function()
...
function OnChatPanelShown(fromPlayer, stayOnScreen)
    for _, chatIndicatorFade in ipairs(m_uiChatIconsVisible) do
        chatIndicatorFade:RegisterEndCallback(
            function() chatIndicatorFade:SetToBeginning() end)
...
function OnRefresh()
    ...
    if table.count(g_kRefreshRequesters) > 0 then
        ...
        UI.DataError(
            "Attempt to refresh diplomacy ribbon stats but no event triggered the refresh!")
    end
...
function OnTreasuryChanged(playerID, yield, balance)
    ...
    if playerID == Game.GetLocalPlayer() and table.count(g_kRefreshRequesters) >
        0 then ContextPtr:ClearRequestRefresh() end
end
...
OnCityFocusChanged =
    function() OnLocalStatUpdateRequest("OnCityFocusChanged") end
...
OnCityWorkerChanged =
    function() OnLocalStatUpdateRequest("OnCityWorkerChanged") end
...
OnGovernmentChanged =
    function() OnLocalStatUpdateRequest("OnGovernmentChanged") end
...
OnGreatWorkCreated =
    function() OnLocalStatUpdateRequest("OnGreatWorkCreated") end
...
OnPlayerAgeChanged =
    function() OnLocalStatUpdateRequest("OnPlayerAgeChanged") end
OnResearchCompleted =
    function() OnLocalStatUpdateRequest("OnResearchCompleted") end
