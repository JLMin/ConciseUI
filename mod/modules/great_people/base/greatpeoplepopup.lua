...
local m_greatPersonPanelIM = InstanceManager:new("PanelInstance", "Top", Controls.PeopleStack)
local m_greatPersonRowIM = InstanceManager:new("PastRecruitmentInstance", "Content", Controls.RecruitedStack)
...
function GetActivationEffectTextByGreatPersonClass(greatPersonClassID)
    local text
    if
        ((GameInfo.GreatPersonClasses["GREAT_PERSON_CLASS_WRITER"] ~= nil and
            greatPersonClassID == GameInfo.GreatPersonClasses["GREAT_PERSON_CLASS_WRITER"].Index) or
            ...
            (GameInfo.GreatPersonClasses["GREAT_PERSON_CLASS_MUSICIAN"] ~= nil and
                greatPersonClassID == GameInfo.GreatPersonClasses["GREAT_PERSON_CLASS_MUSICIAN"].Index))
     then
...
function ViewCurrent(data)
    ...
    for i, kPerson in ipairs(data.Timeline) do
        ...
        if classText then
            ...
            instance.CivilpediaButton:RegisterCallback(
                Mouse.eLClick,
                function()
        ...
        if actionIcon ~= nil and actionIcon ~= "" then
            ...
            if (textureSheet == nil or textureSheet == "") then
                UI.DataError(
                    'Could not find icon in ViewCurrent: icon="' ..
                        actionIcon .. '", iconSize=' .. tostring(SIZE_ACTION_ICON)
                )
            else
        ...
        if (sPassiveName ~= nil and sPassiveName ~= "") and (sActiveName ~= nil and sActiveName ~= "") then
        ...
        if (sActiveName ~= nil and sActiveName ~= "") then
            ...
            if (kPerson.ActionCharges > 0) then
                sFullText =
                    sFullText .. " (" .. Locale.Lookup("LOC_GREATPERSON_ACTION_CHARGES", kPerson.ActionCharges) .. ")"
            end
        ...
        local recruitTable = {}
        instance["PlayerIM"] =
            instance["PlayerIM"] or InstanceManager:new("PlayerInstance", "CivIconBackingFaded", instance.CivIconStack)
                ...
                if i <= 3 or kPlayerPoints.PlayerID == Game.GetLocalPlayer() then
                    ...
                    if (canEarnAnotherOfThisClass) then
                        ...
                        local sRecruitDetails =
                            Locale.Lookup(
                            "LOC_GREAT_PEOPLE_POINT_DETAILS",
                            Round(kPlayerPoints.PointsPerTurn, 1),
                            classData.IconString,
                            classData.Name
                        )
                        ...
                        tPlayerInstance.CivIconBackingFaded:SetColor(tPlayerInstance.UNKNOWN_COLOR)
                        tPlayerInstance.CivIconFaded:SetColor(tPlayerInstance.UNKNOWN_COLOR)
                        --------------------------------------------------------
                        -- Color Icon
                        --------------------------------------------------------
                        tPlayerInstance.FullIcon =
                            tPlayerInstance.FullIcon or
                            CivilizationIcon:new(
                                {
                    ...
                    if kPlayerPoints.PlayerID == Game.GetLocalPlayer() then
                        local sRecruitText = Locale.Lookup("LOC_GREAT_PEOPLE_OR_RECRUIT_WITH_PATRONAGE")
                        ...
                        if (kPerson.EarnConditions ~= nil and kPerson.EarnConditions ~= "") then
                            sRecruitText =
                                "[COLOR_Civ6Red]" ..
                                Locale.Lookup("LOC_GREAT_PEOPLE_CANNOT_EARN_PERSON") .. "[ENDCOLOR]"
                            ...
                            instance.ConnotRecruitButton:SetToolTipString(sRecruitTooltip)
                        ...
                        if (canEarnAnotherOfThisClass) then
                            ...
                            if turnsRemaining >= 1 then
                                if pointPerturn == 0 then
                                    ...
                                    sProgress =
                                        pointPlayer ..
                                        "([COLOR_Civ6Green]+" .. pointPerturn .. "[ENDCOLOR])" .. " / " .. pointTotal
                                end
                                sTurns = turnsRemaining .. "[ICON_TURN]"
                            end
                            instance.Amount:SetText(sProgress .. "  -  " .. sTurns)
                        end
                    end
                end
            end
        ...
        instance.CivIconStack_TTCatcher:SetSizeVal(instance.CivIconStack:GetSizeX(), instance.CivIconStack:GetSizeY())
        LuaEvents.CuiGreatPersonToolTip(instance.CivIconStack_TTCatcher, recruitTable, kPerson)

        if kPerson.IndividualID ~= nil and kPerson.ClassID ~= nil then
            -- Buy via gold
            if
                (HasCapability("CAPABILITY_GREAT_PEOPLE_RECRUIT_WITH_GOLD") and
                    (not kPerson.CanRecruit and not kPerson.CanReject and kPerson.PatronizeWithGoldCost ~= nil and
                        kPerson.PatronizeWithGoldCost < 1000000))
             then
            ...
            -- Buy via Faith
            if
                (HasCapability("CAPABILITY_GREAT_PEOPLE_RECRUIT_WITH_FAITH") and
                    (not kPerson.CanRecruit and not kPerson.CanReject and kPerson.PatronizeWithFaithCost ~= nil and
                        kPerson.PatronizeWithFaithCost < 1000000))
             then
            ...
            -- Recruiting
            if
                (HasCapability("CAPABILITY_GREAT_PEOPLE_CAN_RECRUIT") and kPerson.CanRecruit and
                    kPerson.RecruitCost ~= nil)
             then
                instance.RecruitButton:SetToolTipString(
                    Locale.Lookup("LOC_GREAT_PEOPLE_RECRUIT_DETAILS", kPerson.RecruitCost)
                )
            ...
            if (HasCapability("CAPABILITY_GREAT_PEOPLE_CAN_REJECT") and kPerson.CanReject and kPerson.RejectCost ~= nil) then
                instance.RejectButton:SetToolTipString(
                    Locale.Lookup("LOC_GREAT_PEOPLE_PASS_DETAILS", kPerson.RejectCost)
                )
    ...
    for i = table.count(data.Timeline), 1, -1 do
        ...
        if (kPerson.ClaimantID ~= nil) then
            ...
            if (playerConfig ~= nil) then
                ...
                elseif
                    (Game.GetLocalObserver() == PlayerTypes.OBSERVER or
                        (localPlayer ~= nil and localPlayer:GetDiplomacy() ~= nil and
                            localPlayer:GetDiplomacy():HasMet(kPerson.ClaimantID)))
                 then
                    instance.RecruitedImage:SetIcon(iconName, 55)
        ...
        local isLocalPlayer = (kPerson.ClaimantID ~= nil and kPerson.ClaimantID == localPlayerID)
        instance.YouIndicator:SetHide(not isLocalPlayer)

        local colorName = (isLocalPlayer and "GreatPeopleRow") or "GreatPeopleRowUnOwned"
        ...
        if (kPerson.ActionNameText ~= nil and kPerson.ActionNameText ~= "") then
            ...
            if (kPerson.ActionCharges > 0) then
                fullText =
                    fullText .. " (" .. Locale.Lookup("LOC_GREATPERSON_ACTION_CHARGES", kPerson.ActionCharges) .. ")"
            end
    ...
    for i, entry in ipairs(pTimeline) do
        ...
        if not isPast or entry.Claimant then
            ...
            if (entry.Individual ~= nil) then
                if (Players[displayPlayerID] ~= nil) then
                    ...
                    canPatronizeWithGold =
                        pGreatPeople:CanPatronizePerson(displayPlayerID, entry.Individual, YieldTypes.GOLD)
                    patronizeWithGoldCost =
                        pGreatPeople:GetPatronizeCost(displayPlayerID, entry.Individual, YieldTypes.GOLD)
                    canPatronizeWithFaith =
                        pGreatPeople:CanPatronizePerson(displayPlayerID, entry.Individual, YieldTypes.FAITH)
                    patronizeWithFaithCost =
                        pGreatPeople:GetPatronizeCost(displayPlayerID, entry.Individual, YieldTypes.FAITH)
            ...
            if GameInfo.GreatPersonIndividuals[entry.Individual] ~= nil then
                ...
                personType = (GameInfo.GreatPersonIndividuals[entry.Individual].GreatPersonIndividualType) -- CUI
            end
    ...
    for classInfo in GameInfo.GreatPersonClasses() do
        ...
        for i, player in ipairs(players) do
            ...
            if (player:GetID() == displayPlayerID) then
                playerName =
                    playerName .. Locale.Lookup(PlayerConfigurations[player:GetID()]:GetCivilizationShortDescription())
                isPlayer = true
            elseif
                (Game.GetLocalObserver() == PlayerTypes.OBSERVER or
                    Players[displayPlayerID]:GetDiplomacy():HasMet(player:GetID()))
             then
                playerName =
                    playerName .. Locale.Lookup(PlayerConfigurations[player:GetID()]:GetCivilizationShortDescription())
            else
...
--    Game Engine Event
-- ===========================================================================
function OnUnitGreatPersonActivated(unitOwner, unitID, greatPersonClassID, greatPersonIndividualID)
...
function Initialize()
    ...
    -- Audio Events
    Controls.ButtonGreatPeople:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
    )
    Controls.ButtonPreviouslyRecruited:RegisterCallback(
        Mouse.eMouseEnter,
        function()
            UI.PlaySound("Main_Menu_Mouse_Over")
        end
