...
--	CONSTANTS
-- ===========================================================================
local COLOR_CLAIMED    :number = UI.GetColorValueFromHexLiteral(0xffffffff);
local COLOR_AVAILABLE  :number = UI.GetColorValueFromHexLiteral(0xbbffffff);
local COLOR_UNAVAILABLE:number = UI.GetColorValueFromHexLiteral(0x55ffffff);
...
local m_greatPersonPanelIM	:table	= InstanceManager:new("PanelInstance",				"Top",	Controls.PeopleStack);
...
function ViewCurrent( data:table )
  ...
  for i, kPerson:table in ipairs(data.Timeline) do
    ...
    local individualName:string = "";
    
    if (kPerson.ClassID ~= nil) then classText = Locale.Lookup(classData.Name) end
    if kPerson.IndividualID ~= nil then individualName = Locale.ToUpper(kPerson.Name) end
    ...
    if actionIcon ~= nil and actionIcon ~= "" then
      ...
      if(textureSheet == nil or textureSheet == "") then
        ...
        instance.GreatPersonIcon:SetTexture(textureOffsetX, textureOffsetY, textureSheet);
    ...
    instance.BonusBacking1:SetHide(true); instance.BonusBacking2:SetHide(true); instance.BonusBacking3:SetHide(true);
    if (sPassiveName ~=  nil and sPassiveName ~= "") and (sActiveName ~=  nil and sActiveName ~= "") then
      instance.BonusBacking2:SetHide(false); instance.BonusBacking3:SetHide(false);
      bTwoBoxes = true
    else
    ...
    if (sActiveName ~= nil and sActiveName ~= "") then
      ...
      if (kPerson.ActionCharges > 0) then
        sFullText = sFullText .. " (" .. Locale.Lookup("LOC_GREATPERSON_ACTION_CHARGES", kPerson.ActionCharges) .. ")";
      end
      ...
      for i, kPlayerPoints in ipairs(recruitTable) do
        if i <= 3 or kPlayerPoints.PlayerID == Game.GetLocalPlayer() then
          ...
          if (canEarnAnotherOfThisClass) then
            ...
            local sRecruitDetails = Locale.Lookup("LOC_GREAT_PEOPLE_POINT_DETAILS", Round(kPlayerPoints.PointsPerTurn, 1), classData.IconString, classData.Name);
            ...
            -- Color Icon
            --------------------------------------------------------
            tPlayerInstance.FullIcon = tPlayerInstance.FullIcon or CivilizationIcon:new(
              {
          ...
          if kPlayerPoints.PlayerID == Game.GetLocalPlayer() then
            ...
            if (canEarnAnotherOfThisClass) then
              ...
              if turnsRemaining >= 1 then
                sProgress = pointTotal ..
                            " ( [COLOR_Civ6Red]-"   .. pointRemaining .. "[ENDCOLOR] / " ..
                               "[COLOR_Civ6Green]+" .. pointPerturn   .. "[ENDCOLOR] )";
...
function ViewPast( data:table )
  ...
  -- CUI for i, kPerson:table in ipairs(data.Timeline) do
...
-- CUI ===================================================================================
function CuiSetPanelToDetault(instance:table)
  instance.GreatPersonIcon:SetHide(true);
...
-- CUI ===================================================================================
function CuiSearchInCivilpedia(name:string)
