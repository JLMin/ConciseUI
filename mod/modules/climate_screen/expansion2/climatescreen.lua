...
--	MEMBERS
-- ===========================================================================
local m_kCO2CivsIM = InstanceManager:new("ResourceCO2Instance", "Top",
                                         Controls.GlobalCityStack)
local m_kCivCO2IM = InstanceManager:new("CivCO2Instance", "Top",
                                        Controls.GlobalCityStack)
local m_kYourCO2IM = InstanceManager:new("ResourceCO2Instance", "Top",
                                         Controls.YourCO2Stack)
local m_kSliceIM = InstanceManager:new("PieChartSliceInstance", "Slice")
local m_kAffectedCitiesIM = InstanceManager:new("CityInstance", "Top",
                                                Controls.CitiesStack)
local m_kEventRowInstance = InstanceManager:new("EventRowInstance", "Top",
                                                Controls.EventStack)
local m_kClimateChangeInstance = InstanceManager:new("ClimateChangeInstance",
                                                     "Top", Controls.EventStack)
...
function GetWorstCO2PlayerID()
    ...
    if CO2Total > 0 then
        for _, pPlayer in ipairs(PlayerManager.GetAliveMajors()) do
            local CO2 =
                GameClimate.GetPlayerCO2Footprint(pPlayer:GetID(), false)
...
function RealizeCO2Tabs(selectedTabName)
    ...
    Controls.CO2SelectedByCivilization:SetHide(
        selectedTabName ~= "ByCivilization")
    Controls.CO2ButtonByCivilization:SetSelected(
        selectedTabName == "ByCivilization")
...
function RefreshCurrentEvent()
    ...
    if kCurrentEvent ~= nil then
        local kCurrentEventDef =
            GameInfo.RandomEvents[kCurrentEvent.RandomEvent]
        if kCurrentEventDef ~= nil then
            ...
            if kCurrentEventDef.EffectOperatorType == CLIMATE_CHANGE_EVENT_TYPE then
                ...
                return
            elseif kCurrentEventDef.EffectOperatorType ==
                NUCLEAR_ACCIDENT_EVENT_TYPE then
                -- Don't show nuclear accidents
                Controls.CurrentEventStack:SetHide(true)
                return
            end

            local pCurrentPlot = Map.GetPlotByIndex(
                                     kCurrentEvent.CurrentLocation)
            ...
            if pCurrentPlot ~= nil then
                ...
                if pLocalPlayerVis ~= nil then
                    if pLocalPlayerVis:IsRevealed(pCurrentPlot:GetX(),
                                                  pCurrentPlot:GetY()) then
            ...
            -- Weather type name
            Controls.WeatherStatusText:SetText(
                Locale.ToUpper(kCurrentEventDef.Name))
            ...
            if kCurrentEventDef.IconLarge and kCurrentEventDef.IconLarge ~= "" then
                Controls.WeatherStatusImage:SetTexture(
                    kCurrentEventDef.IconLarge)
            else
                UI.DataError(
                    "Unable to find IconLarge for RandomEvents type: " ..
                        kCurrentEventDef.PrimaryKey)
            end

            -- Get the territory name as a back up to continent name
            local territoryName = nil
            local pTerritory = Territories.GetTerritoryAt(
                                   kCurrentEvent.CurrentLocation)
            ...
            local direction = Locale.Lookup(
                                  GetDirectionText(
                                      kCurrentEvent.CurrentDirection))

            if not bIsEventVisible then
                Controls.WeatherLocation:SetText(
                    Locale.Lookup("LOC_CLIMATE_SCREEN_LOCATION", Locale.Lookup(
                                      "LOC_CIVICS_TREE_UNREVEALED_CIVIC")))
            elseif location ~= "" and direction ~= "" then
                Controls.WeatherLocation:SetText(
                    Locale.Lookup("LOC_CLIMATE_SCREEN_LOCATION_DIRECTION",
                                  location, direction))
            elseif location ~= "" then
                Controls.WeatherLocation:SetText(
                    Locale.Lookup("LOC_CLIMATE_SCREEN_LOCATION", location))
            else
                Controls.WeatherLocation:SetText("")
            end
            ...
            for _, affectedCity in ipairs(kCurrentAffectedCities) do
                ...
                local pLocalPlayerDiplo =
                    Players[Game.GetLocalPlayer()]:GetDiplomacy()
                if pOwner ~= nil and
                    pLocalPlayerDiplo:HasMet(affectedCity.CityOwner) then
                    local pCity = pOwner:GetCities():FindID(affectedCity.CityID)
                    if pCity then
                        local pOwnerConfig =
                            PlayerConfigurations[affectedCity.CityOwner]
                        local iconString =
                            "ICON_" .. pOwnerConfig:GetCivilizationTypeName()
                        local textureOffsetX, textureOffsetY, textureSheet =
                            IconManager:FindIconAtlas(iconString, 30)
                        local secondaryColor, primaryColor =
                            UI.GetPlayerColors(affectedCity.CityOwner)

                        local cityInstance = m_kAffectedCitiesIM:GetInstance()
                        cityInstance.Icon:SetTexture(textureOffsetX,
                                                     textureOffsetY,
                                                     textureSheet)
                        cityInstance.Icon:LocalizeAndSetToolTip(
                            pOwnerConfig:GetCivilizationDescription())
            ...
            Controls.CitiesStackDivider:SetHide(
                not doesAffectACity or not bIsEventVisible)

            if kCurrentEventDef.EffectString ~= nil then
                Controls.WeatherEffect:SetText(
                    Locale.Lookup(kCurrentEventDef.EffectString))
            else
                Controls.WeatherEffect:SetText("")
            end
            ...
            if kCurrentEvent.FertilityAdded ~= nil and
                kCurrentEvent.FertilityAdded < 0 then
                Controls.PlotLostFertileLabel:SetText(
                    math.abs(kCurrentEvent.FertilityAdded))
            ...
            if kCurrentEvent.TilesDamaged ~= nil and kCurrentEvent.TilesDamaged >
                0 then
...
function TabSelectOverview()
    ...
    if (m_currentSeaLevelEvent < 0) then
        ...
        ClimateName = Locale.ToUpper(
                          GameInfo.RandomEvents[m_currentSeaLevelEvent].Name)
    end
    ...
    if (deforestationType >= 0) then
        local kDeforestationLevel =
            GameInfo.DeforestationLevels[deforestationType]
        deforestationName = kDeforestationLevel.Name
        deforestationDescription = kDeforestationLevel.Description
        TempTooltip = Locale.Lookup("LOC_CLIMATE_TEMPERATURE_TOOLTIP",
                                    deforestationName, deforestationDescription,
                                    co2Modifier)
    end
    local co2Modifier = GameClimate.GetCO2FootprintModifier()
    ...
    -- Left
    Controls.ContributeTotal:SetText(Locale.Lookup("LOC_CLIMATE_TOTAL_NUM",
                                                   CO2Total))
    Controls.ContributeTop:SetText(Locale.Lookup(
                                       "LOC_CLIMATE_TOP_CONTRIBUTOR_NUM",
                                       TopContributorName))
    Controls.ContributeMe:SetText(Locale.Lookup(
                                      "LOC_CLIMATE_MY_CONTRIBUTION_NUM",
                                      CO2Player))
    Controls.OverviewCO2Grid:SetToolTipString(
        Locale.Lookup("LOC_CLIMATE_CO2_TOTAL_TOOLTIP", CO2Modifier))

    Controls.ClimateTemperature:SetText(Locale.Lookup(
                                            "LOC_CLIMATE_TEMPERATURE_SUBTEXT"))
    Controls.TemperatureValue:SetText(Locale.Lookup("LOC_CLIMATE_TEMPERATURE",
                                                    TempIncreaseText))
    ...
    if m_worldAgeName then
        Controls.WorldAgeText:SetText(Locale.Lookup("LOC_CLIMATE_WORLD_AGE",
                                                    m_worldAgeName))
    end

    Controls.RealismText:SetHide(m_RealismName == nil)
    if m_RealismName then
        Controls.RealismText:SetText(Locale.Lookup("LOC_CLIMATE_REALISM",
                                                   m_RealismName))
    end

    -- Right
    Controls.StormChanceNum:SetText(Locale.Lookup("LOC_CLIMATE_PERCENT_CHANCE",
                                                  stormChance))
    Controls.StormChanceFromClimateChange:SetText(
        Locale.Lookup("LOC_CLIMATE_AMOUNT_FROM_CLIMATE_CHANGE", stormIncrease))

    Controls.RiverFloodChanceNum:SetText(
        Locale.Lookup("LOC_CLIMATE_PERCENT_CHANCE", riverFloodChance))
    Controls.RiverFloodChanceFromClimateChange:SetText(
        Locale.Lookup("LOC_CLIMATE_AMOUNT_FROM_CLIMATE_CHANGE",
                      riverFloodIncrease))

    Controls.DroughtActivityChanceNum:SetText(
        Locale.Lookup("LOC_CLIMATE_PERCENT_CHANCE", droughtChance))
    Controls.DroughtChanceFromClimateChange:SetText(
        Locale.Lookup("LOC_CLIMATE_AMOUNT_FROM_CLIMATE_CHANGE", droughtIncrease))

    Controls.VolcanicActivityChanceNum:SetText(
        Locale.Lookup("LOC_CLIMATE_PERCENT_CHANCE", volcanoEruptChance))
    Controls.VolatileNum:SetText(Locale.Lookup(
                                     "LOC_CLIMATE_VOLCANO_VOLATILE_NUM",
                                     volcanoNaturalWonder))
    Controls.InactiveNum:SetText(Locale.Lookup(
                                     "LOC_CLIMATE_VOLCANO_INACTIVE_NUM",
                                     volcanoTotalNum - volcanoActiveNum))
    Controls.ActiveNum:SetText(Locale.Lookup("LOC_CLIMATE_VOLCANO_ACTIVE_NUM",
                                             volcanoActiveNum))
    ...
    if nextIceLostTurns > 0 and ClimateLevel < MAX_PHASES then
        ...
        Controls.NextPolarIceLost:SetText(
            Locale.Lookup("LOC_CLIMATE_POLAR_ICE_MELT_X_TURNS", nextIceLostTurns))
    else
        Controls.NextPolarIceLost:SetHide(true)
    end

    Controls.SeaLevel:SetText(Locale.Lookup("LOC_CLIMATE_SEA_LEVEL_RISE",
                                            Locale.Lookup(szSeaLevel)))
    Controls.SeaLevelArea:SetToolTipString(
        Locale.Lookup("LOC_CLIMATE_SEA_LEVEL_RISE_DESCRIPTION_TOOLTIP",
                      szSeaLevel))
    Controls.TilesFlooded:SetText(Locale.Lookup(
                                      "LOC_CLIMATE_COASTAL_TILES_FLOODED_NUM",
                                      tilesFlooded))
    Controls.TilesSubmerged:SetText(Locale.Lookup(
                                        "LOC_CLIMATE_COASTAL_TILES_SUBMERGED_NUM",
                                        tilesSubmerged))
    if nextSeaRiseTurns > 0 and ClimateLevel < MAX_PHASES then
        Controls.NextSeaLevelRise:SetHide(false)
        Controls.NextSeaLevelRise:SetText(
            Locale.Lookup("LOC_CLIMATE_SEA_LEVEL_RISE_X_TURNS", nextSeaRiseTurns))
    else
        Controls.NextSeaLevelRise:SetHide(true)
    end

    Controls.SeaLevelWave:SetOffsetY((ClimateLevel * SEA_LEVEL_WAVE_OFFSET) -
                                         SEA_LEVEL_WAVE_OFFSET)
end
...
function BuildPieChart(uiHolder, sliceIM, kSliceAmounts, kColors)
    ...
    if sliceIM == nil then
        UI.DataError(
            "Cannot build pie chart due to a nil instance manager for generating slices passed in.")
        return {}
    end
    if kSliceAmounts == nil then
        UI.DataError(
            "Cannot build pie chart due to a nil table of slice amounts passed in.")
        return {}
    end
    ...
    if total > 1.0 then
        ...
        UI.DataError("Total of pie chart slices " .. tostring(total) ..
                         " exceeds 1.0 (100%).  Applying multiplier " ..
                         tostring(multiplier))
    elseif total < 0 then
        UI.DataError("Total of slices " .. tostring(total) ..
                         " is less than 0!  Something is fishy with your data.")
        total = 0
    end
    ...
    if kColors == nil or table.count(kColors) == 0 then
        kColors = {
            UI.GetColorValueFromHexLiteral(0xff000099),
            UI.GetColorValueFromHexLiteral(0xff008888),
            UI.GetColorValueFromHexLiteral(0xff009900),
            UI.GetColorValueFromHexLiteral(0xff888800),
            UI.GetColorValueFromHexLiteral(0xff990000),
            UI.GetColorValueFromHexLiteral(0xff880088)
        }
    end
...
function TabSelectCO2Levels()
    ...
    if CO2Modifier ~= 0 then
        sGlobalTotal = Locale.Lookup("LOC_CLIMATE_TOTAL_NUM_W_MOD", CO2Total,
                                     CO2Modifier)
    else
        sGlobalTotal = Locale.Lookup("LOC_CLIMATE_TOTAL_NUM", CO2Total)
    end
    Controls.GlobalContributionsTotalNum:SetText(sGlobalTotal)
    Controls.YourContributionsNum:SetText(
        Locale.Lookup("LOC_CLIMATE_TOTAL_NUM", CO2Player))
    Controls.GlobalContributionsTotalNum:SetToolTipString(
        Locale.Lookup("LOC_CLIMATE_CO2_TOTAL_TOOLTIP", CO2Modifier))
end
    ...
    for i = iCurrentTurn, 0, -1 do
        ...
        if kEvent ~= nil then
            ...
            if kEventDef ~= nil then
                ...
                elseif kEventDef.EffectOperatorType ~=
                    NUCLEAR_ACCIDENT_EVENT_TYPE then
                    local pEventPlot =
                        Map.GetPlotByIndex(kEvent.CurrentLocation)
                    if pEventPlot ~= nil then
                        local pLocalPlayerVis =
                            PlayersVisibility[Game.GetLocalPlayer()]
                        if pLocalPlayerVis ~= nil and
                            pLocalPlayerVis:IsRevealed(pEventPlot:GetX(),
                                                       pEventPlot:GetY()) then
...
function CreateClimateChangeInstance(kEvent, kEventDef, iTurn)
    ...
    kInstance.DateString:SetText("[Icon_Turn]" ..
                                     Locale.Lookup("LOC_CLIMATE_ENTRY_DATE",
                                                   iTurn, strDate))
end
...
function CreateEventInstance(kEvent, kEventDef, iTurn)
    ...
    if pPlot ~= nil then
        ...
        if eContinentType and eContinentType ~= -1 then
            ...
            kInstance.LocationString:SetText(
                Locale.Lookup(kContinentDef.Description))
        elseif territoryName ~= nil then
            kInstance.LocationString:SetText(territoryName)
        else
            kInstance.LocationString:SetText(
                Locale.Lookup("LOC_CLIMATE_SCREEN_WATER"))
        end
    end
    ...
    if kEvent.FertilityAdded > 0 then
        ...
        kInstance.FertilizedContainer:SetToolTipString(
            Locale.Lookup("LOC_CLIMATE_FERTILIZED_TILES"))
    elseif kEvent.FertilityAdded < 0 then
        ...
        kInstance.FertilizedContainer:SetToolTipString(
            Locale.Lookup("LOC_CLIMATE_LOST_FERTILIZED_TILES"))
    else
        kInstance.FertilizedTilesIcon:SetHide(true)
        kInstance.LosingFertilizedTilesIcon:SetHide(true)
...
function RealizePlayerCO2()
    ...
    for kResourceInfo in GameInfo.Resources() do
        ...
        local kConsumption =
            GameInfo.Resource_Consumption[kResourceInfo.ResourceType]
        if kConsumption ~= nil and kConsumption.CO2perkWh ~= nil and
            kConsumption.CO2perkWh > 0 then

            -- Is the player using the resource?
            local amount = GameClimate.GetPlayerResourceCO2Footprint(m_playerID,
                                                                     kResourceInfo.Index,
                                                                     false)
            if amount > 0 then
                ...
                local amountLastTurn =
                    GameClimate.GetPlayerResourceCO2Footprint(m_playerID,
                                                              kResourceInfo.Index,
                                                              true)
                local resourceLastTurn =
                    GameClimate.GetPlayerResourceConsumption(m_playerID,
                                                             kResourceInfo.Index,
                                                             true)
                ...
                uiResource.Top:SetToolTipString(
                    Locale.Lookup("LOC_CLIMATE_RESOURCE_CONSUMED_LAST_TURN",
                                  resourceLastTurn, kResourceInfo.Name,
                                  amountLastTurn))
    ...
    BuildPieChart(Controls.TotalContributionsPie, m_kSliceIM, kSliceAmounts,
                  kColors)
end
...
function TabCO2ByCiviliation()
    ...
    for _, pPlayer in ipairs(pPlayers) do
        ...
        local CO2FootprintNum = GameClimate.GetPlayerCO2Footprint(playerID,
                                                                  false)
    ...
    m_kGlobalPieSlices = BuildPieChart(Controls.GlobalContributionsPie,
                                       m_kSliceIM, kSliceAmounts, kColors)
end
            ...
            for _, pPlayer in ipairs(PlayerManager.GetAliveMajors()) do
                amount = amount +
                             GameClimate.GetPlayerResourceCO2Footprint(
                                 pPlayer:GetID(), kResourceInfo.Index, false)
            end
...
function UpdateClimateChangeEventsData()
    local iCurrentClimateChangePoints =
    ...
    for row in GameInfo.RandomEvents() do
        if (row.EffectOperatorType == "SEA_LEVEL") then
            ...
            if (row.ClimateChangePoints == iCurrentClimateChangePoints) then
                ...
                m_currentSeaLevelPhase =
                    m_currentSeaLevelEvent - m_firstSeaLevelEvent + 1
            end
        end
    end
end
...
function UpdatePhaseBar(phase, realismAmount, globalTemp)
    ...
    if phase < 0 then
        UI.DataError(
            "SetPhaseBar() needs to cap phase of " .. tostring(phase) .. " to 1")
        phase = 1
    end
    if phase > MAX_PHASES then
        UI.DataError(
            "SetPhaseBar() needs to cap phase of " .. tostring(phase) ..
                " to max phase of " .. tostring(MAX_PHASE) .. ".")
        phase = MAX_PHASES
    end
...
function UpdatePhaseTooltips(segmentNum)
    local kEventDef = GameInfo.RandomEvents[m_firstSeaLevelEvent + segmentNum -
                          1]
    ...
    for row in GameInfo.CoastalLowlands() do
        if (row.FloodedEvent == szPhaseType) then
            szAtOrAboveString = Locale.Lookup(
                                    "LOC_CLIMATE_TILES_AT_OR_BELOW_FLOOD_TOOLTIP",
                                    row.Name)
            szLongDescription = GameInfo.RandomEvents[row.FloodedEvent]
                                    .LongDescription
            break
        elseif (row.SubmergedEvent == szPhaseType) then
            szAtOrAboveString = Locale.Lookup(
                                    "LOC_CLIMATE_TILES_AT_OR_BELOW_SUBMERGE_TOOLTIP",
                                    row.Name)
            szLongDescription = GameInfo.RandomEvents[row.SubmergedEvent]
                                    .LongDescription
            break
        end
    end
    ...
    local tooltip = Locale.ToUpper(szPhaseName) .. "[NEWLINE]" .. "[NEWLINE]" ..
                        Locale.Lookup(
                            "LOC_CLIMATE_CLIMATE_CHANGE_POINTS_TOOLTIP",
                            GameClimate.GetClimateChangeLevel(), iPoints) ..
                        "[NEWLINE]" ..
                        Locale.Lookup(
                            "LOC_CLIMATE_FROM_WORLD_REALISM_NUM_TOOLTIP",
                            GameClimate.GetClimateChangeFromRealism()) ..
                        "[NEWLINE]" ..
                        Locale.Lookup(
                            "LOC_CLIMATE_FROM_GLOBAL_TEMPERATURE_NUM_TOOLTIP",
                            GameClimate.GetClimateChangeFromTemperature()) ..
                        "[NEWLINE]" .. "[NEWLINE]" ..
                        Locale.ToUpper(
                            "LOC_CLIMATE_EFFECTS_ADDED_THIS_PHASE_TOOLTIP") ..
                        "[NEWLINE]" .. "[NEWLINE]" ..
                        Locale.Lookup("LOC_CLIMATE_SEA_LEVEL_RISES_NUM_TOOLTIP",
                                      szSeaLevelRise) .. szAtOrAboveString ..
                        "[NEWLINE]" .. "[NEWLINE]" ..
                        Locale.Lookup("LOC_CLIMATE_POLAR_ICE_MELT_TOOLTIP",
                                      kEventDef.IceLoss)

    if (szLongDescription ~= nil and szLongDescription ~= "") then
        tooltip = tooltip .. "[NEWLINE][NEWLINE]" ..
                      Locale.Lookup(szLongDescription)
    end
...
function OnShutdown()
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "isHidden",
                                 ContextPtr:IsHidden())
    LuaEvents.GameDebug_AddValue(RELOAD_CACHE_ID, "m_currentTabName",
                                 m_currentTabName)
end
...
function Initialize()
    ...
    if worldAgeNum then
        query =
            "SELECT * FROM DomainValues where Domain = 'WorldAge' and Value = ? LIMIT 1"
