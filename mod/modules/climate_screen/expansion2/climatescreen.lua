        ...
        if kCurrentEventDef ~= nil then
            ...
            if not bIsEventVisible then
                Controls.WeatherLocation:SetText(
                    Locale.Lookup("LOC_CLIMATE_SCREEN_LOCATION", Locale.Lookup("LOC_CIVICS_TREE_UNREVEALED_CIVIC"))
                )
...
function TabSelectOverview()
    ...
    if (deforestationType >= 0) then
        ...
        TempTooltip =
            Locale.Lookup("LOC_CLIMATE_TEMPERATURE_TOOLTIP", deforestationName, deforestationDescription, co2Modifier)
    end
    local co2Modifier = GameClimate.GetCO2FootprintModifier()
    ...
    Controls.RiverFloodChanceFromClimateChange:SetText(
        Locale.Lookup("LOC_CLIMATE_AMOUNT_FROM_CLIMATE_CHANGE", riverFloodIncrease)
    )
...
function BuildPieChart(uiHolder, sliceIM, kSliceAmounts, kColors)
    ...
    if total > 1.0 then
        ...
        UI.DataError(
            "Total of pie chart slices " ..
                tostring(total) .. " exceeds 1.0 (100%).  Applying multiplier " .. tostring(multiplier)
        )
    elseif total < 0 then
        ...
        if kConsumption ~= nil and kConsumption.CO2perkWh ~= nil and kConsumption.CO2perkWh > 0 then
            ...
            if amount > 0 then
                ...
                uiResource.Top:SetToolTipString(
                    Locale.Lookup(
                        "LOC_CLIMATE_RESOURCE_CONSUMED_LAST_TURN",
                        resourceLastTurn,
                        kResourceInfo.Name,
                        amountLastTurn
                    )
...
function UpdatePhaseBar(phase, realismAmount, globalTemp)
    ...
    if phase > MAX_PHASES then
        UI.DataError(
            "SetPhaseBar() needs to cap phase of " ..
                tostring(phase) .. " to max phase of " .. tostring(MAX_PHASE) .. "."
        )
        phase = MAX_PHASES
    end
                        ...
                        Locale.Lookup(
                            ...
                            "[NEWLINE]" ..
                                Locale.Lookup(
                                    ...
                                    "[NEWLINE]" ..
                                        "[NEWLINE]" ..
                                            Locale.ToUpper("LOC_CLIMATE_EFFECTS_ADDED_THIS_PHASE_TOOLTIP") ..
                                                "[NEWLINE]" ..
                                                    "[NEWLINE]" ..
                                                        Locale.Lookup(
                                                            "LOC_CLIMATE_SEA_LEVEL_RISES_NUM_TOOLTIP",
                                                            szSeaLevelRise
                                                        ) ..
                                                            szAtOrAboveString ..
                                                                "[NEWLINE]" ..
                                                                    "[NEWLINE]" ..
                                                                        Locale.Lookup(
                                                                            "LOC_CLIMATE_POLAR_ICE_MELT_TOOLTIP",
                                                                            kEventDef.IceLoss
                                                                        )
